...
import { IActivityService } from '../common/activity.service.js';
...
let ViewContainerActivityByView = class ViewContainerActivityByView extends Disposable {
    constructor(viewId, viewDescriptorService, activityService) {
        ...
        this._register(
            Event.filter(this.viewDescriptorService.onDidChangeContainer, e => ( e.views.some(view => view.id === viewId)))(() => this.update())
        );
        this._register(
            Event.filter(this.viewDescriptorService.onDidChangeLocation, e => ( e.views.some(view => view.id === viewId)))(() => this.update())
        );
    }
...
ViewContainerActivityByView = ( __decorate([( __param(1, IViewDescriptorService)), ( __param(2, IActivityService))], ViewContainerActivityByView));
...
ActivityService = ( __decorate([( __param(0, IViewDescriptorService)), ( __param(1, IInstantiationService))], ActivityService));
