...
let AuxiliaryWindow = class AuxiliaryWindow extends BaseWindow {
    ...
    registerListeners() {
        this._register(
            addDisposableListener(this.window, EventType.BEFORE_UNLOAD, e => this.handleBeforeUnload(e))
        );
        this._register(
            addDisposableListener(this.window, EventType.UNLOAD, () => this.handleUnload())
        );
        ...
        this._register(
            addDisposableListener(this.container, EventType.SCROLL, () => this.container.scrollTop = 0)
        );
        if (isWeb) {
            this._register(
                addDisposableListener(this.container, EventType.DROP, e => EventHelper.stop(e, true))
            );
            this._register(
                addDisposableListener(this.container, EventType.WHEEL, e => e.preventDefault(), {
                    passive: false
                })
            );
            this._register(
                addDisposableListener(this.container, EventType.CONTEXT_MENU, e => EventHelper.stop(e, true))
            );
        } else {
            this._register(
                addDisposableListener(this.window.document.body, EventType.DRAG_OVER, e => EventHelper.stop(e))
            );
            this._register(
                addDisposableListener(this.window.document.body, EventType.DROP, e => EventHelper.stop(e))
            );
        }
...
AuxiliaryWindow = ( __decorate([( __param(3, IConfigurationService)), ( __param(4, IHostService)), ( __param(5, IWorkbenchEnvironmentService)), ( __param(6, IContextMenuService)), ( __param(7, IWorkbenchLayoutService))], AuxiliaryWindow));
let BrowserAuxiliaryWindowService = class BrowserAuxiliaryWindowService extends Disposable {
    ...
    async open(options) {
        ...
        if (!targetWindow) {
            throw ( new Error(( localize(17379, "Unable to open a new window."))));
        }
        ...
        const {
            container,
            stylesLoaded
        } = this.createContainer(targetWindow, containerDisposables, options);
        ...
        this.telemetryService.publicLog2("auxiliaryWindowOpen", {
    ...
    -webkit-user-select: none;
        ...
        disposables.add(sharedMutationObserver.observe(container, disposables, {
            childList: true,
            subtree: true
        })(mutations => {
            for (const mutation of mutations) {
                if (
                mutation.type !== "childList" || mutation.target.nodeName.toLowerCase() === "title" || mutation.target.nodeName.toLowerCase() === "script" || mutation.target.nodeName.toLowerCase() === "meta") {
                    continue;
                }
    ...
    applyHTML(auxiliaryWindow, disposables) {
        ...
        disposables.add(trackAttributes(
