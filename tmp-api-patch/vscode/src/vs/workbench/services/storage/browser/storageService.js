...
import { Storage, InMemoryStorageDatabase, isStorageItemsChangeEvent } from '../../../../base/parts/storage/common/storage.js';
...
let BrowserStorageService = class BrowserStorageService extends AbstractStorageService {
    ...
    get hasPendingUpdate() {
        return Boolean(
            getIIndexedDBStorageDatabase(this.applicationStorageDatabase)?.hasPendingUpdate || getIIndexedDBStorageDatabase(this.applicationSharedStorageDatabase)?.hasPendingUpdate || getIIndexedDBStorageDatabase(this.profileStorageDatabase)?.hasPendingUpdate || getIIndexedDBStorageDatabase(this.workspaceStorageDatabase)?.hasPendingUpdate
        );
    }
    ...
    async createApplicationSharedStorage() {
        ...
        this._register(
            this.applicationSharedStorage.onDidChangeStorage(e => this.emitDidChangeValue(StorageScope.APPLICATION_SHARED, e))
        );
    ...
    async switchToWorkspace(toWorkspace, preserveData) {
        ...
        this.switchData(
            oldItems,
            assertReturnsDefined(this.workspaceStorage),
    ...
    async clear() {
        ...
        await Promises.settled([
            ...
            getIIndexedDBStorageDatabase(this.applicationSharedStorageDatabase)?.clear() ?? Promise.resolve(),
...
BrowserStorageService = BrowserStorageService_1 = ( __decorate([( __param(2, ILogService))], BrowserStorageService));
