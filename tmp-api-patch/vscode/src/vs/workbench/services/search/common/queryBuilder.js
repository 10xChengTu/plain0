...
let QueryBuilder = class QueryBuilder {
    ...
    commonQuery(folderResources = [], options = {}) {
        ...
        if (options.changedFileUris !== undefined) {
            ...
            this.logService.trace(
                "QueryBuilder#commonQuery - changedFile Query",
                JSON.stringify(changedFilesQueryProps)
            );
            return {
                ...queryProps,
                ...changedFilesQueryProps
            };
        }
    ...
    commonQueryFromFileList(files) {
        ...
        return {
            ...
            excludePattern: hasIncludedFile ? undefined : {
                "**/*": true
            }
        };
    }
    ...
    getExcludesForFolder(folderConfig, options) {
        return options.disregardExcludeSettings ? undefined : getExcludes(folderConfig, !options.disregardSearchExcludeSettings);
    }
    expandSearchPathPatterns(searchPaths) {
        ...
        const expandedSearchPaths = searchPaths.flatMap(searchPath => {
            let {
                pathPortion,
                globPortion
            } = splitGlobFromPath(searchPath);
            if (globPortion) {
                globPortion = normalizeGlobPattern(globPortion);
            }
            const oneExpanded = this.expandOneSearchPath(pathPortion);
            return oneExpanded.flatMap(
                oneExpandedResult => this.resolveOneSearchPathPattern(oneExpandedResult, globPortion)
            );
        });
    ...
    expandOneSearchPath(searchPath) {
        ...
        if (this.workspaceContextService.getWorkbenchState() === WorkbenchState.FOLDER) {
            ...
            if (folderMatches.length) {
                ...
                const searchPathNotFoundError = ( localize(17915, "Workspace folder does not exist: {0}", probableWorkspaceFolderName));
...
QueryBuilder = ( __decorate([( __param(0, IConfigurationService)), ( __param(1, IWorkspaceContextService)), ( __param(2, IEditorGroupsService)), ( __param(3, ILogService)), ( __param(4, IPathService)), ( __param(5, IUriIdentityService))], QueryBuilder));
