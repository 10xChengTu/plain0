...
import { Extensions } from '../../../../platform/theme/common/colorUtils.js';
...
import { Extensions as Extensions$1 } from '../../extensionManagement/common/extensionFeatures.js';
...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(17992, "Contributes extension defined themable colors")),
        type: "array",
        items: {
            type: "object",
            properties: {
                id: {
                    type: "string",
                    description: ( localize(17993, "The identifier of the themable color")),
                ...
                description: {
                    type: "string",
                    description: ( localize(17995, "The description of the themable color"))
                },
                defaults: {
                    type: "object",
                    properties: {
        ...
        configurationExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(18000, "'configuration.colors' must be a array")));
                    return;
                }
                ...
                for (const colorContribution of extensionValue) {
                    if (typeof colorContribution.id !== "string" || colorContribution.id.length === 0) {
                        collector.error(( localize(18002, "'configuration.colors.id' must be defined and can not be empty")));
                        return;
                    }
...
class ColorDataRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [( localize(18008, "ID")), ( localize(18009, "Description")), ( localize(18010, "Dark Default")), ( localize(18011, "Light Default")), ( localize(18012, "High Contrast Default"))];
        ...
        const rows = ( colors.sort((a, b) => a.id.localeCompare(b.id)).map(color => {
            return [( new MarkdownString()).appendMarkdown(`\`${color.id}\``), color.description, toColor(color.defaults.dark) ?? ( new MarkdownString()).appendMarkdown(`\`${color.defaults.dark}\``), toColor(color.defaults.light) ?? ( new MarkdownString()).appendMarkdown(`\`${color.defaults.light}\``), toColor(color.defaults.highContrast) ?? ( new MarkdownString()).appendMarkdown(`\`${color.defaults.highContrast}\``)];
        }));
        return {
            data: {
                headers,
...
let ColorExtensionPointWorkbenchContribution = class ColorExtensionPointWorkbenchContribution {
    static {
        this.ID = "workbench.contrib.colorExtensionPoint";
    }
...
ColorExtensionPointWorkbenchContribution = ( __decorate([( __param(0, IInstantiationService))], ColorExtensionPointWorkbenchContribution));
registerWorkbenchContribution2(
    ColorExtensionPointWorkbenchContribution.ID,
