...
import { THEME_SCOPE_OPEN_PAREN, THEME_SCOPE_CLOSE_PAREN, THEME_SCOPE_WILDCARD, themeScopeRegex, ExtensionData } from './workbenchThemeService.js';
import { convertSettings } from './themeCompatibility.js';
import { localize } from '../../../../nls.js';
import { isString, isObject, isBoolean } from '../../../../base/common/types.js';
import { extname, joinPath, dirname } from '../../../../base/common/resources.js';
import { Extensions, DEFAULT_COLOR_CONFIG_VALUE } from '../../../../platform/theme/common/colorUtils.js';
...
import { getTokenClassificationRegistry, TokenStyle, parseClassifierString, SemanticTokenRule } from '../../../../platform/theme/common/tokenClassificationRegistry.js';
import { createMatchers } from './textMateScopeMatcher.js';
import { CharCode } from '../../../../base/common/charCode.js';
import { StorageScope, StorageTarget } from '../../../../platform/storage/common/storage.js';
import { ColorScheme, ThemeTypeSelector } from '../../../../platform/theme/common/theme.js';
import { MetadataConsts, FontStyle, ColorId } from '../../../../editor/common/encodedTokenAttributes.js';
...
class ColorThemeData {
    ...
    resolveTokenStyleValue(tokenStyleValue) {
        if (tokenStyleValue === undefined) {
            ...
            const {
                ...
                modifiers,
                language
            } = parseClassifierString(tokenStyleValue, "");
    ...
    getTokenFontIndex() {
        if (!this.tokenFontIndex) {
            ...
            this.tokenColors.forEach(
                r => index.add(r.settings.fontFamily, r.settings.fontSize, r.settings.lineHeight)
            );
    ...
    getTokenStyleMetadata(
        ...
        const {
            type,
            language
        } = parseClassifierString(typeWithLanguage, defaultLanguage);
...
async function _loadColorTheme(extensionResourceLoaderService, themeLocation, result) {
    if (extname(themeLocation) === ".json") {
        ...
        if (errors.length > 0) {
            ...
            return Promise.reject(( new Error(( localize(18015, "Invalid format for JSON theme file: Object expected.")))));
        }
    ...
    return extensionResourceLoaderService.readExtensionResource(themeLocation).then(content => {
        try {
            ...
            if (!Array.isArray(settings)) {
                return Promise.reject(( new Error(( localize(18019, "Problem parsing tmTheme file: {0}. 'settings' is not array.")))));
            }
            convertSettings(settings, result);
            return Promise.resolve(null);
        } catch (e) {
            return Promise.reject(( new Error(( localize(18020, "Problems parsing tmTheme file: {0}", e.message)))));
        }
    }, error => {
        return Promise.reject(( new Error(( localize(
            18021,
...
function isSemanticTokenColorizationSetting(style) {
    return style && (isString(style.foreground) || isString(style.fontStyle) || isBoolean(style.italic) || isBoolean(style.underline) || isBoolean(style.strikethrough) || isBoolean(style.bold));
}
