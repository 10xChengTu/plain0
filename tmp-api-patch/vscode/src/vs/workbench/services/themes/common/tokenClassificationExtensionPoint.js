...
const tokenTypeExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(18148, "Contributes semantic token types.")),
        type: "array",
        items: {
            type: "object",
            properties: {
                id: {
                    type: "string",
                    description: ( localize(18149, "The identifier of the semantic token type")),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(18150, "Identifiers should be in the form letterOrDigit[_-letterOrDigit]*"))
                },
                superType: {
                    type: "string",
                    description: ( localize(18151, "The super type of the semantic token type")),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(18152, "Super types should be in the form letterOrDigit[_-letterOrDigit]*"))
                },
                description: {
                    type: "string",
                    description: ( localize(18153, "The description of the semantic token type"))
                }
            }
        }
    }
...
const tokenModifierExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(18154, "Contributes semantic token modifiers.")),
        type: "array",
        items: {
            type: "object",
            properties: {
                id: {
                    type: "string",
                    description: ( localize(18155, "The identifier of the semantic token modifier")),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(18156, "Identifiers should be in the form letterOrDigit[_-letterOrDigit]*"))
                },
                description: {
                    type: "string",
                    description: ( localize(18157, "The description of the semantic token modifier"))
                }
            }
        }
    }
...
const tokenStyleDefaultsExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(18158, "Contributes semantic token scope maps.")),
        type: "array",
        items: {
            type: "object",
            properties: {
                language: {
                    description: ( localize(18159, "Lists the languge for which the defaults are.")),
                    type: "string"
                },
                scopes: {
    ...
    constructor() {
        ...
        tokenTypeExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(18165, "'configuration.semanticTokenType' must be an array")));
                    return;
                }
        ...
        tokenModifierExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(18166, "'configuration.semanticTokenModifier' must be an array")));
                    return;
                }
                for (const contribution of extensionValue) {
        ...
        tokenStyleDefaultsExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(18167, "'configuration.semanticTokenScopes' must be an array")));
                    return;
                }
                for (const contribution of extensionValue) {
                    if (contribution.language && typeof contribution.language !== "string") {
                        collector.error(( localize(18168, "'configuration.semanticTokenScopes.language' must be a string")));
                        continue;
                    }
...
let TokenClassificationExtensionPointWorkbenchContribution = class TokenClassificationExtensionPointWorkbenchContribution {
    static {
        this.ID = "workbench.contrib.tokenClassificationExtensionPoint";
    }
