    ...
    async handleURL(uri, options) {
        ...
        if (!trusted) {
            ...
            const result = await this.dialogService.confirm({
                message: ( localize(17633, "Allow '{0}' extension to open this URI?", extensionDisplayName)),
                checkbox: {
                    label: ( localize(17634, "Do not ask me again for this extension"))
                },
                primaryButton: ( localize(17635, "&&Open")),
                custom: {
                    markdownDetails: [{
                        markdown: ( new MarkdownString(`<div title="${uriString}" aria-label='${uriString}'>${uriLabel}</div>`, {
                            supportHtml: true
                        }))
                    }]
                }
    ...
    async handleUnhandledURL(uri, extensionId, options) {
        try {
            await this.commandService.executeCommand("workbench.extensions.installExtension", extensionId, {
                justification: {
                    reason: `${( localize(17636, "This extension wants to open a URI:"))}\n${( uri.toString())}`,
                    action: ( localize(17637, "Open URI"))
                },
                enable: true,
        ...
        if (extension) {
            ...
            const result = await this.dialogService.confirm({
                ...
                primaryButton: ( localize(17639, "&&Reload Window and Open"))
            });
            if (!result.confirmed) {
                return;
            }
...
ExtensionUrlHandler = ( __decorate([( __param(0, IURLService)), ( __param(1, IExtensionService)), ( __param(2, IDialogService)), ( __param(3, ICommandService)), ( __param(4, IHostService)), ( __param(5, IStorageService)), ( __param(6, IConfigurationService)), ( __param(7, INotificationService)), ( __param(8, IProductService))], ExtensionUrlHandler));
...
ExtensionUrlBootstrapHandler = ExtensionUrlBootstrapHandler_1 = ( __decorate([( __param(0, IURLService))], ExtensionUrlBootstrapHandler));
...
class ManageAuthorizedExtensionURIsAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(17640, "Manage Authorized Extension URIs...")),
            category: ( localize2(17641, "Extensions")),
            menu: {
    ...
    async run(accessor) {
        ...
        const items = ( storage.extensions.map(label => ({
            label,
            picked: true
        })));
        if (items.length === 0) {
            await quickInputService.pick([{
                label: ( localize(17642, "There are currently no authorized extension URIs."))
            }]);
            return;
        }
