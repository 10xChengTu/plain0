...
let FilesConfigurationService = class FilesConfigurationService extends Disposable {
    ...
    static {
        this.READONLY_MESSAGES = {
            ...
            fileReadonly: {
                value: ( localize(17751, "Editor is read-only because the file is read-only.")),
                isTrusted: true
            }
        };
    }
        ...
        if (resource) {
            ...
            if (!resourceAutoSaveConfiguration) {
                resourceAutoSaveConfiguration = this.computeAutoSaveConfiguration(
                    resource,
                    this.textResourceConfigurationService.getValue(resource, "files")
                );
    ...
    getAutoSaveMode(resourceOrEditor, saveReason) {
        ...
        if (resource && ( this.autoSaveAfterShortDelayOverrides.has(resource))) {
            return {
                mode: AutoSaveMode.AFTER_SHORT_DELAY
            };
        }
        ...
        case "afterDelay":
            if (typeof autoSaveConfiguration.autoSaveDelay === "number" && autoSaveConfiguration.autoSaveDelay <= FilesConfigurationService_1.DEFAULT_AUTO_SAVE_DELAY) {
                return {
                    mode: autoSaveConfiguration.autoSaveWhenNoErrors ? AutoSaveMode.AFTER_LONG_DELAY : AutoSaveMode.AFTER_SHORT_DELAY
                };
            }
            return {
                mode: AutoSaveMode.AFTER_LONG_DELAY
            };
        case "onFocusChange":
            return {
