...
let TextFileEditorModel = class TextFileEditorModel extends BaseTextEditorModel {
    ...
    static {
        this.TEXTFILE_SAVE_ENCODING_SOURCE = SaveSourceRegistry.registerSource("textFileEncoding.source", ( localize(17963, "File Encoding Changed")));
    }
    static {
    ...
    registerListeners() {
        ...
        this._register(
            this.filesConfigurationService.onDidChangeFilesAssociation(() => this.onDidChangeFilesAssociation())
        );
        this._register(
    ...
    async backup(token) {
        ...
        const content = await this.textFileService.getEncodedReadable(this.resource, this.createSnapshot() ?? undefined, {
            encoding: UTF8
        });
        return {
            meta,
    ...
    async autoDetectLanguage() {
        ...
        if (
        this.resource.scheme === this.pathService.defaultUriScheme && (!languageId || languageId === PLAINTEXT_LANGUAGE_ID) && !this.resourceHasExtension) {
    ...
    async doSave(options) {
        ...
        return this.progressService.withProgress({
            title: ( localize(17964, "Saving '{0}'", this.name)),
    ...
    doSaveSequential(versionId, options, progress, saveCancellation) {
        return this.saveSequentializer.run(versionId, (async () => {
                ...
                try {
                    if (options.reason === SaveReason.AUTO && typeof this.lastModelContentChangeFromUndoRedo === "number") {
                        ...
                        if (timeFromUndoRedoToSave < TextFileEditorModel_1.UNDO_REDO_SAVE_PARTICIPANTS_AUTO_SAVE_THROTTLE_THRESHOLD) {
                            await timeout(
                                TextFileEditorModel_1.UNDO_REDO_SAVE_PARTICIPANTS_AUTO_SAVE_THROTTLE_THRESHOLD - timeFromUndoRedoToSave
                            );
                        }
                    }
            ...
            this.inErrorMode = false;
            progress.report({
                message: ( localize(17965, "Writing into file..."))
            });
    ...
    async onMaybeShouldChangeEncoding() {
        ...
        const {
            encoding
        } = await this.textFileService.encoding.getPreferredReadEncoding(this.resource);
    ...
    async setEncodingInternal(encoding, mode) {
        if (mode === EncodingMode.Encode) {
            ...
            if (!this.inConflictMode) {
                await this.save({
                    source: TextFileEditorModel_1.TEXTFILE_SAVE_ENCODING_SOURCE
                });
            }
        }
        else {
...
TextFileEditorModel = TextFileEditorModel_1 = ( __decorate([( __param(3, ILanguageService)), ( __param(4, IModelService)), ( __param(5, IFileService)), ( __param(6, ITextFileService)), ( __param(7, IWorkingCopyBackupService)), ( __param(8, ILogService)), ( __param(9, IWorkingCopyService)), ( __param(10, IFilesConfigurationService)), ( __param(11, ILabelService)), ( __param(12, ILanguageDetectionService)), ( __param(13, IAccessibilityService)), ( __param(14, IPathService)), ( __param(15, IExtensionService)), ( __param(16, IProgressService))], TextFileEditorModel));
