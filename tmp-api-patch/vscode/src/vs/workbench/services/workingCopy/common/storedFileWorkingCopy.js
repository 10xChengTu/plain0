...
let StoredFileWorkingCopy = class StoredFileWorkingCopy extends ResourceWorkingCopy {
    ...
    registerListeners() {
        this._register(
            this.filesConfigurationService.onDidChangeReadonly(() => this._onDidChangeReadonly.fire())
        );
    }
    isDirty() {
        return this.dirty;
    ...
    installModelListeners(model) {
        this._register(
            model.onDidChangeContent(e => this.onModelContentChanged(model, e.isUndoing || e.isRedoing))
        );
    ...
    async save(options = Object.create(null)) {
        ...
        if ((this.hasState(StoredFileWorkingCopyState.CONFLICT) || this.hasState(StoredFileWorkingCopyState.ERROR)) && (options.reason === SaveReason.AUTO || options.reason === SaveReason.FOCUS_CHANGE || options.reason === SaveReason.WINDOW_CHANGE)) {
            this.trace(
    ...
    async doSave(options) {
        ...
        return this.progressService.withProgress({
            title: ( localize(18296, "Saving '{0}'", this.name)),
    ...
    doSaveSequential(versionId, options, progress, saveCancellation) {
        return this.saveSequentializer.run(versionId, (async () => {
                ...
                try {
                    if (options.reason === SaveReason.AUTO && typeof this.lastContentChangeFromUndoRedo === "number") {
                        ...
                        if (timeFromUndoRedoToSave < StoredFileWorkingCopy_1.UNDO_REDO_SAVE_PARTICIPANTS_AUTO_SAVE_THROTTLE_THRESHOLD) {
                            await timeout(
                                StoredFileWorkingCopy_1.UNDO_REDO_SAVE_PARTICIPANTS_AUTO_SAVE_THROTTLE_THRESHOLD - timeFromUndoRedoToSave
                            );
                        }
                    }
            ...
            this.inErrorMode = false;
            progress.report({
                message: ( localize(18297, "Writing into file..."))
            });
                ...
                try {
                    ...
                    if (typeof resolvedFileWorkingCopy.model.save === "function") {
                        ...
                        const snapshot = await raceCancellation(
                            resolvedFileWorkingCopy.model.snapshot(SnapshotContext.Save, saveCancellation.token),
    ...
    doHandleSaveError(error, options) {
        ...
        if (fileOperationError.fileOperationResult === FileOperationResult.FILE_MODIFIED_SINCE) {
            ...
            primaryActions.push(toAction({
                ...
                label: ( localize(18299, "Overwrite")),
                run: () => this.save({
                    ...options,
            ...
            if (canSaveElevated && (isPermissionDenied || triedToUnlock)) {
                primaryActions.push(toAction({
                    ...
                    label: triedToUnlock ? isWindows ? ( localize(18301, "Overwrite as Admin...")) : ( localize(18302, "Overwrite as Sudo...")) : isWindows ? ( localize(18303, "Retry as Admin...")) : ( localize(18304, "Retry as Sudo...")),
                    run: () => {
                        this.save({
                            ...options,
                            writeElevated: true,
        ...
        const listener = this._register(
            Event.once(Event.any(this.onDidSave, this.onDidRevert))(() => handle.close())
        );
...
StoredFileWorkingCopy = StoredFileWorkingCopy_1 = ( __decorate([( __param(5, IFileService)), ( __param(6, ILogService)), ( __param(7, IWorkingCopyFileService)), ( __param(8, IFilesConfigurationService)), ( __param(9, IWorkingCopyBackupService)), ( __param(10, IWorkingCopyService)), ( __param(11, INotificationService)), ( __param(12, IWorkingCopyEditorService)), ( __param(13, IEditorService)), ( __param(14, IElevatedFileService)), ( __param(15, IProgressService))], StoredFileWorkingCopy));
