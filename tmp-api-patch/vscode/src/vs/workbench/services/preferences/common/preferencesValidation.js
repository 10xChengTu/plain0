...
import { isUndefinedOrNull, isStringArray, isString, isObject } from '../../../../base/common/types.js';
...
function createValidator(prop) {
    ...
    return value => {
        ...
        if (prop.type === "boolean" && value !== true && value !== false) {
            errors.push(( localize(17860, "Incorrect type. Expected \"boolean\".")));
        }
        if (isNumeric) {
            if (isNullOrEmpty(value) || typeof value === "boolean" || Array.isArray(value) || isNaN(+value)) {
                errors.push(( localize(17861, "Value must be a number.")));
            } else {
        ...
        if (prop.type === "string") {
            if (prop.enum && !isStringArray(prop.enum)) {
                ...
                errors.push(( localize(17863, "Incorrect type. Expected \"string\".")));
            } else {
...
function getStringValidators(prop) {
    ...
    return [{
        ...
        message: ( localize(17866, "Value must be {0} or fewer characters long.", prop.maxLength))
    }, {
        enabled: prop.minLength !== undefined,
        isValid: (value => value.length >= prop.minLength),
        message: ( localize(17867, "Value must be {0} or more characters long.", prop.minLength))
    }, {
        enabled: patternRegex !== undefined,
        isValid: (value => patternRegex.test(value)),
        message: prop.patternErrorMessage || ( localize(17868, "Value must match regex `{0}`.", prop.pattern))
    }, {
        enabled: prop.format === "color-hex",
        isValid: (value => Color.Format.CSS.parseHex(value)),
        message: ( localize(17869, "Invalid color format. Use #RGB, #RGBA, #RRGGBB or #RRGGBBAA."))
    }, {
        enabled: prop.format === "uri" || prop.format === "uri-reference",
        isValid: (value => !!value.length),
        message: ( localize(17870, "URI expected."))
    }, {
        enabled: prop.format === "uri" || prop.format === "uri-reference",
        isValid: (value => uriRegex.test(value)),
        message: ( localize(17871, "URI is expected."))
    }, {
        ...
        message: ( localize(17872, "URI with a scheme is expected."))
    }, {
        ...
        if (propItems && !Array.isArray(propItems.type)) {
            ...
            return value => {
                ...
                if (!Array.isArray(value)) {
                    message += ( localize(17880, "Incorrect type. Expected an array."));
                    message += "\n";
                    return message;
                }
                ...
                if (prop.uniqueItems) {
                    if (( new Set(arrayValue)).size < arrayValue.length) {
                        message += ( localize(17881, "Array has duplicate items"));
                        message += "\n";
                    }
                }
                if (prop.minItems && arrayValue.length < prop.minItems) {
                    message += ( localize(17882, "Array must have at least {0} items", prop.minItems));
                    message += "\n";
                }
                if (prop.maxItems && arrayValue.length > prop.maxItems) {
                    message += ( localize(17883, "Array must have at most {0} items", prop.maxItems));
                    message += "\n";
                }
                if (propItems.type === "string") {
                    if (!isStringArray(arrayValue)) {
                        message += ( localize(17884, "Incorrect type. Expected a string array."));
                        message += "\n";
                        return message;
                    }
...
function getObjectValidator(prop) {
    if (prop.type === "object") {
        ...
        return value => {
            ...
            if (!isObject(value)) {
                errors.push(( localize(17887, "Incorrect type. Expected an object.")));
            } else {
                ( Object.keys(value)).forEach(key => {
                    ...
                    if (propertyNames?.pattern && !propertyNamesErrorShown) {
                        ...
                        if (!patternRegex.test(key)) {
                            const errorMessage = propertyNames.patternErrorMessage || ( localize(17888, "Property name must match pattern `{0}`.", propertyNames.pattern));
                    ...
                    if (additionalProperties === false) {
                        errors.push(( localize(17889, "Property {0} is not allowed.\n", key)));
...
export { createValidator, getInvalidTypeError, validatePropertyName };
