...
import { or, matchesBaseContiguousSubString, matchesWords, matchesCamelCase, matchesContiguousSubString } from '../../../../base/common/filters.js';
import { UserSettingsLabelProvider, AriaLabelProvider, UILabelProvider } from '../../../../base/common/keybindingLabels.js';
...
const SOURCE_SYSTEM = ( localize(17844, "System"));
const SOURCE_EXTENSION = ( localize(17845, "Extension"));
const SOURCE_USER = ( localize(17846, "User"));
...
let KeybindingsEditorModel = KeybindingsEditorModel_1 = class KeybindingsEditorModel extends EditorModel {
    ...
    async resolve(
        ...
        for (const keybinding of this.keybindingsService.getKeybindings()) {
            if (keybinding.command) {
                this._keybindingItemsSortedByPrecedence.push(
                    KeybindingsEditorModel_1.toKeybindingEntry(keybinding.command, keybinding, actionLabels, extensions)
                );
        ...
        for (const command of getAllUnboundCommands(boundCommands)) {
            ...
            this._keybindingItemsSortedByPrecedence.push(
                KeybindingsEditorModel_1.toKeybindingEntry(command, keybindingItem, actionLabels, extensions)
            );
        }
        this._keybindingItemsSortedByPrecedence = distinct(
            this._keybindingItemsSortedByPrecedence,
            keybindingItem => KeybindingsEditorModel_1.getId(keybindingItem)
        );
    ...
    static getCommandDefaultLabel(menuCommand) {
        if (!Language.isDefaultVariant()) {
            if (menuCommand && menuCommand.title && menuCommand.title.original) {
                ...
                return category ? ( localize(17847, "{0}: {1}", category, title)) : title;
            }
        }
        return null;
    }
...
KeybindingsEditorModel = KeybindingsEditorModel_1 = ( __decorate([( __param(1, IKeybindingService)), ( __param(2, IExtensionService))], KeybindingsEditorModel));
class KeybindingItemMatches {
        ...
        if (!completeMatch) {
            ...
            if (isString(keybindingItem.source)) {
                this.sourceMatches = this.matches(
                    ...
                    (word, wordToMatchAgainst) => matchesWords(word, keybindingItem.source, true),
                    words
                );
            } else {
    ...
    matchesKeybinding(keybinding, searchValue, words, completeMatch) {
        ...
        return this.hasAnyMatch(firstPartMatch) || this.hasAnyMatch(chordPartMatch) ? {
            firstPart: firstPartMatch,
            chordPart: chordPartMatch
        } : null;
    }
    ...
    wordMatchesAltModifier(word) {
        ...
        if (equalsIgnoreCase(( localize(17848, "option")), word)) {
            return true;
        }
        return false;
    }
    ...
    wordMatchesMetaModifier(word) {
        ...
        if (equalsIgnoreCase(( localize(17849, "meta")), word)) {
            return true;
        }
        return false;
    }
