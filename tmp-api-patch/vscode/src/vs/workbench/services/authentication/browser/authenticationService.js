...
import { Disposable, DisposableMap, toDisposable, DisposableStore, isDisposable } from '../../../../base/common/lifecycle.js';
import { isFalsyOrWhitespace, equalsIgnoreCase } from '../../../../base/common/strings.js';
import { isString } from '../../../../base/common/types.js';
import { localize } from '../../../../nls.js';
import '../../../../platform/instantiation/common/extensions.js';
import { IAuthenticationAccessService } from './authenticationAccessService.service.js';
import { isAuthenticationWwwAuthenticateRequest } from '../common/authentication.js';
...
const authenticationDefinitionSchema = {
    ...
    properties: {
        id: {
            type: "string",
            description: ( localize(17370, "The id of the authentication provider."))
        },
        label: {
            type: "string",
            description: ( localize(17371, "The human readable name of the authentication provider."))
        },
...
const authenticationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(17374, "Contributes authentication")),
        type: "array",
...
let AuthenticationService = class AuthenticationService extends Disposable {
    ...
    _registerAuthenticationExtensionPointHandler() {
        this._register(authenticationExtPoint.setHandler((
            _extensions,
            {
                added,
                removed
            ...
            added.forEach(point => {
                for (const provider of point.value) {
                    if (isFalsyOrWhitespace(provider.id)) {
                        point.collector.error(( localize(17375, "An authentication contribution must specify an id.")));
                        continue;
                    }
                    if (isFalsyOrWhitespace(provider.label)) {
                        point.collector.error(( localize(17376, "An authentication contribution must specify a label.")));
                        continue;
                    }
    ...
    registerDeclaredAuthenticationProvider(provider) {
        if (isFalsyOrWhitespace(provider.id)) {
            throw ( new Error(( localize(17375, "An authentication contribution must specify an id."))));
        }
        if (isFalsyOrWhitespace(provider.label)) {
            throw ( new Error(( localize(17376, "An authentication contribution must specify a label."))));
        }
        if (( this.declaredProviders.some(p => p.id === provider.id))) {
    ...
    registerAuthenticationProvider(id, authenticationProvider) {
        ...
        disposableStore.add(
            authenticationProvider.onDidChangeSessions(e => this._onDidChangeSessions.fire({
                providerId: id,
        ...
        if (authProvider) {
            ...
            if (server) {
                if (!this.matchesProvider(authProvider, server)) {
                    throw ( new Error(
                        `The authentication provider '${id}' does not support the authorization server '${( server.toString(true))}'.`
                    ));
                }
            }
            if (isAuthenticationWwwAuthenticateRequest(scopeListOrRequest)) {
                if (!authProvider.getSessionsFromChallenges) {
                    throw ( new Error(
                        `The authentication provider '${id}' does not support getting sessions from challenges.`
                    ));
                }
                return await authProvider.getSessionsFromChallenges({
                    challenges: parseWWWAuthenticateHeader(scopeListOrRequest.wwwAuthenticate),
                    fallbackScopes: scopeListOrRequest.fallbackScopes
                }, {
                    ...options
                });
            }
            return await authProvider.getSessions(scopeListOrRequest ? [...scopeListOrRequest] : undefined, {
                ...options
            });
        } else {
            throw ( new Error(`No authentication provider '${id}' is currently registered.`));
        ...
        if (authProvider) {
            if (isAuthenticationWwwAuthenticateRequest(scopeListOrRequest)) {
                if (!authProvider.createSessionFromChallenges) {
                    throw ( new Error(
                        `The authentication provider '${id}' does not support creating sessions from challenges.`
                    ));
                }
            ...
            return await authProvider.createSession([...scopeListOrRequest], {
                ...options
            });
        } else {
            throw ( new Error(`No authentication provider '${id}' is currently registered.`));
    ...
    async getOrActivateProviderIdForServer(authorizationServer, resourceServer) {
        ...
        const providers =
        this._declaredProviders.filter(p => !( this._authenticationProviders.has(p.id))).filter(p => !!p.authorizationServerGlobs?.some(i => match(i, authServerStr, {
            ignoreCase: true
        })));
    ...
    matchesProvider(provider, authorizationServer, resourceServer) {
        ...
        if (provider.authorizationServers) {
            ...
            for (const server of provider.authorizationServers) {
                ...
                if (equalsIgnoreCase(str, authServerStr) || match(str, authServerStr, {
                    ignoreCase: true
                })) {
                    return true;
                }
    ...
    async tryActivateProvider(providerId, activateImmediate) {
        ...
        try {
            ...
            const providerRegistered = raceCancellation(Event.toPromise(
                Event.filter(this.onDidRegisterAuthenticationProvider, e => e.id === providerId, store),
                store
            ...
            if (!result) {
                throw ( new Error(
                    `Timed out waiting for authentication provider '${providerId}' to register.`
                ));
            }
...
AuthenticationService = ( __decorate([( __param(0, IExtensionService)), ( __param(1, IAuthenticationAccessService)), ( __param(2, IBrowserWorkbenchEnvironmentService)), ( __param(3, ILogService))], AuthenticationService));
