...
var ChatEntitlementService_1, ChatEntitlementContext_1;
...
(function(ChatEntitlementContextKeys) {
    ...
    ChatEntitlementContextKeys.clientByokEnabled = ( new RawContextKey("github.copilot.clientByokEnabled", true, true));
    ChatEntitlementContextKeys.hasByokModels = ( new RawContextKey("github.copilot.hasByokModels", false, true));
...
(function(ChatEntitlement) {
    ...
    ChatEntitlement[ChatEntitlement["Max"] = 11] = "Max";
})(ChatEntitlement || (ChatEntitlement = {}));
function chatRequiresSetup(context) {
    return (
    (!context.completed && !context.hasByokModels) || context.disabled || context.untrusted || context.entitlement === ChatEntitlement.Available ||
    (
    context.entitlement === ChatEntitlement.Unknown && !context.anonymous && !context.hasByokModels));
}
function isProUser(chatEntitlement) {
    return chatEntitlement === ChatEntitlement.EDU || chatEntitlement === ChatEntitlement.Pro || chatEntitlement === ChatEntitlement.ProPlus || chatEntitlement === ChatEntitlement.Max || chatEntitlement === ChatEntitlement.Business || chatEntitlement === ChatEntitlement.Enterprise;
}
function getChatPlanName(chatEntitlement) {
    switch (chatEntitlement) {
    case ChatEntitlement.EDU:
        return localize(17383, "Copilot Student");
    case ChatEntitlement.Pro:
        return localize(17384, "Copilot Pro");
    case ChatEntitlement.ProPlus:
        return localize(17385, "Copilot Pro+");
    case ChatEntitlement.Max:
        return localize(17386, "Copilot Max");
    case ChatEntitlement.Business:
        return localize(17387, "Copilot Business");
    case ChatEntitlement.Enterprise:
        return localize(17388, "Copilot Enterprise");
    default:
        return localize(17389, "Copilot Free");
    }
}
...
let ChatEntitlementService = class ChatEntitlementService extends Disposable {
    ...
    constructor(
        ...
        const cachedUBB = this.storageService.getBoolean(ChatEntitlementService_1.CACHED_UBB_STORAGE_KEY, StorageScope.PROFILE);
        this._quotas = cachedUBB !== undefined ? {
            usageBasedBilling: cachedUBB
        } : {};
        ...
        if (this.contextKeyService.getContextKeyValue(ChatEntitlementContextKeys.clientByokEnabled.key) === undefined) {
            ChatEntitlementContextKeys.clientByokEnabled.bindTo(this.contextKeyService);
        }
        this.onDidChangeEntitlement = ( Event.map(Event.filter(
        ...
        this.onDidChangeSentiment = ( Event.map(Event.filter(
            this.contextKeyService.onDidChangeContext,
            e => e.affectsSome(( new Set([
        ...
        this.requests = ( new Lazy(() => this._register(
            instantiationService.createInstance(ChatEntitlementRequests, context.value, {
    ...
    registerListeners() {
        const quotaExceededSet = ( new Set([
            this.ExtensionQuotaContextKeys.chatQuotaExceeded,
    ...
    acceptQuotas(quotas) {
        ...
        if (oldQuota.usageBasedBilling !== quotas.usageBasedBilling) {
            if (quotas.usageBasedBilling !== undefined) {
                this.storageService.store(
                    ChatEntitlementService_1.CACHED_UBB_STORAGE_KEY,
                    quotas.usageBasedBilling,
                    StorageScope.PROFILE,
                    StorageTarget.MACHINE
                );
            } else {
                this.storageService.remove(ChatEntitlementService_1.CACHED_UBB_STORAGE_KEY, StorageScope.PROFILE);
            }
        }
        ...
        const {
            changed: chatChanged
        } = this.compareQuotas(oldQuota.chat, quotas.chat);
        const {
            changed: completionsChanged
        } = this.compareQuotas(oldQuota.completions, quotas.completions);
        const {
            changed: premiumChatChanged
        } = this.compareQuotas(oldQuota.premiumChat, quotas.premiumChat);
        ...
        if (quotas.additionalUsageEnabled && quotas.premiumChat?.percentRemaining === 0 && oldQuota.premiumChat?.percentRemaining !== undefined && oldQuota.premiumChat.percentRemaining > 0) {
    ...
    updateContextKeys() {
        ...
        this.chatQuotaExceededContextKey.set(
            chatExhausted || (premiumChatExhausted && (isManagedPlan || !additionalUsageEnabled))
        );
...
ChatEntitlementService = ChatEntitlementService_1 = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, IProductService)), ( __param(2, IWorkbenchEnvironmentService)), ( __param(3, IContextKeyService)), ( __param(4, IConfigurationService)), ( __param(5, ITelemetryService)), ( __param(6, ILogService)), ( __param(7, IStorageService))], ChatEntitlementService));
function parseQuotas(entitlementsData) {
    ...
    if (entitlementsData.monthly_quotas?.chat && typeof entitlementsData.limited_user_quotas?.chat === "number") {
        quotas.chat = {
            percentRemaining: Math.min(100, Math.max(
                0,
                (entitlementsData.limited_user_quotas.chat / entitlementsData.monthly_quotas.chat) * 100
            )),
            unlimited: false
        };
    }
    if (entitlementsData.monthly_quotas?.completions && typeof entitlementsData.limited_user_quotas?.completions === "number") {
        quotas.completions = {
            percentRemaining: Math.min(100, Math.max(
                0,
                (entitlementsData.limited_user_quotas.completions / entitlementsData.monthly_quotas.completions) * 100
            )),
            unlimited: false
        };
    }
...
let ChatEntitlementRequests = class ChatEntitlementRequests extends Disposable {
    ...
    async doResolveEntitlement(defaultAccount, token) {
        ...
        if (!entitlementsData) {
            ...
            return {
                entitlement: entitlementsData === null ? ChatEntitlement.Unknown : ChatEntitlement.Unresolved
            };
        }
        let entitlement;
        ...
        this.logService.trace(
            `[chat entitlement]: resolved to ${entitlements.entitlement}, quotas: ${JSON.stringify(entitlements.quotas)}`
        );
    ...
    update(state) {
        ...
        this.context.update({
            ...
            sku: this.state.sku,
            copilotTrackingId: this.state.copilotTrackingId
        });
        if (state.quotas) {
    ...
    async doSignUpFree(sessions) {
        ...
        const response = await this.request(
            defaultChatAgent.entitlementSignupLimitedUrl,
            "POST",
            body,
            sessions,
            CancellationToken.None,
            "chatEntitlementService.signUpFree"
        );
        if (!response) {
            const retry = await this.onUnknownSignUpError(( localize(17390, "No response received.")), "[chat entitlement] sign-up: no response");
            return retry ? this.doSignUpFree(sessions) : {
                errorCode: 1
            };
        }
                ...
                try {
                    ...
                    if (responseText) {
                        ...
                        if (typeof responseError.message === "string" && responseError.message) {
                            this.onUnprocessableSignUpError(
                                `[chat entitlement] sign-up: unprocessable entity (${responseError.message})`,
                                responseError.message
                            );
                            return {
                                errorCode: response.res.statusCode
                            };
                        }
                    }
                } catch (error) {}
            }
            const retry = await this.onUnknownSignUpError(( localize(17391, "Unexpected status code {0}.", response.res.statusCode)), `[chat entitlement] sign-up: unexpected status code ${response.res.statusCode}`);
            return retry ? this.doSignUpFree(sessions) : {
                errorCode: response.res.statusCode
            };
        }
        ...
        if (!responseText) {
            const retry = await this.onUnknownSignUpError(( localize(17392, "Response has no contents.")), "[chat entitlement] sign-up: response has no content");
            return retry ? this.doSignUpFree(sessions) : {
                errorCode: 2
            };
        }
        ...
        try {
            ...
            const retry = await this.onUnknownSignUpError(( localize(17393, "Invalid response contents.")), `[chat entitlement] sign-up: error parsing response (${err})`);
            return retry ? this.doSignUpFree(sessions) : {
                errorCode: 3
            };
        }
    ...
    async getSessions() {
        ...
        return [
            ...(await this.authenticationService.getSessions(this.defaultAccountService.getDefaultAccountAuthenticationProvider().id))
        ];
    }
    async onUnknownSignUpError(detail, logMessage) {
        ...
        if (!this.lifecycleService.willShutdown) {
            const {
                ...
                detail,
                primaryButton: ( localize(17395, "Retry"))
            });
            return confirmed;
        }
        return false;
    ...
    onUnprocessableSignUpError(logMessage, logDetails) {
        ...
        if (!this.lifecycleService.willShutdown) {
            this.dialogService.prompt({
                ...
                detail: logDetails,
                buttons: [{
                    label: ( localize(17397, "OK")),
                    run: () => {}
                }, {
                    label: ( localize(17398, "Learn More")),
...
ChatEntitlementRequests = ( __decorate([( __param(2, ITelemetryService)), ( __param(3, ILogService)), ( __param(4, IRequestService)), ( __param(5, IDialogService)), ( __param(6, IOpenerService)), ( __param(7, ILifecycleService)), ( __param(8, IDefaultAccountService)), ( __param(9, IAuthenticationService))], ChatEntitlementRequests));
let ChatEntitlementContext = class ChatEntitlementContext extends Disposable {
    ...
    constructor(
        ...
        const migrated = this.storageService.getBoolean(
            ChatEntitlementContext_1.CHAT_ENTITLEMENT_CONTEXT_MIGRATED_STORAGE_KEY,
            StorageScope.PROFILE
        ) === true;
        if (!migrated) {
            this.storageService.store(
    ...
    withConfiguration(state) {
        if (this._forceHidden || this.configurationService.getValue(ChatEntitlementContext_1.CHAT_DISABLED_CONFIGURATION_KEY) === true) {
            return {
                ...state,
                hidden: true
            };
    ...
    updateContextSync() {
        ...
        this.isInternalContextKey.set(Boolean(state.organisations?.some(
            org => org === "github" || org === "microsoft" || org === "ms-copilot" || org === "MicrosoftCopilot"
        )));
...
ChatEntitlementContext = ChatEntitlementContext_1 = ( __decorate([( __param(0, IContextKeyService)), ( __param(1, IStorageService)), ( __param(2, ILogService)), ( __param(3, IConfigurationService)), ( __param(4, ITelemetryService))], ChatEntitlementContext));

export { ChatEntitlement, ChatEntitlementContext, ChatEntitlementContextKeys, ChatEntitlementRequests, ChatEntitlementService, chatRequiresSetup, getChatPlanName, isProUser, parseQuotas };
