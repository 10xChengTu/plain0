...
import { ThrottledDelayer, RunOnceScheduler, timeout } from '../../../../base/common/async.js';
...
import { asJson, isSuccess, isClientError, readHeader, retryAfterFromHeaders } from '../../../../platform/request/common/request.js';
...
let DefaultAccountProvider = class DefaultAccountProvider extends Disposable {
    ...
    constructor(
        ...
        this.initPromise = this.init().finally(() => {
            this.telemetryService.publicLog2("defaultaccount:status", {
                status: this.defaultAccount ? "available" : "unavailable",
                initial: true
            });
            ...
            try {
                ...
                if (accountId && policyData) {
                    this.logService.debug(
                        "[DefaultAccount] Initializing with cached policy data (migrating old format)"
                    );
                    const result = {
                        accountPolicyData: {
                            accountId,
                ...
                const {
                    accountPolicyData,
                    copilotTokenInfo: wrappedCopilotTokenInfo
                } = parsed;
                ...
                this.logService.error(
                    "[DefaultAccount] Failed to parse cached policy data",
    ...
    async init() {
        ...
        this._register(this.authenticationService.onDidChangeSessions(e => {
            ...
            if (this.defaultAccount && e.event.removed?.some(session => session.id === this.defaultAccount?.sessionId)) {
                ...
                this.logService.debug(
                    "[DefaultAccount] Sessions changed for default account provider, updating default account"
                );
                this.updateDefaultAccount();
            }
        }));
        this._register(
            this.authenticationExtensionsService.onDidChangeAccountPreference(async e => {
                const defaultAccountProvider = this.getDefaultAccountAuthenticationProvider();
                if (e.providerId !== defaultAccountProvider.id) {
                    return;
                }
                this.logService.debug(
                    "[DefaultAccount] Account preference changed for default account provider, updating default account"
                );
                this.updateDefaultAccount();
            })
        );
        this._register(this.authenticationService.onDidRegisterAuthenticationProvider(e => {
            ...
            this.logService.debug(
                "[DefaultAccount] Default account provider registered, updating default account"
            );
            this.updateDefaultAccount();
        }));
        this._register(this.authenticationService.onDidUnregisterAuthenticationProvider(e => {
            ...
            this.logService.debug(
                "[DefaultAccount] Default account provider unregistered, updating default account"
            );
            this.updateDefaultAccount();
        }));
    ...
    async whenDefaultAccountAuthenticationProviderAvailable() {
        ...
        this.logService.debug(
            "[DefaultAccount] Waiting for default account authentication provider to be available."
        );
        ...
        try {
            await ( new Promise(resolve => {
                if (this.isAccountProviderAvailable(provider)) {
                    this.logService.debug(
                        "[DefaultAccount] Default account authentication provider is now available."
                    );
                    resolve();
                    return;
                }
                ...
                this.extensionService.whenInstalledExtensionsRegistered().then(() => {
                    ...
                    this.logService.error(
                        "[DefaultAccount] Error while waiting for installed extensions to be registered",
                        getErrorMessage(error)
                    );
                    resolve();
                });
    ...
    async refetchDefaultAccount() {
        ...
        if (!this.hostService.hasFocus || !this._defaultAccount) {
            ...
            this.logService.debug(
                "[DefaultAccount] Skipping refetching default account. Host is not focused or default account is not set"
            );
            return;
        }
    ...
    async doUpdateDefaultAccount(options) {
        try {
            ...
            this.logService.error(
                "[DefaultAccount] Error while updating default account",
                getErrorMessage(error)
            );
        }
    }
    async fetchDefaultAccount(options) {
        ...
        if (!this.isAccountProviderAvailable(defaultAccountProvider)) {
            this.logService.info(
                `[DefaultAccount] Authentication provider is not available.`,
    ...
    cachePolicyData(accountPolicyData) {
        if (accountPolicyData) {
            this.logService.debug(
                "[DefaultAccount] Caching policy data for account:",
                accountPolicyData.accountId
            );
    ...
    extractFromToken(token) {
        ...
        this.logService.debug(
            `[DefaultAccount] extractFromToken: ${JSON.stringify(Object.fromEntries(result))}`
        );
        return result;
    }
    async getDefaultAccountForAuthenticationProvider(authenticationProvider, options) {
        try {
            ...
            const sessions = await this.findMatchingProviderSession(
                authenticationProvider.id,
                this.defaultAccountConfig.authenticationProvider.scopes
            );
            if (!sessions?.length) {
                this.logService.debug(
                    "[DefaultAccount] No matching session found for provider:",
                    authenticationProvider.id
                );
                return null;
            }
    ...
    async getDefaultAccountFromAuthenticatedSessions(authenticationProvider, sessions, options) {
        try {
            ...
            this.logService.debug(
                "[DefaultAccount] Successfully created default account for provider:",
                authenticationProvider.id
            );
            ...
            this.logService.error(
                "[DefaultAccount] Failed to create default account for provider:",
                authenticationProvider.id,
                getErrorMessage(error)
            );
            return null;
    ...
    async getSessions(authProviderId) {
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                return await this.authenticationService.getSessions(authProviderId, undefined, {
                    account: preferredAccount
                }, true);
            } catch (error) {
                this.logService.warn(
                    `[DefaultAccount] Attempt ${attempt} to get sessions failed:`,
                    getErrorMessage(error)
                );
                if (attempt === 3) {
                    throw error;
    ...
    async getTokenEntitlements(sessions, accountPolicyData, options) {
        if (!options?.forceRefresh && accountPolicyData?.tokenEntitlementsFetchedAt && !this.isDataStale(accountPolicyData.tokenEntitlementsFetchedAt)) {
            ...
            return {
                data: {
                    policyData: accountPolicyData.policyData,
                    copilotTokenInfo: this._copilotTokenInfo ?? {}
                },
                fetchedAt: accountPolicyData.tokenEntitlementsFetchedAt
            };
        }
    ...
    async requestTokenEntitlements(sessions) {
        ...
        if (response.res.statusCode && response.res.statusCode !== 200) {
            this.logService.trace(
                `[DefaultAccount] unexpected status code ${response.res.statusCode} while fetching token entitlements`
            );
            return undefined;
        }
        try {
    ...
    async getEntitlements(sessions, accountPolicyData, options) {
        ...
        if (!options?.forceRefresh && existingData && accountPolicyData?.entitlementsFetchedAt && !this.isDataStale(accountPolicyData.entitlementsFetchedAt)) {
            ...
            return {
                data: existingData,
                fetchedAt: accountPolicyData.entitlementsFetchedAt
            };
        }
        ...
        if (response.res.statusCode && response.res.statusCode !== 200) {
            this.logService.trace(
                `[DefaultAccount] unexpected status code ${response.res.statusCode} while fetching entitlements`
            );
            const data =
            (
            response.res.statusCode === 401 || response.res.statusCode === 404) ? null : undefined;
            return {
                data,
                fetchedAt: Date.now()
            };
    ...
    async getMcpRegistryProvider(sessions, accountPolicyData, options) {
        if (!options?.forceRefresh && accountPolicyData?.mcpRegistryDataFetchedAt && !this.isDataStale(accountPolicyData.mcpRegistryDataFetchedAt)) {
            ...
            const data = accountPolicyData.policyData.mcpRegistryUrl && accountPolicyData.policyData.mcpAccess ? {
                url: accountPolicyData.policyData.mcpRegistryUrl,
                registry_access: accountPolicyData.policyData.mcpAccess
            } : null;
            return {
                data,
                fetchedAt: accountPolicyData.mcpRegistryDataFetchedAt
            };
        }
    ...
    async requestMcpRegistryProvider(sessions) {
        ...
        if (!isSuccess(response)) {
            if (isClientError(response)) {
                this.logService.debug(
                    `[DefaultAccount] Received ${response.res.statusCode} for MCP registry data, treating as no registry available.`
                );
                return null;
            }
            this.logService.debug(
    ...
    async requestManagedSettings(sessions) {
        ...
        if (!managedSettingsUrl) {
            this.logService.debug(
                "[DefaultAccount] No managed settings URL configured; skipping enterprise policy fetch"
            );
        ...
        const response = await this.request(
            ...
            MANAGED_SETTINGS_REQUEST_TIMEOUT_MS
        );
        if (!response) {
            this.logService.debug(
                "[DefaultAccount] Managed settings fetch returned no response (network error, all sessions rejected, or active rate-limit backoff); falling back to local-only policy"
            );
        ...
        if (!isSuccess(response)) {
            ...
            this.logService.warn(
                `[DefaultAccount] Managed settings fetch returned non-success status ${status}; falling back to local-only policy`
            );
        ...
        try {
            const data = await asJson(response);
            this.logService.trace(
                "[DefaultAccount] Managed settings raw response:",
            ...
            if (managedSettingsCount === 0) {
                this.logService.debug(
                    "[DefaultAccount] Managed settings fetched (empty response — no enterprise policy file present)"
                );
            } else {
            ...
            this.logService.error(
                "[DefaultAccount] Failed to parse managed settings response",
                getErrorMessage(error)
            );
    ...
    async request(url, type, body, sessions, token, callSite, requestTimeoutMs) {
        if (Date.now() < this._rateLimitBackoffUntil) {
            ...
            this.logService.debug(
                `[DefaultAccount] Skipping request to ${url} — rate-limit backoff active for ${remainingSec}s more`
            );
            return undefined;
        }
        let lastResponse;
        for (const session of sessions) {
            ...
            try {
                ...
                if (this.isRateLimited(response)) {
                    ...
                    this.logService.warn(
                        `[DefaultAccount] Rate limited by ${url} (status ${status}); backing off for ${retryAfterSec}s`
                    );
                    return response;
                }
                if (status === 401 || status === 404) {
                    this.logService.debug(
                        `[DefaultAccount] Received ${status} for URL ${url} with session ${session.id}, likely due to expired/revoked token or insufficient permissions.`,
                        "Trying next session if available."
                    );
    ...
    getEnterpriseUrl() {
        const value = this.configurationService.getValue(
            this.defaultAccountConfig.authenticationProvider.enterpriseProviderUriSetting
        );
        if (!isString(value)) {
            return undefined;
        }
    ...
    async signOut() {
        ...
        await this.commandService.executeCommand("_signOutOfAccount", {
            providerId: this.defaultAccount.authenticationProvider.id,
            accountLabel: this.defaultAccount.accountName
        });
    }
};
DefaultAccountProvider = ( __decorate([( __param(1, IConfigurationService)), ( __param(2, IAuthenticationService)), ( __param(3, IAuthenticationExtensionsService)), ( __param(4, ITelemetryService)), ( __param(5, IExtensionService)), ( __param(6, IRequestService)), ( __param(7, ILogService)), ( __param(8, IWorkbenchEnvironmentService)), ( __param(9, IContextKeyService)), ( __param(10, IStorageService)), ( __param(11, IHostService)), ( __param(12, ICommandService))], DefaultAccountProvider));
...
DefaultAccountProviderContribution = ( __decorate([( __param(0, IProductService)), ( __param(1, IInstantiationService)), ( __param(2, IDefaultAccountService))], DefaultAccountProviderContribution));
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: DEFAULT_ACCOUNT_SIGN_IN_COMMAND,
            title: ( localize2(17209, "Sign In"))
        });
    }
    async run(accessor) {
...
export { CONTEXT_DEFAULT_ACCOUNT_STATE, DEFAULT_ACCOUNT_SIGN_IN_COMMAND, DefaultAccountStatus };
