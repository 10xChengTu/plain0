...
import { EditorResourceAccessor, SideBySideEditor, isEditorInput, EditorInputCapabilities, EditorsOrder, isEditorInputWithOptionsAndGroup, isEditorInputWithOptions, isResourceMergeEditorInput, isResourceDiffEditorInput, isResourceEditorInput, SaveReason } from '../../../common/editor.js';
...
let EditorService = EditorService_1 = class EditorService extends Disposable {
    constructor(
        ...
        this.editorsObserver = this._register(
            this.instantiationService.createInstance(EditorsObserver, this.editorGroupsContainer)
        );
    ...
    registerListeners() {
        ...
        this._register(
            this.editorGroupsContainer.onDidChangeActiveGroup(group => this.handleActiveEditorChange(group))
        );
        this._register(
            this.editorGroupsContainer.onDidAddGroup(group => this.registerGroupListeners(group))
        );
        this._register(
    ...
    handleVisibleEditorsChange() {
        ...
        for (const editor of this.visibleEditors) {
            const resources = distinct(coalesce([EditorResourceAccessor.getCanonicalUri(editor, {
        ...
        case EditorsOrder.SEQUENTIAL:
            {
                ...
                for (const group of this.editorGroupsContainer.getGroups(GroupsOrder.GRID_APPEARANCE)) {
                    editors.push(...( group.getEditors(EditorsOrder.SEQUENTIAL, options).map(editor => ({
                        editor,
                        groupId: group.id
                    }))));
                }
    ...
    doGetVisibleTextEditorControls(editorPanes) {
        ...
        for (const editorPane of editorPanes) {
            const controls = [];
            if (editorPane instanceof SideBySideEditor$1) {
    ...
    getVisibleTextEditorControls(order) {
        return this.doGetVisibleTextEditorControls(coalesce(( this.editorGroupsContainer.getGroups(
            order === EditorsOrder.SEQUENTIAL ? GroupsOrder.GRID_APPEARANCE : GroupsOrder.MOST_RECENTLY_ACTIVE
    ...
    async openEditor(editor, optionsOrPreferredGroup, preferredGroup) {
        ...
        if (!group) {
            ...
            const findGroupResult = this.instantiationService.invokeFunction(findGroup, {
                editor: typedEditor,
                options
            }, preferredGroup);
    ...
    async handleWorkspaceTrust(editors) {
        ...
        case WorkspaceTrustUriResponse.OpenInNewWindow:
            await this.hostService.openWindow(( resources.map(resource => ({
                fileUri: resource
            }))), {
                forceNewWindow: true,
                diffMode,
    ...
    findEditors(arg1, options, arg2) {
        ...
        if (!isUndefined(arg2)) {
            ...
            for (const group of this.editorGroupsContainer.getGroups(
                options?.order === EditorsOrder.SEQUENTIAL ? GroupsOrder.GRID_APPEARANCE : GroupsOrder.MOST_RECENTLY_ACTIVE
            )) {
                const editors = [];
                if (URI.isUri(arg1)) {
                ...
                result.push(...( editors.map(editor => ({
                    editor,
                    groupId: group.id
                }))));
            }
    ...
    async save(editors, options) {
        ...
        if (options?.saveAs) {
            ...
            for (const {
                ...
                if (editor.hasCapability(EditorInputCapabilities.Untitled)) {
                    editorsToSaveSequentially.push({
                        groupId,
                        editor
                    });
                } else {
                    editorsToSaveParallel.push({
                        groupId,
                        editor
                    });
                }
        ...
        const saveResults = await Promises.settled(( editorsToSaveParallel.map((
            {
                groupId,
                editor
            }
    ...
    async revert(editors, options) {
        ...
        const uniqueEditors = this.getUniqueEditors(editors);
        await Promises.settled(( uniqueEditors.map(async (
            {
                groupId,
                editor
            }
...
EditorService = EditorService_1 = ( __decorate([( __param(1, IEditorGroupsService)), ( __param(2, IInstantiationService)), ( __param(3, IFileService)), ( __param(4, IConfigurationService)), ( __param(5, IWorkspaceContextService)), ( __param(6, IUriIdentityService)), ( __param(7, IEditorResolverService)), ( __param(8, IWorkspaceTrustRequestService)), ( __param(9, IHostService)), ( __param(10, ITextEditorService))], EditorService));
