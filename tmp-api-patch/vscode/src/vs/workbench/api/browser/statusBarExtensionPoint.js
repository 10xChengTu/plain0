...
const statusBarItemSchema = {
    ...
    properties: {
        ...
        tooltip: {
            type: "string",
            description: ( localize(2812, "The tooltip text for the entry."))
        },
        command: {
            type: "string",
            description: ( localize(2813, "The command to execute when the status bar entry is clicked."))
        },
        alignment: {
            type: "string",
            enum: ["left", "right"],
            description: ( localize(2814, "The alignment of the status bar entry."))
        },
        priority: {
            type: "number",
            description: ( localize(
...
const statusBarItemsSchema = {
    description: ( localize(2819, "Contributes items to the status bar.")),
            ...
            for (const entry of extensions) {
                ...
                for (const candidate of Iterable.wrap(value)) {
                    if (!isUserFriendlyStatusItemEntry(candidate)) {
                        collector.error(( localize(2820, "Invalid status bar item contribution.")));
                        continue;
                    }
