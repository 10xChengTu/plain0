    ...
    async $providePortAttributes(handles, ports, pid, commandLine, cancellationToken) {
        ...
        for (const handle of handles) {
            ...
            providedAttributes.push(...(await Promise.all(( ports.map(async port => {
                ...
                try {
                    providedAttributes = await provider.provider.providePortAttributes({
                        port,
                        pid,
                        commandLine
                    }, cancellationToken);
        ...
        if (provider) {
            ...
            if (tunnelFactory) {
                ...
                if (provider.tunnelFeatures?.public && (privacyOptions.length === 0)) {
                    privacyOptions = [{
                        id: "private",
                        label: ( localize(2955, "Private")),
                        themeIcon: "lock"
                    }, {
                        id: "public",
                        label: ( localize(2956, "Public")),
                        themeIcon: "eye"
                    }];
                }
...
ExtHostTunnelService = ( __decorate([( __param(0, IExtHostRpcService)), ( __param(1, IExtHostInitDataService)), ( __param(2, ILogService))], ExtHostTunnelService));
