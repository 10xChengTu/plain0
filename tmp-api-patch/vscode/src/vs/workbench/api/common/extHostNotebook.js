...
class ExtHostNotebookController {
    ...
    static _convertNotebookRegistrationData(extension, registration) {
        ...
        const viewOptionsFilenamePattern = ( registration.filenamePattern.map(pattern => NotebookExclusiveDocumentPattern.from(pattern))).filter(pattern => pattern !== undefined);
    ...
    async showNotebookDocument(notebook, options) {
        ...
        if (typeof options === "object") {
            resolvedOptions = {
                ...
                label: typeof options.asRepl === "string" ? options.asRepl : typeof options.asRepl === "object" ? options.asRepl.label : undefined
            };
        } else {
            resolvedOptions = {
                preserveFocus: false,
    ...
    registerNotebookSerializer(extension, viewType, serializer, options, registration) {
        ...
        this._notebookSerializer.set(handle, {
            viewType,
            serializer,
            options
        });
        this._notebookProxy.$registerNotebookSerializer(handle, {
            id: extension.identifier,
            location: extension.extensionLocation
        }, viewType, NotebookDocumentContentOptions.from(options), ExtHostNotebookController._convertNotebookRegistrationData(extension, registration));
        return toDisposable(() => {
    ...
    async $saveNotebook(handle, uriComponents, versionId, options, token) {
        ...
        try {
            ...
            if (!this._extHostFileSystem.value.isWritableFileSystem(uri.scheme)) {
                throw new FileOperationError(( localize(2949, "Unable to modify read-only file '{0}'", this._resourceForError(uri))), FileOperationResult.FILE_PERMISSION_DENIED);
            }
            const data = {
                metadata: filter(
            ...
            try {
                ...
                simpleCells.forEach((cell, index) => {
                    ...
                    const webviewResults = ( outputMatches.flatMap(
                        outputMatch => genericCellMatchesToTextSearchMatches(outputMatch.matches, outputMatch.textBuffer)
                    ).map((textMatch, index) => {
    ...
    async _validateWriteFile(uri, options) {
        ...
        if (typeof options?.mtime === "number" && typeof options.etag === "string" && options.etag !== ETAG_DISABLED && typeof stat.mtime === "number" && typeof stat.size === "number" && options.mtime < stat.mtime && options.etag !== etag({
            ...
            throw new FileOperationError(( localize(2950, "File Modified Since")), FileOperationResult.FILE_MODIFIED_SINCE, options);
        }
        return;
    }
...
export { ExtHostNotebookController, NotebookSaveError };
