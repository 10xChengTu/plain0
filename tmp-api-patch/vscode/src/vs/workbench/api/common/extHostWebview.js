...
import { serializeWebviewMessage, deserializeWebviewMessage } from './extHostWebviewMessaging.js';
    ...
    set options(newOptions) {
        ...
        if (!equals(this.#options, newOptions)) {
            this.#proxy.$setOptions(
                this.#handle,
                serializeWebviewOptions(this.#extension, this.#workspace, newOptions)
            );
        }
...
class ExtHostWebviews extends Disposable {
    ...
    $onMessage(handle, jsonMessage, buffers) {
        ...
        if (webview) {
            const {
                message
            } = deserializeWebviewMessage(jsonMessage, buffers.value);
