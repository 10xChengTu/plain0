...
import { isFalsyOrEmpty, coalesce, isNonEmptyArray, asArray } from '../../../base/common/arrays.js';
...
class CodeLensAdapter {
    ...
    async provideCodeLenses(resource, token) {
        ...
        for (let i = 0; i < lenses.length; i++) {
            if (!Range$2.isRange(lenses[i].range)) {
                console.warn(
                    "INVALID code lens, range is not defined",
        ...
        const otherDocuments = ( otherResources.map(r => {
            try {
                ...
                this._logService.error(
                    "Error: Unable to retrieve document from URI: " + r + ". Error message: " + err
                );
                return undefined;
            }
...
class CodeActionAdapter {
    static {
        this._maxCodeActionsPerFile = 1000;
    }
    constructor(
        _documents,
        _commands,
    ...
    async provideCodeActions(resource, rangeOrSelection, context, token) {
        const doc = this._documents.getDocument(resource);
        const ran = Selection.isISelection(rangeOrSelection) ? Selection$1.to(rangeOrSelection) : Range.to(rangeOrSelection);
            ...
            if (CodeActionAdapter._isCommand(candidate) && !(candidate instanceof CodeAction)) {
                ...
                if (codeActionContext.only) {
                    if (!toConvert.kind) {
                        ...
                        this._logService.warn(
                            `${this._extension.identifier.value} - Code actions of kind '${codeActionContext.only.value}' requested but returned code action is of kind '${toConvert.kind.value}'. Code action will be dropped. Please check 'CodeActionContext.only' to only return requested code actions.`
                        );
                    }
                }
                ...
                actions.push({
                    ...
                    edit: toConvert.edit && WorkspaceEdit.from(toConvert.edit, undefined),
                    kind: toConvert.kind && toConvert.kind.value,
                    isPreferred: toConvert.isPreferred,
                    isAI: isProposedApiEnabled(this._extension) ? toConvert.isAI : false,
                    ranges: isProposedApiEnabled(this._extension) ? coalesce(( range.map(Range.from))) : undefined,
...
class DocumentPasteEditProvider {
    ...
    async prepareDocumentPaste(resource, ranges, dataTransferDto, token) {
        ...
        const items = await Promise.all(Array.from(newEntries, async ([mime, value]) => {
            ...
            return [mime, await DataTransferItem.from(mime, value, id)];
        }));
    ...
    async providePasteEdits(requestId, resource, ranges, dataTransferDto, context, token) {
        ...
        const items = ( dataTransferDto.items.map(([mime, value]) => {
            ...
            return [mime, DataTransferItem.to(mime, value, async id => {
                return (await this._proxy.$resolvePasteFileData(this._handle, requestId, id)).buffer;
            })];
        }));
        const dataTransfer = ( new DataTransfer$1(items));
        ...
        return (edits.map((edit, i) => ({
            ...
            insertText: typeof edit.insertText === "string" ? edit.insertText : {
    ...
    async resolvePasteEdit(id, token) {
        ...
        return {
            ...
            additionalEdit: resolvedItem.additionalEdit ? WorkspaceEdit.from(resolvedItem.additionalEdit, undefined) : undefined
        };
    }
...
class NewSymbolNamesAdapter {
    static {
        this.languageTriggerKindToVSCodeTriggerKind = {
            [NewSymbolNameTriggerKind$1.Invoke]: NewSymbolNameTriggerKind.Invoke,
            [NewSymbolNameTriggerKind$1.Automatic]: NewSymbolNameTriggerKind.Automatic
        };
    }
    constructor(_documents, _provider, _logService) {
        this._documents = _documents;
...
class DocumentSemanticTokensAdapter {
    ...
    async provideDocumentSemanticTokens(resource, previousResultId, token) {
        ...
        return this._send(
            DocumentSemanticTokensAdapter._convertToEdits(previousResult, value),
            value
        );
    }
...
class CompletionsAdapter {
    ...
    async provideCompletionItems(resource, position, context, token) {
        ...
        const insertRange = replaceRange.with({
            end: pos
        });
        ...
        const result = {
            ...
            [ISuggestResultDtoField.defaultRanges]: {
                replace: Range.from(replaceRange),
                insert: Range.from(insertRange)
            },
    ...
    async resolveCompletionItem(id, token) {
        ...
        if (dto1[ISuggestDataDtoField.insertText] !== dto2[ISuggestDataDtoField.insertText] || dto1[ISuggestDataDtoField.insertTextRules] !== dto2[ISuggestDataDtoField.insertTextRules]) {
            this._apiDeprecation.report(
        ...
        if (dto1[ISuggestDataDtoField.commandIdent] !== dto2[ISuggestDataDtoField.commandIdent] || dto1[ISuggestDataDtoField.commandId] !== dto2[ISuggestDataDtoField.commandId] || !equals(
            dto1[ISuggestDataDtoField.commandArguments],
            dto2[ISuggestDataDtoField.commandArguments]
        )) {
            this._apiDeprecation.report(
    ...
    _convertCompletionItem(item, id, defaultInsertRange, defaultReplaceRange) {
        ...
        if (Range$2.isRange(range)) {
            result[ISuggestDataDtoField.range] = Range.from(range);
...
class InlineCompletionAdapter {
    constructor(_extension, _documents, _provider, _commands) {
        ...
        this._isAdditionsProposedApiEnabled = isProposedApiEnabled(this._extension);
    }
    get supportsHandleEvents() {
        return isProposedApiEnabled(this._extension) && (typeof this._provider.handleDidShowCompletionItem === "function" || typeof this._provider.handleDidPartiallyAcceptCompletionItem === "function" || typeof this._provider.handleDidRejectCompletionItem === "function" || typeof this._provider.handleEndOfLifetime === "function");
    }
    get supportsSetModelId() {
        return isProposedApiEnabled(this._extension) && typeof this._provider.setCurrentModelId === "function";
    }
    get supportsSetProviderOption() {
        return isProposedApiEnabled(this._extension) && typeof this._provider.setProviderOptionValue === "function";
    }
    get modelInfo() {
    ...
    async provideInlineCompletions(resource, position, context, token) {
        ...
        const items = {
            ...
            items: ( resultItems.map((item, idx) => {
                ...
                return ({
                    insertText: insertText === undefined ? undefined : (typeof insertText === "string" ? insertText : {
                        snippet: insertText.value
                    }),
                    range: item.range ? Range.from(item.range) : undefined,
                    showRange: (this._isAdditionsProposedApiEnabled && item.showRange) ? Range.from(item.showRange) : undefined,
                    command,
                    ...
                    jumpToPosition: (this._isAdditionsProposedApiEnabled && item.jumpToPosition) ? Position.from(item.jumpToPosition) : undefined
                });
            })),
        ...
        if (this._provider.handleListEndOfLifetime && this._isAdditionsProposedApiEnabled && completionList?.list) {
            function translateReason(reason) {
                ...
                case "tokenCancellation":
                    return {
                        kind: InlineCompletionsDisposeReasonKind.TokenCancellation
                    };
                case "other":
                    return {
                        kind: InlineCompletionsDisposeReasonKind.Other
                    };
                case "empty":
                    return {
                        kind: InlineCompletionsDisposeReasonKind.Empty
                    };
                case "notTaken":
                    return {
                        kind: InlineCompletionsDisposeReasonKind.NotTaken
                    };
                default:
                    return {
                        kind: InlineCompletionsDisposeReasonKind.Other
...
class LinkProviderAdapter {
    ...
    async provideLinks(resource, token) {
        ...
        if (typeof this._provider.resolveDocumentLink !== "function") {
            return {
                links: ( links.filter(LinkProviderAdapter._validateLink).map(DocumentLink.from))
            };
        } else {
...
class ColorProviderAdapter {
    ...
    async provideColorPresentations(resource, raw, token) {
        ...
        const value = await this._provider.provideColorPresentations(color, {
            document,
            range
        }, token);
        if (!Array.isArray(value)) {
...
class ExtHostLanguageFeatures extends Disposable {
    ...
    $provideDefinition(handle, resource, position, token) {
        return this._withAdapter(
            handle,
            DefinitionAdapter,
            adapter => adapter.provideDefinition(URI.revive(resource), position, token),
            [],
            token
        );
    }
    ...
    $provideDeclaration(handle, resource, position, token) {
        return this._withAdapter(
            handle,
            DeclarationAdapter,
            adapter => adapter.provideDeclaration(URI.revive(resource), position, token),
            [],
            token
        );
    }
    ...
    $provideImplementation(handle, resource, position, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideImplementation(URI.revive(resource), position, token),
            [],
            token
        );
    }
    ...
    $provideTypeDefinition(handle, resource, position, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideTypeDefinition(URI.revive(resource), position, token),
            [],
            token
        );
    }
    ...
    $provideHover(handle, resource, position, context, token) {
        return this._withAdapter(
            handle,
            HoverAdapter,
            adapter => adapter.provideHover(URI.revive(resource), position, context, token),
            undefined,
            token
        );
    }
    ...
    $provideDocumentHighlights(handle, resource, position, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideDocumentHighlights(URI.revive(resource), position, token),
            undefined,
            token
        );
    }
    $provideMultiDocumentHighlights(handle, resource, position, otherModels, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideMultiDocumentHighlights(URI.revive(resource), position, ( otherModels.map(model => URI.revive(model))), token),
            undefined,
            token
        );
    }
    ...
    $provideReferences(handle, resource, position, context, token) {
        return this._withAdapter(
            handle,
            ReferenceAdapter,
            adapter => adapter.provideReferences(URI.revive(resource), position, context, token),
            undefined,
            token
        );
    }
    ...
    $provideCodeActions(handle, resource, rangeOrSelection, context, token) {
        return this._withAdapter(
            handle,
            CodeActionAdapter,
            adapter => adapter.provideCodeActions(URI.revive(resource), rangeOrSelection, context, token),
            undefined,
            token
        );
    }
    ...
    $provideDocumentFormattingEdits(handle, resource, options, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideDocumentFormattingEdits(URI.revive(resource), options, token),
            undefined,
            token
        );
    }
    ...
    $provideDocumentRangeFormattingEdits(handle, resource, range, options, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideDocumentRangeFormattingEdits(URI.revive(resource), range, options, token),
            undefined,
            token
        );
    }
    $provideDocumentRangesFormattingEdits(handle, resource, ranges, options, token) {
        return this._withAdapter(
            handle,
            RangeFormattingAdapter,
            adapter => adapter.provideDocumentRangesFormattingEdits(URI.revive(resource), ranges, options, token),
            undefined,
            token
        );
    }
    ...
    $provideOnTypeFormattingEdits(handle, resource, position, ch, options, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideOnTypeFormattingEdits(URI.revive(resource), position, ch, options, token),
            undefined,
            token
        );
    }
    ...
    $provideRenameEdits(handle, resource, position, newName, token) {
        return this._withAdapter(
            handle,
            RenameAdapter,
            adapter => adapter.provideRenameEdits(URI.revive(resource), position, newName, token),
            undefined,
            token
        );
    }
    $resolveRenameLocation(handle, resource, position, token) {
        return this._withAdapter(
            handle,
            RenameAdapter,
            adapter => adapter.resolveRenameLocation(URI.revive(resource), position, token),
            undefined,
            token
        );
    }
    ...
    $provideNewSymbolNames(handle, resource, range, triggerKind, token) {
        return this._withAdapter(
            handle,
            NewSymbolNamesAdapter,
            adapter => adapter.provideNewSymbolNames(URI.revive(resource), range, triggerKind, token),
            undefined,
            token
        );
    }
    ...
    $provideCompletionItems(handle, resource, position, context, token) {
        return this._withAdapter(
            handle,
            CompletionsAdapter,
            adapter => adapter.provideCompletionItems(URI.revive(resource), position, context, token),
            undefined,
            token
        );
    }
    ...
    registerInlineCompletionsProvider(extension, selector, provider, metadata) {
        ...
        let result = this._createDisposable(handle);
        const supportsOnDidChange = isProposedApiEnabled(extension) && typeof provider.onDidChange === "function";
        if (supportsOnDidChange) {
            const subscription = provider.onDidChange(e => this._proxy.$emitInlineCompletionsChange(handle, e ? {
                data: e.data
            } : undefined));
            result = Disposable$1.from(result, subscription);
        }
        const supportsOnDidChangeModelInfo = isProposedApiEnabled(extension) && typeof provider.onDidChangeModelInfo === "function";
        if (supportsOnDidChangeModelInfo) {
            const subscription = provider.onDidChangeModelInfo(
                _ => this._proxy.$emitInlineCompletionModelInfoChange(handle, adapter.modelInfo)
            );
            result = Disposable$1.from(result, subscription);
        }
        const supportsOnDidChangeProviderOptions = isProposedApiEnabled(extension) && typeof provider.onDidChangeProviderOptions === "function";
        if (supportsOnDidChangeProviderOptions) {
            const subscription = provider.onDidChangeProviderOptions(
                _ => this._proxy.$emitInlineCompletionProviderOptionsChange(handle, adapter.providerOptions)
            );
            result = Disposable$1.from(result, subscription);
        }
    ...
    $provideSignatureHelp(handle, resource, position, context, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideSignatureHelp(URI.revive(resource), position, context, token),
            undefined,
            token
        );
    }
    ...
    $provideColorPresentations(handle, resource, colorInfo, token) {
        return this._withAdapter(
            handle,
            ColorProviderAdapter,
            adapter => adapter.provideColorPresentations(URI.revive(resource), colorInfo, token),
            undefined,
            token
        );
    }
    ...
    $provideSelectionRanges(handle, resource, positions, token) {
        return this._withAdapter(
            ...
            adapter => adapter.provideSelectionRanges(URI.revive(resource), positions, token),
            [],
            token
        );
    }
    ...
    $provideDocumentOnDropEdits(handle, requestId, resource, position, dataTransferDto, token) {
        return this._withAdapter(handle, DocumentDropEditAdapter, adapter => Promise.resolve(
            adapter.provideDocumentOnDropEdits(requestId, URI.revive(resource), position, dataTransferDto, token)
        ), undefined, undefined);
    }
    ...
    $prepareDocumentPaste(handle, resource, ranges, dataTransfer, token) {
        return this._withAdapter(
            ...
            adapter => adapter.prepareDocumentPaste(URI.revive(resource), ranges, dataTransfer, token),
            undefined,
            token
        );
    }
    $providePasteEdits(handle, requestId, resource, ranges, dataTransferDto, context, token) {
        return this._withAdapter(
            handle,
            DocumentPasteEditProvider,
            adapter => adapter.providePasteEdits(requestId, URI.revive(resource), ranges, dataTransferDto, context, token),
            undefined,
            token
        );
    }
