...
import { LanguageModelToolSource, LanguageModelToolResult, MarkdownString, IconPath } from './extHostTypeConverters.js';
...
class ExtHostLanguageModelTools {
    ...
    getTools(extension) {
        const hasParticipantAdditions = isProposedApiEnabled(extension);
        return ( Array.from(( this._allTools.values())).map(
            ...
            case SearchExtensionsToolId:
                return isProposedApiEnabled(extension);
            default:
                return true;
            }
        });
    ...
    async $invokeTool(dto, token) {
        ...
        if (isProposedApiEnabled(item.extension)) {
        ...
        if (isProposedApiEnabled(item.extension) && dto.chatStreamToolCallId) {
    ...
    async $prepareToolInvocation(toolId, context, token) {
        ...
        if (item.tool.prepareInvocation) {
            ...
            const approveCombinationLabel = approveCombination ? MarkdownString.fromStrict(approveCombination.message) : undefined;
            const approveCombinationKey = approveCombinationLabel ? await computeCombinationKey(toolId, context.parameters) : undefined;
            return {
                confirmationMessages: result.confirmationMessages ? {
                    ...
                    approveCombination: approveCombinationLabel && approveCombinationKey ? {
                        label: approveCombinationLabel,
                        key: approveCombinationKey,
    ...
    registerToolDefinition(extension, definition, tool) {
        ...
        const dto = {
            ...
            icon: IconPath.from(definition.icon),
            models: definition.models,
            toolSet: definition.toolSet,
            tags: definition.tags,
            fullReferenceName: undefined
        };
        this._registeredTools.set(id, {
            extension,
            tool
