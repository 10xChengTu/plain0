...
let ExtHostGitExtensionService = class ExtHostGitExtensionService extends Disposable {
    ...
    async $getRefs(handle, query, token) {
        ...
        try {
            ...
            const result = ( refs.map(ref => {
                ...
                const id = ref.type === GitRefType.Head ? `refs/heads/${ref.name}` : ref.type === GitRefType.RemoteHead ? `refs/remotes/${ref.remote}/${ref.name}` : `refs/tags/${ref.name}`;
                return {
                    id,
                    name: ref.name,
    ...
    async _ensureGitApi() {
        ...
        try {
            await this._extHostExtensionService.activateByIdWithErrors(( new ExtensionIdentifier(GIT_EXTENSION_ID)), {
                startup: false,
...
ExtHostGitExtensionService = ExtHostGitExtensionService_1 = ( __decorate([( __param(0, IExtHostRpcService)), ( __param(1, IExtHostExtensionService))], ExtHostGitExtensionService));

export { ExtHostGitExtensionService, IExtHostGitExtensionService };
