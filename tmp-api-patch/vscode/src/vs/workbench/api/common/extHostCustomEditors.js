...
class ExtHostCustomEditors {
    ...
    registerCustomEditorProvider(extension, viewType, provider, options) {
        ...
        if (isCustomTextEditorProvider(provider)) {
            ...
            this._proxy.$registerTextEditorProvider(toExtensionData(extension), viewType, options.webviewOptions || {}, {
                ...
                supportsInlineDiff: isProposedApiEnabled(extension) && isCustomTextEditorProviderWithInlineDiffCapability(provider),
                supportsSideBySideDiff: isProposedApiEnabled(extension) && isCustomTextEditorProviderWithSideBySideDiffCapability(provider)
            }, shouldSerializeBuffersForPostMessage(extension));
        } else {
            disposables.add(this._editorProviders.addCustomProvider(viewType, extension, provider));
            const supportsCustomEditorDiffs = isProposedApiEnabled(extension);
    ...
    async $createCustomDocument(resource, viewType, backupId, untitledDocumentData, cancellation) {
        ...
        const document = await entry.provider.openCustomDocument(revivedResource, {
            backupId,
            untitledDocumentData: untitledDocumentData?.buffer
        }, cancellation);
        let storageRoot;
    ...
    async $disposeCustomDocument(resource, viewType) {
        ...
        const {
            document
        } = this.getCustomDocumentEntry(viewType, revivedResource);
    ...
    async $resolveCustomEditorInlineDiff(
        ...
        const {
            entry,
            panel
        } = this.createCustomEditorDiffPanel(handle, viewType, initData, position);
        ...
        if (entry.type === CustomEditorType.Text) {
            if (!isCustomTextEditorProviderWithInlineDiffCapability(entry.provider)) {
                throw ( new Error(
                    `Provider for '${viewType}' does not support inline custom text editor diffs`
                ));
            }
            ...
            return entry.provider.resolveCustomTextEditorInlineDiff({
        ...
        } = this.getCustomDocumentEntry(viewType, revivedOriginalResource);
        const {
            document: modifiedDocument
        } = this.getCustomDocumentEntry(viewType, revivedModifiedResource);
        return entry.provider.resolveCustomEditorInlineDiff({
            original: originalDocument,
            modified: modifiedDocument
        }, panel, cancellation);
    }
    async $resolveCustomEditorSideBySideDiff(
        ...
        const {
            entry,
            panel: originalPanel
        } = this.createCustomEditorDiffPanel(webviewHandles.original, viewType, initData.original, position);
        const {
            panel: modifiedPanel
        } = this.createCustomEditorDiffPanel(webviewHandles.modified, viewType, initData.modified, position);
        const revivedOriginalResource = URI.revive(originalResource);
        const revivedModifiedResource = URI.revive(modifiedResource);
        if (entry.type === CustomEditorType.Text) {
            if (!isCustomTextEditorProviderWithSideBySideDiffCapability(entry.provider)) {
                throw ( new Error(
                    `Provider for '${viewType}' does not support side by side custom text editor diffs`
                ));
            }
            const originalDocument = this._extHostDocuments.getDocument(revivedOriginalResource);
            const modifiedDocument = this._extHostDocuments.getDocument(revivedModifiedResource);
            return entry.provider.resolveCustomTextEditorSideBySideDiff({
                original: originalDocument,
                modified: modifiedDocument
            }, {
        ...
        if (!isCustomEditorProviderWithSideBySideDiffCapability(entry.provider)) {
            throw ( new Error(
                `Provider for '${viewType}' does not support side by side custom editor diffs`
            ));
        }
        const {
            document: originalDocument
        ...
        return entry.provider.resolveCustomEditorSideBySideDiff({
            original: originalDocument,
            modified: modifiedDocument
        }, {
            original: originalPanel,
