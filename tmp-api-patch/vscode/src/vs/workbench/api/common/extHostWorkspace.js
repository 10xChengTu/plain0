...
import { Barrier, AsyncIterableProducer } from '../../../base/common/async.js';
...
import { ExtUri, dirname, basename, basenameOrAuthority, relativePath } from '../../../base/common/resources.js';
...
import { ExcludeSettingOptions, TextSearchMatch2, TextSearchContext2 } from '../../services/search/common/searchExtTypes.js';
import { bufferToStream, VSBuffer, readableToBuffer } from '../../../base/common/buffer.js';
import { toDecodeStream, UTF8, toEncodeReadable } from '../../services/textfile/common/encoding.js';
...
function delta(oldFolders, newFolders, compare, extHostFileSystemInfo) {
    ...
    return delta$1(
        oldSortedFolders,
        newSortedFolders,
        (a, b) => compare(a, b, extHostFileSystemInfo)
    );
}
...
class ExtHostWorkspaceImpl extends Workspace$1 {
    static toExtHostWorkspace(
        ...
        if (previousConfirmedWorkspace) {
            ...
            newWorkspaceFolders.push(...( folders.map((
                {
                    uri,
                    name,
                    index
...
let ExtHostWorkspace = class ExtHostWorkspace {
    ...
    findFiles(include, exclude, maxResults, extensionId, token = CancellationToken.None) {
        ...
        }, extensionId, "findFiles", {
            useIgnoreFilesLocal: undefined,
            excludeWasNull: exclude === null
        }, token);
    }
    findFiles2(filePatterns, options = {}, extensionId, token = CancellationToken.None) {
        ...
        return this._findFilesImpl({
            type: "filePatterns",
            value: filePatterns
        }, options, extensionId, "findFiles2", {
            useIgnoreFilesLocal: options.useIgnoreFiles?.local,
            excludeWasNull: false
        }, token);
    }
    async _findFilesImpl(
        ...
        try {
            ...
            respectedIgnoreFiles = queryOptions.every(
                q => q.options.disregardIgnoreFiles === true ? false : q.options.disregardIgnoreFiles === false ? true : userHonorsIgnore
            );
    ...
    async _findFilesBase(queryOptions, token) {
        ...
        const result = await Promise.all(queryOptions?.map(
            option => this._proxy.$startFileSearch(option.folder ?? null, option.options, tokenToUse).then(data => Array.isArray(data) ? ( data.map(d => URI.revive(d))) : [])
        ) ?? []);
    ...
    findTextInFiles2(query, options, extensionId, token = CancellationToken.None) {
        ...
        const complete = this.findTextInFilesBase(query, queryOptions, (result, uri) => progressEmitter.fire({
            result,
            uri
        }), token);
    ...
    async findTextInFilesBase(query, queryOptions, callback, token = CancellationToken.None) {
        ...
        try {
            const result = await Promise.all(queryOptions?.map(
                option => this._proxy.$startTextSearch(query, option.folder ?? null, option.options, requestId, token) || {}
            ) ?? []);
    ...
    async findTextInFiles(query, options, callback, extensionId, token = CancellationToken.None) {
        ...
        const excludePattern = (typeof options.exclude === "string") ? options.exclude : options.exclude ? options.exclude.pattern : undefined;
        const queryOptions = {
            ignoreSymlinks: typeof options.followSymlinks === "boolean" ? !options.followSymlinks : undefined,
        ...
        return this.findTextInFilesBase(query, [{
    ...
    async $provideCanonicalUri(uri, targetScheme, cancellationToken) {
        return this.provideCanonicalUri(URI.revive(uri), {
            targetScheme
        }, cancellationToken);
    }
    ...
    async encode(content, args) {
        ...
        const {
            encoding,
            addBOM
        } = await this._proxy.$resolveEncoding(uri, options);
...
ExtHostWorkspace = ( __decorate([( __param(0, IExtHostRpcService)), ( __param(1, IExtHostInitDataService)), ( __param(2, IExtHostFileSystemInfo)), ( __param(3, ILogService)), ( __param(4, IURITransformerService))], ExtHostWorkspace));
