...
function convertChatSessionDeltaToDto(delta) {
    return {
        addedOrUpdated: delta.addedOrUpdated ? Array.from(( delta.addedOrUpdated.values()), ChatSessionItem.from) : [],
...
class ChatSessionItemCollectionImpl {
    ...
    add(item) {
        ...
        void this.#proxy.$addOrUpdateChatSessionItem(this.#controllerHandle, ChatSessionItem.from(item));
    }
    delete(resource) {
...
class ExtHostChatSession {
    constructor(session, extension, request, proxy, commandsConverter, sessionDisposables) {
        ...
        this._stream = ( new ChatAgentResponseStream(
            extension,
            request,
            proxy,
            commandsConverter,
    ...
    getActiveRequestStream(request) {
        return ( new ChatAgentResponseStream(
            this.extension,
            request,
            this.proxy,
...
let ExtHostChatSessions = class ExtHostChatSessions extends Disposable {
        ...
        commands.registerArgumentProcessor({
            processArgument: arg => {
                if (arg && arg.$mid === MarshalledId.AgentSessionContext) {
                    ...
                    for (const {
                        controller
                    } of ( this._chatSessionItemControllers.values())) {
            ...
            createChatSessionItem: (resource, label) => {
                ...
                const item = ( new ChatSessionItemImpl(resource, label, () => {
                    if (collection.get(resource) === item) {
                        void this._proxy.$addOrUpdateChatSessionItem(controllerHandle, ChatSessionItem.from(item));
                    }
                }));
                return item;
            },
    ...
    registerChatSessionContentProvider(extension, chatSessionScheme, chatParticipant, provider, capabilities) {
        ...
        const disposables = ( new DisposableStore());
        this._chatSessionContentProviders.set(handle, {
            chatSessionScheme,
            provider,
            extension,
            capabilities,
    ...
    async $provideChatSessionContent(handle, sessionResourceComponents, context, token) {
        ...
        this._extHostChatSessions.set(sessionResource, {
            sessionObj: chatSession,
            disposeCts
        });
        if (session.activeResponseCallback) {
            Promise.resolve(
                session.activeResponseCallback(chatSession.activeResponseStream.apiObject, disposeCts.token)
            ).finally(() => {
        ...
        if (provider.provider.provideHandleOptionsChange) {
            try {
                ...
                this._logService.error(
                    `Error calling provideHandleOptionsChange for handle ${handle}, sessionResource ${sessionResource}:`,
                    error
                );
            }
            return;
    ...
    async $provideChatSessionProviderOptions(handle, token) {
        ...
        try {
            ...
            this._logService.error(
                `Error calling provideChatSessionProviderOptions for handle ${handle}:`,
                error
            );
            return;
        }
    ...
    async $invokeChatSessionRequestHandler(handle, sessionResource, request, history, token) {
        ...
        const chatRequest = ChatAgentRequest.to(
            request,
            undefined,
            await this.getModelForRequest(request, entry.sessionObj.extension),
            request.modelConfiguration,
            [],
            ( new Map()),
            entry.sessionObj.extension,
            this._logService
        );
        const stream = entry.sessionObj.getActiveRequestStream(request);
        await entry.sessionObj.session.requestHandler(chatRequest, {
            history,
            yieldRequested: false
        }, stream.apiObject, token);
        return {};
    }
    async $forkChatSession(handle, sessionResourceComponents, request, token) {
        ...
        if (controllerData?.controller.forkHandler) {
            ...
            return ChatSessionItem.from(item);
        }
    ...
    async getInputStateForSession(sessionResource, initialSessionOptions, token) {
        ...
        if (controllerData?.controller.getChatSessionInputState) {
            const result = await controllerData.controller.getChatSessionInputState(resolvedResource, {
        ...
        return ( groups.map(group => {
            ...
            return {
                ...
                commands: ( group.commands.map(command => {
                    ...
                    this._proxyCommands.set(proxyId, {
                        originalCommandId: command.command,
                        controllerHandle
                    });
                    this.commands.registerCommand(true, proxyId, async (...args) => {
                        const sessionResource = args[0] instanceof URI ? args[0] : undefined;
                        const inputState = await this.getInputStateForSession(sessionResource, undefined, CancellationToken.None);
                        return this.commands.executeCommand(command.command, {
                            inputState,
                            sessionResource
                        }, ...(command.arguments ?? []));
                    });
    ...
    convertReferenceToVariable(ref) {
        const value = ref.value && typeof ref.value === "object" && "uri" in ref.value && "range" in ref.value ? Location.from(ref.value) : ref.value;
        ...
        if (Location$1.isLocation(ref.value) && ref.name.startsWith(`sym:`)) {
            const loc = Location.from(ref.value);
            return {
                id: ref.id,
                name: ref.name,
    ...
    async $resolveChatSessionItem(handle, sessionResourceComponents, token) {
        ...
        return ChatSessionItem.from(updatedItem);
    }
    async $provideChatSessionInputState(controllerHandle, sessionResourceComponents, token) {
        ...
        const inputState = await handler(
            !sessionResource || isUntitledChatSession(sessionResource) ? undefined : sessionResource,
            {
...
ExtHostChatSessions = ( __decorate([( __param(2, IExtHostRpcService)), ( __param(3, ILogService))], ExtHostChatSessions));
