...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(2958, "Contributes json schema configuration.")),
        type: "array",
        defaultSnippets: [{
            body: [{
        ...
        configurationExtPoint.setHandler(extensions => {
            for (const extension of extensions) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(2961, "'configuration.jsonValidation' must be a array")));
                    return;
                }
                extensionValue.forEach(extension => {
                    ...
                    if (!isString(uri)) {
                        collector.error(( localize(2963, "'configuration.jsonValidation.url' must be a URL or relative path")));
                        return;
                    }
                    if (uri.startsWith("./")) {
                        try {
...
class JSONValidationDataRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [( localize(2967, "File Match")), ( localize(2968, "Schema"))];
...
( Registry.as(Extensions.ExtensionFeaturesRegistry)).registerExtensionFeature({
    id: "jsonValidation",
    label: ( localize(2969, "JSON Validation")),
    access: {
        canToggle: false
    },
...
registerWorkbenchContribution2(
    JSONValidationExtensionPointWorkbenchContribution.ID,
