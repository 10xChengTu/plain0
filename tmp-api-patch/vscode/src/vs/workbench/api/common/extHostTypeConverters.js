...
import { ToolInvocationPresentation, ToolDataSource } from '../../contrib/chat/common/tools/languageModelToolsService.js';
...
(function(TokenType) {
    function to(type) {
        ...
        case StandardTokenType.Comment:
            return StandardTokenType$1.Comment;
        case StandardTokenType.Other:
            return StandardTokenType$1.Other;
        case StandardTokenType.RegEx:
            return StandardTokenType$1.RegEx;
        case StandardTokenType.String:
            return StandardTokenType$1.String;
        }
    }
    TokenType.to = to;
})(TokenType || (TokenType = {}));
    ...
    function from(markup) {
        ...
        const collectUri = (
            ...
            try {
                ...
                uri = uri.with({
                    query: _uriMassage(uri.query, resUris)
                });
                resUris[href] = uri;
            } catch (e) {}
            return "";
    ...
    if (isDecorationOptionsArr(ranges)) {
        return ( ranges.map(r => {
            return {
                ...
                hoverMessage: Array.isArray(r.hoverMessage) ? MarkdownString.fromMany(r.hoverMessage) : (r.hoverMessage ? MarkdownString.from(r.hoverMessage) : undefined),
...
(function(SymbolTag) {
    function from(kind) {
        switch (kind) {
        case SymbolTag$1.Deprecated:
            return SymbolTag$2.Deprecated;
        }
    }
    SymbolTag.from = from;
    function to(kind) {
        switch (kind) {
        case SymbolTag$2.Deprecated:
            return SymbolTag$1.Deprecated;
        }
    }
    SymbolTag.to = to;
})(SymbolTag || (SymbolTag = {}));
...
(function(DocumentSymbol) {
    ...
    function to(info) {
        const result = new DocumentSymbol$1(
            info.name,
            info.detail,
            SymbolKind.to(info.kind),
            Range.to(info.range),
...
(function(CallHierarchyItem) {
    function to(item) {
        const result = new CallHierarchyItem$1(
...
(function(DefinitionLink) {
    function from(value) {
        ...
        return {
            originSelectionRange: definitionLink.originSelectionRange ? Range.from(definitionLink.originSelectionRange) : undefined,
            uri: definitionLink.targetUri ? definitionLink.targetUri : location.uri,
            range: Range.from(definitionLink.targetRange ? definitionLink.targetRange : location.range),
            targetSelectionRange: definitionLink.targetSelectionRange ? Range.from(definitionLink.targetSelectionRange) : undefined
        };
    }
...
(function(CompletionItemTag) {
    function from(kind) {
        switch (kind) {
        case CompletionItemTag$1.Deprecated:
            return CompletionItemTag$2.Deprecated;
        }
    }
    CompletionItemTag.from = from;
    function to(kind) {
        switch (kind) {
        case CompletionItemTag$2.Deprecated:
            return CompletionItemTag$1.Deprecated;
        }
    }
...
(function(CompletionCommand) {
    function from(c, converter, disposables) {
        ...
        return {
            command: converter.toInternal(c, disposables)
        };
    }
...
(function(CompletionItemKind) {
    const _from = ( new Map([
        ...
        [
            CompletionItemKind$1.Constructor,
            CompletionItemKind$2.Constructor
        ],
        ...
        [
            CompletionItemKind$1.EnumMember,
            CompletionItemKind$2.EnumMember
        ],
        ...
        [
            CompletionItemKind$1.TypeParameter,
            CompletionItemKind$2.TypeParameter
        ],
...
(function(ProgressLocation) {
    function from(loc) {
        ...
        switch (loc) {
        case ProgressLocation$1.SourceControl:
            return ProgressLocation$2.Scm;
        case ProgressLocation$1.Window:
            return ProgressLocation$2.Window;
        case ProgressLocation$1.Notification:
            return ProgressLocation$2.Notification;
        }
...
(function(TestCoverage) {
    ...
    function to(serialized) {
        if (serialized.type === DetailType.Statement) {
            ...
            return new StatementCoverage(
                ...
                serialized.branches?.map(b => new BranchCoverage(b.count, toLocation(b.location), b.label))
            );
        } else {
    ...
    function fromFile(controllerId, id, coverage) {
        ...
        return {
            ...
            testIds: coverage instanceof FileCoverage && coverage.includesTests.length ? ( coverage.includesTests.map(t => ( TestId.fromExtHostTestItem(t, controllerId).toString()))) : undefined
        };
    }
...
(function(TypeHierarchyItem) {
    function to(item) {
        const result = new TypeHierarchyItem$1(
            SymbolKind.to(item.kind),
            item.name,
            item.detail || "",
            URI.revive(item.uri),
...
(function(DataTransferItem) {
    function to(mime, item, resolveFileData) {
        ...
        if (file) {
            return new InternalFileDataTransferItem(new DataTransferFile(
                file.name,
                URI.revive(file.uri),
                file.id,
...
(function(LanguageModelChatMessageRole) {
    function to(role) {
        ...
        case ChatMessageRole.System:
            return LanguageModelChatMessageRole$1.System;
        case ChatMessageRole.User:
            return LanguageModelChatMessageRole$1.User;
        case ChatMessageRole.Assistant:
            return LanguageModelChatMessageRole$1.Assistant;
        }
    }
    LanguageModelChatMessageRole.to = to;
    function from(role) {
        switch (role) {
        case LanguageModelChatMessageRole$1.System:
            return ChatMessageRole.System;
        case LanguageModelChatMessageRole$1.User:
            return ChatMessageRole.User;
        case LanguageModelChatMessageRole$1.Assistant:
...
(function(LanguageModelChatMessage2) {
    function to(message) {
        ...
        const role = LanguageModelChatMessageRole.to(message.role);
        const result = new LanguageModelChatMessage2$1(role, content, message.name);
        return result;
    }
...
(function(ChatResponseCodeblockUriPart) {
    ...
    function to(part) {
        return new ChatResponseCodeblockUriPart$1(URI.revive(part.uri), part.isEdit, part.undoStopId);
    }
...
(function(ChatResponseQuestionCarouselPart) {
    ...
    function stringToQuestionType(type) {
        switch (type) {
        case "text":
            return ChatQuestionType.Text;
        case "singleSelect":
            return ChatQuestionType.SingleSelect;
        case "multiSelect":
            return ChatQuestionType.MultiSelect;
        default:
            return ChatQuestionType.Text;
        }
    }
    ...
    function to(part) {
        const questions = ( part.questions.map(q => new ChatQuestion(q.id, stringToQuestionType(q.type), q.title, {
            message: q.message ? (typeof q.message === "string" ? new MarkdownString$1(q.message) : MarkdownString.to(q.message)) : undefined,
            options: q.options?.map(opt => ({
                id: opt.id,
                label: opt.label,
                value: opt.value
        ...
        return new ChatResponseQuestionCarouselPart$1(questions, part.allowSkip);
    }
...
(function(ChatResponseMultiDiffPart) {
    ...
    function to(part) {
        ...
        return new ChatResponseMultiDiffPart$1(resources, part.multiDiffData.title, part.readOnly);
    }
...
(function(ChatResponseAnchorPart) {
    ...
    function to(part) {
        ...
        return new ChatResponseAnchorPart$1(
...
(function(ChatResponseHookPart) {
    ...
    function to(part) {
        return new ChatResponseHookPart$1(part.hookType, part.stopReason, part.systemMessage, part.metadata);
    }
    ChatResponseHookPart.to = to;
})(ChatResponseHookPart || (ChatResponseHookPart = {}));
var ChatResponseAutoModeResolutionPart;
(function(ChatResponseAutoModeResolutionPart) {
    const validLabels = ( new Set(["needs_reasoning", "no_reasoning", "fallback"]));
    function from(part) {
    ...
    ChatResponseAutoModeResolutionPart.from = from;
    function to(part) {
        return new ChatResponseAutoModeResolutionPart$1(
            part.resolvedModel,
    ...
    ChatResponseAutoModeResolutionPart.to = to;
})(
...
(function(ChatToolInvocationPart) {
    function from(part) {
        ...
        const presentation = part.presentation === "hidden" ? ToolInvocationPresentation.Hidden : part.presentation === "hiddenAfterComplete" ? ToolInvocationPresentation.HiddenAfterComplete : undefined;
    ...
    function convertToolSpecificData(data) {
        ...
        } else if (data instanceof ChatSubagentToolInvocationData) {
            return {
                kind: "subagent",
    ...
    function todoStatusEnumToString(status) {
        switch (status) {
        case ChatTodoStatus.NotStarted:
            return "not-started";
        case ChatTodoStatus.InProgress:
            return "in-progress";
        case ChatTodoStatus.Completed:
            return "completed";
        default:
            return "not-started";
        }
    ...
    function todoStatusStringToEnum(status) {
        switch (status) {
        case "not-started":
            return ChatTodoStatus.NotStarted;
        case "in-progress":
            return ChatTodoStatus.InProgress;
        case "completed":
            return ChatTodoStatus.Completed;
        default:
            return ChatTodoStatus.NotStarted;
        }
    }
    function to(part) {
        const toolInvocation = new ChatToolInvocationPart$1(part.toolId || part.toolName, part.toolCallId, part.errorMessage);
...
(function(ChatResponseCommandButtonPart) {
    ...
    function to(part, commandsConverter) {
        return new ChatResponseCommandButtonPart$1(commandsConverter.fromInternal(part.command) ?? {
...
(function(ChatResponseReferencePart) {
    function from(part) {
        const iconPath = ThemeIcon.isThemeIcon(part.iconPath) ? part.iconPath : URI.isUri(part.iconPath) ? {
            light: URI.revive(part.iconPath)
        } : (part.iconPath && "light" in part.iconPath && "dark" in part.iconPath && URI.isUri(part.iconPath.light) && URI.isUri(part.iconPath.dark) ? {
    ...
    function to(part) {
        ...
        return new ChatResponseReferencePart$1(
...
(function(ChatResponsePart) {
    function from(part, commandsConverter, commandDisposables) {
        ...
        } else if (part instanceof ChatResponseHookPart$1) {
        ...
        } else if (part instanceof ChatResponseWorkspaceEditPart$1) {
            return ChatResponseWorkspaceEditPart.from(part);
        } else if (part instanceof ChatResponseAutoModeResolutionPart$1) {
...
(function(ChatLocation) {
    function to(loc) {
        ...
        case ChatAgentLocation.Notebook:
            return ChatLocation$1.Notebook;
        case ChatAgentLocation.Terminal:
            return ChatLocation$1.Terminal;
        case ChatAgentLocation.Chat:
            return ChatLocation$1.Panel;
        case ChatAgentLocation.EditorInline:
            return ChatLocation$1.Editor;
        }
    }
    ChatLocation.to = to;
    function from(loc) {
        switch (loc) {
        case ChatLocation$1.Notebook:
            return ChatAgentLocation.Notebook;
        case ChatLocation$1.Terminal:
            return ChatAgentLocation.Terminal;
        case ChatLocation$1.Panel:
            return ChatAgentLocation.Chat;
        case ChatLocation$1.Editor:
...
(function(ChatSessionCustomizationType) {
    ...
    function to(id) {
        switch (id) {
        case "agent":
            return ChatSessionCustomizationType$1.Agent;
        case "skill":
            return ChatSessionCustomizationType$1.Skill;
        case "instructions":
            return ChatSessionCustomizationType$1.Instructions;
        case "prompt":
            return ChatSessionCustomizationType$1.Prompt;
        case "hook":
            return ChatSessionCustomizationType$1.Hook;
        case "plugins":
            return ChatSessionCustomizationType$1.Plugins;
        default:
            return new ChatSessionCustomizationType$1(id);
        }
    }
    ChatSessionCustomizationType.to = to;
...
(function(ChatPromptReference) {
    function to(variable, diagnostics, logService) {
        ...
        if (isUriComponents(value)) {
            ...
            value = new ChatReferenceBinaryData(variable.mimeType ?? "image/png", () => Promise.resolve(( new Uint8Array(( Object.values(variable.value))))), ref && URI.isUri(ref) ? ref : undefined, variable.isPasted, variable.isURL);
    ...
    function reviveMetadata(metadata) {
        return cloneAndChange(metadata, value => {
            if (value.$mid === MarshalledId.LanguageModelToolResult) {
                ...
                return new LanguageModelDataPart(buffer, value.mimeType, value.audience);
            }
            return undefined;
        });
    }
...
(function(InlineCompletionHintStyle) {
    function from(value) {
        if (value === InlineCompletionDisplayLocationKind.Label) {
            return InlineCompletionHintStyle$1.Label;
        } else {
            return InlineCompletionHintStyle$1.Code;
        }
    }
    InlineCompletionHintStyle.from = from;
    function to(kind) {
        switch (kind) {
        case InlineCompletionHintStyle$1.Label:
...
(function(LanguageModelToolResult) {
    function to(result) {
        const toolResult = new LanguageModelToolResult$1(( result.content.map(item => {
            if (item.kind === "text") {
...
(function(McpServerDefinition) {
    ...
    function to(dto) {
        ...
        if (launch.type === McpServerTransportType.HTTP) {
            ...
            const result = new McpStdioServerDefinition(
                dto.label,
                launch.command,
                [...launch.args],
...
(function(SourceControlInputBoxValidationType) {
    function from(type) {
        switch (type) {
        case SourceControlInputBoxValidationType$1.Error:
            return InputValidationType.Error;
        case SourceControlInputBoxValidationType$1.Warning:
            return InputValidationType.Warning;
        case SourceControlInputBoxValidationType$1.Information:
