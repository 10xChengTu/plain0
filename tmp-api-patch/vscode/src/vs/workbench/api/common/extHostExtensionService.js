...
import { Barrier, timeout, IntervalTimer, asPromise } from '../../../base/common/async.js';
...
import { ExtensionsActivator, HostExtension, EmptyExtension, ExtensionActivationTimes, ExtensionActivationTimesBuilder, ActivatedExtension } from './extHostExtensionActivator.js';
...
import { ExtensionIdentifierSet, ExtensionIdentifier, ExtensionIdentifierMap } from '../../../platform/extensions/common/extensions.js';
import { VSBuffer } from '../../../base/common/buffer.js';
import { ExtensionGlobalMemento, ExtensionMemento } from './extHostMemento.js';
import { ExtensionMode, ExtensionKind, RemoteAuthorityResolverError, ManagedResolvedAuthority } from './extHostTypes.js';
...
let AbstractExtHostExtensionService = AbstractExtHostExtensionService_1 = class AbstractExtHostExtensionService extends Disposable {
    constructor(
        ...
        _extHostManagedSockets,
        _extHostLanguageModels
    ) {
        super();
    ...
    _doActivateExtension(extensionDescription, reason) {
        ...
        return Promise.all([isESM ? this._loadESMModule(
            ...
            joinPath(extensionDescription.extensionLocation, entryPoint),
            activationTimesBuilder
        ), this._loadExtensionContext(extensionDescription, extensionInternalStore)]).then(values => {
            mark(
                `code/extHost/willActivateExtension/${extensionDescription.identifier.value}`
            );
            return AbstractExtHostExtensionService_1._callActivate(
                this._logService,
                extensionDescription.identifier,
                values[0],
                values[1],
    ...
    _loadExtensionContext(extensionDescription, extensionInternalStore) {
        ...
        return Promise.all([
            ...
            const messagePort = isProposedApiEnabled(extensionDescription) ? this._initData.messagePorts?.get(ExtensionIdentifier.toKey(extensionDescription.identifier)) : undefined;
            return ( Object.freeze({
                ...
                asAbsolutePath(relativePath) {
                    return join(extensionDescription.extensionLocation.fsPath, relativePath);
                },
                get storagePath() {
                ...
                get messagePassingProtocol() {
                    if (!messagePassingProtocol) {
                        ...
                        const onDidReceiveMessage = Event.buffer(
                            Event.fromDOMEventEmitter(messagePort, "message", e => e.data),
                            "onDidReceiveMessage"
                        );
                        messagePort.start();
    ...
    _handleEagerExtensions() {
        ...
        this._register(
            this._extHostWorkspace.onDidChangeWorkspace(e => this._handleWorkspaceContainsEagerExtensions(e.added))
        );
    ...
    async _doHandleExtensionTests() {
        ...
        if (!extensionDevelopmentLocationURI || !extensionTestsLocationURI) {
            throw ( new Error(( localize(2941, "Cannot load test runner."))));
        }
    ...
    async _activateAndGetResolver(remoteAuthority) {
        ...
        return {
            authorityPrefix,
            resolver: this._resolvers[authorityPrefix]
        };
    }
    async $resolveAuthority(remoteAuthorityChain, resolveAttempt) {
            ...
            try {
                if (i === resolvers.length - 1) {
                    ...
                    this._register(await this._extHostTunnelService.setTunnelFactory(
                        resolver,
                        ManagedResolvedAuthority.isManagedResolvedAuthority(result) ? result : undefined
                    ));
                } else {
        ...
        logInfo(
            `returned ${ManagedResolvedAuthority.isManagedResolvedAuthority(result) ? "managed authority" : `${result.host}:${result.port}`}`
        );
        let authority;
    ...
    async $startExtensionHost(extensionsDelta) {
        extensionsDelta.toAdd.forEach(
            extension => extension.extensionLocation = URI.revive(extension.extensionLocation)
        );
        const {
            globalRegistry,
            myExtensions
        } = applyExtensionsDelta(
            this._activationEventsReader,
...
AbstractExtHostExtensionService = AbstractExtHostExtensionService_1 = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, IHostUtils)), ( __param(2, IExtHostRpcService)), ( __param(3, IExtHostWorkspace)), ( __param(4, IExtHostConfiguration)), ( __param(5, ILogService)), ( __param(6, IExtHostInitDataService)), ( __param(7, IExtensionStoragePaths)), ( __param(8, IExtHostTunnelService)), ( __param(9, IExtHostTerminalService)), ( __param(10, IExtHostLocalizationService)), ( __param(11, IExtHostManagedSockets)), ( __param(12, IExtHostLanguageModels))], AbstractExtHostExtensionService));
