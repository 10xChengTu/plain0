...
let ExtHostAuthentication = class ExtHostAuthentication {
    ...
    getExtensionScopedSessionsEvent(extensionId) {
        ...
        return Event.chain(this._onDidChangeSessions.event, $ => ( $.filter(
    ...
    async $registerXaaAuthProvider(issuerComponents, serverMetadata, clientId, clientSecret, initialTokens) {
        ...
        if (!clientId) {
            this._logService.info(
                `Prompting user for client registration details for XAA issuer ${( issuer.toString())}`
            );
...
ExtHostAuthentication = ( __decorate([( __param(0, IExtHostRpcService)), ( __param(1, IExtHostInitDataService)), ( __param(2, IExtHostWindow)), ( __param(3, IExtHostUrlsService)), ( __param(4, IExtHostProgress)), ( __param(5, ILoggerService)), ( __param(6, ILogService))], ExtHostAuthentication));
class TaskSingler {
    constructor() {
...
let DynamicAuthProvider = class DynamicAuthProvider {
    constructor(
        ...
        this._logger = loggerService.createLogger(this.id, {
        ...
        this._disposable.add(
            this._tokenStore.onDidChangeSessions(e => this._onDidChangeSessions.fire(e))
        );
        this._createFlows = [];
        if (_serverMetadata.authorization_endpoint) {
            this._createFlows.push({
                label: ( localize(2935, "URL Handler")),
    ...
    async createSession(scopes, _options) {
        ...
        for (let i = 0; i < this._createFlows.length; i++) {
            ...
            try {
                token = await this._extHostProgress.withProgressFromSource({
                    ...
                    title: ( localize(2936, "Authenticating to '{0}'", this.label)),
                    cancellable: true
        ...
        this._logger.info(
            `Created ${token.refresh_token ? "refreshable" : "non-refreshable"} session for scopes: ${token.scope}${token.expires_in ? ` that expires in ${token.expires_in} seconds` : ""}`
        );
        return session;
    }
    ...
    async _createWithUrlHandler(scopes, progress, token) {
        ...
        progress.report({
            message: ( localize(2939, "Complete the authentication in the browser window that has opened."))
        });
        let code;
        try {
    ...
    async _generateNewClientId() {
        try {
            const registration = await fetchDynamicRegistration(
                ...
                this._initData.environment.appName,
                this._resourceMetadata?.scopes_supported
            );
...
DynamicAuthProvider = ( __decorate([( __param(0, IExtHostWindow)), ( __param(1, IExtHostUrlsService)), ( __param(2, IExtHostInitDataService)), ( __param(3, IExtHostProgress)), ( __param(4, ILoggerService))], DynamicAuthProvider));
class TokenStore {
    constructor(_persistence, initialTokens, _logger) {
        ...
        this._sessionsObservable = derivedOpts({
            equalsFn: (a, b) => equals(a, b, (a, b) => a.accessToken === b.accessToken)
        ...
        this._disposable.add(
            this._persistence.onDidChange(tokens => this._tokensObservable.set(tokens, undefined))
        );
    }
    get tokens() {
...
export { DynamicAuthProvider, ExtHostAuthentication, IExtHostAuthentication, TokenStore };
