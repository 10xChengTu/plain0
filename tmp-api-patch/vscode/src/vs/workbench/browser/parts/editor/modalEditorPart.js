...
import { $, addDisposableListener, EventType, EventHelper, Dimension, append, hide, show, setVisibility, isHTMLElement, getActiveElement } from '../../../../base/browser/dom.js';
...
import { WorkbenchToolBar, HiddenItemStrategy, MenuWorkbenchToolBar } from '../../../../platform/actions/browser/toolbar.js';
...
"workbench.action.files.save", "workbench.action.files.saveAll", "workbench.action.files.revert",
...
let ModalEditorPart = class ModalEditorPart {
    ...
    async create(options) {
        ...
        const sidebarToggleAction = disposables.add(( new Action(TOGGLE_MODAL_EDITOR_SIDEBAR_COMMAND_ID, ( localize(3773, "Toggle Sidebar")), ThemeIcon.asClassName(sidebarToggleIcon), true)));
        ...
        disposables.add(
            addDisposableListener(navigationContainer, EventType.DBLCLICK, e => EventHelper.stop(e, true))
        );
        const previousButton = disposables.add(( new Button(navigationContainer, {
            title: ( localize(3774, "Previous"))
        })));
        ...
        const nextButton = disposables.add(( new Button(navigationContainer, {
            title: ( localize(3775, "Next"))
        })));
        ...
        const sidebarResult = this.createSidebar(
            editorPartContainer,
            headerElement,
            options?.sidebar,
            modalContextKeyService,
            disposables
        );
        if (sidebarResult) {
        ...
        const modalInstantiationService = disposables.add(this.instantiationService.createChild(( new ServiceCollection([IContextKeyService, modalContextKeyService]))));
        ...
        disposables.add(Event.runAndSubscribe(editorPart.onDidChangeNavigation, (navigation => {
            if (navigation && navigation.total > 1) {
                ...
                navigationLabel.textContent = ( localize(3776, "{0} of {1}", navigation.current + 1, navigation.total));
        ...
        const editorActionsToolbar = disposables.add(
            scopedInstantiationService.createInstance(WorkbenchToolBar, editorActionsToolbarContainer, {
        ...
        disposables.add(
            Event.runAndSubscribe(modalEditorService.onDidActiveEditorChange, () => updateEditorActions())
        );
        disposables.add(
            modalEditorService.onDidEditorsChange(() => editorPart.enforceModalPartOptions())
        );
        disposables.add(
            scopedInstantiationService.createInstance(MenuWorkbenchToolBar, actionBarContainer, MenuId.ModalEditorTitle, {
                hiddenItemStrategy: HiddenItemStrategy.NoHide,
                highlightToggledItems: true,
                menuOptions: {
        ...
        const updateLabel = () => {
            ...
            if (activeEditor) {
                ...
                label.element.setResource({
                    resource: EditorResourceAccessor.getOriginalUri(activeEditor, {
                        supportSideBySide: SideBySideEditor.BOTH
                    }),
                    name: activeEditor.getName(),
                    description: activeEditor.getDescription(
                        labelFormat === "short" ? Verbosity.SHORT : labelFormat === "long" ? Verbosity.LONG : Verbosity.MEDIUM
                    ) || ""
                }, {
        ...
        disposables.add(
            Event.runAndSubscribe(modalEditorService.onDidActiveEditorChange, updateLabel)
        );
        ...
        const layout = sizeChanged => {
            const {
                width: modalWidth,
                height: modalHeight
            } = resizableElement.size;
            const {
                top: topPx,
                left: leftPx
            } = resizableElement.domNode.style;
            ...
            const onStop = () => {
                ...
                if (didDrag) {
                    ...
                    const centerTop = Math.max(
                        titleBarOffset,
                        (containerDimension.height - resizableElement.size.height) / 2
                    );
        ...
        const layoutModal = () => {
            ...
            if (!editorPart.maximized && editorPart.position) {
                ...
                const clampedTop = Math.max(
                    titleBarOffset,
                    Math.min(editorPart.position.top, titleBarOffset + availableHeight - height)
                );
...
ModalEditorPart = ( __decorate([( __param(1, IInstantiationService)), ( __param(2, IEditorService)), ( __param(3, IWorkbenchLayoutService)), ( __param(4, IKeybindingService)), ( __param(5, IHostService)), ( __param(6, IConfigurationService)), ( __param(7, IContextMenuService)), ( __param(8, IContextKeyService))], ModalEditorPart));
let ModalEditorPartImpl = class ModalEditorPartImpl extends EditorPart {
    ...
    constructor(
        ...
        super(editorPartsView, `workbench.parts.modalEditor.${id}`, ( localize(3777, "Modal Editor Area")), windowId, instantiationService, themeService, configurationService, storageService, layoutService, hostService, modalContextKeyService);
    ...
    doRemoveLastGroup() {
        ...
        if (
        !isHTMLElement(this.previousMainWindowActiveElement) || !this.previousMainWindowActiveElement.isConnected || mainEditorPartContainer?.contains(this.previousMainWindowActiveElement)) {
...
ModalEditorPartImpl = ModalEditorPartImpl_1 = ( __decorate([( __param(4, IInstantiationService)), ( __param(5, IThemeService)), ( __param(6, IConfigurationService)), ( __param(7, IStorageService)), ( __param(8, IWorkbenchLayoutService)), ( __param(9, IHostService)), ( __param(10, IContextKeyService))], ModalEditorPartImpl));
