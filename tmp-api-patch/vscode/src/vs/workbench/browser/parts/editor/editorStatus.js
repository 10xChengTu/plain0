...
import * as editorstatus from './media/editorstatus.css';
...
import { isEqual, extname, basename } from '../../../../base/common/resources.js';
...
TabFocusMode = ( __decorate([( __param(0, IConfigurationService))], TabFocusMode));
class StatusInputMode extends Disposable {
    constructor() {
        ...
        this._register(
            InputMode.onDidChangeInputMode(inputMode => this._onDidChange.fire(inputMode))
        );
    }
}
const nlsSingleSelectionRange = ( localize(3710, "Ln {0}, Col {1} ({2} selected)"));
const nlsSingleSelection = ( localize(3711, "Ln {0}, Col {1}"));
const nlsMultiSelectionRange = ( localize(3712, "{0} selections ({1} characters selected)"));
const nlsMultiSelection = ( localize(3713, "{0} selections"));
const nlsEOLLF = ( localize(3714, "LF"));
const nlsEOLCRLF = ( localize(3715, "CRLF"));
let EditorStatus = class EditorStatus extends Disposable {
    constructor(
        ...
        this.currentMarkerStatus = this._register(
            instantiationService.createInstance(ShowCurrentMarkerInStatusbarContribution)
        );
    ...
    registerListeners() {
        ...
        this._register(
            this.textFileService.untitled.onDidChangeEncoding(model => this.onResourceEncodingChange(model.resource))
        );
        this._register(
            this.textFileService.files.onDidChangeEncoding(model => this.onResourceEncodingChange((model.resource)))
        );
        ...
        this._register(Event.runAndSubscribe(
            this.inputMode.onDidChange,
            inputMode => this.onInputModeChange(inputMode ?? "insert")
        ));
    }
    registerCommands() {
    ...
    async showIndentationPicker() {
        ...
        if (!activeTextEditorControl) {
            return this.quickInputService.pick([{
                label: ( localize(3716, "No text editor active at this time"))
            }]);
        }
        if (this.editorService.activeEditor?.isReadonly()) {
            return this.quickInputService.pick([{
                label: ( localize(3717, "The active code editor is read-only."))
            }]);
        }
        const picks = ( [
        ...
        picks.splice(3, 0, {
            type: "separator",
            label: ( localize(3718, "convert file"))
        });
        picks.unshift({
            type: "separator",
            label: ( localize(3719, "change view"))
        });
        const action = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(3720, "Select Action")),
            matchOnDetail: true
        });
        return action?.run();
    }
    updateTabFocusModeElement(visible) {
        if (visible) {
            if (!this.tabFocusModeElement.value) {
                const text = ( localize(3721, "Tab Moves Focus"));
                this.tabFocusModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(3722, "Accessibility Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(3723, "Disable Accessibility Mode")),
    ...
    updateInputModeElement(inputMode) {
        if (inputMode === "overtype") {
            if (!this.inputModeElement.value) {
                const text = ( localize(3724, "OVR"));
                const name = ( localize(3725, "Enable Insert Mode"));
    ...
    updateColumnSelectionModeElement(visible) {
        if (visible) {
            if (!this.columnSelectionModeElement.value) {
                const text = ( localize(3726, "Column Selection"));
                this.columnSelectionModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(3727, "Column Selection Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(3728, "Disable Column Selection Mode")),
    ...
    updateSelectionElement(text) {
        ...
        const props = {
            name: ( localize(3729, "Editor Selection")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3730, "Go to Line/Column")),
    ...
    updateIndentationElement(text) {
        ...
        const props = {
            name: ( localize(3731, "Editor Indentation")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3732, "Select Indentation")),
    ...
    updateEncodingElement(text) {
        ...
        const props = {
            name: ( localize(3733, "Editor Encoding")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3734, "Select Encoding")),
    ...
    updateEOLElement(text) {
        ...
        const props = {
            name: ( localize(3735, "Editor End of Line")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3736, "Select End of Line Sequence")),
    ...
    updateLanguageIdElement(text) {
        ...
        const props = {
            name: ( localize(3737, "Editor Language")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3738, "Select Language Mode")),
    ...
    updateMetadataElement(text) {
        ...
        const props = {
            name: ( localize(3739, "File Information")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3740, "File Information"))
        };
        this.updateElement(
    ...
    onIndentationChange(editorWidget) {
        ...
        if (editorWidget) {
            ...
            if (model) {
                ...
                update.indentation = (modelOpts.insertSpaces ? modelOpts.tabSize === modelOpts.indentSize ? ( localize(3741, "Spaces: {0}", modelOpts.indentSize)) : ( localize(
                    3742,
                    "Spaces: {0} (Tab Size: {1})",
                    modelOpts.indentSize,
                    modelOpts.tabSize
                )) : ( localize(3743, "Tab Size: {0}", modelOpts.tabSize)));
            }
        }
...
EditorStatus = ( __decorate([( __param(1, IEditorService)), ( __param(2, IQuickInputService)), ( __param(3, ILanguageService)), ( __param(4, ITextFileService)), ( __param(5, IStatusbarService)), ( __param(6, IInstantiationService)), ( __param(7, IConfigurationService))], EditorStatus));
let EditorStatusContribution = class EditorStatusContribution extends Disposable {
    ...
    constructor(editorGroupService) {
        ...
        this._register(
            editorGroupService.onDidCreateAuxiliaryEditorPart(part => this.createEditorStatus(part))
        );
    }
...
EditorStatusContribution = ( __decorate([( __param(0, IEditorGroupsService))], EditorStatusContribution));
let ShowCurrentMarkerInStatusbarContribution = class ShowCurrentMarkerInStatusbarContribution extends Disposable {
    constructor(statusbarService, markerService, configurationService) {
        ...
        this._register(
            markerService.onMarkerChanged(changedResources => this.onMarkerChanged(changedResources))
        );
        this._register(Event.filter(
            configurationService.onDidChangeConfiguration,
            e => e.affectsConfiguration("problems.showCurrentInStatus")
        )(() => this.updateStatus()));
    }
    update(editor) {
        this.editor = editor;
            ...
            if (this.currentMarker) {
                ...
                if (!this.statusBarEntryAccessor.value) {
                    this.statusBarEntryAccessor.value = this.statusbarService.addEntry({
                        name: ( localize(3744, "Current Problem")),
                        text,
                        ariaLabel: text
...
ShowCurrentMarkerInStatusbarContribution = ( __decorate([( __param(0, IStatusbarService)), ( __param(1, IMarkerService)), ( __param(2, IConfigurationService))], ShowCurrentMarkerInStatusbarContribution));
class ChangeLanguageAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(3745, "Change Language Mode")),
            f1: true,
            keybinding: {
            ...
            metadata: {
                description: ( localize(3746, "Change the language mode of the active text editor.")),
                args: [{
                    name: ( localize(3747, "The name of the language mode to change to.")),
    ...
    async run(accessor, languageMode) {
        ...
        const picks = ( languages.map((
            ...
            if (currentLanguageName === languageName) {
                description = ( localize(3748, "({0}) - Configured Language", languageId));
            } else {
                description = ( localize(3749, "({0})", languageId));
            }
            return {
                id: languageId,
                label: languageName,
        ...
        picks.unshift({
            type: "separator",
            label: ( localize(3750, "languages (identifier)"))
        });
        ...
        if (hasLanguageSupport && resource) {
            ...
            if (galleryService.isEnabled()) {
                galleryAction = toAction({
                    ...
                    label: ( localize(3751, "Search Marketplace Extensions for '{0}'...", ext)),
            ...
            configureLanguageSettings = {
                label: ( localize(3752, "Configure '{0}' language based settings...", currentLanguageName))
            };
            picks.unshift(configureLanguageSettings);
            configureLanguageAssociations = {
                label: ( localize(3753, "Configure File Association for '{0}'...", ext))
            };
            picks.unshift(configureLanguageAssociations);
        }
        const autoDetectLanguage = {
            label: ( localize(3754, "Auto Detect"))
        };
        ...
        const pick = typeof languageMode === "string" ? {
            ...
            placeHolder: ( localize(3755, "Select Language Mode")),
    ...
    configureFileAssociation(resource, languageService, quickInputService, configurationService) {
        ...
        const picks = ( languages.map((
            ...
            return {
                ...
                label: languageName,
                iconClasses: getIconClassesForLanguageId(languageId),
                description: (languageId === currentAssociation) ? ( localize(3756, "Current Association")) : undefined
            };
        }));
        setTimeout(async () => {
            const language = await quickInputService.pick(picks, {
                placeHolder: ( localize(3757, "Select Language Mode to Associate with '{0}'", extension || base))
            });
            if (language) {
...
class ChangeEOLAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3758, "Change End of Line Sequence")),
            f1: true
        });
    }
    async run(accessor) {
        ...
        const eol = await quickInputService.pick(EOLOptions, {
            placeHolder: ( localize(3759, "Select End of Line Sequence")),
...
class ChangeEncodingAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3760, "Change File Encoding")),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (!encodingSupport) {
            await quickInputService.pick([{
                label: ( localize(3761, "No file active at this time"))
            }]);
            return;
        }
        const saveWithEncodingPick = {
            label: ( localize(3762, "Save with Encoding"))
        };
        const reopenWithEncodingPick = {
            label: ( localize(3763, "Reopen with Encoding"))
        };
        ...
        if (guessedEncoding && configuredEncoding !== guessedEncoding && SUPPORTED_ENCODINGS[guessedEncoding]) {
            ...
            picks.unshift({
                ...
                description: ( localize(3764, "Guessed from content"))
            });
        }
        const encoding = await quickInputService.pick(picks, {
            placeHolder: isReopenWithEncoding ? ( localize(3765, "Select File Encoding to Reopen File")) : ( localize(3766, "Select File Encoding to Save with")),
        ...
        if (typeof encoding.id !== "undefined" && activeEncodingSupport) {
            if (isReopenWithEncoding && editorService.activeEditorPane.input.isDirty()) {
                const {
                    ...
                    detail: ( localize(3768, "This will discard any unsaved changes.")),
                    primaryButton: ( localize(3769, "Discard Changes and Reopen"))
                });
                if (!confirmed) {
                    return;
                }
...
export { ChangeEOLAction, ChangeEncodingAction, ChangeLanguageAction, EditorStatusContribution };
