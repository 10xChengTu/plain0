...
import * as multieditortabscontrol from './media/multieditortabscontrol.css';
import { isWindows, isLinux, isMacintosh } from '../../../../base/common/platform.js';
import { shorten } from '../../../../base/common/labels.js';
import { DEFAULT_EDITOR_ASSOCIATION, EditorsOrder, preventEditorClose, EditorCloseMethod, Verbosity, EditorInputCapabilities, EditorResourceAccessor, SideBySideEditor } from '../../../common/editor.js';
...
import { DraggedEditorGroupIdentifier, DraggedEditorIdentifier, isWindowDraggedOver, extractTreeDropData, ResourcesDropHandler } from '../../dnd.js';
import { INotificationService } from '../../../../platform/notification/common/notification.service.js';
import { MergeGroupMode } from '../../../services/editor/common/editorGroupsService.js';
import { Dimension, $, addDisposableListener, EventType, EventHelper, DragAndDropObserver, isMouseEvent, getWindow, clearNode, scheduleAtNextAnimationFrame, findParentWithClass } from '../../../../base/browser/dom.js';
...
registerCss(multieditortabscontrol);
let MultiEditorTabsControl = class MultiEditorTabsControl extends EditorTabsControl {
    ...
    constructor(
        ...
        this.closeEditorAction = this._register(
            this.instantiationService.createInstance(CloseEditorTabAction, CloseEditorTabAction.ID, CloseEditorTabAction.LABEL)
        );
        this.unpinEditorAction = this._register(
            this.instantiationService.createInstance(UnpinEditorAction, UnpinEditorAction.ID, UnpinEditorAction.LABEL)
        );
        this.tabResourceLabels = this._register(
        ...
        this._register(
            this.tabResourceLabels.onDidChangeDecorations(() => this.doHandleDecorationsChange())
        );
    }
    create(parent) {
        super.create(parent);
    ...
    createAddTabControl() {
        ...
        const menu = this._register(
            this.menuService.createMenu(MenuId.EditorTabsBarAddTab, this.contextKeyService)
        );
        const getActions = () => getFlatActionBarActions(menu.getActions({
            shouldForwardArgs: true
        }));
        const addTabAction = toAction({
            id: "editor.tabs.addTab",
            label: ( localize(3778, "Add Tab")),
            class: ThemeIcon.asClassName(Codicon.add),
            run: () => {}
        });
        const dropdown = this._register(( new DropdownMenuActionViewItem(addTabAction, {
            getActions
    ...
    registerTabsContainerListeners(tabsContainer, tabsScrollbar) {
        ...
        this._register(
            addDisposableListener(tabsContainer, EventType$1.Contextmenu, e => showContextMenu(e))
        );
        this._register(
            addDisposableListener(tabsContainer, EventType.CONTEXT_MENU, e => showContextMenu(e))
        );
    }
    ...
    handleOpenedEditors() {
        ...
        let didChange = false;
        if (
        activeEditorChanged || oldTabLabels.length !== this.tabLabels.length || ( oldTabLabels.some(
            (label, index) => !this.equalsEditorInputLabel(label, this.tabLabels.at(index))
        ))) {
            this.redraw({
    ...
    didActiveEditorChange() {
        if (
        !this.activeTabLabel?.editor && this.tabsModel.activeEditor || this.activeTabLabel?.editor && !this.tabsModel.activeEditor || (!this.activeTabLabel?.editor || !this.tabsModel.isActive(this.activeTabLabel.editor))) {
            return true;
        }
        return false;
    }
    ...
    pinEditor(editor) {
        this.withTab(
            editor,
            (editor, tabIndex, tabContainer, tabLabelWidget, tabLabel) => this.redrawTabLabel(editor, tabIndex, tabContainer, tabLabelWidget, tabLabel)
        );
    }
    stickEditor(editor) {
    ...
    doHandleStickyEditorChange(editor) {
        this.withTab(
            editor,
            (editor, tabIndex, tabContainer, tabLabelWidget, tabLabel, tabActionBar) => this.redrawTab(editor, tabIndex, tabContainer, tabLabelWidget, tabLabel, tabActionBar)
        );
    ...
    updateEditorDirty(editor) {
        this.withTab(
            editor,
            (editor, tabIndex, tabContainer, tabLabelWidget, tabLabel, tabActionBar) => this.redrawTabSelectedActiveAndDirty(
                this.groupsView.activeGroup === this.groupView,
                editor,
                tabContainer,
                tabActionBar
    ...
    createTab(tabIndex, tabsContainer, tabsScrollbar) {
        ...
        const tabActionBar = ( new ActionBar(tabActionsContainer, {
            ariaLabel: ( localize(3779, "Tab actions")),
        ...
        this.tabDisposables.push(
            combinedDisposable(gestureDisposable, eventsDisposable, tabActionBarDisposable, editorLabel)
        );
        return tabContainer;
    }
    ...
    registerTabListeners(tab, tabIndex, tabsContainer, tabsScrollbar) {
        ...
        disposables.add(
            addDisposableListener(tab, EventType.MOUSE_DOWN, e => handleClickOrTouch(e, false))
        );
        disposables.add(
            addDisposableListener(tab, EventType$1.Tap, e => handleClickOrTouch(e, true))
        );
        ...
        disposables.add(addDisposableListener(tab, EventType.KEY_UP, e => {
            ...
            if (event.equals(KeyCode.Enter) || event.equals(KeyCode.Space)) {
                ...
                if (target) {
                    handled = true;
                    this.groupView.openEditor(target, {
                        preserveFocus: true
                    }, {
                        focusTabControl: true
                    });
    ...
    computeDropTarget(e, tabIndex, targetTab) {
        ...
        if (isLeftSideOfTab && isFirstTab) {
            return {
                ...
                rightElement: targetTab
            };
        }
        if (!isLeftSideOfTab && isLastTab) {
            return {
                leftElement: targetTab,
                rightElement: undefined
            };
        }
        ...
        for (const [, duplicateLabels] of mapNameToDuplicates) {
            ...
            for (const [, duplicateLabels] of mapDescriptionToDuplicates) {
                if (!useLongDescriptions && duplicateLabels.length > 1) {
                    const [first, ...rest] = ( duplicateLabels.map((
                        {
                            editor
                        }
    ...
    redrawTabLabel(editor, tabIndex, tabContainer, tabLabelWidget, tabLabel) {
        ...
        tabLabelWidget.setResource({
            ...
            extraClasses: coalesce(
                ["tab-label", fileDecorationBadges ? "tab-label-has-badge" : undefined].concat(editor.getLabelExtraClasses())
            ),
    ...
    doLayoutTabsWrapping(dimensions) {
        ...
        function updateTabsWrapping(enabled) {
            ...
            tabsContainer.style.setProperty(
                ...
                tabsWrapMultiLine ? `${editorToolbarWidth()}px` : "0"
            );
            tabsAndActionsContainer.style.setProperty(
                "--last-tab-layout-actions-width",
                `${layoutActionsContainer?.offsetWidth ?? 0}px`
            );
        ...
        if (this.groupsView.partOptions.wrapTabs) {
            ...
            if (tabsWrapMultiLine) {
                if (
                (tabsContainer.offsetHeight > dimensions.available.height) || (allTabsWidth === visibleTabsWidth && tabsContainer.offsetHeight === this.tabHeight) || (!lastTabFitsWrapped())) {
    ...
    doLayoutTabsNonWrapping(options) {
        ...
        const {
            width: oldVisibleTabsWidth,
            scrollWidth: oldAllTabsWidth
        ...
        if (
        this.blockRevealActiveTab || typeof activeTabPosX !== "number" || typeof activeTabWidth !== "number" || activeTabPositionStatic || (!dimensionsChanged && !options?.forceRevealActiveTab)) {
...
MultiEditorTabsControl = MultiEditorTabsControl_1 = ( __decorate([( __param(5, IContextMenuService)), ( __param(6, IInstantiationService)), ( __param(7, IContextKeyService)), ( __param(8, IKeybindingService)), ( __param(9, INotificationService)), ( __param(10, IQuickInputService)), ( __param(11, IThemeService)), ( __param(12, IEditorService)), ( __param(13, IPathService)), ( __param(14, ITreeViewsDnDService)), ( __param(15, IEditorResolverService)), ( __param(16, IHostService)), ( __param(17, IMenuService))], MultiEditorTabsControl));
