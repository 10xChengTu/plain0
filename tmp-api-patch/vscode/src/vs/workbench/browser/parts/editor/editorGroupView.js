...
import * as editorgroupview from './media/editorgroupview.css';
...
import { EditorResourceAccessor, SideBySideEditor, EditorInputCapabilities, TEXT_DIFF_EDITOR_ID, GroupModelChangeKind, DEFAULT_EDITOR_ASSOCIATION, EditorsOrder, EditorCloseContext, SaveReason, CloseDirection } from '../../../common/editor.js';
...
import { $, addDisposableListener, EventType, EventHelper, getWindow, trackFocus, isMouseEvent, findParentWithClass, getActiveElement, isActiveElement, getRootContainer, isAncestor, Dimension } from '../../../../base/browser/dom.js';
...
import { isMacintosh, isNative, isWindows, isLinux } from '../../../../base/common/platform.js';
...
let EditorGroupView = EditorGroupView_1 = class EditorGroupView extends Themable {
    ...
    constructor(
        ...
        }
        {
            ...
            this.element.classList.add(
                ...coalesce(["editor-group-container", this.model.isLocked ? "locked" : undefined])
            );
            this.registerContainerListeners();
            this.createContainerToolbar();
            this.createContainerContextMenu();
            this._register(
                this.instantiationService.createInstance(EditorGroupWatermark, this.element)
            );
            ...
            this.titleControl = this._register(this.scopedInstantiationService.createInstance(
                EditorTitleControl,
                this.titleContainer,
            ...
            this.editorPane = this._register(
                this.scopedInstantiationService.createInstance(EditorPanes, this.element, this.editorContainer, this)
            );
    ...
    handleGroupContextKeys() {
        ...
        const observeActiveEditor = () => {
            ...
            this.scopedContextKeyService.bufferChangeEvents(() => {
                ...
                if (activeEditorPane) {
                    ...
                    const primaryEditorResource = EditorResourceAccessor.getOriginalUri(activeEditorPane.input, {
                        supportSideBySide: SideBySideEditor.PRIMARY
                    });
                    const secondaryEditorResource = EditorResourceAccessor.getOriginalUri(activeEditorPane.input, {
                        supportSideBySide: SideBySideEditor.SECONDARY
                    });
                    groupActiveCompareEditorCanSwap.set(
                        activeEditorPane.input instanceof DiffEditorInput && !activeEditorPane.input.original.isReadonly() && !!primaryEditorResource && (this.fileService.hasProvider(primaryEditorResource) || primaryEditorResource.scheme === Schemas.untitled) && !!secondaryEditorResource && (this.fileService.hasProvider(secondaryEditorResource) || secondaryEditorResource.scheme === Schemas.untitled)
                    );
                    groupActiveEditorCanToggleReadonly.set(
                        !!primaryEditorResource && this.fileService.hasProvider(primaryEditorResource) && !this.fileService.hasCapability(primaryEditorResource, FileSystemProviderCapabilities.Readonly)
                    );
        ...
        const updateGroupContextKeys = e => {
            ...
            case GroupModelChangeKind.EDITORS_SELECTION:
                ...
                selectedEditorsHaveFileOrUntitledResourceContext.set(this.model.selectedEditors.every(
                    e => e.resource && (this.fileService.hasProvider(e.resource) || e.resource.scheme === Schemas.untitled)
                ));
                break;
            }
    ...
    createContainerToolbar() {
        ...
        const containerToolbar = this._register(( new ActionBar(toolbarContainer, {
            ariaLabel: ( localize(3673, "Empty editor group actions")),
            highlightToggledItems: true
        })));
        const containerToolbarMenu = this._register(
            this.menuService.createMenu(MenuId.EmptyEditorGroup, this.scopedContextKeyService)
        );
        const updateContainerToolbar = () => {
            this.containerToolBarMenuDisposable.value = toDisposable(() => containerToolbar.clear());
            const actions = getActionBarActions(containerToolbarMenu.getActions({
                arg: {
                    groupId: this.id
                },
    ...
    doTrackFocus() {
        ...
        this._register(
            addDisposableListener(this.titleContainer, EventType.MOUSE_DOWN, e => handleTitleClickOrTouch(e))
        );
        this._register(
            addDisposableListener(this.titleContainer, EventType$1.Tap, e => handleTitleClickOrTouch(e))
        );
    ...
    updateContainer() {
        if (this.isEmpty) {
            ...
            this.element.setAttribute("aria-label", ( localize(3674, "{0} (empty)", this.ariaLabel)));
        }
        else {
    ...
    registerListeners() {
        ...
        this._register(
            this.groupsView.onDidChangeEditorPartOptions(e => this.onDidChangeEditorPartOptions(e))
        );
    ...
    get label() {
        if (this.groupsLabel) {
            return localize(3675, "{0}: Group {1}", this.groupsLabel, this._index + 1);
        }
        return localize(3676, "Group {0}", this._index + 1);
    }
    get ariaLabel() {
        if (this.groupsLabel) {
            return localize(3677, "{0}: Editor Group {1}", this.groupsLabel, this._index + 1);
        }
        return localize(3678, "Editor Group {0}", this._index + 1);
    }
    get disposed() {
    ...
    async setSelection(activeSelectedEditor, inactiveSelectedEditors) {
        if (!this.isActive(activeSelectedEditor)) {
            await this.openEditor(activeSelectedEditor, {
    ...
    async doOpenEditor(editor, options, internalOptions) {
        ...
        const {
            editor: openedEditor,
            isNew
        } = this.model.openEditor(editor, openEditorOptions);
        if (
        isNew && this.count === 1 && this.editorPartsView.groups.length > 1) {
            if (openedEditor.editorId && this.groupsView.partOptions.autoLockGroups?.has(openedEditor.editorId)) {
                this.lock(true);
            }
        }
        const showEditorResult = this.doShowEditor(openedEditor, {
            active: !!openEditorOptions.active,
            isNew
    ...
    async openEditors(editors) {
        ...
        await Promises.settled(( inactiveEditors.map((
            {
                editor,
                options
            },
        ...
        this.titleControl.openEditors(( inactiveEditors.map((
            {
                editor
            }
        ) => editor)));
    ...
    doMoveOrCopyEditorAcrossGroups(editor, target, openOptions, internalOptions) {
        ...
        if (!keepCopy || editor.hasCapability(EditorInputCapabilities.Singleton) ) {
            ...
            if (typeof canMoveVeto === "string") {
                this.dialogService.error(canMoveVeto, ( localize(3679, "Try saving or reverting the editor first and then try again.")));
                return false;
            }
        }
    ...
    copyEditors(editors, target) {
        ...
        if (internalOptions.skipTitleUpdate) {
            const copiedEditors = ( editors.map((
                {
                    editor
                }
            ) => editor));
        ...
        for (const {
            ...
            if (!editor.matches(replacement)) {
                ...
                if (forceReplaceDirty) {
                    ...
                    closed = await this.doCloseEditorWithConfirmationHandling(editor, {
                        preserveFocus: true
                    }, {
                        context: EditorCloseContext.REPLACE
                    });
        ...
        if (activeReplacement) {
            ...
            if (!activeReplacement.editor.matches(activeReplacement.replacement)) {
                if (activeReplacement.forceReplaceDirty) {
                    ...
                    await this.doCloseEditorWithConfirmationHandling(activeReplacement.editor, {
                        preserveFocus: true
                    }, {
                        context: EditorCloseContext.REPLACE
                    });
    ...
    createEditorActions(disposables, menuId = MenuId.EditorTitle) {
        ...
        if (activeEditorPane instanceof EditorPane) {
            ...
            const editorTitleMenu = disposables.add(this.menuService.createMenu(menuId, editorScopedContextKeyService, {
            ...
            actions = getActionBarActions(editorTitleMenu.getActions({
                arg: this.resourceContext.get(),
                shouldForwardArgs: true,
                renderShortTitle: true
            }), "navigation", shouldInlineGroup);
        } else {
...
EditorGroupView = EditorGroupView_1 = ( __decorate([( __param(6, IInstantiationService)), ( __param(7, IContextKeyService)), ( __param(8, IThemeService)), ( __param(9, ITelemetryService)), ( __param(10, IKeybindingService)), ( __param(11, IMenuService)), ( __param(12, IContextMenuService)), ( __param(13, IFileDialogService)), ( __param(14, IEditorService)), ( __param(15, IFilesConfigurationService)), ( __param(16, IUriIdentityService)), ( __param(17, ILogService)), ( __param(18, IEditorResolverService)), ( __param(19, IHostService)), ( __param(20, IDialogService)), ( __param(21, IFileService))], EditorGroupView));
