...
import * as editorplaceholder from './media/editorplaceholder.css';
...
import { FileOperationResult, FileChangeType } from '../../../../platform/files/common/files.js';
...
let WorkspaceTrustRequiredPlaceholderEditor = class WorkspaceTrustRequiredPlaceholderEditor extends EditorPlaceholder {
    ...
    static {
        this.LABEL = ( localize(3695, "Workspace Trust Required"));
    }
    static {
        this.DESCRIPTOR = EditorPaneDescriptor.create(this, this.ID, this.LABEL);
    }
    constructor(
        group,
        telemetryService,
        ...
        super(
            WorkspaceTrustRequiredPlaceholderEditor_1.ID,
            group,
            telemetryService,
            themeService,
            storageService
    ...
    async getContents() {
        return {
            ...
            actions: [{
                label: ( localize(3698, "Manage Workspace Trust")),
...
let ErrorPlaceholderEditor = class ErrorPlaceholderEditor extends EditorPlaceholder {
    ...
    static {
        this.LABEL = ( localize(3699, "Error Editor"));
    }
    static {
        this.DESCRIPTOR = EditorPaneDescriptor.create(this, this.ID, this.LABEL);
    }
    ...
    async getContents(input, options, disposables) {
        ...
        let label;
        if (isFileNotFound) {
            label = ( localize(3700, "The editor could not be opened because the file was not found."));
            ...
            label = ( localize(3702, "The editor could not be opened due to an unexpected error."));
        }
        let icon = "$(error)";
        ...
        if (isEditorOpenError(error) && error.actions.length > 0) {
            ...
            actions = [{
                label: ( localize(3703, "Try Again")),
                run: () => this.group.openEditor(input, {
                    ...options,
                    source: EditorOpenSource.USER
                })
            }, {
                label: ( localize(3704, "Show Logs")),
...
ErrorPlaceholderEditor = ErrorPlaceholderEditor_1 = ( __decorate([( __param(1, ITelemetryService)), ( __param(2, IThemeService)), ( __param(3, IStorageService)), ( __param(4, IFileService)), ( __param(5, IDialogService)), ( __param(6, ICommandService))], ErrorPlaceholderEditor));
