...
import * as editortabscontrol from './media/editortabscontrol.css';
import { localize } from '../../../../nls.js';
import { DataTransfers } from '../../../../base/browser/dnd.js';
import { $, setVisibility, getActiveWindow, isMouseEvent, getWindow } from '../../../../base/browser/dom.js';
...
let EditorTabsControl = class EditorTabsControl extends Themable {
    ...
    doCreateEditorActionsToolBar(container) {
        ...
        this.editorActionsToolbar = this.editorActionsToolbarDisposables.add(this.instantiationService.createInstance(WorkbenchToolBar, container, {
            ...
            ariaLabel: ( localize(3770, "Editor actions")),
    ...
    doCreateEditorLayoutActionsToolBar(container) {
        ...
        this.editorLayoutActionsToolbar = this.editorLayoutActionsToolbarDisposables.add(this.instantiationService.createInstance(WorkbenchToolBar, container, {
            actionViewItemProvider: (action, options) => this.actionViewItemProvider(action, options),
            orientation: ActionsOrientation.HORIZONTAL,
            ariaLabel: ( localize(3771, "Editor layout actions")),
            getKeyBinding: action => this.getKeybinding(action),
    ...
    updateEditorLayoutActionsToolbar() {
        ...
        const {
            primary,
            secondary
        } = this.prepareEditorLayoutActions(editorActions.actions);
        ...
        if (this.editorLayoutActionsSeparator) {
            setVisibility(
                ...
                this.editorLayoutActionsSeparator
            );
        }
    }
    ...
    onGroupDragStart(e, element) {
        ...
        if (this.groupView.activeEditor) {
            ...
            if (this.groupsView.partOptions.showTabs === "multiple" && this.groupView.count > 1) {
                label = ( localize(3772, "{0} (+{1})", label, this.groupView.count - 1));
            }
    ...
    get tabHeight() {
        ...
        if (this.parent.classList.contains("tabs") && this.parent.closest(".style-override")) {
            return isCompact ? EditorTabsControl_1.EDITOR_TAB_HEIGHT.styleOverrideCompact : EditorTabsControl_1.EDITOR_TAB_HEIGHT.styleOverride;
        }
        return isCompact ? EditorTabsControl_1.EDITOR_TAB_HEIGHT.compact : EditorTabsControl_1.EDITOR_TAB_HEIGHT.normal;
    }
