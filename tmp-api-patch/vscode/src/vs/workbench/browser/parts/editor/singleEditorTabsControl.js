...
import * as singleeditortabscontrol from './media/singleeditortabscontrol.css';
...
registerCss(singleeditortabscontrol);
class SingleEditorTabsControl extends EditorTabsControl {
    ...
    create(parent) {
        ...
        this.editorLabel = this._register(
            this.instantiationService.createInstance(ResourceLabel, labelContainer, {})
        ).element;
        this._register(
            addDisposableListener(this.editorLabel.element, EventType.CLICK, e => this.onTitleLabelClick(e))
        );
        this.breadcrumbsControlFactory = this._register(
            this.instantiationService.createInstance(BreadcrumbsControlFactory, labelContainer, this.groupView, {
                showFileIcons: false,
        ...
        this._register(
            this.breadcrumbsControlFactory.onDidEnablementChange(() => this.handleBreadcrumbsEnablementChange())
        );
    ...
    registerContainerListeners(titleContainer) {
        ...
        this._register(
            addDisposableListener(titleContainer, EventType.AUXCLICK, e => this.onTitleAuxClick(e))
        );
        this._register(
            addDisposableListener(titleContainer, EventType$1.Tap, e => this.onTitleTap(e))
        );
    ...
    ifActiveEditorChanged(fn) {
        if (
        !this.activeLabel.editor && this.tabsModel.activeEditor || this.activeLabel.editor && !this.tabsModel.activeEditor || (!this.activeLabel.editor || !this.tabsModel.isActive(this.activeLabel.editor))) {
            fn();
            return true;
        }
        return false;
