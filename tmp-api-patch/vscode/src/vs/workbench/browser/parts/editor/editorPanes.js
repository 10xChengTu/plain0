...
import { getActiveElement, getWindowById, isEditableElement, isAncestor, show, Dimension, $, hide } from '../../../../base/browser/dom.js';
...
let EditorPanes = class EditorPanes extends Disposable {
    ...
    registerListeners() {
        this._register(
            this.workspaceTrustService.onDidChangeTrust(() => this.onDidChangeWorkspaceTrust())
        );
    }
    ...
    async doShowErrorDialog(error, editor) {
        ...
        if (!message) {
            message = ( localize(3692, "Unable to open '{0}'", editor.getName()));
        }
        const buttons = [];
    ...
    async doOpenEditor(
        ...
        const {
            changed,
            cancelled
        } = await this.doSetInput(pane, editor, options, context);
        if (!cancelled) {
    ...
    doSetActiveEditorPane(editorPane) {
        ...
        if (editorPane) {
            this.activeEditorPaneDisposables.add(
                editorPane.onDidChangeSizeConstraints(e => this._onDidChangeSizeConstraints.fire(e))
            );
...
EditorPanes = ( __decorate([( __param(3, IWorkbenchLayoutService)), ( __param(4, IInstantiationService)), ( __param(5, IEditorProgressService)), ( __param(6, IWorkspaceTrustManagementService)), ( __param(7, ILogService)), ( __param(8, IDialogService)), ( __param(9, IHostService))], EditorPanes));
