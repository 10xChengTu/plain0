...
import { GroupActivationReason, GroupsOrder, GroupLocation } from '../../../services/editor/common/editorGroupsService.js';
import { IInstantiationService } from '../../../../platform/instantiation/common/instantiation.js';
import { Emitter } from '../../../../base/common/event.js';
import { DisposableMap, toDisposable, DisposableStore } from '../../../../base/common/lifecycle.js';
...
let EditorParts = class EditorParts extends MultiWindowParts {
            ...
            if (!mainPartInstantiationService) {
                mainPartInstantiationService = this.instantiationService.invokeFunction(accessor => {
                    ...
                    const mainPartInstantiationService = this._register(this.mainPart.scopedInstantiationService.createChild(( new ServiceCollection(
                        ...
                        [
                            IStatusbarService,
                            statusbarService.createScoped(statusbarService, this._store)
                        ]
                    ))));
    ...
    async createAuxiliaryEditorPart(options) {
        const {
            ...
            instantiationService,
            disposables
        } = await this.instantiationService.createInstance(AuxiliaryEditorPart, this).create(this.getGroupsLabel(this._parts.size), options);
        this.mapPartToInstantiationService.set(part.windowId, instantiationService);
        disposables.add(
            toDisposable(() => this.mapPartToInstantiationService.delete(part.windowId))
        );
    ...
    async createModalEditorPart(options) {
        ...
        const {
            ...
            instantiationService,
            disposables
        } = await this.instantiationService.createInstance(ModalEditorPart, this).create({
            ...options,
    ...
    registerEditorPartListeners(part, disposables) {
        ...
        disposables.add(
            part.onDidChangeActiveGroup(group => this._onDidActiveGroupChange.fire(group))
        );
        ...
        disposables.add(
            part.onDidChangeGroupMaximized(maximized => this._onDidChangeGroupMaximized.fire(maximized))
        );
        disposables.add(
            part.onDidChangeGroupIndex(group => this._onDidChangeGroupIndex.fire(group))
        );
        disposables.add(
            part.onDidChangeGroupLocked(group => this._onDidChangeGroupLocked.fire(group))
        );
    }
    ...
    getGroupsLabel(index) {
        return localize(3694, "Window {0}", index + 1);
    }
    ...
    static {
        this.MODAL_EDITOR_STATE_STORAGE_KEY = "editorparts.modalState";
    }
    get isReady() {
    ...
    saveModalState() {
        ...
        if (this.modalEditorMaximized || this.modalEditorSize || this.modalEditorPosition || this.modalEditorSidebarWidth || this.modalEditorSidebarHidden) {
            this.profileMemento[EditorParts_1.MODAL_EDITOR_STATE_STORAGE_KEY] = {
                ...
                size: this.modalEditorSize ? {
                    width: this.modalEditorSize.width,
                    height: this.modalEditorSize.height
                } : undefined,
    ...
    createState() {
        return {
            auxiliary: ( ( this.parts.map(part => ({
                part,
                auxiliaryWindow: this.auxiliaryWindowService.getWindow(part.windowId)
            }))).filter((
                {
                    auxiliaryWindow
                }
            ) => auxiliaryWindow !== undefined).map((
                {
                    part,
                    auxiliaryWindow
                }
    ...
    registerGroupsContextKeyListeners() {
        ...
        this._register(
            this.onDidAddGroup(group => this.registerGroupContextKeyProvidersListeners(group))
        );
...
EditorParts = EditorParts_1 = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, IStorageService)), ( __param(2, IThemeService)), ( __param(3, IAuxiliaryWindowService)), ( __param(4, IContextKeyService))], EditorParts));
