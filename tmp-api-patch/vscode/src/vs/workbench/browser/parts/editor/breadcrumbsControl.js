...
import { $, hide, DragAndDropObserver, createElement, append, getWindow, getDomNodePagePosition, trackFocus } from '../../../../base/browser/dom.js';
...
import { DisposableStore, toDisposable, MutableDisposable, combinedDisposable } from '../../../../base/common/lifecycle.js';
...
import { fillEditorsDragData, DraggedEditorIdentifier } from '../../dnd.js';
...
import * as breadcrumbscontrol from './media/breadcrumbscontrol.css';
import { ScrollbarVisibility } from '../../../../base/common/scrollable.js';
import { CancellationToken } from '../../../../base/common/cancellation.js';

var OutlineItem_1, FileItem_1, BreadcrumbsControl_1;
registerCss(breadcrumbscontrol);
let OutlineItem = OutlineItem_1 = class OutlineItem extends BreadcrumbsItem {
    ...
    render(container) {
        ...
        if (!this.options.showSymbolIcons) {
            hide(template.iconClass);
        }
        this._disposables.add(toDisposable(() => {
            renderer.disposeTemplate(template);
        }));
        if (element instanceof OutlineElement && outline.uri) {
            this._disposables.add(this._instantiationService.invokeFunction(
                accessor => createBreadcrumbDndObserver(accessor, container, element.symbol.name, {
...
OutlineItem = OutlineItem_1 = ( __decorate([( __param(3, IInstantiationService))], OutlineItem));
let FileItem = FileItem_1 = class FileItem extends BreadcrumbsItem {
    ...
    render(container) {
        ...
        this._disposables.add(
            this._instantiationService.invokeFunction(accessor => createBreadcrumbDndObserver(
                accessor,
                container,
                basename(this.element.uri),
                this.element.uri,
                this.model,
                this.options.dragEditor
            ))
        );
    }
};
FileItem = FileItem_1 = ( __decorate([( __param(5, IInstantiationService))], FileItem));
        ...
        onDragStart: event => {
            ...
            instantiationService.invokeFunction(accessor => {
                if (URI.isUri(item)) {
                    ...
                    fillEditorsDragData(accessor, [{
                        resource: item.uri,
                        selection: item.symbol.range
                    }], event);
                ...
                if (dragEditor && model.editor?.input) {
                    ...
                    editorTransfer.setData([( new DraggedEditorIdentifier({
...
const separatorIcon = registerIcon("breadcrumb-separator", Codicon.chevronRight, ( localize(3353, "Icon for the separator in the breadcrumbs.")));
let BreadcrumbsControl = class BreadcrumbsControl {
    ...
    static {
        this.CK_BreadcrumbsPossible = ( new RawContextKey("breadcrumbsPossible", false, ( localize(3354, "Whether the editor can show breadcrumbs"))));
    }
    static {
        this.CK_BreadcrumbsVisible = ( new RawContextKey("breadcrumbsVisible", false, ( localize(3355, "Whether breadcrumbs are currently visible"))));
    }
    static {
        this.CK_BreadcrumbsActive = ( new RawContextKey("breadcrumbsActive", false, ( localize(3356, "Whether breadcrumbs have focus"))));
    }
    static {
        this.CK_BreadcrumbsHasSymbols = ( new RawContextKey("breadcrumbsHasSymbols", false, ( localize(3357, "Whether breadcrumbs contain symbol items"))));
    }
    ...
    constructor(
        ...
        this._widget = ( new BreadcrumbsWidget(
            ...
            BreadcrumbsControl_1.SCROLLBAR_VISIBILITY[visibility],
            separatorIcon,
            styles
        ));
        ...
        this._ckBreadcrumbsHasSymbols = BreadcrumbsControl_1.CK_BreadcrumbsHasSymbols.bindTo(this._contextKeyService);
    ...
    update() {
        ...
        const updateBreadcrumbs = () => {
            ...
            const items = ( elements.map(
                element => element instanceof FileElement ? this._instantiationService.createInstance(FileItem, model, element, options, this._labels, this._hoverDelegate) : this._instantiationService.createInstance(OutlineItem, model, element, options)
            ));
            if (items.length === 0) {
                this._widget.setEnabled(false);
                this._widget.setItems([new (class extends BreadcrumbsItem {
                    render(container) {
                        container.textContent = ( localize(3358, "no elements"));
                    }
                    equals(other) {
                        return other === this;
                    }
    ...
    _onSelectEvent(event) {
        ...
        this._contextViewService.showContextView({
            render: parent => {
                ...
                const zoomListener = PixelRatio.getInstance(getWindow(this.domNode)).onDidChange(() => this._contextViewService.hideContextView({
                    source: this
                }));
            ...
            getAnchor: () => {
                if (!pickerAnchor) {
                    ...
                    const data = getDomNodePagePosition(event.node);
...
BreadcrumbsControl = BreadcrumbsControl_1 = ( __decorate([( __param(3, IContextKeyService)), ( __param(4, IContextViewService)), ( __param(5, IInstantiationService)), ( __param(6, IQuickInputService)), ( __param(7, IFileService)), ( __param(8, IEditorService)), ( __param(9, ILabelService)), ( __param(10, IConfigurationService)), ( __param(11, IBreadcrumbsService))], BreadcrumbsControl));
let BreadcrumbsControlFactory = class BreadcrumbsControlFactory {
    ...
    createControl() {
        const control = this._controlDisposables.add(
            this._instantiationService.createInstance(BreadcrumbsControl, this._container, this._options, this._editorGroup)
        );
...
BreadcrumbsControlFactory = ( __decorate([( __param(3, IConfigurationService)), ( __param(4, IInstantiationService)), ( __param(5, IFileService))], BreadcrumbsControlFactory));
registerAction2(class ToggleBreadcrumb extends Action2 {
    constructor() {
        super({
            id: "breadcrumbs.toggle",
            title: ( localize2(3359, "Toggle Breadcrumbs")),
            shortTitle: ( localize2(3360, "Breadcrumbs")),
            category: Categories.View,
            toggled: {
                condition: ( ContextKeyExpr.equals("config.breadcrumbs.enabled", true)),
                title: ( localize(3361, "Breadcrumbs")),
                mnemonicTitle: ( localize(3362, "&&Breadcrumbs"))
            },
            menu: [{
                ...
                id: MenuId.NotebookToolbarContext,
                group: "notebookView",
                order: 2
            }]
        });
...
registerAction2(class FocusAndSelectBreadcrumbs extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3363, "Focus and Select Breadcrumbs")),
...
registerAction2(class FocusBreadcrumbs extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3364, "Focus Breadcrumbs")),
            precondition: BreadcrumbsControl.CK_BreadcrumbsVisible,
            keybinding: {
                weight: KeybindingWeight.WorkbenchContrib,
...
registerAction2(class CopyBreadcrumbPath extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3365, "Copy Breadcrumbs Path")),
            category: Categories.View,
            precondition: ( ContextKeyExpr.and(
                BreadcrumbsControl.CK_BreadcrumbsVisible,
                BreadcrumbsControl.CK_BreadcrumbsHasSymbols
            )),
            f1: true,
            menu: [{
                ...
                order: 100,
                when: ( ContextKeyExpr.and(
                    BreadcrumbsControl.CK_BreadcrumbsPossible,
                    BreadcrumbsControl.CK_BreadcrumbsHasSymbols
                ))
            }]
        });
    ...
    async run(accessor) {
        ...
        const outline = await outlineService.createOutline(
            groups.activeGroup.activeEditorPane,
            OutlineTarget.Breadcrumbs,
        ...
        const separator = config.getValue(resource && {
            resource
        }) ?? ".";
        config.dispose();
