...
let BaseBinaryResourceEditor = class BaseBinaryResourceEditor extends EditorPlaceholder {
    ...
    getTitle() {
        return this.input ? this.input.getName() : ( localize(3308, "Binary Viewer"));
    }
    async getContents(input, options) {
        ...
        return {
            ...
            actions: [{
                label: ( localize(3310, "Open Anyway")),
                run: async () => {
...
BaseBinaryResourceEditor = ( __decorate([( __param(5, IStorageService))], BaseBinaryResourceEditor));
