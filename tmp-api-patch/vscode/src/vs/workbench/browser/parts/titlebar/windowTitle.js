...
let WindowTitle = class WindowTitle extends Disposable {
    ...
    static {
        this.NLS_USER_IS_ADMIN = isWindows ? ( localize(3970, "[Administrator]")) : ( localize(3971, "[Superuser]"));
    }
    static {
        this.NLS_EXTENSION_HOST = ( localize(3972, "[Extension Development Host]"));
    }
    static {
        this.TITLE_DIRTY = "● ";
    }
    ...
    registerListeners() {
        ...
        this._register(
            this.contextService.onDidChangeWorkspaceFolders(() => this.titleUpdater.schedule())
        );
        this._register(
            this.contextService.onDidChangeWorkbenchState(() => this.titleUpdater.schedule())
        );
        this._register(
            this.contextService.onDidChangeWorkspaceName(() => this.titleUpdater.schedule())
        );
        this._register(
            this.labelService.onDidChangeFormatters(() => this.titleUpdater.schedule())
        );
        this._register(
            this.userDataProfileService.onDidChangeCurrentProfile(() => this.titleUpdater.schedule())
        );
        ...
        this._register(
            this.accessibilityService.onDidChangeScreenReaderOptimized(() => this.titleUpdater.schedule())
        );
    }
    ...
    onActiveEditorChange() {
        ...
        if (this.titleIncludesEditorState) {
            this.activeEditorListeners.add(
                this.decorationsService.onDidChangeDecorations(() => this.titleUpdater.schedule())
            );
        }
    }
    doUpdateTitle() {
...
WindowTitle = WindowTitle_1 = ( __decorate([( __param(1, IConfigurationService)), ( __param(2, IContextKeyService)), ( __param(3, IEditorService)), ( __param(4, IBrowserWorkbenchEnvironmentService)), ( __param(5, IWorkspaceContextService)), ( __param(6, ILabelService)), ( __param(7, IUserDataProfileService)), ( __param(8, IProductService)), ( __param(9, IViewsService)), ( __param(10, IDecorationsService)), ( __param(11, IAccessibilityService))], WindowTitle));
