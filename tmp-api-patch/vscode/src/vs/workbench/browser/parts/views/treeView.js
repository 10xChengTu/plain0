...
import { show, hide, append, $, trackFocus, createElement, isHTMLElement, clearNode, getTotalHeight, getLargestChildWidth, getActiveElement, prepend } from '../../../../base/browser/dom.js';
...
import { toDisposable, Disposable, MutableDisposable, DisposableStore } from '../../../../base/common/lifecycle.js';
...
import * as views from './media/views.css';
...
import { Extensions, TreeItemCollapsibleState, ResolvableTreeItem } from '../../../common/views.js';
...
import { ITreeViewsDnDService } from '../../../../editor/common/services/treeViewsDndService.service.js';
...
let TreeViewPane = class TreeViewPane extends ViewPane {
    constructor(
        ...
        }, keybindingService, contextMenuService, configurationService, contextKeyService, viewDescriptorService, instantiationService, openerService, themeService, hoverService, accessibleViewService);
        const {
            treeView
        } = ( Registry.as(Extensions.ViewsRegistry)).getView(options.id);
        this.treeView = treeView;
        this._register(this.treeView.onDidChangeActions(() => this.updateActions(), this));
        this._register(this.treeView.onDidChangeTitle(newTitle => this.updateTitle(newTitle)));
        this._register(
            this.treeView.onDidChangeDescription(newDescription => this.updateTitleDescription(newDescription))
        );
        ...
        this._register(
            this.treeView.onDidChangeWelcomeState(() => this._onDidChangeViewWelcomeState.fire())
        );
...
TreeViewPane = ( __decorate([( __param(1, IKeybindingService)), ( __param(2, IContextMenuService)), ( __param(3, IConfigurationService)), ( __param(4, IContextKeyService)), ( __param(5, IViewDescriptorService)), ( __param(6, IInstantiationService)), ( __param(7, IOpenerService)), ( __param(8, IThemeService)), ( __param(9, INotificationService)), ( __param(10, IHoverService)), ( __param(11, IAccessibleViewInformationService))], TreeViewPane));
class Root {
    constructor() {
        this.label = {
            label: "root"
...
const noDataProviderMessage = ( localize(3975, "There is no data provider registered that can provide view data."));
...
let AbstractTreeView = class AbstractTreeView extends Disposable {
    ...
    initialize() {
        ...
        this._register(this.viewDescriptorService.onDidChangeLocation((
            {
                views,
                from,
                to
    ...
    initializeShowCollapseAllAction(startingValue = false) {
        if (!this.collapseAllContext) {
            this.collapseAllContextKey = ( new RawContextKey(`treeView.${this.id}.enableCollapseAll`, startingValue, ( localize(3976, "Whether the tree view with id {0} enables collapse all.", this.id))));
    ...
    initializeShowRefreshAction(startingValue = false) {
        if (!this.refreshContext) {
            this.refreshContextKey = ( new RawContextKey(`treeView.${this.id}.enableRefresh`, startingValue, ( localize(3977, "Whether the tree view with id {0} enables refresh.", this.id))));
        ...
        this._register(registerAction2(class extends Action2 {
            constructor() {
                super({
                    ...
                    title: ( localize(3979, "Collapse All")),
                    menu: {
                        id: MenuId.ViewTitle,
    ...
    createTree() {
        ...
        const dataSource = this.instantiationService.createInstance(TreeDataSource, this, task => this.progressService.withProgress({
            location: this.id
        }, () => task));
        ...
        const renderer = this.treeDisposables.add(this.instantiationService.createInstance(
            TreeRenderer,
            this.id,
            treeMenus,
            this.treeLabels,
        ...
        this.treeDisposables.add(
            renderer.onDidChangeCheckboxState(e => this._onDidChangeCheckboxState.fire(e))
        );
        const widgetAriaLabel = this._title;
        this.tree = this.treeDisposables.add(
            this.instantiationService.createInstance(Tree, this.id, this.treeContainer, ( new TreeViewDelegate()), [renderer], dataSource, {
                ...
                accessibilityProvider: {
                    getAriaLabel(element) {
                        ...
                        if (isString(element.tooltip)) {
                            return treeMenus.getResourceActions([element]).length > 0 ? ( localize(3980, "{0}, has actions", element.tooltip)) : element.tooltip;
                        } else {
                            ...
                            if (treeMenus.getResourceActions([element]).length > 0) {
                                buildAriaLabel = buildAriaLabel ? ( localize(3981, "{0}, has actions", buildAriaLabel.trim())) : ( localize(3982, "has actions"));
                            }
        ...
        this.treeDisposables.add(
            renderer.onDidChangeMenuContext(e => e.forEach(e => this.tree?.rerender(e)))
        );
        ...
        this.treeDisposables.add(
            this.tree.onContextMenu(e => this.onContextMenu(treeMenus, e, actionRunner))
        );
    ...
    processMessage(message, disposables) {
        ...
        for (const line of lines) {
            ...
            if (linkedText.nodes.length === 1 && typeof linkedText.nodes[0] !== "string") {
                ...
                const rendered = this.markdownRendererService.render(( new MarkdownString(line, {
...
AbstractTreeView = ( __decorate([( __param(2, IThemeService)), ( __param(3, IInstantiationService)), ( __param(4, ICommandService)), ( __param(5, IConfigurationService)), ( __param(6, IProgressService)), ( __param(7, IContextMenuService)), ( __param(8, IKeybindingService)), ( __param(9, INotificationService)), ( __param(10, IViewDescriptorService)), ( __param(11, IHoverService)), ( __param(12, IContextKeyService)), ( __param(13, IActivityService)), ( __param(14, ILogService)), ( __param(15, IOpenerService)), ( __param(16, IMarkdownRendererService))], AbstractTreeView));
...
let TreeRenderer = class TreeRenderer extends Disposable {
    ...
    getHover(label, resource, node) {
        if (!(node instanceof ResolvableTreeItem) || !node.hasResolve) {
            if (resource && !node.tooltip) {
                ...
                if (isMarkdownString(label)) {
                    return {
                        markdown: label,
                        markdownNotSupportedFallback: label.value
                    };
                } else {
                    return label;
                }
        ...
        return {
            ...
            markdownNotSupportedFallback: resource ? undefined : (label ? (isMarkdownString(label) ? label.value : label) : "")
        };
    }
        ...
        function checkSyntaxes() {
            ...
            for (const syntax of syntaxes) {
                if (text.startsWith(syntax.open) && text.endsWith(syntax.close)) {
                    if (matches?.some(
                        match => match.start < syntax.open.length || match.end > text.length - syntax.close.length
                    )) {
                        return false;
                    }
                    syntax.mark();
    ...
    renderElement(element, index, templateData) {
        ...
        const treeItemLabel = node.label ? node.label : (resource ? {
        ...
        const {
            ...
            strikethrough,
            supportIcons
        } = this.processLabel(treeItemLabel?.label, matches);
        ...
        templateData.actionBar.push(menuActions, {
            icon: true,
            label: false
        });
        if (menuActions.length > 0) {
            const itemName = [label, description].filter(part => !!part).join(" ").trim();
            templateData.actionBar.setAriaLabel(itemName ? ( localize(3984, "Actions for {0}", itemName)) : ( localize(3985, "Actions")));
        } else {
        ...
        this._renderedElements.set(element.element.handle, [...renderedItems, {
            original: element,
...
TreeRenderer = TreeRenderer_1 = ( __decorate([( __param(7, IThemeService)), ( __param(8, IConfigurationService)), ( __param(9, ILabelService)), ( __param(10, IContextKeyService)), ( __param(11, IHoverService)), ( __param(12, IInstantiationService))], TreeRenderer));
        ...
        if (this._tree) {
            ...
            if (this.hasIconOrCheckbox(parent)) {
                return !!parent.children && ( parent.children.some(
                    c => c.collapsibleState !== TreeItemCollapsibleState.None && !this.hasIconOrCheckbox(c)
                ));
            }
...
let TreeMenus = class TreeMenus {
    ...
    getElementOverlayContexts(element) {
        return ( new Map([["view", this.id], ["viewItem", element.contextValue]]));
    }
...
TreeMenus = ( __decorate([( __param(1, IMenuService))], TreeMenus));
    ...
    activate() {
        if (!this.activated) {
            ...
            this.progressService.withProgress({
                location: this.id
            }, () => this.extensionService.activateByEvent(`onView:${this.id}`)).then(() => timeout(2000)).then(() => {
                this.updateMessage();
            });
            this.activated = true;
        }
...
CustomTreeView = ( __decorate([( __param(3, IThemeService)), ( __param(4, IInstantiationService)), ( __param(5, ICommandService)), ( __param(6, IConfigurationService)), ( __param(7, IProgressService)), ( __param(8, IContextMenuService)), ( __param(9, IKeybindingService)), ( __param(10, INotificationService)), ( __param(11, IViewDescriptorService)), ( __param(12, IContextKeyService)), ( __param(13, IHoverService)), ( __param(14, IExtensionService)), ( __param(15, IActivityService)), ( __param(16, ITelemetryService)), ( __param(17, ILogService)), ( __param(18, IOpenerService)), ( __param(19, IMarkdownRendererService))], CustomTreeView));
...
let CustomTreeViewDragAndDrop = class CustomTreeViewDragAndDrop {
    ...
    addExtensionProvidedTransferTypes(originalEvent, itemHandles) {
        ...
        this.treeViewsDragAndDropService.addDragOperationTransfer(
            uuid,
            this.handleDragAndLog(this.dndController, itemHandles, uuid, this.dragCancellationToken.token)
        );
...
CustomTreeViewDragAndDrop = ( __decorate([( __param(1, ILabelService)), ( __param(2, IInstantiationService)), ( __param(3, ITreeViewsDnDService)), ( __param(4, ILogService))], CustomTreeViewDragAndDrop));
