...
class TreeItemCheckbox extends Disposable {
    ...
    createCheckbox(node) {
        if (node.checkbox) {
            this.toggle = ( new Checkbox("", node.checkbox.isChecked, {
    ...
    setHover(checkbox) {
        if (this.toggle) {
            if (!this.hover) {
                this.hover = this._register(this.hoverService.setupManagedHover(
                    this.hoverDelegate,
                    this.toggle.domNode,
    ...
    checkboxHoverContent(checkbox) {
        return checkbox.tooltip ? checkbox.tooltip : checkbox.isChecked ? ( localize(3973, "Checked")) : ( localize(3974, "Unchecked"));
    }
