...
MenuRegistry.appendMenuItem(viewFilterMenu, {
    ...
    title: ( localize(3987, "More Filters...")),
    group: "navigation",
    icon: Codicon.filter
});
...
let FilterWidget = class FilterWidget extends Widget {
    ...
    _updateFilterInputAriaLabel() {
        let ariaLabel = this.options.ariaLabel || ( localize(3988, "Filter"));
        if (!this._accessibilityHelpHintAnnounced && this.configurationService.getValue("accessibility.verbosity.find") && this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (keybinding) {
                ariaLabel += ", " + ( localize(3989, "Press {0} for accessibility help", keybinding));
                this._accessibilityHelpHintAnnounced = true;
                this._labelResetTimeout?.dispose();
                this._labelResetTimeout = disposableTimeout(() => {
                    this.filterInputBox.setAriaLabel(this.options.ariaLabel || ( localize(3988, "Filter")));
                }, 1000);
            }
        }
    ...
    createInput(container) {
        ...
        this._register(inputBox.onDidChange(
            filter => this.delayedFilterUpdate.trigger(() => this.onDidInputChange(inputBox))
        ));
        this._register(
            addStandardDisposableListener(inputBox.inputElement, EventType.KEY_DOWN, e => this.onInputKeyDown(e))
        );
        this._register(
            addStandardDisposableListener(container, EventType.KEY_DOWN, e => this.handleKeyboardEvent(e))
        );
        this._register(
            addStandardDisposableListener(container, EventType.KEY_UP, e => this.handleKeyboardEvent(e))
        );
        this._register(
            addStandardDisposableListener(inputBox.inputElement, EventType.CLICK, e => {
                e.stopPropagation();
                e.preventDefault();
            })
        );
...
FilterWidget = ( __decorate([( __param(1, IInstantiationService)), ( __param(2, IContextViewService)), ( __param(3, IContextKeyService)), ( __param(4, IKeybindingService)), ( __param(5, IConfigurationService)), ( __param(6, IAccessibilityService))], FilterWidget));
