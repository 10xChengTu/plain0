...
import { addDisposableListener, EventType, append, $, hide, clearNode, show, EventHelper, getDomNodePagePosition } from '../../../base/browser/dom.js';
...
let CompositeBarActionViewItem = class CompositeBarActionViewItem extends BaseActionViewItem {
    ...
    updateActivity() {
        ...
        const {
            badges,
            type
        } = this.getVisibleBadges(this.getActivities());
        ...
        if (badges.length > 0 && shouldRenderBadges) {
            ...
            if (type === "progress") {
                ...
                this.badgeDisposable.value.add(
                    toDisposable(() => this.badgeContent?.classList.remove(...badgeContentClassess))
                );
                show(this.badge);
            }
            if (classes.length) {
    ...
    computeTitle() {
        this.keybindingLabel = this.computeKeybindingLabel();
        let title = this.keybindingLabel ? ( localize(3289, "{0} ({1})", this.compositeBarActionItem.name, this.keybindingLabel)) : this.compositeBarActionItem.name;
...
CompositeBarActionViewItem = ( __decorate([( __param(3, IThemeService)), ( __param(4, IHoverService)), ( __param(5, IConfigurationService)), ( __param(6, IKeybindingService))], CompositeBarActionViewItem));
class CompositeOverflowActivityAction extends CompositeBarAction {
    constructor(showMenu) {
        super({
            id: "additionalComposites.action",
            name: ( localize(3290, "Additional Views")),
...
let CompositeOverflowActivityActionViewItem = class CompositeOverflowActivityActionViewItem extends CompositeBarActionViewItem {
    ...
    getActions() {
        return (this.getOverflowingComposites().map(composite => {
            ...
            if (suffix) {
                action.label = ( localize(3291, "{0} ({1})", composite.name, suffix));
            } else {
...
CompositeOverflowActivityActionViewItem = ( __decorate([( __param(7, IContextMenuService)), ( __param(8, IThemeService)), ( __param(9, IHoverService)), ( __param(10, IConfigurationService)), ( __param(11, IKeybindingService))], CompositeOverflowActivityActionViewItem));
let CompositeActionViewItem = class CompositeActionViewItem extends CompositeBarActionViewItem {
    ...
    render(container) {
        ...
        this._register(
            CompositeDragAndDropObserver.INSTANCE.registerDraggable(this.container, () => {
                return {
                    type: "composite",
    ...
    updateFromDragging(element, showFeedback, event) {
        ...
        return {
            verticallyBefore: top,
            horizontallyBefore: left
        };
    }
    showContextMenu(container) {
        const actions = [];
        if (this.compositeBarActionItem.keybindingId) {
            actions.push(createConfigureKeybindingAction(
                this.commandService,
        ...
        if (isPinned) {
            this.toggleCompositePinnedAction.label = ( localize(3292, "Hide '{0}'", this.compositeBarActionItem.name));
            this.toggleCompositePinnedAction.checked = false;
            this.toggleCompositePinnedAction.enabled = this.compositeBar.getPinnedCompositeIds().length > 1;
        } else {
            this.toggleCompositePinnedAction.label = ( localize(3293, "Keep '{0}'", this.compositeBarActionItem.name));
        ...
        if (isBadgeEnabled) {
            this.toggleCompositeBadgeAction.label = ( localize(3294, "Hide Badge"));
        } else {
            this.toggleCompositeBadgeAction.label = ( localize(3295, "Show Badge"));
        }
...
CompositeActionViewItem = ( __decorate([( __param(8, IContextMenuService)), ( __param(9, IKeybindingService)), ( __param(10, IInstantiationService)), ( __param(11, IThemeService)), ( __param(12, IHoverService)), ( __param(13, IConfigurationService)), ( __param(14, ICommandService))], CompositeActionViewItem));
class ToggleCompositePinnedAction extends Action {
    constructor(activity, compositeBar) {
        super("show.toggleCompositePinned", activity ? activity.name : ( localize(3296, "Toggle View Pinned")));
...
class ToggleCompositeBadgeAction extends Action {
    constructor(compositeBarActionItem, compositeBar) {
        super(
            ...
            compositeBarActionItem ? compositeBarActionItem.name : ( localize(3297, "Toggle View Badge"))
        );
