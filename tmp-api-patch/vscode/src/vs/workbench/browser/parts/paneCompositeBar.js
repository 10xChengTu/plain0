...
import { Disposable, DisposableMap, combinedDisposable, DisposableStore } from '../../../base/common/lifecycle.js';
...
let PaneCompositeBar = class PaneCompositeBar extends Disposable {
    constructor(
        ...
        this.dndHandler = ( new CompositeDragAndDrop(
            ...
            (from, to, before) => this.compositeBar.move(
                from,
                to,
                this.options.orientation === ActionsOrientation.VERTICAL ? before?.verticallyBefore : before?.horizontallyBefore
            ),
    ...
    getContextMenuActionsForComposite(compositeId) {
        ...
        actions.push(( new SubmenuAction("moveToMenu", ( localize(3870, "Move To")), moveActions)));
        if (defaultLocation !== currentLocation) {
            actions.push(toAction({
                id: "resetLocationAction",
                label: ( localize(3871, "Reset Location")),
                run: () => {
    ...
    createMoveAction(viewContainer, newLocation, defaultLocation) {
        return toAction({
            ...
            label: newLocation === ViewContainerLocation.Panel ? ( localize(3872, "Panel")) : newLocation === ViewContainerLocation.Sidebar ? ( localize(3873, "Primary Side Bar")) : ( localize(3874, "Secondary Side Bar")),
            run: () => {
                let index;
    ...
    registerListeners() {
        ...
        this.extensionService.whenInstalledExtensionsRegistered().then(() => {
            ...
            this._register(
                this.storageService.onDidChangeValue(StorageScope.PROFILE, this.options.pinnedViewContainersKey, this._store)(() => this.updateCompositeBarItemsFromStorage(false))
            );
        });
    }
    onDidChangeViewContainers(added, removed) {
        ...
        ) => location === this.location).forEach((
            {
                container
            }
        ) => this.onDidDeregisterViewContainer(container));
        this.onDidRegisterViewContainers(( added.filter((
            {
                location
            }
    ...
    getCompositeActions(compositeId) {
        ...
        if (!compositeActions) {
            ...
            if (viewContainer) {
                ...
                const actionItem = this.toCompositeBarActionItem(
                    compositeId,
    ...
    onDidRegisterViewContainers(viewContainers) {
        for (const viewContainer of viewContainers) {
            ...
            disposables.add(
                viewContainerModel.onDidChangeContainerInfo(() => this.updateCompositeBarActionItem(viewContainer, viewContainerModel))
            );
            disposables.add(
                viewContainerModel.onDidChangeActiveViewDescriptors(() => this.showOrHideViewContainer(viewContainer))
            );
            disposables.add(
                viewContainerModel.onDidChangeEnablement(() => this.showOrHideViewContainer(viewContainer))
            );
    ...
    updateCompositeBarActionItem(viewContainer, viewContainerModel) {
        ...
        const {
            activityAction,
            pinnedAction
        } = this.getCompositeActions(viewContainer.id);
    ...
    shouldBeHidden(viewContainerOrId, cachedViewContainer) {
        ...
        if (!this.hasExtensionsRegistered && !(this.part === Parts.SIDEBAR_PART && this.environmentService.remoteAuthority && isNative)) {
            ...
            if (cachedViewContainer?.views?.length) {
                ...
                ) => !!when && !this.contextKeyService.contextMatchesRules(ContextKeyExpr.deserialize(when)));
            }
        }
        return true;
    }
    ...
    saveCachedViewContainers() {
        ...
        for (const compositeItem of compositeItems) {
            ...
            if (viewContainer) {
                ...
                for (const {
                    when
                } of viewContainerModel.allViewDescriptors) {
                    views.push({
        ...
        if (this._cachedViewContainers === undefined) {
            ...
            for (const placeholderViewContainer of this.getPlaceholderViewContainers()) {
                ...
                if (cachedViewContainer) {
                    ...
                    cachedViewContainer.icon = placeholderViewContainer.themeIcon ? placeholderViewContainer.themeIcon : placeholderViewContainer.iconUrl ? URI.revive(placeholderViewContainer.iconUrl) : undefined;
    ...
    storeCachedViewContainersState(cachedViewContainers) {
        ...
        this.setPinnedViewContainers(( cachedViewContainers.map((
            {
                id,
                pinned,
                order
            ...
            id,
            pinned,
            visible: Boolean(pinnedViewContainers.find((
                {
                    id: pinnedId
                }
            ) => pinnedId === id)?.visible),
        ...
        this.setPlaceholderViewContainers(( cachedViewContainers.map((
            {
                id,
                icon,
                name,
        ...
        this.setViewContainersWorkspaceState(( cachedViewContainers.map((
            {
                id,
                visible
            }
...
PaneCompositeBar = ( __decorate([( __param(4, IInstantiationService)), ( __param(5, IStorageService)), ( __param(6, IExtensionService)), ( __param(7, IViewDescriptorService)), ( __param(8, IViewsService)), ( __param(9, IContextKeyService)), ( __param(10, IWorkbenchEnvironmentService)), ( __param(11, IWorkbenchLayoutService))], PaneCompositeBar));
...
ViewContainerActivityAction = ViewContainerActivityAction_1 = ( __decorate([( __param(3, IWorkbenchLayoutService)), ( __param(4, IConfigurationService)), ( __param(5, IActivityService))], ViewContainerActivityAction));
