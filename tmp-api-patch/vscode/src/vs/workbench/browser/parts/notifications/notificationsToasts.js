...
import { Dimension, getActiveElement, isHTMLElement, isEditableElement, scheduleAtNextAnimationFrame, getWindow, getDomNodePagePosition, getClientArea, $, addDisposableListener, EventType, isAncestorOfActiveElement } from '../../../../base/browser/dom.js';
...
let NotificationsToasts = class NotificationsToasts extends Themable {
    ...
    addToast(item) {
        ...
        itemDisposables.add(
            scheduleAtNextAnimationFrame(getWindow(this.container), () => this.doAddToast(item, itemDisposables))
        );
    }
    ...
    doAddToast(item, itemDisposables) {
        ...
        const notificationList = this.instantiationService.createInstance(NotificationsList, notificationToast, {
            ...
            widgetAriaLabel: (() => {
                if (!item.source) {
                    return withSeverityPrefix(( localize(3863, "{0}, notification", item.message.raw)), item.severity);
                }
                return withSeverityPrefix(( localize(3864, "{0}, source: {1}, notification", item.message.raw, item.source)), item.severity);
            })()
        });
    ...
    purgeNotification(item, notificationToastContainer, notificationList, disposables) {
        ...
        const hideAfterTimeout = () => {
            purgeTimeoutHandle = setTimeout(() => {
                ...
                item.sticky || notificationList.hasFocus() || isMouseOverToast) {
                    hideAfterTimeout();
                } else {
                    this.removeToast(item);
                }
    ...
    updateStyles() {
        this.mapNotificationToToast.forEach((
            {
                toast
            }
        ) => {
...
NotificationsToasts = NotificationsToasts_1 = ( __decorate([( __param(2, IInstantiationService)), ( __param(3, IWorkbenchLayoutService)), ( __param(4, IThemeService)), ( __param(5, IEditorGroupsService)), ( __param(6, IContextKeyService)), ( __param(7, ILifecycleService)), ( __param(8, IHostService)), ( __param(9, IWorkbenchEnvironmentService)), ( __param(10, IConfigurationService))], NotificationsToasts));
