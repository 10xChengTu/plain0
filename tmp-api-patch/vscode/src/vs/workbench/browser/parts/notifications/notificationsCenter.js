...
import * as notificationsCenter from './media/notificationsCenter.css';
...
import { NotificationsSettings, getNotificationsPosition, NotificationsPosition, NotificationChangeType, NotificationViewItemContentChangeKind } from '../../../common/notifications.js';
...
import { hideUpIcon, hideIcon, ConfigureNotificationsPositionAction, ConfigureDoNotDisturbAction, ToggleDoNotDisturbAction, ToggleDoNotDisturbBySourceAction, ClearAllNotificationsAction, HideNotificationsCenterAction } from './notificationsActions.js';
...
registerCss(notificationsCenter);
...
let NotificationsCenter = class NotificationsCenter extends Themable {
    ...
    registerListeners() {
        ...
        this._register(
            this.layoutService.onDidLayoutMainContainer(dimension => this.layout(Dimension.lift(dimension)))
        );
    ...
    updateTitle() {
        ...
        if (this.model.notifications.length === 0) {
            notificationsCenterTitle.textContent = ( localize(3824, "No new notifications"));
            clearAllAction.enabled = false;
        } else {
            notificationsCenterTitle.textContent = ( localize(3825, "Notifications"));
    ...
    create() {
        ...
        const notificationsToolBar = this._register(( new ActionBar(toolbarContainer, {
            ariaLabel: ( localize(3826, "Notification Center Actions")),
            actionRunner,
            actionViewItemProvider: (action, options) => {
                if (action.id === ConfigureNotificationsPositionAction.ID) {
                    return this._register(
                        this.instantiationService.createInstance(DropdownMenuActionViewItem, action, {
                        ...
                        this.instantiationService.createInstance(DropdownMenuActionViewItem, action, {
                            getActions() {
                                const actions = [toAction({
                                    ...
                                    label: that.notificationService.getFilter() === NotificationsFilter.OFF ? ( localize(3827, "Enable Do Not Disturb Mode")) : ( localize(3828, "Disable Do Not Disturb Mode")),
                                ...
                                if (sortedFilters.length > NotificationsCenter_1.MAX_NOTIFICATION_SOURCES) {
                                    actions.push(( new Separator()));
                                    actions.push(that._register(that.instantiationService.createInstance(
        ...
        this.clearAllAction = this._register(this.instantiationService.createInstance(
        ...
        this.configureDoNotDisturbAction = this._register(this.instantiationService.createInstance(
        ...
        const configureNotificationsPositionAction = this._register(this.instantiationService.createInstance(
            ConfigureNotificationsPositionAction,
            ConfigureNotificationsPositionAction.ID,
            ConfigureNotificationsPositionAction.LABEL
        ));
        notificationsToolBar.push(configureNotificationsPositionAction, {
            icon: true,
            label: false
        });
        this.hideAction = this._register(this.instantiationService.createInstance(
            HideNotificationsCenterAction,
            HideNotificationsCenterAction.ID,
            HideNotificationsCenterAction.LABEL
        ));
        this.updateHideActionIcon();
        notificationsToolBar.push(this.hideAction, {
            icon: true,
            label: false,
            keybinding: this.getKeybindingLabel(this.hideAction)
        });
        this.notificationsList = this.instantiationService.createInstance(NotificationsList, this.notificationsCenterContainer, {
            widgetAriaLabel: ( localize(3830, "Notifications Center"))
        });
...
NotificationsCenter = NotificationsCenter_1 = ( __decorate([( __param(2, IThemeService)), ( __param(3, IInstantiationService)), ( __param(4, IWorkbenchLayoutService)), ( __param(5, IContextKeyService)), ( __param(6, IEditorGroupsService)), ( __param(7, IKeybindingService)), ( __param(8, INotificationService)), ( __param(9, IAccessibilitySignalService)), ( __param(10, IContextMenuService)), ( __param(11, IConfigurationService)), ( __param(12, IMenuService))], NotificationsCenter));
