...
import * as notificationsList from './media/notificationsList.css';
...
let NotificationsList = class NotificationsList extends Disposable {
    ...
    createNotificationsList() {
        ...
        const copyAction = this._register(this.instantiationService.createInstance(
...
NotificationsList = ( __decorate([( __param(2, IInstantiationService)), ( __param(3, IContextMenuService))], NotificationsList));
let NotificationAccessibilityProvider = class NotificationAccessibilityProvider {
    ...
    getAriaLabel(element) {
        ...
        if (this._configurationService.getValue("accessibility.verbosity.notification")) {
            accessibleViewHint = keybinding ? ( localize(3844, "Inspect the response in the accessible view with {0}", keybinding)) : ( localize(
                3845,
        ...
        if (!element.source) {
            return withSeverityPrefix(accessibleViewHint ? ( localize(3846, "{0}, notification, {1}", element.message.raw, accessibleViewHint)) : ( localize(3847, "{0}, notification", element.message.raw)), element.severity);
        }
        return withSeverityPrefix(accessibleViewHint ? ( localize(
            3848,
    ...
    getWidgetAriaLabel() {
        return this._options.widgetAriaLabel ?? ( localize(3850, "Notifications List"));
    }
    getRole() {
        return "dialog";
    }
};
NotificationAccessibilityProvider = ( __decorate([( __param(1, IKeybindingService)), ( __param(2, IConfigurationService))], NotificationAccessibilityProvider));

export { NotificationAccessibilityProvider, NotificationsList };
