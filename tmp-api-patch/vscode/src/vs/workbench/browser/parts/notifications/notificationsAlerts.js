...
class NotificationsAlerts extends Disposable {
    ...
    doTriggerAriaAlert(notification) {
        ...
        if (notification.severity === Severity$1.Error) {
            alertText = ( localize(3821, "Error: {0}", (notification.message.linkedText.toString())));
        } else if (notification.severity === Severity$1.Warning) {
            alertText = ( localize(3822, "Warning: {0}", (notification.message.linkedText.toString())));
        } else {
            alertText = ( localize(3823, "Info: {0}", (notification.message.linkedText.toString())));
        }
        alert(alertText);
    }
}
