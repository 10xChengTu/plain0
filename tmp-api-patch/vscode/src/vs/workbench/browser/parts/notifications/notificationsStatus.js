...
import { NotificationsSettings, NotificationChangeType, getNotificationsPosition, NotificationsPosition, StatusMessageChangeType } from '../../../common/notifications.js';
...
let NotificationsStatus = class NotificationsStatus extends Disposable {
    ...
    registerListeners() {
        ...
        this._register(
            this.notificationService.onDidChangeFilter(() => this.updateNotificationsCenterStatusItem())
        );
    ...
    updateNotificationsCenterStatusItem() {
        ...
        if (this.notificationService.getFilter() === NotificationsFilter.ERROR) {
            statusProperties = {
                ...
                ariaLabel: ( localize(3852, "Do Not Disturb")),
                tooltip: ( localize(3853, "Do Not Disturb Mode is Enabled"))
            };
        }
        ...
        if (position === NotificationsPosition.TOP_RIGHT) {
            ...
            if (!this.notificationsCenterStatusItem) {
                this.notificationsCenterStatusItem = this.statusbarService.addEntry(
                    ...
                    Number.MAX_SAFE_INTEGER : Number.NEGATIVE_INFINITY
                );
            } else {
    ...
    getTooltip(notificationsInProgress) {
        if (this.isNotificationsCenterVisible) {
            return localize(3854, "Hide Notifications");
        }
        if (this.model.notifications.length === 0) {
            return localize(3855, "No Notifications");
        }
        if (notificationsInProgress === 0) {
            if (this.newNotificationsCount === 0) {
                return localize(3856, "No New Notifications");
            }
            if (this.newNotificationsCount === 1) {
                return localize(3857, "1 New Notification");
            }
            return localize(3858, "{0} New Notifications", this.newNotificationsCount);
        }
        if (this.newNotificationsCount === 0) {
            return localize(3859, "No New Notifications ({0} in progress)", notificationsInProgress);
        }
        if (this.newNotificationsCount === 1) {
            return localize(3860, "1 New Notification ({0} in progress)", notificationsInProgress);
        }
        return localize(
            3861,
    ...
    doSetStatusMessage(item) {
        ...
        let showHandle = setTimeout(() => {
            statusMessageEntry = this.statusbarService.addEntry({
                name: ( localize(3862, "Status Message")),
                text: message,
                ariaLabel: message
            }, "status.message", StatusbarAlignment.LEFT, Number.NEGATIVE_INFINITY );
...
NotificationsStatus = ( __decorate([( __param(1, IStatusbarService)), ( __param(2, INotificationService)), ( __param(3, IConfigurationService))], NotificationsStatus));
