...
import { $, clearNode, createTextNode, isEventLike, EventHelper, EventType, addDisposableListener } from '../../../../base/browser/dom.js';
...
import { NotificationViewItem, getNotificationsPosition, NotificationsSettings, NotificationViewItemContentChangeKind, ChoiceAction } from '../../../common/notifications.js';
import { ConfigureNotificationAction, getNotificationExpandIcon, getNotificationCollapseIcon, ClearNotificationAction, ExpandNotificationAction, CollapseNotificationAction } from './notificationsActions.js';
    ...
    static render(message, actionHandler) {
        ...
        for (const node of message.linkedText.nodes) {
            if (typeof node === "string") {
                messageContainer.appendChild(createTextNode(node));
            } else {
...
let NotificationRenderer = class NotificationRenderer {
    ...
    renderTemplate(container) {
        ...
        data.toolbar = ( new ActionBar(toolbarContainer, {
            ariaLabel: ( localize(3866, "Notification Actions")),
                        ...
                        getActions() {
                            ...
                            if (isNotificationSource(source)) {
                                ...
                                actions.push(toAction({
                                    id: source.id,
                                    label: isSourceFiltered ? ( localize(3867, "Turn On All Notifications from '{0}'", source.label)) : ( localize(3868, "Turn Off Info and Warning Notifications from '{0}'", source.label)),
...
NotificationRenderer = NotificationRenderer_1 = ( __decorate([( __param(1, IContextMenuService)), ( __param(2, IInstantiationService)), ( __param(3, INotificationService))], NotificationRenderer));
let NotificationTemplateRenderer = class NotificationTemplateRenderer extends Disposable {
    ...
    static {
        this.SEVERITIES = [Severity$1.Info, Severity$1.Warning, Severity$1.Error];
    }
    constructor(
        template,
        actionRunner,
        ...
        if (!NotificationTemplateRenderer_1.closeNotificationAction) {
            NotificationTemplateRenderer_1.closeNotificationAction = instantiationService.createInstance(
                ClearNotificationAction,
                ClearNotificationAction.ID,
                ClearNotificationAction.LABEL
            );
            NotificationTemplateRenderer_1.expandNotificationAction = instantiationService.createInstance(
                ExpandNotificationAction,
                ExpandNotificationAction.ID,
                ExpandNotificationAction.LABEL
            );
            NotificationTemplateRenderer_1.collapseNotificationAction = instantiationService.createInstance(
    ...
    render(notification) {
        ...
        const messageCustomHover = this.inputDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.template.message, "")
        );
        ...
        const sourceCustomHover = this.inputDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.template.source, "")
        );
    ...
    renderSecondaryActions(notification, messageOverflows) {
        ...
        if (showExpandCollapseAction) {
            actions.push(
                notification.expanded ? NotificationTemplateRenderer_1.collapseNotificationAction : NotificationTemplateRenderer_1.expandNotificationAction
            );
        }
        if (!notification.hasProgress) {
    ...
    renderSource(notification, sourceCustomHover) {
        if (notification.expanded && notification.source) {
            this.template.source.textContent = ( localize(3869, "Source: {0}", notification.source));
...
NotificationTemplateRenderer = NotificationTemplateRenderer_1 = ( __decorate([( __param(2, IOpenerService)), ( __param(3, IInstantiationService)), ( __param(4, IKeybindingService)), ( __param(5, IContextMenuService)), ( __param(6, IHoverService)), ( __param(7, IConfigurationService))], NotificationTemplateRenderer));
