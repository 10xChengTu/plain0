...
import * as compositepart from './media/compositepart.css';
import { localize } from '../../../nls.js';
import { defaultGenerator } from '../../../base/common/idGenerator.js';
import { MutableDisposable, DisposableStore, dispose } from '../../../base/common/lifecycle.js';
import { Emitter } from '../../../base/common/event.js';
import { isCancellationError } from '../../../base/common/errors.js';
import { prepareActions, ActionsOrientation } from '../../../base/browser/ui/actionbar/actionbar.js';
...
import { $, show, hide, append, Dimension } from '../../../base/browser/dom.js';
...
class CompositePart extends Part {
    ...
    createComposite(id, isActive) {
        ...
        if (compositeDescriptor) {
            ...
            const compositeProgressIndicator = ( new ScopedProgressIndicator(
                ...
                this._register(new (class extends AbstractProgressScope {
                    constructor() {
                        ...
                        this._register(
                            that.onDidCompositeOpen.event(e => this.onScopeOpened(e.composite.getId()))
                        );
            ...
            disposable.add(
                composite.onTitleAreaUpdate(() => this.onTitleAreaUpdate(composite.getId()), this)
            );
    ...
    updateTitle(compositeId, compositeTitle) {
        ...
        this.toolBar?.setAriaLabel(( localize(3298, "{0} actions", compositeTitle)));
    }
    ...
    createTitleArea(parent) {
        ...
        this.toolBar = this._register(
            this.instantiationService.createInstance(WorkbenchToolBar, titleActionsContainer, {
                actionViewItemProvider: (action, options) => this.actionViewItemProvider(action, options),
                orientation: ActionsOrientation.HORIZONTAL,
                getKeyBinding: action => this.keybindingService.lookupKeybinding(action.id),
                anchorAlignmentProvider: () => this.getTitleAreaDropDownAnchorAlignment(),
                toggleMenuTitle: ( localize(3299, "Views and More Actions...")),
    ...
    createTitleLabel(parent) {
        ...
        const hover = this._register(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), titleLabel, "")
        );
        const $this = this;
        return {
            updateTitle: (id, title, keybinding) => {
                if (!this.activeComposite || this.activeComposite.getId() === id) {
                    ...
                    hover.update(keybinding ? ( localize(3300, "{0} ({1})", title, keybinding)) : title);
                }
            },
            updateStyles: () => {
