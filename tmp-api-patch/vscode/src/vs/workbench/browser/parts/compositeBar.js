...
import { isAncestor, $, addDisposableListener, EventType, getWindow, EventHelper } from '../../../base/browser/dom.js';
...
import { toggleDropEffect, CompositeDragAndDropObserver } from '../dnd.js';
...
class CompositeBarDndCallbacks {
    ...
    updateFromDragging(element, showFeedback, front, isDragging) {
        ...
        return {
            verticallyBefore: front,
            horizontallyBefore: front
        };
    }
}
let CompositeBar = class CompositeBar extends Widget {
    ...
    create(parent) {
        ...
        this.compositeSwitcherBar = this._register(( new ActionBar(actionBarDiv, {
            ...
            ariaLabel: ( localize(3288, "Active View Switcher")),
            ariaRole: "tablist",
    ...
    computeSizes(items) {
        ...
        if (size) {
            ...
            if (compositeSwitcherBar && this.dimension && this.dimension.height !== 0 && this.dimension.width !== 0) {
                ...
                items.forEach((composite, index) => this.compositeSizeInBar.set(
                    composite.id,
    ...
    updateCompositeSwitcher(donotTrigger) {
        ...
        if (totalComposites > compositesToShow.length && !this.compositeOverflowAction.value) {
            ...
            compositeSwitcherBar.push(this.compositeOverflowAction.value, {
                label: false,
                icon: true
            });
        }
...
CompositeBar = ( __decorate([( __param(2, IInstantiationService)), ( __param(3, IContextMenuService)), ( __param(4, IViewDescriptorService))], CompositeBar));
