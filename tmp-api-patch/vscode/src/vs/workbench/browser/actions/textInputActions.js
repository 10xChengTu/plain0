...
function createTextInputActions(clipboardService, logService) {
    return [toAction({
        id: "undo",
        label: ( localize(3169, "Undo")),
        run: () => getActiveDocument().execCommand("undo")
    }), toAction({
        id: "redo",
        label: ( localize(3170, "Redo")),
        run: () => getActiveDocument().execCommand("redo")
    }), ( new Separator()), toAction({
        id: "editor.action.clipboardCutAction",
        label: ( localize(3171, "Cut")),
        run: () => {
        ...
        label: ( localize(3174, "Select All")),
...
let TextInputActionsProvider = class TextInputActionsProvider extends Disposable {
    ...
    registerListeners() {
        this._register(Event$1.runAndSubscribe(this.layoutService.onDidAddContainer, (
            {
                container,
                disposables
            }
...
TextInputActionsProvider = ( __decorate([( __param(0, IWorkbenchLayoutService)), ( __param(1, IContextMenuService)), ( __param(2, IClipboardService)), ( __param(3, ILogService))], TextInputActionsProvider));
