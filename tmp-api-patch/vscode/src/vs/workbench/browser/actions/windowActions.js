...
class BaseOpenRecentAction extends Action2 {
    ...
    async run(accessor) {
        ...
        const [mainWindows, recentlyOpened, dirtyWorkspacesAndFolders] = await Promise.all([hostService.getWindows({
            includeAuxiliaryWindows: false
        }), workspacesService.getRecentlyOpened(), workspacesService.getDirtyWorkspaces()]);
        ...
        for (const window of mainWindows) {
            ...
            if (isSingleFolderWorkspaceIdentifier(window.workspace)) {
                openedInWindows.set(window.workspace.uri, {
                    isActive
                });
            } else if (isWorkspaceIdentifier(window.workspace)) {
                openedInWindows.set(window.workspace.configPath, {
                    isActive
                });
            }
        }
        ...
        for (const recent of recentlyOpened.workspaces) {
            ...
            workspacePicks.push(this.toQuickPick(modelService, languageService, labelService, recent, {
                isDirty,
                windowState
            }));
        }
        for (const dirtyWorkspaceOrFolder of dirtyWorkspacesAndFolders) {
            if (isFolderBackupInfo(dirtyWorkspaceOrFolder) && !( recentFolders.has(dirtyWorkspaceOrFolder.folderUri))) {
                workspacePicks.push(
                    this.toQuickPick(modelService, languageService, labelService, dirtyWorkspaceOrFolder, {
                        isDirty: true,
        ...
        const filePicks = ( recentlyOpened.files.map(p => this.toQuickPick(modelService, languageService, labelService, p, {
            isDirty: false,
            windowState: undefined
        })));
        ...
        const workspaceSeparator = {
            type: "separator",
            label: hasWorkspaces ? ( localize(3182, "folders & workspaces")) : ( localize(3183, "folders"))
        };
        const fileSeparator = {
            type: "separator",
            label: ( localize(3184, "files"))
        };
            ...
            onDidTriggerItemButton: async context => {
                if (context.button === this.removeFromRecentlyOpened || context.button === this.windowOpenedRecentlyOpenedFolder || context.button === this.windowOpenedRecentlyOpenedWorkspace) {
                    ...
                    const {
                        ...
                        title: isDirtyWorkspace ? ( localize(3187, "Workspace with Unsaved Files")) : ( localize(3188, "Folder with Unsaved Files")),
                        message: isDirtyWorkspace ? ( localize(3189, "Do you want to open the workspace to review the unsaved files?")) : ( localize(3190, "Do you want to open the folder to review the unsaved files?")),
    ...
    toQuickPick(modelService, languageService, labelService, recent, kind) {
        ...
        if (kind.isDirty) {
            ...
            if (kind.windowState.isActive) {
                buttons.push(
                    isWorkspace ? this.activeWindowOpenedRecentlyOpenedWorkspace : this.activeWindowOpenedRecentlyOpenedFolder
                );
            } else {
                buttons.push(
                    isWorkspace ? this.windowOpenedRecentlyOpenedWorkspace : this.windowOpenedRecentlyOpenedFolder
                );
            }
        } else {
        ...
        return {
            iconClasses,
            label: name,
            ariaLabel: kind.isDirty ? isWorkspace ? ( localize(3193, "{0}, workspace with unsaved changes", name)) : ( localize(3194, "{0}, folder with unsaved changes", name)) : name,
...
class OpenRecentAction extends BaseOpenRecentAction {
    ...
    constructor() {
        super({
            ...
            title: {
                ...( localize2(3195, "Open Recent...")),
                mnemonicTitle: ( localize(3196, "&&More..."))
            },
            ...
            keybinding: {
                ...
                mac: {
                    primary: KeyMod$1.WinCtrl | KeyCode.KeyR
                }
            },
            menu: {
...
class QuickPickRecentAction extends BaseOpenRecentAction {
    constructor() {
        super({
            ...
            title: ( localize2(3197, "Quick Open Recent...")),
            category: Categories.File,
            f1: false
        });
    }
...
class ToggleFullScreenAction extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(3198, "Toggle Full Screen")),
                mnemonicTitle: ( localize(3199, "&&Full Screen"))
            },
...
class ReloadWindowAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(3200, "Reload Window")),
...
class ShowAboutDialogAction extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(3201, "About")),
                mnemonicTitle: ( localize(3202, "&&About"))
            },
...
class NewWindowAction extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(3203, "New Window")),
                mnemonicTitle: ( localize(3204, "New &&Window"))
            },
            f1: true,
            precondition: ( IsSandboxWorkspaceContext.toNegated()),
            keybinding: {
...
class BlurAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3205, "Remove keyboard focus from focused element"))
        });
    }
    run() {
...
MenuRegistry.appendMenuItem(MenuId.MenubarFileMenu, {
    ...
    command: {
        ...
        title: ( localize(3206, "Confirm Before Close")),
...
MenuRegistry.appendMenuItem(MenuId.MenubarFileMenu, {
    title: ( localize(3207, "Open &&Recent")),
