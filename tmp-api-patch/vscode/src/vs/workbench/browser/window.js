...
import { getWindows, getWindowsCount, getWindow, hasAppFocus, getWindowById, addDisposableListener, EventType, EventHelper, windowOpenWithSuccess, windowOpenPopup, windowOpenNoOpener } from '../../base/browser/dom.js';
...
let BaseWindow = class BaseWindow extends Disposable {
    ...
    static {
        this.TIMEOUT_DISPOSABLES = ( new Map());
    }
    constructor(
        targetWindow,
        dom = {
    ...
    enableMultiWindowAwareTimeout(
        ...
        function getOverridenWindow(targetWindow) {
        ...
        Object.defineProperty(getOverridenWindow(targetWindow), "vscodeOriginalSetTimeout", {
            get: () => originalSetTimeout
        });
        const originalClearTimeout = getOverridenWindow(targetWindow).clearTimeout;
        Object.defineProperty(getOverridenWindow(targetWindow), "vscodeOriginalClearTimeout", {
            get: () => originalClearTimeout
        });
        getOverridenWindow(targetWindow).setTimeout = function(handler, timeout = 0, ...args) {
            ...
            for (const {
                ...
                let didClear = false;
                const handle = getOverridenWindow(targetWindow).vscodeOriginalSetTimeout?.apply(this, [(...args) => {
                    if (didClear) {
                        return;
                    }
                    handlerFn(...args);
                ...
                const timeoutDisposable = toDisposable(() => {
                    didClear = true;
                    getOverridenWindow(targetWindow).vscodeOriginalClearTimeout?.apply(this, [handle]);
        ...
        getOverridenWindow(targetWindow).clearTimeout = function(timeoutHandle) {
    ...
    static async confirmOnShutdown(accessor, reason) {
        ...
        const message = reason === ShutdownReason.QUIT ? (isMacintosh ? ( localize(4002, "Are you sure you want to quit?")) : ( localize(4003, "Are you sure you want to exit?"))) : ( localize(4004, "Are you sure you want to close the window?"));
        const primaryButton = reason === ShutdownReason.QUIT ? (isMacintosh ? ( localize(4005, "&&Quit")) : ( localize(4006, "&&Exit"))) : ( localize(4007, "&&Close Window"));
        const res = await dialogService.confirm({
            message,
            primaryButton,
            checkbox: {
                label: ( localize(4008, "Do not ask me again"))
            }
        });
    ...
    registerFullScreenListeners(targetWindowId) {
        this._register(this.hostService.onDidChangeFullScreen((
            {
                windowId,
                fullscreen
            }
...
BaseWindow = BaseWindow_1 = ( __decorate([( __param(2, IHostService)), ( __param(3, IWorkbenchEnvironmentService)), ( __param(4, IContextMenuService)), ( __param(5, IWorkbenchLayoutService))], BaseWindow));
    ...
    onWillShutdown() {
        Event.toPromise(Event.any(Event.once(( new DomEmitter(mainWindow.document.body, EventType.KEY_DOWN, true)).event), Event.once(( new DomEmitter(mainWindow.document.body, EventType.MOUSE_DOWN, true)).event))).then(async () => {
            ...
            await this.dialogService.prompt({
                ...
                message: ( localize(4009, "An unexpected error occurred that requires a reload of this page.")),
                detail: ( localize(4010, "The workbench was unexpectedly disposed while running.")),
                buttons: [{
                    label: ( localize(4011, "&&Reload")),
                ...
                if (matchesScheme(href, Schemas.http) || matchesScheme(href, Schemas.https)) {
                    ...
                    const showProtocolUrlOpenedDialog = async () => {
                        ...
                        await this.hostService.withExpectedShutdown(() => this.dialogService.prompt({
                            ...
                            message: ( localize(4019, "All done. You can close this tab now.")),
                            detail,
                            buttons,
                            cancelButton: true
                        }));
...
BrowserWindow = ( __decorate([( __param(0, IOpenerService)), ( __param(1, ILifecycleService)), ( __param(2, IDialogService)), ( __param(3, ILabelService)), ( __param(4, IProductService)), ( __param(5, IBrowserWorkbenchEnvironmentService)), ( __param(6, IWorkbenchLayoutService)), ( __param(7, IInstantiationService)), ( __param(8, IHostService)), ( __param(9, IContextMenuService))], BrowserWindow));
