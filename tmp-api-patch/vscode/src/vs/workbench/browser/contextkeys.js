...
import { IsMacContext, IsLinuxContext, IsWindowsContext, IsWebContext, IsMacNativeContext, IsIOSContext, IsMobileContext, IsDevelopmentContext, ProductQualityContext } from '../../platform/contextkey/common/contextkeys.js';
...
import { RemoteNameContext, VirtualWorkspaceContext, TemporaryWorkspaceContext, IsSessionsWindowContext, HasWebFileSystemAccess, EmbedderIdentifierContext, InAutomationContext, ActiveEditorGroupEmptyContext, ActiveEditorGroupIndexContext, ActiveEditorGroupLastContext, ActiveEditorGroupLockedContext, MultipleEditorGroupsContext, EditorsVisibleContext, DirtyWorkingCopiesContext, WorkbenchStateContext, WorkspaceFolderCountContext, OpenFolderWorkspaceSupportContext, EmptyWorkspaceSupportContext, EnterMultiRootWorkspaceSupportContext, SplitEditorsVertically, IsMainWindowFullscreenContext, IsAuxiliaryWindowFocusedContext, InEditorZenModeContext, IsMainEditorCenteredLayoutContext, MainEditorAreaVisibleContext, EditorTabsVisibleContext, SideBarVisibleContext, TitleBarVisibleContext, TitleBarStyleContext, PanelPositionContext, PanelVisibleContext, PanelMaximizedContext, PanelAlignmentContext, AuxiliaryBarVisibleContext, AuxiliaryBarMaximizedContext, IsSandboxWorkspaceContext } from '../common/contextkeys.js';
...
import { Parts, positionToString } from '../services/layout/browser/layoutService.js';
...
let WorkbenchContextKeysHandler = class WorkbenchContextKeysHandler extends Disposable {
    ...
    registerListeners() {
        ...
        this._register(
            this.editorService.onDidActiveEditorChange(() => this.updateActiveEditorGroupContextKeys())
        );
        this._register(
            this.editorService.onDidVisibleEditorsChange(() => this.updateVisiblePanesContextKeys())
        );
        this._register(
            this.editorGroupService.onDidAddGroup(() => this.updateEditorGroupsContextKeys())
        );
        this._register(
            this.editorGroupService.onDidRemoveGroup(() => this.updateEditorGroupsContextKeys())
        );
        this._register(
            this.editorGroupService.onDidChangeGroupIndex(() => this.updateActiveEditorGroupContextKeys())
        );
        this._register(
            this.editorGroupService.onDidChangeGroupLocked(() => this.updateActiveEditorGroupContextKeys())
        );
        this._register(
            this.editorGroupService.onDidChangeEditorPartOptions(() => this.updateEditorAreaContextKeys())
        );
        this._register(
            this.contextService.onDidChangeWorkbenchState(() => this.updateWorkbenchStateContextKey())
        );
        ...
        this._register(
            this.layoutService.onDidChangeZenMode(enabled => this.inZenModeContext.set(enabled))
        );
        this._register(this.layoutService.onDidChangeActiveContainer(
            () => this.isAuxiliaryWindowFocusedContext.set(this.layoutService.activeContainer !== this.layoutService.mainContainer)
        ));
        ...
        this._register(
            this.layoutService.onDidChangeMainEditorCenteredLayout(centered => this.isMainEditorCenteredLayoutContext.set(centered))
        );
        this._register(
            this.layoutService.onDidChangePanelPosition(position => this.panelPositionContext.set(position))
        );
        this._register(
        ...
        this._register(this.workingCopyService.onDidChangeDirty(
            workingCopy => this.dirtyWorkingCopiesContext.set(workingCopy.isDirty() || this.workingCopyService.hasDirty)
        ));
    }
...
WorkbenchContextKeysHandler = ( __decorate([( __param(0, IContextKeyService)), ( __param(1, IWorkspaceContextService)), ( __param(2, IConfigurationService)), ( __param(3, IWorkbenchEnvironmentService)), ( __param(4, IProductService)), ( __param(5, IEditorGroupsService)), ( __param(6, IEditorService)), ( __param(7, IWorkbenchLayoutService)), ( __param(8, IWorkingCopyService))], WorkbenchContextKeysHandler));
