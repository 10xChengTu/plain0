    ...
    return ( new Promise(resolve => {
        ...
        const listener = editorService.onDidCloseEditor(async event => {
            ...
            if (event.context === EditorCloseContext.REPLACE) {
                const newPrimaryResource = EditorResourceAccessor.getOriginalUri(editorService.activeEditor, {
                    supportSideBySide: SideBySideEditor.PRIMARY
                });
                const newSecondaryResource = EditorResourceAccessor.getOriginalUri(editorService.activeEditor, {
                    supportSideBySide: SideBySideEditor.SECONDARY
                });
            ...
            remainingResources = remainingResources.filter(resource => {
                ...
                if (event.context !== EditorCloseContext.REPLACE) {
                    if ((primaryResource?.scheme === Schemas.untitled && uriIdentityService.extUri.isEqual(resource, primaryResource.with({
                        scheme: resource.scheme
                    }))) || (secondaryResource?.scheme === Schemas.untitled && uriIdentityService.extUri.isEqual(resource, secondaryResource.with({
                        scheme: resource.scheme
                    })))) {
                        return false;
                    }
...
function computeEditorAriaLabel(input, index, group, groupCount) {
    ...
    if (group && !group.isPinned(input)) {
        ariaLabel = ( localize(3231, "{0}, preview", ariaLabel));
    }
    if (group?.isSticky(index ?? input)) {
        ariaLabel = ( localize(3232, "{0}, pinned", ariaLabel));
    }
