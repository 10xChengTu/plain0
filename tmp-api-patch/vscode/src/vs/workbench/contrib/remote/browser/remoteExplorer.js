...
import { TUNNEL_VIEW_ID, TUNNEL_VIEW_CONTAINER_ID, PORT_AUTO_SOURCE_SETTING, PORT_AUTO_FALLBACK_SETTING, PORT_AUTO_SOURCE_SETTING_PROCESS, PORT_AUTO_SOURCE_SETTING_HYBRID, PORT_AUTO_SOURCE_SETTING_OUTPUT, PORT_AUTO_FORWARD_SETTING, PortsEnablement } from '../../../services/remote/common/remoteExplorerService.js';
import { IRemoteExplorerService } from '../../../services/remote/common/remoteExplorerService.service.js';
import { forwardedPortsFeaturesEnabled, forwardedPortsViewEnabled, TunnelSource, OnPortForward, makeAddress, mapHasAddressLocalhostOrAllInterfaces, TunnelCloseReason, AutoTunnelSource } from '../../../services/remote/common/tunnelModel.js';
...
let ForwardedPortsView = class ForwardedPortsView extends Disposable {
    ...
    async getViewContainer() {
        return ( Registry.as(Extensions.ViewContainersRegistry)).registerViewContainer({
            ...
            title: ( localize2(13453, "Ports")),
            icon: portsViewIcon,
            ctorDescriptor: ( new SyncDescriptor(ViewPaneContainer, [TUNNEL_VIEW_CONTAINER_ID, {
    ...
    enableBadgeAndStatusBar() {
        const disposable = ( Registry.as(Extensions.ViewsRegistry)).onViewsRegistered(e => {
            if (e.find(
                view => view.views.find(viewDescriptor => viewDescriptor.id === TUNNEL_VIEW_ID)
            )) {
                this._register(
                    Event.debounce(this.remoteExplorerService.tunnelModel.onForwardPort, (_last, e) => e, 50)(() => {
                        this.updateActivityBadge();
                        this.updateStatusBar();
                    })
                );
                this._register(
                    Event.debounce(this.remoteExplorerService.tunnelModel.onClosePort, (_last, e) => e, 50)(() => {
                        this.updateActivityBadge();
                        this.updateStatusBar();
                    })
                );
        ...
        if (this.remoteExplorerService.tunnelModel.forwarded.size > 0) {
            this.activityBadge.value = this.activityService.showViewActivity(TUNNEL_VIEW_ID, {
                badge: ( new NumberBadge(
                    ...
                    n => n === 1 ? ( localize(13454, "1 forwarded port")) : ( localize(13455, "{0} forwarded ports", n))
                ))
            });
        } else {
    ...
    updateStatusBar() {
        ...
        if (!this.entryAccessor) {
            this._register(
                this.entryAccessor = this.statusbarService.addEntry(this.entry, "status.forwardedPorts", StatusbarAlignment.LEFT, 40)
            );
        } else {
    ...
    get entry() {
        ...
        const text = `${count}`;
        if (count === 0) {
            tooltip = ( localize(13456, "No Ports Forwarded"));
        } else {
        ...
        return {
            name: ( localize(13458, "Forwarded Ports")),
...
ForwardedPortsView = ( __decorate([( __param(0, IContextKeyService)), ( __param(1, IWorkbenchEnvironmentService)), ( __param(2, IRemoteExplorerService)), ( __param(3, ITunnelService)), ( __param(4, IActivityService)), ( __param(5, IStatusbarService))], ForwardedPortsView));
...
PortRestore = ( __decorate([( __param(0, IRemoteExplorerService)), ( __param(1, ILogService))], PortRestore));
                    ...
                    this.notificationService.notify({
                        ...
                        actions: {
                            primary: [toAction({
                                ...
                                label: ( localize(13461, "Show Setting")),
                                run: async () => {
    ...
    setup(environment) {
        ...
        if (environment?.os !== OperatingSystem.Linux) {
            if (this.configurationService.inspect(PORT_AUTO_SOURCE_SETTING).default?.value !== PORT_AUTO_SOURCE_SETTING_OUTPUT) {
                ( Registry.as(Extensions$1.Configuration)).registerDefaultConfigurations([{
                    overrides: {
...
class OnAutoForwardedAction extends Disposable {
    ...
    linkMessage() {
        return localize(13463, "[See all forwarded ports]({0})", `command:${TunnelPanel.ID}.focus`);
    }
    async showNotification(tunnel) {
        ...
        this.lastNotification = this.notificationService.prompt(Severity$1.Info, message, choices, {
            neverShowAgain: {
    ...
    makePublicChoice(tunnel) {
        return {
            label: ( localize(13465, "Make Public")),
            run: async () => {
    ...
    elevateChoice(tunnel) {
        return {
            label: ( localize(13466, "Use Port {0} as Sudo...", tunnel.tunnelRemotePort)),
            run: async () => {
                await this.remoteExplorerService.close({
                    host: tunnel.tunnelRemoteHost,
                    port: tunnel.tunnelRemotePort
...
class OutputAutomaticPortForwarding extends Disposable {
    constructor(
        ...
        if (configurationService.getValue(PORT_AUTO_SOURCE_SETTING) === PORT_AUTO_SOURCE_SETTING_HYBRID) {
            this._register(
                this.tunnelService.onTunnelClosed(tunnel => this.notifier.hide([tunnel.port]))
            );
        }
    }
...
class ProcAutomaticPortForwarding extends Disposable {
    constructor(
        unforwardOnly,
        alreadyAutoForwarded,
        needsInitialCandidates,
        configurationService,
        remoteExplorerService,
        notificationService,
        openerService,
    ...
    async startCandidateListener() {
        ...
        if (this.configurationService.getValue(PORT_AUTO_FORWARD_SETTING)) {
            this.candidateListener = this._register(
                this.remoteExplorerService.tunnelModel.onCandidatesChanged(this.handleCandidateUpdate, this)
            );
        }
    }
