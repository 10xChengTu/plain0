...
import { ObservableLazyPromise } from '../../../../base/common/observableInternal/utils/promise.js';
import { observableFromValueWithChangeEvent, ValueWithChangeEventFromObservable } from '../../../../base/common/observableInternal/utils/valueWithChangeEvent.js';
import { constObservable } from '../../../../base/common/observableInternal/observables/constObservable.js';
import { derived } from '../../../../base/common/observableInternal/observables/derived.js';
import { mapObservableArrayCached, recomputeInitiallyAndOnChange } from '../../../../base/common/observableInternal/utils/utils.js';
...
let MultiDiffEditorInput = class MultiDiffEditorInput extends EditorInput {
    ...
    constructor(
        ...
        this._isDirtyObservables = mapObservableArrayCached(this, ( this.resources.map(r => r ?? [])), res => {
            ...
            return derived(
                reader =>  isModifiedDirty.read(reader) || isOriginalDirty.read(reader)
            );
        }, i => i.getKey());
        ...
        this._register(autorun(reader => {
            ...
            const label = this.label ?? ( localize(12350, "Multi Diff Editor"));
            if (resources && resources.length === 1) {
                this._name = ( localize(12351, "{0} (1 file)", label));
            } else if (resources) {
                this._name = ( localize(12352, "{0} ({1} files)", label, resources.length));
            } else {
                this._name = label;
            }
