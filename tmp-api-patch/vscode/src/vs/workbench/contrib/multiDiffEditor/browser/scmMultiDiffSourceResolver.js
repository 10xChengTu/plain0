...
let ScmMultiDiffSourceResolver = class ScmMultiDiffSourceResolver {
    ...
    static parseUri(uri) {
        ...
        return {
            repositoryUri: ( URI.parse(repositoryUri)),
            groupId
        };
    }
    ...
    async resolveDiffSource(uri) {
        const {
            repositoryUri,
            groupId
        } = ScmMultiDiffSourceResolver_1.parseUri(uri);
...
ScmMultiDiffSourceResolver = ScmMultiDiffSourceResolver_1 = ( __decorate([( __param(0, ISCMService)), ( __param(1, IActivityService))], ScmMultiDiffSourceResolver));
let ScmHistoryItemResolver = class ScmHistoryItemResolver {
    ...
    static parseUri(uri) {
        ...
        if (typeof repositoryId !== "string" || typeof historyItemId !== "string" || (typeof historyItemParentId !== "string" && historyItemParentId !== undefined) || (typeof historyItemDisplayId !== "string" && historyItemDisplayId !== undefined)) {
            return undefined;
        }
        return {
            repositoryId,
    ...
    async resolveDiffSource(uri) {
        const {
            ...
            historyItemParentId,
            historyItemDisplayId
        } = ScmHistoryItemResolver_1.parseUri(uri);
        ...
        const resources = ValueWithChangeEvent.const(( historyItemChanges.map(change => {
            const goToFileEditorTitle = change.modifiedUri ? `${basename(change.modifiedUri.fsPath)} (${historyItemDisplayId ?? historyItemId})` : undefined;
...
ScmHistoryItemResolver = ScmHistoryItemResolver_1 = ( __decorate([( __param(0, ISCMService))], ScmHistoryItemResolver));
...
let ScmMultiDiffSourceResolverContribution = class ScmMultiDiffSourceResolverContribution extends Disposable {
    ...
    constructor(instantiationService, multiDiffSourceResolverService) {
        super();
        this._register(
            multiDiffSourceResolverService.registerResolver(instantiationService.createInstance(ScmHistoryItemResolver))
        );
        this._register(
            multiDiffSourceResolverService.registerResolver(instantiationService.createInstance(ScmMultiDiffSourceResolver))
        );
    }
};
ScmMultiDiffSourceResolverContribution = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, IMultiDiffSourceResolverService))], ScmMultiDiffSourceResolverContribution));
class OpenScmGroupAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(12353, "Open Changes")),
            f1: false
        });
    }
