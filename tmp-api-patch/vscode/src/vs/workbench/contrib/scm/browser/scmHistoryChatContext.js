...
import { ISCMViewService, ISCMService } from '../common/scm.service.js';
...
let SCMHistoryItemContextContribution = class SCMHistoryItemContextContribution extends Disposable {
    ...
    constructor(contextPickService, instantiationService, textModelResolverService) {
        super();
        this._store.add(
            contextPickService.registerChatContextItem(instantiationService.createInstance(SCMHistoryItemContext))
        );
...
SCMHistoryItemContextContribution = ( __decorate([( __param(0, IChatContextPickService)), ( __param(1, IInstantiationService)), ( __param(2, ITextModelService))], SCMHistoryItemContextContribution));
let SCMHistoryItemContext = SCMHistoryItemContext_1 = class SCMHistoryItemContext {
    ...
    constructor(_scmViewService) {
        ...
        this.label = ( localize(13767, "Source Control..."));
    ...
    asPicker(_widget) {
        return {
            placeholder: ( localize(13768, "Select a change")),
            picks: picksWithPromiseFn((query, token) => {
                ...
                return this._delayer.trigger(() => {
                    return historyProvider.provideHistoryItems({
                        historyItemRefs,
                        filterText,
                        limit: 100
                    }, token).then(historyItems => {
                        if (!historyItems) {
                            return [];
                        }
                        return (historyItems.map(historyItem => {
                            ...
                            if (historyItem.statistics) {
                                details.push(`${historyItem.statistics.files} ${( localize(13769, "file(s)"))}`);
                            }
...
SCMHistoryItemContext = SCMHistoryItemContext_1 = ( __decorate([( __param(0, ISCMViewService))], SCMHistoryItemContext));
...
SCMHistoryItemContextContentProvider = ( __decorate([( __param(0, IModelService)), ( __param(1, ISCMService))], SCMHistoryItemContextContentProvider));
...
SCMHistoryItemChangeRangeContentProvider = SCMHistoryItemChangeRangeContentProvider_1 = ( __decorate([( __param(0, IModelService)), ( __param(1, ISCMService))], SCMHistoryItemChangeRangeContentProvider));
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: "workbench.scm.action.graph.addHistoryItemToChat",
            title: ( localize(13770, "Add to Chat")),
            f1: false,
            menu: {
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize(13771, "Explain Changes")),
            f1: false,
            menu: {
                id: MenuId.SCMHistoryItemContext,
                group: "z_chat",
...
export { SCMHistoryItemChangeRangeContentProvider, SCMHistoryItemContextContribution, extractSCMHistoryItemDropData };
