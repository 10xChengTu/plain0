...
class SnippetCompletion {
    constructor(snippet, range) {
        ...
        this.detail = ( localize(14234, "{0} ({1})", snippet.description || snippet.name, snippet.source));
...
let SnippetCompletionProvider = class SnippetCompletionProvider {
    ...
    async provideCompletionItems(model, position, context) {
        ...
        for (const snippet of snippets) {
            ...
            suggestions.push(( new SnippetCompletion(snippet, {
                replace,
                insert
            })));
    ...
    _disambiguateSnippets(suggestions) {
        ...
        for (let i = 0; i < suggestions.length; i++) {
            ...
            if (to > i + 1) {
                suggestions[i].label.label = ( localize(14235, "{0}, {1}", suggestions[i].label.label, suggestions[i].snippet.name));
                i = to;
            }
        }
    }
...
SnippetCompletionProvider = ( __decorate([( __param(0, ILanguageService)), ( __param(1, ISnippetsService)), ( __param(2, ILanguageConfigurationService))], SnippetCompletionProvider));
