...
import { joinPath, isEqualOrParent, extname, basename } from '../../../../base/common/resources.js';
...
(function(snippetExt) {
    ...
    snippetExt.snippetsContribution = {
        description: ( localize(14260, "Contributes snippets.")),
        type: "array",
        defaultSnippets: [{
            body: [{
                language: "",
        ...
        items: {
            ...
            properties: {
                language: {
                    description: ( localize(14261, "Language identifier for which this snippet is contributed to.")),
                    type: "string"
                },
                path: {
...
SnippetEnablement = SnippetEnablement_1 = ( __decorate([( __param(0, IStorageService))], SnippetEnablement));
let SnippetUsageTimestamps = class SnippetUsageTimestamps {
    static {
        SnippetUsageTimestamps_1 = this;
    }
    static {
...
SnippetUsageTimestamps = SnippetUsageTimestamps_1 = ( __decorate([( __param(0, IStorageService))], SnippetUsageTimestamps));
let SnippetsService = class SnippetsService {
    constructor(
        ...
        this._pendingWork.push(
            Promise.resolve(lifecycleService.when(LifecyclePhase.Restored).then(() => {
        ...
        if (languageId) {
            if (this._languageService.isRegisteredLanguageId(languageId)) {
                for (const file of ( this._files.values())) {
                    promises.push(
                        file.load().then(file => file.select(languageId, result)).catch(err => this._logService.error(err, ( file.location.toString())))
                    );
                }
            }
        } else {
            for (const file of ( this._files.values())) {
                promises.push(
                    file.load().then(file => insertInto(result, result.length, file.data)).catch(err => this._logService.error(err, ( file.location.toString())))
                );
            }
        }
    ...
    _initWorkspaceSnippets() {
        ...
        const updateWorkspaceSnippets = () => {
            disposables.clear();
            this._pendingWork.push(
                this._initWorkspaceFolderSnippets(this._contextService.getWorkspace(), disposables)
            );
        };
    ...
    async _initUserSnippets() {
        ...
        this._disposables.add(
            this._userDataProfileService.onDidChangeCurrentProfile(e => e.join((async () => {
