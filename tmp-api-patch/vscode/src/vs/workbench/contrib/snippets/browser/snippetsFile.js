    ...
    isFileIncluded(resourceUri) {
        ...
        if (this.exclude) {
            for (const pattern of this.exclude.filter(Boolean)) {
                if (match(pattern, getMatchTarget(pattern), {
                    ignoreCase: true
                })) {
                    return false;
                }
...
class SnippetFile {
    ...
    _parseSnippet(name, snippet, bucket) {
        ...
        if (this._extension) {
            ...
            source = ( localize(14253, "Workspace Snippet"));
        } else {
            if (this.isGlobalSnippets) {
                source = ( localize(14254, "Global User Snippet"));
            } else {
                source = ( localize(14255, "User Snippet"));
            }
        }
