...
let LiveTestResult = class LiveTestResult extends Disposable {
    ...
    constructor(id, persist, request, insertOrder, telemetry) {
        ...
        this.tasks = [];
        this.name = ( localize(16142, "Test run at {0}", ( new Date()).toLocaleString(language)));
    ...
    appendOutput(output, taskId, location, testId) {
        ...
        const {
            offset,
            length
        } = task.output.append(output, marker);
        const message = {
            location,
            message: preview,
            offset,
    ...
    updateState(testId, taskId, state, duration) {
        ...
        if (oldTerminalStatePrio !== undefined && (newTerminalStatePrio === undefined || newTerminalStatePrio < oldTerminalStatePrio)) {
            return;
        }
...
LiveTestResult = ( __decorate([( __param(4, ITelemetryService))], LiveTestResult));
class HydratedTestResult {
    ...
    constructor(identity, serialized, persist = true) {
        ...
        this.tasks = ( serialized.tasks.map((task, i) => ({
            id: task.id,
            name: task.name || ( localize(16143, "Unnamed Task")),
            ctrlId: task.ctrlId,
            running: false,
            coverage: observableValue(this, undefined),
