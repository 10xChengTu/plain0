...
let DirtyFilesIndicator = class DirtyFilesIndicator extends Disposable {
    ...
    registerListeners() {
        this._register(
            this.workingCopyService.onDidChangeDirty(workingCopy => this.onWorkingCopyDidChangeDirty(workingCopy))
        );
    }
    ...
    updateActivityBadge() {
        ...
        if (dirtyCount > 0) {
            this.badgeHandle.value = this.activityService.showViewContainerActivity(VIEWLET_ID, {
                badge: ( new NumberBadge(dirtyCount, num => num === 1 ? ( localize(11243, "1 unsaved file")) : ( localize(11244, "{0} unsaved files", dirtyCount))))
            });
        } else {
