...
let TextFileEditor = class TextFileEditor extends AbstractTextCodeEditor {
    ...
    getTitle() {
        ...
        return localize(10878, "Text File Editor");
    }
    get input() {
        return this._input;
    }
    ...
    async handleSetInputError(error, input, options) {
        ...
        if (error.fileOperationResult === FileOperationResult.FILE_IS_DIRECTORY) {
            ...
            actions.push(toAction({
                ...
                label: ( localize(10879, "Open Folder")),
                run: async () => {
            ...
            if (this.contextService.isInsideWorkspace(input.preferredResource)) {
                actions.push(toAction({
                    ...
                    label: ( localize(10880, "Reveal Folder")),
                    run: async () => {
        ...
        if (error.fileOperationResult === FileOperationResult.FILE_NOT_FOUND && !this.filesConfigurationService.isReadonly(input.preferredResource) && (await this.pathService.hasValidBasename(input.preferredResource))) {
            const fileNotFoundError = createEditorOpenError(( new FileOperationError(( localize(10884, "The editor could not be opened because the file was not found.")), FileOperationResult.FILE_NOT_FOUND)), [toAction({
                id: "workbench.files.action.createMissingFile",
                label: ( localize(10885, "Create File")),
                run: async () => {
