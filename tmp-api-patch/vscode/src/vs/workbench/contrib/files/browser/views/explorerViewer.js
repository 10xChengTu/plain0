...
import { scheduleAtNextAnimationFrame, getWindow, addDisposableListener, addStandardDisposableListener, EventType, getActiveElement, isActiveElement, isHTMLElement, hasParentWithClass } from '../../../../../base/browser/dom.js';
...
import { FileKind, FileChangeType, FileSystemProviderCapabilities, FileOperationResult } from '../../../../../platform/files/common/files.js';
...
import { DisposableStore, toDisposable, dispose, Disposable } from '../../../../../base/common/lifecycle.js';
...
import { relativePath, dirname, joinPath, distinctParents } from '../../../../../base/common/resources.js';
...
import { Emitter, EventMultiplexer, Event } from '../../../../../base/common/event.js';
...
import { listFilterMatchHighlightBorder, listFilterMatchHighlight } from '../../../../../platform/theme/common/colors/listColors.js';
...
ExplorerDataSource = ( __decorate([( __param(2, IProgressService)), ( __param(3, IConfigurationService)), ( __param(4, INotificationService)), ( __param(5, IWorkbenchLayoutService)), ( __param(6, IFileService)), ( __param(7, IExplorerService)), ( __param(8, IWorkspaceContextService)), ( __param(9, IFilesConfigurationService))], ExplorerDataSource));
...
let ExplorerFindProvider = class ExplorerFindProvider {
    ...
    async doFilterFind(pattern, matchType, token) {
        ...
        return {
            ...
            matchCount: searchResults.reduce((
                acc,
                {
                    files,
                    directories
                }
            ) => acc + files.length + directories.length, 0),
            warningMessage: hitMaxResults ? ( localize(
                11217,
    ...
    async getSearchResults(pattern, roots, matchType, token) {
        ...
        return await Promise.all(( roots.map(
            (root, index) => this.searchInWorkspace(patternLowercase, root, index, isFuzzyMatch, token)
        )));
    }
    async searchInWorkspace(patternLowercase, root, rootIndex, isFuzzyMatch, token) {
        ...
        const searchExcludePattern = getExcludes(this.configurationService.getValue({
        ...
        let folderResults;
        try {
            [fileResults, folderResults] = await Promise.all([this.searchService.fileSearch({
                ...searchOptions,
                filePattern: `**/${segmentMatchPattern}`,
                maxResults: 512
            }, token), this.searchService.fileSearch({
                ...searchOptions,
                filePattern: `**/${segmentMatchPattern}/**`
            }, token)]);
        } catch (e) {
...
ExplorerFindProvider = ( __decorate([( __param(2, ISearchService)), ( __param(3, IFileService)), ( __param(4, IConfigurationService)), ( __param(5, IFilesConfigurationService)), ( __param(6, IProgressService)), ( __param(7, IExplorerService)), ( __param(8, IContextKeyService))], ExplorerFindProvider));
function getMatchingDirectoriesFromFiles(resources, root, segmentMatchPattern) {
    ...
    for (const dirResource of uniqueDirectories) {
        ...
        if (!dirStat || !match(segmentMatchPattern, dirStat, {
            ignoreCase: true
        })) {
            continue;
        }
...
let FilesRenderer = class FilesRenderer {
    ...
    getWidgetAriaLabel() {
        return localize(11218, "Files Explorer");
    }
    get templateId() {
    ...
    renderTemplate(container) {
        ...
        templateDisposables.add(label.onDidRender(() => {
            scheduleAtNextAnimationFrame(getWindow(templateData.container), () => {
                try {
    ...
    renderCompressedElements(node, index, templateData) {
        ...
        if (!editableData) {
            ...
            templateData.elementDisposables.add(
                this._onDidChangeActiveDescendant.add(compressedNavigationController.onDidChange)
            );
    ...
    renderStat(stat, label, domId, filterData, templateData) {
        ...
        if (highlightResults > 0) {
            ...
            badge.setTitleFormat(( localize(11219, "Directory contains {0} matches", highlightResults)));
    ...
    renderInputBox(container, stat, editableData) {
        ...
        const inputBox = ( new InputBox(label.element, this.contextViewService, {
            ...
            ariaLabel: ( localize(11220, "Type file name. Press Enter to confirm or Escape to cancel.")),
        ...
        const toDispose = [inputBox, inputBox.onDidChange(value => {
            label.setFile(joinPath(parent, value || " "), labelOptions);
            ...
            while (true) {
                ...
                if (isActiveElement(inputBox.inputElement)) {
                    return;
                } else if (isHTMLElement(activeElement) && hasParentWithClass(activeElement, "context-view")) {
...
FilesRenderer = FilesRenderer_1 = ( __decorate([( __param(4, IContextViewService)), ( __param(5, IThemeService)), ( __param(6, IConfigurationService)), ( __param(7, IExplorerService)), ( __param(8, ILabelService)), ( __param(9, IWorkspaceContextService)), ( __param(10, IContextMenuService)), ( __param(11, IInstantiationService))], FilesRenderer));
let FilesFilter = class FilesFilter {
    ...
    updateConfiguration() {
        ...
        this.contextService.getWorkspace().folders.forEach(folder => {
            ...
            this.hiddenExpressionPerRoot.set(( folder.uri.toString()), {
                ...
                parsed: parse(excludesConfigCopy)
            });
        });
    ...
    isVisible(stat, parentVisibility) {
        ...
        const globMatch = cached?.parsed(
            relative(stat.root.resource.path, stat.resource.path),
            stat.name,
        ...
        if (isHiddenResource || stat.parent?.isExcluded) {
            ...
            const editor = editors.find(
                e => e.resource && this.uriIdentityService.extUri.isEqualOrParent(e.resource, stat.resource)
            );
...
FilesFilter = ( __decorate([( __param(0, IWorkspaceContextService)), ( __param(1, IConfigurationService)), ( __param(2, IExplorerService)), ( __param(3, IEditorService)), ( __param(4, IUriIdentityService)), ( __param(5, IFileService))], FilesFilter));
...
FileSorter = ( __decorate([( __param(0, IExplorerService)), ( __param(1, IWorkspaceContextService))], FileSorter));
let FileDragAndDrop = class FileDragAndDrop {
    ...
    constructor(
        ...
        this.disposables.add(
            this.configurationService.onDidChangeConfiguration(e => updateDropEnablement(e))
        );
    }
    ...
    async handleExplorerDrop(data, target, targetIndex, targetSector, originalEvent) {
        ...
        if (confirmDragAndDrop) {
            ...
            const confirmation = await this.dialogService.confirm({
                ...
                detail,
                checkbox: {
                    label: ( localize(11225, "Do not ask me again"))
                },
                primaryButton: ( localize(11226, "&&Move"))
            });
    ...
    async doHandleExplorerDropOnCopy(sources, target) {
        ...
        for (const {
            ...
            const newResource = await findValidPasteFileTarget(this.explorerService, this.fileService, this.dialogService, target, {
                resource,
                isDirectory,
                allowOverwrite
        ...
        await this.explorerService.applyBulkEdit(resourceFileEdits, {
            ...
            undoLabel: ( localize(11227, "Copy {0}", labelSuffix)),
            progressLabel: ( localize(11228, "Copying {0}", labelSuffix))
        });
    ...
    async doHandleExplorerDropOnMove(sources, target) {
        ...
        const options = {
            ...
            undoLabel: ( localize(11229, "Move {0}", labelSuffix)),
            progressLabel: ( localize(11230, "Moving {0}", labelSuffix))
        };
        try {
            ...
            if (error.fileOperationResult === FileOperationResult.FILE_MOVE_CONFLICT) {
                ...
                const {
                    confirmed
                } = await this.dialogService.confirm(confirm);
                if (confirmed) {
                    await this.explorerService.applyBulkEdit(( resourceFileEdits.map(re => ( new ResourceFileEdit(re.oldResource, re.newResource, {
                        overwrite: true
                    })))), options);
                }
            }
...
FileDragAndDrop = FileDragAndDrop_1 = ( __decorate([( __param(1, IExplorerService)), ( __param(2, IEditorService)), ( __param(3, IDialogService)), ( __param(4, IWorkspaceContextService)), ( __param(5, IFileService)), ( __param(6, IConfigurationService)), ( __param(7, IInstantiationService)), ( __param(8, IWorkspaceEditingService)), ( __param(9, IUriIdentityService))], FileDragAndDrop));
...
function getFileOrFolderLabelSuffix(items) {
    ...
    if (items.every(i => i.isDirectory)) {
        return localize(11231, "{0} folders", items.length);
    }
    if (items.every(i => !i.isDirectory)) {
        return localize(11232, "{0} files", items.length);
    }
    return `${items.length} files and folders`;
}

export { CompressedNavigationController, ExplorerCompressionDelegate, ExplorerDataSource, ExplorerDelegate, ExplorerFindProvider, FileDragAndDrop, FileSorter, FilesFilter, FilesRenderer, PhantomExplorerItem, explorerRootErrorEmitter, isCompressedFolderName };
