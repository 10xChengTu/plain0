...
import { EditorInputCapabilities, EditorResourceAccessor, SideBySideEditor, EditorsOrder, SaveReason } from '../../../common/editor.js';
        ...
        if (!isCancellationError(error)) {
            ...
            if (editorsToRevert.length > 0) {
                actions.push(toAction({
                    ...
                    label: editorsToRevert.length > 1 ? ( localize(11051, "Revert All")) : ( localize(11052, "Revert")),
            ...
            notificationService.notify({
                ...
                message: ( localize(11053, "Failed to save '{0}': {1}", ( editors.map((
                    {
                        editor
                    }
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    win: {
        primary: KeyChord(
            ...
            KeyMod$1.CtrlCmd | KeyMod$1.Shift | KeyCode.KeyS
        )
    },
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    primary: undefined,
    mac: {
        primary: KeyMod$1.CtrlCmd | KeyMod$1.Alt | KeyCode.KeyS
    },
    win: {
        primary: KeyChord(KeyMod$1.CtrlCmd | KeyCode.KeyK, KeyCode.KeyS)
    },
...
CommandsRegistry.registerCommand({
    ...
    handler: (accessor, _, editorContext) => {
        ...
        if (!resolvedContext.groupedEditors.length) {
            ...
            groups = ( resolvedContext.groupedEditors.map((
                {
                    group
                }
            ) => group));
...
CommandsRegistry.registerCommand({
    ...
    handler: async accessor => {
        ...
        try {
            ...
            const notificationService = accessor.get(INotificationService);
            notificationService.error(( localize(11054, "Failed to revert '{0}': {1}", ( editors.map((
                {
                    editor
                }
            ) => editor.getName())).join(", "), toErrorMessage(error, false))));
