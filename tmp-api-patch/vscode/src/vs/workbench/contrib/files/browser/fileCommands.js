...
import { ExplorerFocusCondition, FilesExplorerFocusCondition, ExplorerFolderContext, TextFileContentProvider, VIEW_ID, VIEWLET_ID, ExplorerCompressedFocusContext, ExplorerCompressedFirstFocusContext, ExplorerCompressedLastFocusContext } from '../common/files.js';
...
import { getMultiSelectedResources, getResourceForCommand } from './files.js';
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    when: undefined,
    weight: KeybindingWeight.WorkbenchContrib,
    primary: KeyChord(KeyMod$1.CtrlCmd | KeyCode.KeyK, KeyCode.KeyD),
    handler: async (accessor, resource) => {
        ...
        if (uri && fileService.hasProvider(uri)) {
            ...
            const editorLabel = ( localize(11055, "{0} (in file) ↔ {1}", name, name));
            try {
...
async function resourcesToClipboard(resources, relative, clipboardService, labelService, configurationService) {
    if (resources.length) {
        ...
        const text = ( resources.map(resource => labelService.getUriLabel(resource, {
            relative,
            noPrefix: true,
            separator
        }))).join(lineDelimiter);
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    primary: KeyMod$1.CtrlCmd | KeyMod$1.Alt | KeyCode.KeyC,
    win: {
        primary: KeyMod$1.Shift | KeyMod$1.Alt | KeyCode.KeyC
    },
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    primary: KeyChord(KeyMod$1.CtrlCmd | KeyCode.KeyK, KeyMod$1.CtrlCmd | KeyMod$1.Alt | KeyCode.KeyC),
    win: {
        primary: KeyMod$1.Shift | KeyMod$1.Alt | KeyCode.KeyC
    },
    id: COPY_PATH_COMMAND_ID,
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    weight: KeybindingWeight.WorkbenchContrib,
    when: ( EditorContextKeys.focus.toNegated()),
    primary: KeyMod$1.CtrlCmd | KeyMod$1.Shift | KeyMod$1.Alt | KeyCode.KeyC,
    win: {
        primary: KeyChord(
            KeyMod$1.CtrlCmd | KeyCode.KeyK,
            KeyMod$1.CtrlCmd | KeyMod$1.Shift | KeyCode.KeyC
        )
    },
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    primary: KeyChord(
        KeyMod$1.CtrlCmd | KeyCode.KeyK,
        KeyMod$1.CtrlCmd | KeyMod$1.Shift | KeyMod$1.Alt | KeyCode.KeyC
    ),
    win: {
        primary: KeyChord(
            KeyMod$1.CtrlCmd | KeyCode.KeyK,
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    weight: KeybindingWeight.WorkbenchContrib,
    when: undefined,
    primary: KeyChord(KeyMod$1.CtrlCmd | KeyCode.KeyK, KeyCode.KeyP),
...
CommandsRegistry.registerCommand({
    ...
    handler: (accessor, resource) => {
        ...
        const resources = getMultiSelectedResources(
            ...
            resource => ( workspace.folders.some(folder => uriIdentityService.extUri.isEqual(folder.uri, resource)))
        );
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    weight: KeybindingWeight.WorkbenchContrib,
    when: null,
    primary: isWeb ? (isWindows ? KeyChord(KeyMod$1.CtrlCmd | KeyCode.KeyK, KeyCode.KeyN) : KeyMod$1.CtrlCmd | KeyMod$1.Alt | KeyCode.KeyN) : KeyMod$1.CtrlCmd | KeyCode.KeyN,
    secondary: isWeb ? [KeyMod$1.CtrlCmd | KeyCode.KeyN] : undefined,
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor, args) => {
        ...
        const fileService = accessor.get(IFileService);
        const createFileLocalized = ( localize(11056, "Create File"));
        ...
        await fileService.createFile(saveUri, undefined, {
            overwrite: true
        });
        await editorService.openEditor({
            resource: saveUri,
