...
import { distinctParents, basenameOrAuthority, joinPath, basename as basename$1, isEqualOrParent } from '../../../../base/common/resources.js';
...
import { ActiveEditorContext, IsSessionsWindowContext, EmptyWorkspaceSupportContext, ActiveEditorCanToggleReadonlyContext } from '../../../common/contextkeys.js';
...
const NEW_FILE_LABEL = ( localize2(10956, "New File..."));
const NEW_FOLDER_COMMAND_ID = "explorer.newFolder";
const NEW_FOLDER_LABEL = ( localize2(10957, "New Folder..."));
const TRIGGER_RENAME_LABEL = ( localize(10958, "Rename..."));
const MOVE_FILE_TO_TRASH_LABEL = ( localize(10959, "Delete"));
const COPY_FILE_LABEL = ( localize(10960, "Copy"));
const PASTE_FILE_LABEL = ( localize(10961, "Paste"));
const FileCopiedContext = ( new RawContextKey("fileCopied", false));
const DOWNLOAD_COMMAND_ID = "explorer.download";
const DOWNLOAD_LABEL = ( localize(10962, "Download..."));
const UPLOAD_COMMAND_ID = "explorer.upload";
const UPLOAD_LABEL = ( localize(10963, "Upload..."));
...
async function deleteFiles(
    ...
    let primaryButton;
    if (useTrash) {
        primaryButton = isWindows ? ( localize(10964, "&&Move to Recycle Bin")) : ( localize(10965, "&&Move to Trash"));
    } else {
    ...
    if (dirtyWorkingCopies.size) {
        ...
        const response = await dialogService.confirm({
            type: "warning",
            message,
            detail: ( localize(10971, "Your changes will be lost if you don't save them.")),
            primaryButton
        });
    ...
    if (!skipConfirm) {
        ...
        if (readonlyResources.length) {
            ...
            const response = await dialogService.confirm({
                type: "warning",
                message,
                detail: ( localize(10975, "The read-only protection will be overridden if you continue.")),
                primaryButton: ( localize(10976, "Continue"))
            });
            if (!response.confirmed) {
                return;
            }
    ...
    let confirmation;
    const deleteDetail = ( distinctElements.some(e => e.isDirectory)) ? ( localize(10977, "This action is irreversible!")) : distinctElements.length > 1 ? ( localize(10978, "You can restore these files using the Undo command.")) : ( localize(10979, "You can restore this file using the Undo command."));
    if (skipConfirm || configurationService.getValue(CONFIRM_DELETE_SETTING_KEY) === false) {
        ...
        detail += detail ? "\n" : "";
        if (isWindows) {
            detail += distinctElements.length > 1 ? ( localize(10980, "You can restore these files from the Recycle Bin.")) : ( localize(10981, "You can restore this file from the Recycle Bin."));
        } else {
            detail += distinctElements.length > 1 ? ( localize(10982, "You can restore these files from the Trash.")) : ( localize(10983, "You can restore this file from the Trash."));
        }
        confirmation = await dialogService.confirm({
            ...
            primaryButton,
            checkbox: {
                label: ( localize(10984, "Do not ask me again"))
            }
        });
    }
    else {
    ...
    try {
        ...
        const options = {
            undoLabel: distinctElements.length > 1 ? ( localize(10985, "Delete {0} files", distinctElements.length)) : ( localize(10986, "Delete {0}", distinctElements[0].name)),
            progressLabel: distinctElements.length > 1 ? ( localize(10987, "Deleting {0} files", distinctElements.length)) : ( localize(10988, "Deleting {0}", distinctElements[0].name))
        };
        ...
        if (useTrash) {
            ...
            primaryButton = ( localize(10991, "&&Delete Permanently"));
        } else {
            errorMessage = toErrorMessage(error, false);
            primaryButton = ( localize(10992, "&&Retry"));
        }
...
function getMoveToTrashMessage(distinctElements) {
    ...
    return {
        message: ( localize(10997, "Are you sure you want to delete '{0}'?", distinctElements[0].name)),
        detail: ""
    };
}
...
async function findValidPasteFileTarget(
    explorerService,
    fileService,
    dialogService,
    targetFolder,
...
async function askForOverwrite(fileService, dialogService, targetResource) {
    ...
    const {
        ...
        primaryButton: ( localize(11004, "&&Replace"))
    });
    return confirmed;
}
class GlobalCompareResourcesAction extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(11005, "Compare Active File With..."));
    }
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(11006, "Opens a picker to select a file to diff with the active editor."))
            }
        });
    }
    async run(accessor) {
...
class ToggleAutoSaveAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(11007, "Toggle Auto Save")),
            f1: true,
            category: Categories.File,
            precondition: ( IsSessionsWindowContext.negate()),
            metadata: {
                description: ( localize2(11008, "Toggle the ability to save files automatically after typing"))
            }
        });
    }
    run(accessor) {
...
let BaseSaveAllAction = class BaseSaveAllAction extends Action {
    ...
    registerListeners() {
        this._register(
            this.workingCopyService.onDidChangeDirty(workingCopy => this.updateEnablement(workingCopy))
        );
    }
...
BaseSaveAllAction = ( __decorate([( __param(2, ICommandService)), ( __param(3, INotificationService)), ( __param(4, IWorkingCopyService))], BaseSaveAllAction));
class SaveAllInGroupAction extends BaseSaveAllAction {
    static {
        this.ID = "workbench.files.action.saveAllInGroup";
    }
    static {
        this.LABEL = ( localize(11009, "Save All in Group"));
    }
    get class() {
...
let CloseGroupAction = class CloseGroupAction extends Action {
    ...
    static {
        this.LABEL = ( localize(11010, "Close Group"));
    }
...
CloseGroupAction = ( __decorate([( __param(2, ICommandService))], CloseGroupAction));
class FocusFilesExplorer extends Action2 {
    static {
        this.ID = "workbench.files.action.focusFilesExplorer";
    }
    static {
        this.LABEL = ( localize2(11011, "Focus on Files Explorer"));
    }
    constructor() {
        super({
            ...
            precondition: ( IsSessionsWindowContext.negate()),
            metadata: {
                description: ( localize2(11012, "Moves focus to the file explorer view container."))
            }
        });
    }
    async run(accessor) {
...
class ShowActiveFileInExplorer extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(11013, "Reveal Active File in Explorer View"));
    }
    constructor() {
        super({
            ...
            precondition: ( IsSessionsWindowContext.negate()),
            metadata: {
                description: ( localize2(11014, "Reveals and selects the active file within the explorer view."))
            }
        });
    }
    async run(accessor) {
...
class OpenActiveFileInEmptyWorkspace extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(11015, "Open Active Editor in New Empty Workspace"));
    }
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(11016, "Opens the active editor in a new window with no folders open."))
            }
        });
    }
    async run(accessor) {
        ...
        if (fileResource && fileService.hasProvider(fileResource)) {
            ...
            dialogService.error(( localize(11017, "The active editor must contain an openable resource.")));
        }
    }
}
function validateFileName(pathService, item, name, os) {
    ...
    if (!name || name.length === 0 || /^\s+$/.test(name)) {
        return {
            content: ( localize(11018, "A file or folder name must be provided.")),
            severity: Severity$1.Error
        };
    }
    if (name[0] === "/" || name[0] === "\\") {
        return {
            content: ( localize(11019, "A file or folder name cannot start with a slash.")),
            severity: Severity$1.Error
        };
    }
    ...
    if (( names.some(name => /^\s|\s$/.test(name)))) {
        return {
            content: ( localize(11022, "Leading or trailing whitespace detected in file or folder name.")),
...
class CompareNewUntitledTextFilesAction extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(11023, "Compare New Untitled Text Files"));
    }
    constructor() {
        super({
            ...
            precondition: ( IsSessionsWindowContext.negate()),
            metadata: {
                description: ( localize2(11024, "Opens a new diff editor with two untitled files."))
            }
        });
    }
    async run(accessor) {
...
class CompareWithClipboardAction extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(11025, "Compare Active File with Clipboard"));
    }
    static {
    ...
    constructor() {
        super({
            ...
            precondition: ( IsSessionsWindowContext.negate()),
            keybinding: {
                primary: KeyChord(KeyMod$1.CtrlCmd | KeyCode.KeyK, KeyCode.KeyC),
    ...
    async run(accessor) {
        ...
        if (resource && (fileService.hasProvider(resource) || resource.scheme === Schemas.untitled)) {
            ...
            const name = basename$1(resource);
            const editorLabel = ( localize(11027, "Clipboard ↔ {0}", name));
            await editorService.openEditor({
                original: {
...
ClipboardContentProvider = ( __decorate([( __param(0, IClipboardService)), ( __param(1, ILanguageService)), ( __param(2, IModelService))], ClipboardContentProvider));
...
async function openExplorerAndCreate(accessor, isFolder) {
    ...
    const onSuccess = async value => {
        try {
            ...
            await explorerService.applyBulkEdit([( new ResourceFileEdit(undefined, resourceToCreate, {
                folder: isFolder
            }))], {
                undoLabel: ( localize(11029, "Create {0}", value)),
                progressLabel: ( localize(11030, "Creating {0}", value)),
                ...
                if (( stat.resource.toString()) !== ( targetResource.toString())) {
                    try {
                        await explorerService.applyBulkEdit([( new ResourceFileEdit(stat.resource, targetResource))], {
                            ...
                            undoLabel: ( localize(11031, "Rename {0} to {1}", stat.name, value)),
                            progressLabel: ( localize(11032, "Renaming {0} to {1}", stat.name, value))
                        });
                        await refreshIfSeparator(value, explorerService);
                    } catch (e) {
...
const moveFileToTrashHandler = async accessor => {
    ...
    if (stats.length) {
        await deleteFiles(
            ...
            accessor.get(IFilesConfigurationService),
            stats,
            true
        );
    }
...
const pasteFileHandler = async (accessor, fileList) => {
    ...
    if (confirmPasteNative && toPaste.files.length >= 1) {
        const message = toPaste.files.length > 1 ? ( localize(
            11033,
            ...
            basename(toPaste.type === "paths" ? toPaste.files[0].fsPath : toPaste.files[0].name)
        ));
        ...
        const confirmation = await dialogService.confirm({
            ...
            primaryButton: ( localize(11035, "&&Paste"))
        });
        if (!confirmation.confirmed) {
            return;
        }
        ...
        if (toPaste.type === "paths") {
            const sourceTargetPairs = coalesce(await Promise.all(( toPaste.files.map(async fileToPaste => {
                if (( element.resource.toString()) !== ( fileToPaste.toString()) && isEqualOrParent(element.resource, fileToPaste)) {
                    throw ( new Error(( localize(11036, "File to paste is an ancestor of the destination folder"))));
                }
            ...
            if (sourceTargetPairs.length >= 1) {
                if (pasteShouldMove) {
                    ...
                    const options = {
                        confirmBeforeUndo: configurationService.getValue().explorer.confirmUndo === UndoConfirmLevel.Verbose,
                        progressLabel: sourceTargetPairs.length > 1 ? ( localize(11037, "Moving {0} files", sourceTargetPairs.length)) : ( localize(11038, "Moving {0}", basenameOrAuthority(sourceTargetPairs[0].target))),
                        undoLabel: sourceTargetPairs.length > 1 ? ( localize(11039, "Move {0} files", sourceTargetPairs.length)) : ( localize(11040, "Move {0}", basenameOrAuthority(sourceTargetPairs[0].target)))
                    };
                    await explorerService.applyBulkEdit(resourceFileEdits, options);
                } else {
                    const resourceFileEdits = ( sourceTargetPairs.map(pair => ( new ResourceFileEdit(pair.source, pair.target, {
    ...
    async function applyCopyResourceEdit(targets, resourceFileEdits) {
        ...
        const options = {
            ...
            progressLabel: targets.length > 1 ? ( localize(11042, "Copying {0} files", targets.length)) : ( localize(11043, "Copying {0}", basenameOrAuthority(targets[0]))),
            undoLabel: targets.length > 1 ? ( localize(11044, "Paste {0} files", targets.length)) : ( localize(11045, "Paste {0}", basenameOrAuthority(targets[0])))
        };
        await explorerService.applyBulkEdit(resourceFileEdits, options);
    }
};
async function getFilesToPaste(fileList, clipboardService, hostService) {
    if (fileList && fileList.length > 0) {
        ...
        return {
            type: "paths",
            files: distinctParents(await clipboardService.readResources(), resource => resource)
        };
    }
}
...
class SetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(11046, "Set Active Editor Read-only in Session"));
    }
    constructor() {
        super(
...
class SetActiveEditorWriteableInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(11047, "Set Active Editor Writeable in Session"));
    }
    constructor() {
        super(
...
class ToggleActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(11048, "Toggle Active Editor Read-only in Session"));
    }
    constructor() {
        super(
...
class ResetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(11049, "Reset Active Editor Read-only in Session"));
    }
    constructor() {
        super(
