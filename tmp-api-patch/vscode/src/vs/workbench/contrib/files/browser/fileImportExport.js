...
import { RunOnceWorker, Limiter, Promises } from '../../../../base/common/async.js';
...
let BrowserFileUpload = class BrowserFileUpload {
    ...
    upload(target, source) {
        ...
        const uploadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: true,
            title: ( localize(11063, "Uploading"))
            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([( new ResourceFileEdit(joinPath(target.resource, entry.name), undefined, {
                        ...
                        undoLabel: ( localize(11064, "Overwrite {0}", entry.name)),
                        progressLabel: ( localize(11065, "Overwriting {0}", entry.name))
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
        ...
        if (entry.isFile) {
            ...
            await Promises.settled(( fileChildEntries.map(fileChildEntry => {
                return fileUploadQueue.queue(
                    () => this.doUploadEntry(fileChildEntry, resource, folderTarget, progress, operation, token)
                );
            })));
...
let ExternalFileImport = class ExternalFileImport {
    ...
    async doImport(target, source, targetWindow, token) {
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            const buttons = [{
                label: folders.length > 1 ? ( localize(11069, "&&Copy Folders")) : ( localize(11070, "&&Copy Folder")),
            ...
            if (( folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0))) {
                buttons.unshift({
                    label: folders.length > 1 ? ( localize(11071, "&&Add Folders to Workspace")) : ( localize(11072, "&&Add Folder to Workspace")),
                ...
                message = folders.length > 1 ? ( localize(11075, "Are you sure to want to copy folders?")) : ( localize(11076, "Are you sure to want to copy '{0}'?", basename(folders[0].uri)));
            }
            const {
                result
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                ...
                )) : ( localize(11078, "The dropped file could not be accessed for import.")));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ? ( localize(11079, "Import {0}", basename(resourcesFiltered[0]))) : ( localize(11080, "Import {0} resources", resourcesFiltered.length)),
                progressLabel: resourcesFiltered.length === 1 ? ( localize(11081, "Copying {0}", basename(resourcesFiltered[0]))) : ( localize(11082, "Copying {0} resources", resourcesFiltered.length)),
...
let FileDownload = class FileDownload {
    ...
    download(source) {
        ...
        const downloadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: isWeb,
            title: ( localize(11083, "Downloading"))
    ...
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const lastUsedDownloadPath = this.storageService.get(
            FileDownload_1.LAST_USED_DOWNLOAD_PATH_STORAGE_KEY,
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: ( localize(11086, "Download")),
            title: ( localize(11087, "Choose Where to Download")),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([( new ResourceFileEdit(explorerItem.resource, destination, {
                ...
                undoLabel: ( localize(11088, "Download {0}", explorerItem.name)),
                progressLabel: ( localize(11089, "Downloading {0}", explorerItem.name)),
                progressLocation: ProgressLocation.Window
            });
        } else {
            cts.cancel();
...
function getFileOverwriteConfirm(name) {
    return {
        ...
        detail: ( localize(11091, "This action is irreversible!")),
        primaryButton: ( localize(11092, "&&Replace")),
        type: "warning"
    };
}
function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            ...
            detail: getFileNamesMessage(files) + "\n" + ( localize(11091, "This action is irreversible!")),
            primaryButton: ( localize(11092, "&&Replace")),
            type: "warning"
        };
    }
