...
async function getExperimentalExtensionToggleData(chatEntitlementService, extensionGalleryService, productService) {
    ...
    if (productService.extensionRecommendations) {
        ...
        for (const key in settingsEditorRecommendedExtensions) {
            ...
            try {
                const extensions = await raceTimeout(extensionGalleryService.getExtensions([{
                    id: extensionId,
                    preRelease: !isStable
                }], CancellationToken.None), EXTENSION_FETCH_TIMEOUT_MS);
...
function wordifyKey(key) {
    key =
    key.replace(/\.([a-z0-9])/g, (_, p1) => ` \u203A ${p1.toUpperCase()}`).replace(/([a-z0-9])([A-Z])/g, "$1 $2").replace(/([A-Z]{1,})([A-Z][a-z])/g, "$1 $2").replace(/^[a-z]/g, match => match.toUpperCase()).replace(/\b\w+\b/g, match => {
