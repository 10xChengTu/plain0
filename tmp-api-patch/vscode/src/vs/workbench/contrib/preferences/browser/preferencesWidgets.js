...
import { $, addDisposableListener, EventType, EventHelper, append, trackFocus, getTotalWidth } from '../../../../base/browser/dom.js';
...
let FolderSettingsActionViewItem = class FolderSettingsActionViewItem extends BaseActionViewItem {
    constructor(action, contextService, contextMenuService, hoverService) {
        ...
        this._register(
            this.contextService.onDidChangeWorkspaceFolders(() => this.onWorkspaceFoldersChanged())
        );
    }
    get folder() {
        return this._folder;
    ...
    render(container) {
        ...
        this.anchorElementHover = this._register(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.anchorElement, "")
        );
        this._register(
            addDisposableListener(this.anchorElement, EventType.MOUSE_DOWN, e => EventHelper.stop(e))
        );
        this._register(
            addDisposableListener(this.anchorElement, EventType.CLICK, e => this.onClick(e))
        );
        this._register(
            addDisposableListener(this.container, EventType.KEY_UP, e => this.onKeyUp(e))
        );
...
FolderSettingsActionViewItem = ( __decorate([( __param(1, IWorkspaceContextService)), ( __param(2, IContextMenuService)), ( __param(3, IHoverService))], FolderSettingsActionViewItem));
let SettingsTargetsWidget = class SettingsTargetsWidget extends Widget {
    constructor(
        ...
        this.create(parent);
        this._register(
            this.contextService.onDidChangeWorkbenchState(() => this.onWorkbenchStateChanged())
        );
    ...
    resetLabels() {
        ...
        this.userLocalSettings.label = ( localize(13146, "User"));
        this.userRemoteSettings.label = ( localize(13147, "Remote")) + (hostLabel ? ` [${hostLabel}]` : "");
        this.workspaceSettings.label = this.contextService.getWorkspace().name || ( localize(13148, "Workspace"));
        this.folderSettingsAction.label = ( localize(13149, "Folder"));
    }
    create(parent) {
        ...
        this.settingsSwitcherBar = this._register(( new ActionBar(settingsTabsWidget, {
            ...
            ariaLabel: ( localize(13150, "Settings Switcher")),
            ariaRole: "tablist",
        ...
        this.userLocalSettings.tooltip = ( localize(13146, "User"));
        ...
        const remoteAuthority = this.environmentService.remoteAuthority;
        const hostLabel = remoteAuthority && this.labelService.getHostLabel(Schemas.vscodeRemote, remoteAuthority);
        this.userRemoteSettings.tooltip = ( localize(13147, "Remote")) + (hostLabel ? ` [${hostLabel}]` : "");
        ...
        this.folderSettings = this._register(
            this.instantiationService.createInstance(FolderSettingsActionViewItem, this.folderSettingsAction)
        );
        this.resetLabels();
        this.update();
    ...
    setResultCount(settingsTarget, count) {
        if (settingsTarget === ConfigurationTarget.WORKSPACE) {
            let label = this.contextService.getWorkspace().name ?? ( localize(13148, "Workspace"));
            if (count) {
                label += ` (${count})`;
            }
            this.workspaceSettings.label = label;
        } else if (settingsTarget === ConfigurationTarget.USER_LOCAL) {
            let label = ( localize(13146, "User"));
            if (count) {
                label += ` (${count})`;
            }
    ...
    async update() {
        ...
        this.workspaceSettings.tooltip = ( localize(13148, "Workspace"));
    }
};
SettingsTargetsWidget = ( __decorate([( __param(2, IWorkspaceContextService)), ( __param(3, IInstantiationService)), ( __param(4, IWorkbenchEnvironmentService)), ( __param(5, ILabelService)), ( __param(6, ILanguageService))], SettingsTargetsWidget));
let SearchWidget = class SearchWidget extends Widget {
    ...
    createInputBox(parent) {
        ...
        return ( new ContextScopedHistoryInputBox(parent, this.contextViewService, {
            ...this.options,
            showHistoryHint
...
SearchWidget = ( __decorate([( __param(2, IContextViewService)), ( __param(3, IInstantiationService)), ( __param(4, IContextKeyService)), ( __param(5, IKeybindingService))], SearchWidget));
