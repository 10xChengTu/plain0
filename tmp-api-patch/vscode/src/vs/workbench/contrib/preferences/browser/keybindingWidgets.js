...
import { editorWidgetBackground, editorWidgetForeground } from '../../../../platform/theme/common/colors/editorColors.js';
...
let KeybindingsSearchWidget = class KeybindingsSearchWidget extends SearchWidget {
    ...
    startRecordingKeys() {
        this.recordDisposables.add(addDisposableListener(
            this.inputBox.inputElement,
            EventType.KEY_DOWN,
            e => this._onKeyDown(( new StandardKeyboardEvent(e)))
        ));
        this.recordDisposables.add(
            addDisposableListener(this.inputBox.inputElement, EventType.BLUR, () => this._onBlur.fire())
        );
        this.recordDisposables.add(
            addDisposableListener(this.inputBox.inputElement, EventType.INPUT, () => {
...
KeybindingsSearchWidget = ( __decorate([( __param(2, IContextViewService)), ( __param(3, IInstantiationService)), ( __param(4, IContextKeyService)), ( __param(5, IKeybindingService))], KeybindingsSearchWidget));
let DefineKeybindingWidget = class DefineKeybindingWidget extends Widget {
    ...
    constructor(parent, instantiationService) {
        ...
        const message = ( localize(13015, "Press desired key combination and then press ENTER."));
        ...
        this._keybindingInputWidget = this._register(
            this.instantiationService.createInstance(KeybindingsSearchWidget, this._domNode.domNode, {
                ariaLabel: message,
                history: ( new Set([])),
        ...
        this._register(
            this._keybindingInputWidget.onKeybinding(keybinding => this.onKeybinding(keybinding))
        );
    ...
    printExisting(numberOfExisting) {
        if (numberOfExisting > 0) {
            ...
            const text = numberOfExisting === 1 ? ( localize(13016, "1 existing command has this keybinding", numberOfExisting)) : ( localize(13017, "{0} existing commands have this keybinding", numberOfExisting));
    ...
    onKeybinding(keybinding) {
        ...
        if (this._chords) {
            for (let i = 1; i < this._chords.length; i++) {
                this._outputNode.appendChild(createTextNode(( localize(13018, "chord to"))));
...
DefineKeybindingWidget = DefineKeybindingWidget_1 = ( __decorate([( __param(1, IInstantiationService))], DefineKeybindingWidget));
...
DefineKeybindingOverlayWidget = DefineKeybindingOverlayWidget_1 = ( __decorate([( __param(1, IInstantiationService))], DefineKeybindingOverlayWidget));
