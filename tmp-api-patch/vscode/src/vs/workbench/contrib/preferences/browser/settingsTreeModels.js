...
import { MODIFIED_SETTING_TAG, REQUIRE_TRUSTED_WORKSPACE_SETTING_TAG, POLICY_SETTING_TAG, AGENTS_WINDOW_SETTING_TAG, wordifyKey, compareTwoNullableNumbers } from '../common/preferences.js';
...
SettingsTreeModel = ( __decorate([( __param(2, IWorkbenchConfigurationService)), ( __param(3, ILanguageService)), ( __param(4, IUserDataProfileService)), ( __param(5, IProductService)), ( __param(6, IWorkbenchEnvironmentService))], SettingsTreeModel));
function inspectSetting(key, target, languageFilter, configurationService) {
    ...
    if (overrideIdentifiers) {
        for (const overrideIdentifier of overrideIdentifiers) {
            inspectedLanguageOverrides.set(overrideIdentifier, configurationService.inspect(key, {
                overrideIdentifier
            }));
        }
        if (languageFilter) {
...
function getObjectSettingSchemaType(
    ...
    if ((objectAdditionalProperties === true || objectAdditionalProperties === undefined) && !( Object.keys(objectPatternProperties ?? {})).includes(".*")) {
        return false;
    }
    ...
    sortResults(filterMatches) {
        ...
        if (!this._viewState.query) {
            return filterMatches.sort(
                (a, b) => compareTwoNullableNumbers(a.setting.internalOrder, b.setting.internalOrder)
            );
        }
...
SearchResultModel = ( __decorate([( __param(3, IWorkbenchConfigurationService)), ( __param(4, IWorkbenchEnvironmentService)), ( __param(5, ILanguageService)), ( __param(6, IUserDataProfileService)), ( __param(7, IProductService))], SearchResultModel));
...
export { SearchResultIdx, SearchResultModel, SettingsTreeElement, SettingsTreeGroupElement, SettingsTreeModel, SettingsTreeNewExtensionsElement, SettingsTreeSettingElement, inspectSetting, objectSettingSupportsRemoveDefaultValue, parseQuery, sanitizeId, settingKeyToDisplayFormat };
