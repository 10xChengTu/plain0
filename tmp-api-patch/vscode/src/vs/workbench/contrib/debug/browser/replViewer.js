...
import { $ as $$1, append, clearNode, addDisposableListener } from '../../../../base/browser/dom.js';
...
let ReplGroupRenderer = class ReplGroupRenderer {
    ...
    renderElement(element, _index, templateData) {
        ...
        templateData.elementDisposable = this.expressionRenderer.renderValue(templateData.label, replGroup.name, {
            wasANSI: true,
            session: element.element.session
        });
...
ReplGroupRenderer = ReplGroupRenderer_1 = ( __decorate([( __param(1, IInstantiationService))], ReplGroupRenderer));
...
let ReplOutputElementRenderer = class ReplOutputElementRenderer {
    ...
    renderElement(
        ...
        templateData.elementDisposable.add(
            element.onDidChangeCount(() => this.setElementCount(element, templateData))
        );
        ...
        templateData.value.classList.add(
            (element.severity === Severity$1.Warning) ? "warn" : (element.severity === Severity$1.Error) ? "error" : (element.severity === Severity$1.Ignore) ? "ignore" : "info"
        );
...
ReplOutputElementRenderer = ReplOutputElementRenderer_1 = ( __decorate([( __param(1, IInstantiationService))], ReplOutputElementRenderer));
...
ReplVariablesRenderer = ReplVariablesRenderer_1 = ( __decorate([( __param(1, IDebugService)), ( __param(2, IContextViewService)), ( __param(3, IHoverService))], ReplVariablesRenderer));
...
class ReplDelegate extends CachedListVirtualDelegate {
    ...
    estimateHeight(element, ignoreValueLength = false) {
        ...
        if (hasValue(element) && !isNestedVariable(element)) {
            ...
            const valueRows =
            countNumberOfLines(value) + (ignoreValueLength ? 0 : Math.floor(value.length / 70)) + (element instanceof ReplOutputElement ? 0 : 1);
...
class ReplAccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(9941, "Debug Console");
    }
    getAriaLabel(element) {
        if (element instanceof Variable) {
            return localize(9942, "Variable {0}, value {1}", element.name, element.value);
        }
        if (element instanceof ReplOutputElement || element instanceof ReplEvaluationInput || element instanceof ReplEvaluationResult) {
            return element.value + (element instanceof ReplOutputElement && element.count > 1 ? ( localize(9943, ", occurred {0} times", element.count)) : "");
        }
        if (element instanceof RawObjectReplElement) {
            return localize(9944, "Debug console variable {0}, value {1}", element.name, element.value);
        }
        if (element instanceof ReplGroup) {
            return localize(9945, "Debug console group {0}", element.name);
        }
        return "";
    }
}
let SourceWidget = class SourceWidget extends Disposable {
    ...
    setSource(source) {
        ...
        this.hover ??= this._register(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.el, "")
        );
        this.hover.update(
...
SourceWidget = ( __decorate([( __param(1, IEditorService)), ( __param(2, IHoverService)), ( __param(3, ILabelService))], SourceWidget));
