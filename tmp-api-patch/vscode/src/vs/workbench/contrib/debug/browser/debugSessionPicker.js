...
async function showDebugSessionMenu(accessor, selectAndStartID) {
    ...
    quickPick.placeholder = ( localize(9826, "Search debug sessions by name"));
...
function _getPicksAndActiveItem(filter, selectAndStartID, debugService, viewsService, commandService) {
    ...
    const createDebugSessionLabel = ( localize(9827, "Start a New Debug Session"));
    debugConsolePicks.push({
...
function _getSessionInfo(session) {
    ...
    let ariaLabel = "";
    if (parentName) {
        ariaLabel = ( localize(9828, "Session {0} spawned from {1}", label, parentName));
