...
import { IExtensionHostDebugService } from '../../../../platform/debug/common/extensionHostDebug.service.js';
...
import { ResourceContextKey, ActiveEditorContext, PanelFocusContext } from '../../../common/contextkeys.js';
...
import { REPL_VIEW_ID, isFrameDeemphasized, CONTEXT_DISASSEMBLY_VIEW_FOCUS, CONTEXT_JUMP_TO_CURSOR_SUPPORTED, CONTEXT_IN_DEBUG_REPL, CONTEXT_IN_DEBUG_MODE, CONTEXT_DEBUG_STATE, CONTEXT_STEP_INTO_TARGETS_SUPPORTED, CONTEXT_FOCUSED_SESSION_IS_ATTACH, CONTEXT_DEBUGGERS_AVAILABLE, getStateLabel, State, CONTEXT_BREAKPOINTS_FOCUSED, CONTEXT_WATCH_EXPRESSIONS_FOCUSED, CONTEXT_VARIABLES_FOCUSED, CONTEXT_EXPRESSION_SELECTED, DataBreakpointSetType, CONTEXT_BREAKPOINT_INPUT_FOCUSED, EDITOR_CONTRIBUTION_ID, VIEWLET_ID } from '../common/debug.js';
...
import { resolveChildSession, saveAllBeforeDebugStart } from '../common/debugUtils.js';
...
const DEBUG_COMMAND_CATEGORY = ( localize2(9658, "Debug"));
const RESTART_LABEL = ( localize2(9659, "Restart"));
const STEP_OVER_LABEL = ( localize2(9660, "Step Over"));
const STEP_INTO_LABEL = ( localize2(9661, "Step Into"));
const STEP_INTO_TARGET_LABEL = ( localize2(9662, "Step Into Target"));
const STEP_OUT_LABEL = ( localize2(9663, "Step Out"));
const PAUSE_LABEL = ( localize2(9664, "Pause"));
const DISCONNECT_LABEL = ( localize2(9665, "Disconnect"));
const DISCONNECT_AND_SUSPEND_LABEL = ( localize2(9666, "Disconnect and Suspend"));
const STOP_LABEL = ( localize2(9667, "Stop"));
const CONTINUE_LABEL = ( localize2(9668, "Continue"));
const FOCUS_SESSION_LABEL = ( localize2(9669, "Focus Session"));
const SELECT_AND_START_LABEL = ( localize2(9670, "Select and Start Debugging"));
const DEBUG_CONFIGURE_LABEL = ( localize(9671, "Open '{0}'", "launch.json"));
const DEBUG_START_LABEL = ( localize2(9672, "Start Debugging"));
const DEBUG_RUN_LABEL = ( localize2(9673, "Start Without Debugging"));
const NEXT_DEBUG_CONSOLE_LABEL = ( localize2(9674, "Focus Next Debug Console"));
const PREV_DEBUG_CONSOLE_LABEL = ( localize2(9675, "Focus Previous Debug Console"));
const OPEN_LOADED_SCRIPTS_LABEL = ( localize2(9676, "Open Loaded Script..."));
const CALLSTACK_TOP_LABEL = ( localize2(9677, "Navigate to Top of Call Stack"));
const CALLSTACK_BOTTOM_LABEL = ( localize2(9678, "Navigate to Bottom of Call Stack"));
const CALLSTACK_UP_LABEL = ( localize2(9679, "Navigate Up Call Stack"));
const CALLSTACK_DOWN_LABEL = ( localize2(9680, "Navigate Down Call Stack"));
const COPY_EVALUATE_PATH_LABEL = ( localize2(9681, "Copy as Expression"));
const COPY_VALUE_LABEL = ( localize2(9682, "Copy Value"));
const COPY_ADDRESS_LABEL = ( localize2(9683, "Copy Address"));
const ADD_TO_WATCH_LABEL = ( localize2(9684, "Add to Watch"));
const SELECT_DEBUG_CONSOLE_LABEL = ( localize2(9685, "Select Debug Console"));
const SELECT_DEBUG_SESSION_LABEL = ( localize2(9686, "Select Debug Session"));
...
async function changeDebugConsoleFocus(accessor, next) {
    ...
    await debugService.focusStackFrame(undefined, undefined, sessions[nextIndex], {
        explicit: true
    });
...
CommandsRegistry.registerCommand({
    ...
    handler: async accessor => {
                ...
                if (targets && targets.length) {
                    ...
                    if (targets.length > 1) {
                        ...
                        const pick = await quickInputService.pick(picks, {
                            placeHolder: ( localize(9687, "Choose the specific location"))
                        });
                        if (!pick) {
                            return;
                        }
        ...
        return notificationService.warn(( localize(9688, "No executable code is associated at the current cursor position.")));
    }
});
CommandsRegistry.registerCommand({
    id: CALLSTACK_TOP_ID,
...
MenuRegistry.appendMenuItem(MenuId.EditorContext, {
    command: {
        id: JUMP_TO_CURSOR_ID,
        title: ( localize(9689, "Jump to Cursor")),
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    mac: {
        primary: KeyMod$1.Shift | KeyMod$1.CtrlCmd | KeyCode.BracketRight
    },
    handler: async (accessor, _, context) => {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    mac: {
        primary: KeyMod$1.Shift | KeyMod$1.CtrlCmd | KeyCode.BracketLeft
    },
    handler: async (accessor, _, context) => {
    ...
    handler: async (accessor, _, context) => {
        ...
        session.stepInTargets(frame.frameId).then(targets => {
            ...
            if (targets?.length) {
                ...
                qp.placeholder = ( localize(9690, "No step targets available"));
            }
        });
    }
});
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    handler: async (accessor, debugStartOptions) => {
        ...
        await debugService.startDebugging(launch, configOrName, {
            noDebug: debugStartOptions?.noDebug,
            startedByUser: true
        }, false);
    }
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    mac: {
        primary: KeyMod$1.WinCtrl | KeyCode.F5
    },
...
registerAction2(class AddConfigurationAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9691, "Add Configuration...")),
            category: DEBUG_COMMAND_CATEGORY,
            f1: true,
            menu: {
...
MenuRegistry.appendMenuItem(MenuId.EditorContext, {
    command: {
        id: TOGGLE_INLINE_BREAKPOINT_ID,
        title: ( localize(9692, "Add Inline Breakpoint")),
        category: DEBUG_COMMAND_CATEGORY
    },
...
registerAction2(class ToggleExceptionBreakpointsAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9693, "Toggle Exception Breakpoints")),
            category: DEBUG_COMMAND_CATEGORY,
            f1: true,
    ...
    async run(accessor) {
        ...
        quickPick.placeholder = ( localize(9694, "Pick enabled exception breakpoints"));
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9695, "Attach to Current Code Renderer"))
        });
    }
    async run(accessor) {
