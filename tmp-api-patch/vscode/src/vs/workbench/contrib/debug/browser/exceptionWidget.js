...
const $ = $$1;
const debugExceptionWidgetBorder = registerColor("debugExceptionWidget.border", "#a31515", ( localize(9863, "Exception widget border color.")));
const debugExceptionWidgetBackground = registerColor("debugExceptionWidget.background", {
    dark: "#420b0d",
    light: "#f1dfde",
    hcDark: "#420b0d",
    hcLight: "#f1dfde"
}, ( localize(9864, "Exception widget background color.")));
let ExceptionWidget = class ExceptionWidget extends ZoneWidget {
    ...
    _fillContainer(container) {
        ...
        label.textContent = this.exceptionInfo.id ? ( localize(9865, "Exception has occurred: {0}", this.exceptionInfo.id)) : ( localize(9866, "Exception has occurred."));
        let ariaLabel = label.textContent;
        const actionBar = this._disposables.add(( new ActionBar(actions)));
        actionBar.push(( new Action("editor.copyExceptionInfo", ( localize(9867, "Copy")), ThemeIcon.asClassName(Codicon.copy), true, async () => {
        ...
        actionBar.push(( new Action("editor.closeExceptionWidget", ( localize(9868, "Close")), ThemeIcon.asClassName(widgetClose), true, async () => {
    ...
    buildExceptionText() {
        ...
        if (this.exceptionInfo.id) {
            parts.push(( localize(9865, "Exception has occurred: {0}", this.exceptionInfo.id)));
        } else {
            parts.push(( localize(9866, "Exception has occurred.")));
        }
        if (this.exceptionInfo.description) {
...
ExceptionWidget = ( __decorate([( __param(4, IThemeService)), ( __param(5, IInstantiationService)), ( __param(6, IClipboardService))], ExceptionWidget));
