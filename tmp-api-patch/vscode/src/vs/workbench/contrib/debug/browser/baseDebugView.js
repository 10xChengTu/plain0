...
const splitExpressionOrScopeHighlights = (e, highlights) => {
    ...
    for (const hl of highlights) {
        if (hl.start < nameEndsAt) {
            name.push({
                start: hl.start,
                end: Math.min(hl.end, nameEndsAt)
            });
        }
...
AbstractExpressionDataSource = ( __decorate([( __param(0, IDebugService)), ( __param(1, IDebugVisualizerService))], AbstractExpressionDataSource));
let AbstractExpressionsRenderer = class AbstractExpressionsRenderer {
    ...
    renderTemplate(container) {
        ...
        templateDisposable.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), lazyButton, ( localize(9364, "Click to expand")))
        );
        ...
        if (element === selectedExpression?.expression || (element instanceof Variable && element.errorMessage)) {
            ...
            if (options) {
                data.elementDisposable.add(
                    this.renderInputBox(data.name, data.value, data.inputBoxContainer, options)
                );
            }
        }
    }
...
AbstractExpressionsRenderer = ( __decorate([( __param(0, IDebugService)), ( __param(1, IContextViewService)), ( __param(2, IHoverService))], AbstractExpressionsRenderer));

export { AbstractExpressionDataSource, AbstractExpressionsRenderer, expressionAndScopeLabelProvider, renderViewTree, splitExpressionOrScopeHighlights };
