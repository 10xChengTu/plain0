...
import { $ as $$1, addDisposableListener, EventType, getWindow } from '../../../../base/browser/dom.js';
import { StandardMouseEvent } from '../../../../base/browser/mouseEvent.js';
import { toAction, SubmenuAction, Separator } from '../../../../base/common/actions.js';
...
import { TrackedRangeStickiness, GlyphMarginLane, OverviewRulerLane } from '../../../../editor/common/model.js';
...
import { debugBreakpointHint, breakpoint, allBreakpoints, debugBreakpointUnsupported, debugStackframeFocused, debugStackframe } from './debugIcons.js';
import { DebuggerString, CONTEXT_BREAKPOINT_WIDGET_VISIBLE, BreakpointWidgetContext, State, BREAKPOINT_EDITOR_CONTRIBUTION_ID } from '../common/debug.js';
...
const breakpointHelperDecoration = {
    ...
    glyphMarginHoverMessage: ( new MarkdownString()).appendText(( localize(9365, "Click to add a breakpoint"))),
    stickiness: TrackedRangeStickiness.NeverGrowsWhenTypingAtEdges
};
function createBreakpointDecorations(
    accessor,
    model,
    breakpoints,
    state,
...
let BreakpointEditorContribution = class BreakpointEditorContribution {
    ...
    registerListeners() {
        this.toDispose.push(this.editor.onMouseDown(async e => {
            ...
            if (!e.target.position || !model || e.target.type !== MouseTargetType.GUTTER_GLYPH_MARGIN || e.target.detail.isAfterLines ||
            !this.marginFreeFromNonDebugDecorations(e.target.position.lineNumber) && !e.target.element?.className.includes("breakpoint")) {
                return;
            }
            ...
            if (e.event.rightButton || (isMacintosh && e.event.leftButton && e.event.ctrlKey)) {
                ...
                if (breakpoints.length) {
                    ...
                    if (isAltPressed) {
                        ...
                        const breakpointType = logPoint ? ( localize(9366, "Logpoint")) : ( localize(9367, "Breakpoint"));
                        const disabledBreakpointDialogMessage = ( localize(
                            ...
                            logPoint ? ( localize(9369, "message")) : ( localize(9370, "condition"))
                        ));
                        ...
                        await this.dialogService.prompt({
                            ...
                            buttons: [{
                                label: ( localize(9372, "&&Remove {0}", breakpointType)),
                                run: () => breakpoints.forEach(bp => this.debugService.removeBreakpoints(bp.getId()))
                            }, {
                                label: ( localize(
                                    9373,
                                    "{0} {1}",
                                    enabled ? ( localize(9374, "&&Disable")) : ( localize(9375, "&&Enable")),
                                    breakpointType
                                )),
        ...
        this.toDispose.push(
            this.editor.onDidChangeModelDecorations(() => this.onModelDecorationsChanged())
        );
    ...
    getContextMenuActions(breakpoints, uri, lineNumber, column) {
        ...
        if (breakpoints.length === 1) {
            const breakpointType = breakpoints[0].logMessage ? ( localize(9366, "Logpoint")) : ( localize(9367, "Breakpoint"));
            actions.push(toAction({
                id: "debug.removeBreakpoint",
                label: ( localize(9376, "Remove {0}", breakpointType)),
                enabled: true,
                run: async () => {
            ...
            actions.push(toAction({
                ...
                label: ( localize(9377, "Edit {0}...", breakpointType)),
                enabled: true,
                run: () => Promise.resolve(
                    this.showBreakpointWidget(breakpoints[0].lineNumber, breakpoints[0].column)
                )
            ...
            actions.push(toAction({
                ...
                label: breakpoints[0].enabled ? ( localize(9378, "Disable {0}", breakpointType)) : ( localize(9379, "Enable {0}", breakpointType)),
                enabled: true,
            ...
            actions.push(( new SubmenuAction("debug.removeBreakpoints", ( localize(9380, "Remove Breakpoints")), ( sorted.map(bp => toAction({
                id: "removeInlineBreakpoint",
                label: bp.column ? ( localize(9381, "Remove Inline Breakpoint on Column {0}", bp.column)) : ( localize(9382, "Remove Line Breakpoint")),
                enabled: true,
                run: () => this.debugService.removeBreakpoints(bp.getId())
            }))))));
            actions.push(( new SubmenuAction("debug.editBreakpoints", ( localize(9383, "Edit Breakpoints")), ( sorted.map(bp => toAction({
                id: "editBreakpoint",
                label: bp.column ? ( localize(9384, "Edit Inline Breakpoint on Column {0}", bp.column)) : ( localize(9385, "Edit Line Breakpoint")),
                enabled: true,
                run: () => Promise.resolve(this.showBreakpointWidget(bp.lineNumber, bp.column))
            }))))));
            actions.push(( new SubmenuAction("debug.enableDisableBreakpoints", ( localize(9386, "Enable/Disable Breakpoints")), ( sorted.map(bp => toAction({
                id: bp.enabled ? "disableColumnBreakpoint" : "enableColumnBreakpoint",
                label: bp.enabled ? (bp.column ? ( localize(9387, "Disable Inline Breakpoint on Column {0}", bp.column)) : ( localize(9388, "Disable Line Breakpoint"))) : (bp.column ? ( localize(9389, "Enable Inline Breakpoint on Column {0}", bp.column)) : ( localize(9390, "Enable Line Breakpoint"))),
                enabled: true,
            ...
            actions.push(toAction({
                id: "addBreakpoint",
                label: ( localize(9391, "Add Breakpoint")),
                enabled: true,
                run: () => this.debugService.addBreakpoints(uri, [{
                    lineNumber,
                    column
                }])
            }));
            actions.push(toAction({
                ...
                label: ( localize(9392, "Add Conditional Breakpoint...")),
                enabled: true,
                run: () => Promise.resolve(
                    this.showBreakpointWidget(lineNumber, column, BreakpointWidgetContext.CONDITION)
                )
            }));
            actions.push(toAction({
                id: "addLogPoint",
                label: ( localize(9393, "Add Logpoint...")),
                enabled: true,
                run: () => Promise.resolve(
                    this.showBreakpointWidget(lineNumber, column, BreakpointWidgetContext.LOG_MESSAGE)
                )
            }));
            actions.push(toAction({
                id: "addTriggeredBreakpoint",
                label: ( localize(9394, "Add Triggered Breakpoint...")),
                enabled: true,
                run: () => Promise.resolve(
                    this.showBreakpointWidget(lineNumber, column, BreakpointWidgetContext.TRIGGER_POINT)
                )
            }));
        }
        if (this.debugService.state === State.Stopped) {
            ...
            actions.push(toAction({
                id: "runToLine",
                label: ( localize(9395, "Run to Line")),
                enabled: true,
    ...
    async setDecorations() {
        ...
        const desiredBreakpointDecorations = this.instantiationService.invokeFunction(accessor => createBreakpointDecorations(
            accessor,
            model,
            breakpoints,
            this.debugService.state,
        ...
        if (desiredCandidatePositionsRaced === undefined) {
            desiredCandidatePositions.then(
                v => activeCodeEditor.changeDecorations(d => setCandidateDecorations(d, v))
            );
        }
        try {
...
BreakpointEditorContribution = ( __decorate([( __param(1, IDebugService)), ( __param(2, IContextMenuService)), ( __param(3, IInstantiationService)), ( __param(4, IContextKeyService)), ( __param(5, IDialogService)), ( __param(6, IConfigurationService)), ( __param(7, ILabelService))], BreakpointEditorContribution));
...
InlineBreakpointWidget.__decorator = ( __decorate([memoize], InlineBreakpointWidget.prototype, "getId", null));
registerThemingParticipant((theme, collector) => {
    ...
    if (debugIconBreakpointColor) {
        collector.addRule(`${scope} {
            ${( allBreakpoints.map(b => `${ThemeIcon.asCSSSelector(b.regular)}`)).join(",\n\t\t")},
            ${ThemeIcon.asCSSSelector(debugBreakpointUnsupported)},
            ${ThemeIcon.asCSSSelector(debugBreakpointHint)}:not([class*='codicon-debug-breakpoint']):not([class*='codicon-debug-stackframe']),
            ${ThemeIcon.asCSSSelector(breakpoint.regular)}${ThemeIcon.asCSSSelector(debugStackframeFocused)}::after,
            ${ThemeIcon.asCSSSelector(breakpoint.regular)}${ThemeIcon.asCSSSelector(debugStackframe)}::after {
                color: ${debugIconBreakpointColor} !important;
            }
        }`);
        collector.addRule(`${scope} {
            ${ThemeIcon.asCSSSelector(breakpoint.pending)} {
                color: ${debugIconBreakpointColor} !important;
    ...
    if (debugIconBreakpointDisabledColor) {
        collector.addRule(`${scope} {
            ${( allBreakpoints.map(b => ThemeIcon.asCSSSelector(b.disabled))).join(",\n\t\t")} {
    ...
    if (debugIconBreakpointUnverifiedColor) {
        collector.addRule(`${scope} {
            ${( allBreakpoints.map(b => ThemeIcon.asCSSSelector(b.unverified))).join(",\n\t\t")} {
    ...
    if (debugIconBreakpointCurrentStackframeForegroundColor) {
        ...
        ${scope} {
            ${ThemeIcon.asCSSSelector(debugStackframe)} {
                color: ${debugIconBreakpointCurrentStackframeForegroundColor} !important;
            }
        }
        `);
    ...
    if (debugIconBreakpointStackframeFocusedColor) {
        collector.addRule(`${scope} {
            ${ThemeIcon.asCSSSelector(debugStackframeFocused)} {
                color: ${debugIconBreakpointStackframeFocusedColor} !important;
            }
        }`);
    }
});
const debugIconBreakpointForeground = registerColor("debugIcon.breakpointForeground", "#E51400", ( localize(9396, "Icon color for breakpoints.")));
const debugIconBreakpointDisabledForeground = registerColor("debugIcon.breakpointDisabledForeground", "#848484", ( localize(9397, "Icon color for disabled breakpoints.")));
const debugIconBreakpointUnverifiedForeground = registerColor("debugIcon.breakpointUnverifiedForeground", "#848484", ( localize(9398, "Icon color for unverified breakpoints.")));
const debugIconBreakpointCurrentStackframeForeground = registerColor("debugIcon.breakpointCurrentStackframeForeground", {
    dark: "#FFCC00",
    light: "#BE8700",
    hcDark: "#FFCC00",
    hcLight: "#BE8700"
}, ( localize(9399, "Icon color for the current breakpoint stack frame.")));
const debugIconBreakpointStackframeForeground = registerColor("debugIcon.breakpointStackframeForeground", "#89D185", ( localize(9400, "Icon color for all breakpoint stack frames.")));
