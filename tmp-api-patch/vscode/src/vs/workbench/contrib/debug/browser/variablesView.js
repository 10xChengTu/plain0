...
import { CONTEXT_VARIABLES_FOCUSED, CONTEXT_BREAK_WHEN_VALUE_CHANGES_SUPPORTED, CONTEXT_BREAK_WHEN_VALUE_IS_ACCESSED_SUPPORTED, CONTEXT_BREAK_WHEN_VALUE_IS_READ_SUPPORTED, DebugVisualizationType, WATCH_VIEW_ID, VARIABLES_VIEW_ID } from '../common/debug.js';
import { IDebugService } from '../common/debug.service.js';
import { getContextForVariable } from '../common/debugContext.js';
import { Variable, VisualizedExpression, Scope, Expression, StackFrame, ErrorScope, getUriForDebugMemory } from '../common/debugModel.js';
import { IDebugVisualizerService } from '../common/debugVisualizers.service.js';
import { renderViewTree, expressionAndScopeLabelProvider, AbstractExpressionDataSource, AbstractExpressionsRenderer } from './baseDebugView.js';
import { setDataBreakpointInfoResponse, COPY_VALUE_ID, COPY_VALUE_LABEL, COPY_EVALUATE_PATH_ID, COPY_EVALUATE_PATH_LABEL, ADD_TO_WATCH_ID, ADD_TO_WATCH_LABEL } from './debugCommands.js';
...
let VariablesView = class VariablesView extends ViewPane {
    ...
    openContextMenuForScope(e, scope) {
        ...
        const menu = this.menuService.getMenuActions(MenuId.DebugScopesContext, this.contextKeyService, {
            arg: context,
...
VariablesView = ( __decorate([( __param(1, IContextMenuService)), ( __param(2, IDebugService)), ( __param(3, IKeybindingService)), ( __param(4, IConfigurationService)), ( __param(5, IInstantiationService)), ( __param(6, IViewDescriptorService)), ( __param(7, IContextKeyService)), ( __param(8, IOpenerService)), ( __param(9, IThemeService)), ( __param(10, IHoverService)), ( __param(11, IMenuService))], VariablesView));
...
let VisualizedVariableRenderer = class VisualizedVariableRenderer extends AbstractExpressionsRenderer {
    ...
    static rendererOnVisualizationRange(model, tree) {
        return model.onDidChangeVisualization((
            {
                original
            }
        ) => {
    ...
    getInputBoxOptions(expression) {
        ...
        return {
            ...
            ariaLabel: ( localize(9966, "Type new variable value")),
            validationOptions: {
                validation: () => viz.errorMessage ? ({
                    content: viz.errorMessage
                }) : null
            },
    ...
    renderActionBar(actionBar, expression, _data) {
        ...
        if (viz.original) {
            const action = toAction({
                id: "debugViz",
                label: ( localize(9967, "Remove Visualizer")),
                class: ThemeIcon.asClassName(Codicon.eye),
                run: () => this.debugService.getViewModel().setVisualizedExpression(viz.original, undefined)
            });
            action.checked = true;
            primary.push(action);
...
VisualizedVariableRenderer = VisualizedVariableRenderer_1 = ( __decorate([( __param(1, IDebugService)), ( __param(2, IContextViewService)), ( __param(3, IHoverService)), ( __param(4, IMenuService)), ( __param(5, IContextKeyService))], VisualizedVariableRenderer));
let VariablesRenderer = class VariablesRenderer extends AbstractExpressionsRenderer {
    ...
    getInputBoxOptions(expression) {
        ...
        return {
            ...
            ariaLabel: ( localize(9966, "Type new variable value")),
            validationOptions: {
                validation: () => variable.errorMessage ? ({
    ...
    renderActionBar(actionBar, expression, data) {
        ...
        this.visualization.getApplicableFor(expression, cts.token).then(result => {
            ...
            const actions = ( result.object.map(v => toAction({
                id: "debugViz",
                label: v.name,
                class: v.iconClass || "debug-viz-icon",
                run: this.useVisualizer(v, originalExpression, cts.token)
            })));
            if (actions.length === 0)
                ; else if (actions.length === 1) {
                actionBar.push(actions[0], {
                    icon: true,
                    label: false
                });
            } else {
                actionBar.push(toAction({
                    id: "debugViz",
                    label: ( localize(9968, "Visualize Variable...")),
                    class: ThemeIcon.asClassName(Codicon.eye),
                    run: () => this.pickVisualizer(actions, originalExpression, data)
                }), {
                    icon: true,
                    label: false
                });
...
VariablesRenderer = VariablesRenderer_1 = ( __decorate([( __param(1, IMenuService)), ( __param(2, IContextKeyService)), ( __param(3, IDebugVisualizerService)), ( __param(4, IContextMenuService)), ( __param(5, IDebugService)), ( __param(6, IContextViewService)), ( __param(7, IHoverService))], VariablesRenderer));
class VariablesAccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(9969, "Debug Variables");
    }
    getAriaLabel(element) {
        if (element instanceof Scope) {
            return localize(9970, "Scope {0}", element.name);
        }
        if (element instanceof Variable) {
            return localize(9971, "{0}, value {1}", element.name, element.value);
        }
        return null;
    }
}
...
async function tryInstallHexEditor(extensionsWorkbenchService, notificationService) {
    try {
        await extensionsWorkbenchService.install(HEX_EDITOR_EXTENSION_ID, {
            justification: ( localize(9972, "Inspecting binary data requires this extension.")),
            enable: true
...
registerAction2(class extends ViewAction {
    constructor() {
        super({
            ...
            title: ( localize(9973, "Collapse All")),
            f1: false,
...
export { SET_VARIABLE_ID, VIEW_MEMORY_ID, VariablesRenderer, VariablesView, VisualizedVariableRenderer, openContextMenuForVariableTreeElement };
