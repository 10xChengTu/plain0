...
import { createElement, createTextNode, isHTMLElement, reset, addDisposableListener, getWindow } from '../../../../base/browser/dom.js';
...
let LinkDetector = class LinkDetector {
    ...
    _linkify(
        ...
        if (splitLines) {
            ...
            const elements = ( lines.map(line => this._linkify(
                line,
                hoverBehavior,
                false,
                workspaceFolder,
    ...
    applyHighlights(node, startIndex, length, highlights) {
        ...
        for (const highlight of highlights || []) {
            ...
            if (highlight.start > currentIndex) {
                children.push(
                    node.textContent.substring(currentIndex - startIndex, highlight.start - startIndex)
                );
    ...
    createWebLink(fulltext, url, hoverBehavior) {
        ...
        this.decorateLink(link, uri, fulltext, hoverBehavior, async () => {
            if (uri.scheme === Schemas.file) {
                ...
                const fileUrl = normalize(
                    ((path.sep === posix.sep) && isWindows) ? fsPath.replace(/\\/g, posix.sep) : fsPath
                );
                ...
                await this.editorService.openEditor({
                    ...
                    options: {
                        ...
                        selection: lineCol ? {
                            ...
                            startColumn: lineCol[2] ? +lineCol[2] : 1
                        } : undefined
                    }
                });
                return;
    ...
    createPathLink(
        ...
        const options = lineNumber > 0 ? {
            selection: {
                ...
                startColumn: columnNumber > 0 ? columnNumber : 1
            }
        } : {};
        if (path[0] === ".") {
            if (!workspaceFolder) {
    ...
    decorateLink(link, uri, fulltext, hoverBehavior, onClick) {
        ...
        const followLink = uri && this.tunnelService.canTunnel(uri) ? ( localize(9869, "follow link using forwarded port")) : ( localize(9870, "follow link"));
        const title = link.ariaLabel = fulltext ? (isMacintosh ? ( localize(9871, "Cmd + click to {0}\n{1}", followLink, fulltext)) : ( localize(9872, "Ctrl + click to {0}\n{1}", followLink, fulltext))) : (isMacintosh ? ( localize(9873, "Cmd + click to {0}", followLink)) : ( localize(9874, "Ctrl + click to {0}", followLink)));
...
LinkDetector = ( __decorate([( __param(0, IEditorService)), ( __param(1, IFileService)), ( __param(2, IOpenerService)), ( __param(3, IPathService)), ( __param(4, ITunnelService)), ( __param(5, IWorkbenchEnvironmentService)), ( __param(6, IConfigurationService)), ( __param(7, IHoverService))], LinkDetector));
