...
let BreakpointWidget = class BreakpointWidget extends ZoneWidget {
    constructor(
        ...
        this.store.add(this.debugService.getModel().onDidChangeBreakpoints(e => {
            ...
            if (this.context === BreakpointWidgetContext.TRIGGER_POINT && this.selectBreakpointBox) {
    ...
    _fillContainer(container) {
        ...
        const selectBox = this.store.add(( new SelectBox([{
            text: ( localize(9404, "Expression"))
        }, {
            text: ( localize(9405, "Hit Count"))
        }, {
            text: ( localize(9406, "Log Message"))
        }, {
            text: ( localize(9407, "Wait for Breakpoint"))
        }], this.context, this.contextViewService, defaultSelectBoxStyles, {
            ariaLabel: ( localize(9408, "Breakpoint Type")),
        ...
        this.store.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.inputContainer, this.placeholder)
        );
    ...
    createTriggerBreakpointInput(container) {
        ...
        const selectBreakpointBox = this.selectBreakpointBox = this.store.add(( new SelectBox(
            ...
            {
                ariaLabel: ( localize(9410, "Select breakpoint")),
                useCustomDrawn: !hasNativeContextMenu(this._configurationService)
            }
        )));
        ...
        this.store.add(
            addDisposableListener(this.selectBreakpointContainer, EventType.KEY_DOWN, e => {
    ...
    relative: true
        ...
        for (const [i, bp] of this.availableBreakpoints.entries()) {
            this.textModelService.createModelReference(bp.uri).then(ref => {
                ...
                breakpointOptions[i + 1].description = ( localize(9414, "Could not load source."));
            });
        }
    ...
    createBreakpointInput(container) {
        ...
        this.store.add(this.languageFeaturesService.completionProvider.register({
            scheme: DEBUG_SCHEME,
            ...
            provideCompletionItems: (model, position, _context, token) => {
                ...
                if (underlyingModel && (this.context === BreakpointWidgetContext.CONDITION || (this.context === BreakpointWidgetContext.LOG_MESSAGE && isPositionInCurlyBracketBlock(this.input)))) {
                    suggestionsPromise = provideSuggestionItems(
                        ...
                        ( new CompletionOptions(undefined, ( new Set()).add(CompletionItemKind.Snippet))),
                        _context,
                        token
                    ).then(suggestions => {
...
BreakpointWidget = ( __decorate([( __param(4, IContextViewService)), ( __param(5, IDebugService)), ( __param(6, IThemeService)), ( __param(7, IInstantiationService)), ( __param(8, IModelService)), ( __param(9, ICodeEditorService)), ( __param(10, IConfigurationService)), ( __param(11, ILanguageFeaturesService)), ( __param(12, IKeybindingService)), ( __param(13, ILabelService)), ( __param(14, ITextModelService)), ( __param(15, IHoverService))], BreakpointWidget));
