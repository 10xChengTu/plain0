...
import { $ as $$1, isMouseEvent, isEditableElement, append, hide, show, addStandardDisposableListener, addDisposableListener } from '../../../../base/browser/dom.js';
...
let BreakpointsView = class BreakpointsView extends ViewPane {
    ...
    renderBody(container) {
        ...
        this._register(this.tree.onMouseMiddleClick(async (
            {
                element
            }
        ) => {
        ...
        this._register(this.tree.onDidOpen(async e => {
            ...
            if (element instanceof InstructionBreakpoint) {
                ...
                disassemblyView.goToInstructionAndOffset(
                    ...
                    element.offset,
                    isMouseEvent(e.browserEvent) && e.browserEvent.detail === 2
                );
            }
        ...
        this._register(this.tree.onKeyDown(e => {
            ...
            if (event.equals(KeyCode.Space) && !isEditableElement(e.target)) {
        ...
        this._register(this.configurationService.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration("debug.breakpointsView.presentation")) {
                ...
                this.tree.updateOptions({
                    compressionEnabled: presentation === "tree"
                });
    ...
    renderHeaderTitle(container, title) {
        ...
        this.hintContainer = this._register(( new IconLabel(iconLabelContainer, {
            supportIcons: true,
            hoverDelegate: {
                showHover: (options, focus) => this.hoverService.showInstantHover({
    ...
    onTreeContextMenu(e) {
        ...
        if (element instanceof BreakpointsFolderItem) {
            ...
            const {
                secondary
            } = getContextMenuActions(this.menu.getActions({
                arg: element,
        ...
        const type = element instanceof Breakpoint ? "breakpoint" : element instanceof ExceptionBreakpoint ? "exceptionBreakpoint" : element instanceof FunctionBreakpoint ? "functionBreakpoint" : element instanceof DataBreakpoint ? "dataBreakpoint" : element instanceof InstructionBreakpoint ? "instructionBreakpoint" : undefined;
        ...
        this.breakpointHasMultipleModes.set(
            this.debugService.getModel().getBreakpointModes(getModeKindForBreakpoint(element)).length > 1
        );
        const {
            secondary
        } = getContextMenuActions(this.menu.getActions({
...
BreakpointsView = ( __decorate([( __param(1, IContextMenuService)), ( __param(2, IDebugService)), ( __param(3, IKeybindingService)), ( __param(4, IInstantiationService)), ( __param(5, IThemeService)), ( __param(6, IEditorService)), ( __param(7, IContextViewService)), ( __param(8, IConfigurationService)), ( __param(9, IViewDescriptorService)), ( __param(10, IContextKeyService)), ( __param(11, IOpenerService)), ( __param(12, ILabelService)), ( __param(13, IMenuService)), ( __param(14, IHoverService)), ( __param(15, ILanguageService))], BreakpointsView));
...
let BreakpointsFolderRenderer = class BreakpointsFolderRenderer {
    static {
        BreakpointsFolderRenderer_1 = this;
    }
    static {
        this.ID = "breakpointFolder";
    }
    ...
    renderTemplate(container) {
        ...
        append(data.container, data.checkbox.domNode);
    ...
    renderElement(node, _index, data) {
        ...
        const fullPath = this.labelService.getUriLabel(folderItem.uri, {
            relative: true
        });
        data.elementDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.container, fullPath)
        );
        ...
        const removeAction = data.elementDisposables.add(( new Action("debug.removeBreakpointsInFile", ( localize(9415, "Remove Breakpoints in File")), ThemeIcon.asClassName(Codicon.close), true, async () => {
            for (const bp of folderItem.breakpoints) {
                await this.debugService.removeBreakpoints(bp.getId());
            }
        })));
        data.actionBar.push(removeAction, {
            icon: true,
            label: false
        });
    }
...
BreakpointsFolderRenderer = BreakpointsFolderRenderer_1 = ( __decorate([( __param(0, IDebugService)), ( __param(1, ILabelService)), ( __param(2, IHoverService))], BreakpointsFolderRenderer));
let BreakpointsRenderer = class BreakpointsRenderer {
    ...
    renderTemplate(container) {
        ...
        append(data.breakpoint, data.checkbox.domNode);
    ...
    renderBreakpointCommon(breakpoint, data) {
        ...
        data.elementDisposables.add(this.hoverService.setupManagedHover(
        ...
        const {
            primary
        } = getActionBarActions(this.menu.getActions({
            arg: breakpoint,
    ...
    renderBreakpointFileLabel(breakpoint, data) {
        ...
        data.filePath.textContent = this.labelService.getUriLabel(dirname(breakpoint.uri), {
            relative: true
        });
    }
    renderBreakpointLineLabel(breakpoint, data) {
        data.name.textContent = ( localize(9416, "Loading..."));
        ...
        this.textModelService.createModelReference(breakpoint.uri).then(reference => {
            ...
            if (model && breakpoint.lineNumber <= model.getLineCount()) {
                ...
                data.name.textContent = lineContent || ( localize(9417, "(empty line)"));
            } else {
                data.name.textContent = ( localize(9418, "(line not found)"));
            }
        }).catch(() => {
            if (data.context === breakpoint) {
                data.name.textContent = ( localize(9419, "(cannot load line)"));
            }
        });
    }
...
BreakpointsRenderer = BreakpointsRenderer_1 = ( __decorate([( __param(4, IDebugService)), ( __param(5, IHoverService)), ( __param(6, ILabelService)), ( __param(7, ITextModelService))], BreakpointsRenderer));
class ExceptionBreakpointsRenderer {
    ...
    renderExceptionBreakpoint(exceptionBreakpoint, data) {
        ...
        const exceptionBreakpointtitle = exceptionBreakpoint.verified ? (exceptionBreakpoint.description || data.name.textContent) : exceptionBreakpoint.message || ( localize(9420, "Unverified Exception Breakpoint"));
        data.elementDisposables.add(this.hoverService.setupManagedHover(
            getDefaultHoverDelegate("mouse"),
            data.breakpoint,
        ...
        data.elementDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.condition, ( localize(9421, "Expression condition: {0}", exceptionBreakpoint.condition)))
        );
...
let FunctionBreakpointsRenderer = class FunctionBreakpointsRenderer {
    ...
    renderFunctionBreakpoint(functionBreakpoint, data) {
        ...
        data.icon.className = ThemeIcon.asClassName(icon);
        data.elementDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.icon, message ? message : "")
        );
        data.checkbox.checked = functionBreakpoint.enabled;
        data.elementDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.breakpoint, message ? message : "")
        );
        ...
        if (session && !session.capabilities.supportsFunctionBreakpoints) {
            data.elementDisposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.breakpoint, ( localize(9423, "Function breakpoints are not supported by this debug type")))
            );
        }
        this.breakpointSupportsCondition.set(!session || !!session.capabilities.supportsConditionalBreakpoints);
...
FunctionBreakpointsRenderer = FunctionBreakpointsRenderer_1 = ( __decorate([( __param(3, IDebugService)), ( __param(4, IHoverService)), ( __param(5, ILabelService))], FunctionBreakpointsRenderer));
let DataBreakpointsRenderer = class DataBreakpointsRenderer {
    ...
    renderDataBreakpoint(dataBreakpoint, data) {
        ...
        if (session && !session.capabilities.supportsDataBreakpoints) {
            data.elementDisposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.breakpoint, ( localize(9424, "Data breakpoints are not supported by this debug type")))
            );
        }
        if (dataBreakpoint.accessType) {
            const accessType = dataBreakpoint.accessType === "read" ? ( localize(9425, "Read")) : dataBreakpoint.accessType === "write" ? ( localize(9426, "Write")) : ( localize(9427, "Access"));
...
DataBreakpointsRenderer = DataBreakpointsRenderer_1 = ( __decorate([( __param(5, IDebugService)), ( __param(6, IHoverService)), ( __param(7, ILabelService))], DataBreakpointsRenderer));
let InstructionBreakpointsRenderer = class InstructionBreakpointsRenderer {
    ...
    renderInstructionBreakpoint(breakpoint, data) {
        ...
        data.elementDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.name, ( localize(9428, "Decimal Address: {0}", (breakpoint.address.toString()))))
        );
        data.checkbox.checked = breakpoint.enabled;
        const {
            message,
...
InstructionBreakpointsRenderer = InstructionBreakpointsRenderer_1 = ( __decorate([( __param(0, IDebugService)), ( __param(1, IHoverService)), ( __param(2, ILabelService))], InstructionBreakpointsRenderer));
class FunctionBreakpointInputRenderer {
    ...
    renderTemplate(container) {
        ...
        append(breakpoint, template.checkbox.domNode);
    ...
    renderElement(node, _index, data) {
        ...
        let placeholder = ( localize(9429, "Function to break on"));
        let ariaLabel = ( localize(9430, "Type function breakpoint."));
        if (data.type === "condition") {
            data.inputBox.value = functionBreakpoint.condition || "";
            placeholder = ( localize(9431, "Break when expression evaluates to true"));
            ariaLabel = ( localize(
                9432,
            ...
            placeholder = ( localize(9433, "Break when hit count is met"));
            ariaLabel = ( localize(
                9434,
...
class DataBreakpointInputRenderer {
    ...
    renderElement(node, _index, data) {
        ...
        data.icon.className = ThemeIcon.asClassName(icon);
        data.elementDisposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), data.icon, message ?? "")
        );
        data.checkbox.checked = dataBreakpoint.enabled;
        data.checkbox.disable();
        data.inputBox.value = "";
        ...
        if (data.type === "condition") {
            ...
            placeholder = ( localize(9435, "Break when expression evaluates to true"));
            ariaLabel = ( localize(
                9436,
            ...
            placeholder = ( localize(9437, "Break when hit count is met"));
            ariaLabel = ( localize(9438, "Type hit count. Data breakpoint will break when hit count is met."));
        }
        data.inputBox.setAriaLabel(ariaLabel);
        data.inputBox.setPlaceHolder(placeholder);
        setTimeout(() => {
...
class ExceptionBreakpointInputRenderer {
    ...
    renderTemplate(container) {
        ...
        append(breakpoint, checkbox.domNode);
        this.view.breakpointInputFocused.set(true);
        const inputBoxContainer = append(breakpoint, $(".inputBoxContainer"));
        const inputBox = ( new InputBox(inputBoxContainer, this.contextViewService, {
            ariaLabel: ( localize(9439, "Type exception breakpoint condition")),
            inputBoxStyles: defaultInputBoxStyles
        }));
        toDispose.add(inputBox);
        const wrapUp = success => {
    ...
    renderElement(node, _index, data) {
        const exceptionBreakpoint = node.element;
        const placeHolder = exceptionBreakpoint.conditionDescription || ( localize(9440, "Break when expression evaluates to true"));
        data.inputBox.setPlaceHolder(placeHolder);
...
class BreakpointsAccessibilityProvider {
    ...
    getWidgetAriaLabel() {
        return localize(9441, "Breakpoints");
    }
    getRole() {
        return "checkbox";
    }
...
function getBreakpointMessageAndIcon(state, breakpointsActivated, breakpoint$1, labelService, debugModel) {
    ...
    if (!breakpoint$1.enabled || !breakpointsActivated) {
        return {
            ...
            message: breakpoint$1.logMessage ? ( localize(9443, "Disabled Logpoint")) : ( localize(9444, "Disabled Breakpoint"))
        };
    }
    ...
    if (debugActive && !breakpoint$1.verified) {
        return {
            ...
            message: breakpoint$1.message ? breakpoint$1.message : (breakpoint$1.logMessage ? ( localize(9445, "Unverified Logpoint")) : ( localize(9446, "Unverified Breakpoint"))),
    ...
    if (breakpoint$1 instanceof DataBreakpoint) {
        if (!breakpoint$1.supported) {
            return {
                icon: breakpointIcon.unverified,
                message: ( localize(9447, "Data breakpoints not supported by this debug type"))
            };
        }
        return {
            icon: breakpointIcon.regular,
            message: breakpoint$1.message || ( localize(9448, "Data Breakpoint"))
        };
    }
    if (breakpoint$1 instanceof FunctionBreakpoint) {
        if (!breakpoint$1.supported) {
            return {
                icon: breakpointIcon.unverified,
                message: ( localize(9449, "Function breakpoints not supported by this debug type"))
            };
        }
        const messages = [];
        messages.push(breakpoint$1.message || ( localize(9450, "Function Breakpoint")));
        if (breakpoint$1.condition) {
            messages.push(( localize(9451, "Condition: {0}", breakpoint$1.condition)));
        }
        if (breakpoint$1.hitCondition) {
            messages.push(( localize(9452, "Hit Count: {0}", breakpoint$1.hitCondition)));
        }
        return {
            icon: breakpointIcon.regular,
    ...
    if (breakpoint$1 instanceof InstructionBreakpoint) {
        if (!breakpoint$1.supported) {
            return {
                icon: breakpointIcon.unverified,
                message: ( localize(9453, "Instruction breakpoints not supported by this debug type"))
            };
        }
        const messages = [];
        if (breakpoint$1.message) {
            ...
            messages.push(( localize(9455, "Instruction breakpoint")));
        }
        if (breakpoint$1.hitCondition) {
            messages.push(( localize(9452, "Hit Count: {0}", breakpoint$1.hitCondition)));
        }
    ...
    if (breakpoint$1.logMessage || breakpoint$1.condition || breakpoint$1.hitCondition || triggeringBreakpoint) {
        ...
        if (!breakpoint$1.supported) {
            ...
            messages.push(( localize(9456, "Breakpoints of this type are not supported by the debugger")));
        }
        if (breakpoint$1.logMessage) {
            messages.push(( localize(9457, "Log Message: {0}", breakpoint$1.logMessage)));
        }
        if (breakpoint$1.condition) {
            messages.push(( localize(9451, "Condition: {0}", breakpoint$1.condition)));
        }
        ...
        if (triggeringBreakpoint) {
            messages.push(( localize(
                ...
                `${labelService.getUriLabel(triggeringBreakpoint.uri, {
    relative: true
})}: ${triggeringBreakpoint.lineNumber}`
            )));
        }
        return {
            icon,
    ...
    const message = breakpoint$1.message ? breakpoint$1.message : breakpoint$1 instanceof Breakpoint && labelService ? labelService.getUriLabel(breakpoint$1.uri) : ( localize(9459, "Breakpoint"));
    return {
        icon: breakpointIcon.regular,
        message
    };
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(9460, "Add Function Breakpoint")),
                mnemonicTitle: ( localize(9461, "&&Function Breakpoint..."))
            },
            f1: true,
...
class MemoryBreakpointAction extends Action2 {
    async run(accessor, existingBreakpoint) {
        ...
        if (info.accessTypes && info.accessTypes?.length > 1) {
            ...
            const selectedAccessType = await quickInput.pick(accessTypes, {
                placeHolder: ( localize(9463, "Select the access type to monitor"))
            });
    ...
    getRange(quickInput, defaultValue) {
        return (new Promise(resolve => {
            ...
            input.prompt = ( localize(9464, "Enter a memory range in which to break"));
    ...
    parseAddress(range, isFinal) {
        ...
        if (sign === "-") {
            if (start > end) {
                return {
                    error: ( localize(9468, "End ({1}) should be greater than Start ({0})", startStr, endStr))
                };
            }
            return {
                address,
                bytes: Number(end - start)
            };
        }
        return {
            address,
...
registerAction2(class extends MemoryBreakpointAction {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(9469, "Add Data Breakpoint at Address")),
                mnemonicTitle: ( localize(9470, "&&Data Breakpoint..."))
            },
            f1: true,
...
registerAction2(class extends MemoryBreakpointAction {
    constructor() {
        super({
            ...
            title: ( localize2(9471, "Edit Address...")),
            menu: [{
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9472, "Toggle Activate Breakpoints")),
            f1: true,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize(9473, "Remove Breakpoint")),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(9474, "Remove All Breakpoints")),
                mnemonicTitle: ( localize(9475, "Remove &&All Breakpoints"))
            },
            f1: true,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(9476, "Enable All Breakpoints")),
                mnemonicTitle: ( localize(9477, "&&Enable All Breakpoints"))
            },
            f1: true,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(9478, "Disable All Breakpoints")),
                mnemonicTitle: ( localize(9479, "Disable A&&ll Breakpoints"))
            },
            f1: true,
            precondition: CONTEXT_DEBUGGERS_AVAILABLE,
            menu: [{
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9480, "Reapply All Breakpoints")),
            f1: true,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9481, "Toggle Breakpoints View Presentation")),
            f1: true,
...
registerAction2(class extends ViewAction {
    constructor() {
        super({
            ...
            title: ( localize(9482, "Edit Condition...")),
            icon: Codicon.edit,
    ...
    async runInView(accessor, view, breakpoint) {
        ...
        if (breakpoint instanceof Breakpoint) {
            ...
            const actions = [( new Action("breakpoint.editCondition", ( localize(9482, "Edit Condition...")), undefined, true, async () => view.renderInputBox({
                breakpoint,
                type: "condition"
            }))), ( new Action("breakpoint.editCondition", ( localize(9483, "Edit Hit Count...")), undefined, true, async () => view.renderInputBox({
                breakpoint,
                type: "hitCount"
            })))];
...
registerAction2(class extends ViewAction {
    constructor() {
        super({
            ...
            viewId: BREAKPOINTS_VIEW_ID,
            title: ( localize(9484, "Edit Function Condition...")),
            menu: [{
                id: MenuId.DebugBreakpointsContext,
                group: "navigation",
                order: 10,
...
registerAction2(class extends ViewAction {
    constructor() {
        super({
            ...
            viewId: BREAKPOINTS_VIEW_ID,
            title: ( localize(9483, "Edit Hit Count...")),
            precondition: CONTEXT_BREAKPOINT_SUPPORTS_CONDITION,
            menu: [{
                id: MenuId.DebugBreakpointsContext,
                group: "navigation",
...
registerAction2(class extends ViewAction {
    ...
    async runInView(accessor, view, breakpoint) {
        ...
        const picked = await accessor.get(IQuickInputService).pick(( modes.map(mode => ({
            ...
            placeHolder: ( localize(9486, "Select Breakpoint Mode"))
        });
        if (!picked) {
            return;
        }
