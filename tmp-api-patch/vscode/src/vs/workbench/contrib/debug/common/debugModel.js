...
import { DebugTreeItemCollapsibleState, isFrameDeemphasized, DEBUG_MEMORY_SCHEME, MemoryRangeType, DataBreakpointSetType, State } from './debug.js';
...
class ExpressionContainer {
    ...
    constructor(
        ...
        valueLocationReference = undefined
    ) {
        ...
        try {
            ...
            const vars = ( response.body.variables.filter(v => !!v).map(v => {
                ...
                return (new Variable(
                    ...
                    "",
                    undefined,
                    localize(10063, "Invalid variable attributes"),
                    0,
                    0,
                    undefined,
                    {
    ...
    async evaluateExpression(expression, session, stackFrame, context, keepLazyVars = false, location) {
        if (!session || (!stackFrame && context !== "repl")) {
            this.value = context === "repl" ? ( localize(10064, "Please start a debug session to evaluate expressions")) : Expression.DEFAULT_VALUE;
            this.reference = 0;
            return false;
        }
        this.session = session;
...
class Expression extends ExpressionContainer {
    static {
        this.DEFAULT_VALUE = ( localize(10065, "not available"));
    }
...
class Variable extends ExpressionContainer {
    constructor(
        ...
        presentationHint,
        type = undefined,
        variableMenuContext = undefined,
        available = true,
        startOfVariables = 0,
        idDuplicationIndex = "",
        declarationLocationReference = undefined,
        valueLocationReference = undefined
    ) {
        super(
            session,
...
class StackFrame {
    ...
    async getMostSpecificScopes(range) {
        ...
        const scopesContainingRange = nonExpensiveScopes.filter(scope => scope.range && Range.containsRange(scope.range, range)).sort(
...
class Thread {
    ...
    getTopStackFrame() {
        ...
        const firstAvailableStackFrame = callStack.find(
            sf => !!(((stopReason === "instruction breakpoint" || (stopReason === "step" && this.lastSteppingGranularity === "instruction")) && sf.instructionPointerReference) || (sf.source && sf.source.available && (KEEP_SUBTLE_FRAME_AT_TOP_REASONS.includes(stopReason) || !isFrameDeemphasized(sf))))
        );
        return firstAvailableStackFrame;
    }
    get stateLabel() {
        if (this.stoppedDetails) {
            return this.stoppedDetails.description || (this.stoppedDetails.reason ? ( localize(10066, "Paused on {0}", this.stoppedDetails.reason)) : ( localize(10067, "Paused")));
        }
        return localize(10068, "Running");
    }
...
DebugModel = ( __decorate([( __param(1, ITextFileService)), ( __param(2, IUriIdentityService)), ( __param(3, ILogService))], DebugModel));
