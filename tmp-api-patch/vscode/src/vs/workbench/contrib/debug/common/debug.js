...
const CONTEXT_DEBUG_TYPE = ( new RawContextKey("debugType", undefined, {
    type: "string",
    description: ( localize(9992, "Debug type of the active debug session. For example 'python'."))
}));
...
const CONTEXT_IN_DEBUG_MODE = ( new RawContextKey("inDebugMode", false, {
    type: "boolean",
    description: ( localize(9997, "True when debugging, false otherwise."))
}));
const CONTEXT_IN_DEBUG_REPL = ( new RawContextKey("inDebugRepl", false, {
    type: "boolean",
    description: ( localize(9998, "True when focus is in the debug console, false otherwise."))
}));
...
const CONTEXT_BREAKPOINTS_FOCUSED = ( new RawContextKey("breakpointsFocused", true, {
    type: "boolean",
    description: ( localize(10001, "True when the BREAKPOINTS view is focused, false otherwise."))
}));
const CONTEXT_WATCH_EXPRESSIONS_FOCUSED = ( new RawContextKey("watchExpressionsFocused", true, {
    type: "boolean",
    description: ( localize(10002, "True when the WATCH view is focused, false otherwise."))
}));
const CONTEXT_WATCH_EXPRESSIONS_EXIST = ( new RawContextKey("watchExpressionsExist", false, {
    type: "boolean",
    description: ( localize(10003, "True when at least one watch expression exists, false otherwise."))
}));
const CONTEXT_VARIABLES_FOCUSED = ( new RawContextKey("variablesFocused", true, {
    type: "boolean",
    description: ( localize(10004, "True when the VARIABLES views is focused, false otherwise"))
}));
...
const CONTEXT_BREAKPOINT_INPUT_FOCUSED = ( new RawContextKey("breakpointInputFocused", false, {
    type: "boolean",
    description: ( localize(10006, "True when the input box has focus in the BREAKPOINTS view."))
}));
...
const CONTEXT_CALLSTACK_FOCUSED = ( new RawContextKey("callStackFocused", true, {
    type: "boolean",
    description: ( localize(10011, "True when the CALLSTACK view is focused, false otherwise."))
}));
...
const CONTEXT_BREAKPOINT_ITEM_IS_DATA_BYTES = ( new RawContextKey("breakpointItemBytes", undefined, {
    type: "boolean",
    description: ( localize(10015, "Whether the breakpoint item is a data breakpoint on a byte range."))
}));
const CONTEXT_BREAKPOINT_HAS_MODES = ( new RawContextKey("breakpointHasModes", false, {
    type: "boolean",
    description: ( localize(10016, "Whether the breakpoint has multiple modes it can switch to."))
}));
const CONTEXT_BREAKPOINT_SUPPORTS_CONDITION = ( new RawContextKey("breakpointSupportsCondition", false, {
    type: "boolean",
    description: ( localize(10017, "True when the focused breakpoint supports conditions."))
}));
const CONTEXT_LOADED_SCRIPTS_SUPPORTED = ( new RawContextKey("loadedScriptsSupported", false, {
    type: "boolean",
    description: ( localize(10018, "True when the focused sessions supports the LOADED SCRIPTS view"))
}));
...
const CONTEXT_FOCUSED_SESSION_IS_ATTACH = ( new RawContextKey("focusedSessionIsAttach", false, {
    type: "boolean",
    description: ( localize(10020, "True when the focused session is 'attach'."))
}));
const CONTEXT_FOCUSED_SESSION_IS_NO_DEBUG = ( new RawContextKey("focusedSessionIsNoDebug", false, {
    type: "boolean",
    description: ( localize(10021, "True when the focused session is run without debugging."))
}));
const CONTEXT_STEP_BACK_SUPPORTED = ( new RawContextKey("stepBackSupported", false, {
    type: "boolean",
    description: ( localize(10022, "True when the focused session supports 'stepBack' requests."))
}));
const CONTEXT_RESTART_FRAME_SUPPORTED = ( new RawContextKey("restartFrameSupported", false, {
    type: "boolean",
    description: ( localize(10023, "True when the focused session supports 'restartFrame' requests."))
}));
const CONTEXT_STACK_FRAME_SUPPORTS_RESTART = ( new RawContextKey("stackFrameSupportsRestart", false, {
    type: "boolean",
    description: ( localize(10024, "True when the focused stack frame supports 'restartFrame'."))
}));
const CONTEXT_JUMP_TO_CURSOR_SUPPORTED = ( new RawContextKey("jumpToCursorSupported", false, {
    type: "boolean",
    description: ( localize(10025, "True when the focused session supports 'jumpToCursor' request."))
}));
const CONTEXT_STEP_INTO_TARGETS_SUPPORTED = ( new RawContextKey("stepIntoTargetsSupported", false, {
    type: "boolean",
    description: ( localize(10026, "True when the focused session supports 'stepIntoTargets' request."))
}));
const CONTEXT_BREAKPOINTS_EXIST = ( new RawContextKey("breakpointsExist", false, {
    type: "boolean",
    description: ( localize(10027, "True when at least one breakpoint exists."))
}));
const CONTEXT_DEBUGGERS_AVAILABLE = ( new RawContextKey("debuggersAvailable", false, {
    type: "boolean",
    description: ( localize(10028, "True when there is at least one debug extensions active."))
}));
...
const CONTEXT_SET_VARIABLE_SUPPORTED = ( new RawContextKey("debugSetVariableSupported", false, {
    type: "boolean",
    description: ( localize(10031, "True when the focused session supports 'setVariable' request."))
}));
...
const CONTEXT_SET_EXPRESSION_SUPPORTED = ( new RawContextKey("debugSetExpressionSupported", false, {
    type: "boolean",
    description: ( localize(10033, "True when the focused session supports 'setExpression' request."))
}));
...
const CONTEXT_TERMINATE_THREADS_SUPPORTED = ( new RawContextKey("terminateThreadsSupported", false, {
    type: "boolean",
    description: ( localize(
        10039,
...
const CONTEXT_VARIABLE_EVALUATE_NAME_PRESENT = ( new RawContextKey("variableEvaluateNamePresent", false, {
    type: "boolean",
    description: ( localize(10040, "True when the focused variable has an 'evaluateName' field set."))
}));
const CONTEXT_VARIABLE_IS_READONLY = ( new RawContextKey("variableIsReadonly", false, {
    type: "boolean",
    description: ( localize(10041, "True when the focused variable is read-only."))
}));
const CONTEXT_VARIABLE_VALUE = ( new RawContextKey("variableValue", false, {
    type: "string",
    description: ( localize(10042, "Value of the variable, present for debug visualization clauses."))
}));
const CONTEXT_VARIABLE_TYPE = ( new RawContextKey("variableType", false, {
    type: "string",
    description: ( localize(10043, "Type of the variable, present for debug visualization clauses."))
}));
...
const CONTEXT_VARIABLE_NAME = ( new RawContextKey("variableName", false, {
    type: "string",
    description: ( localize(10045, "Name of the variable, present for debug visualization clauses."))
}));
...
const CONTEXT_EXCEPTION_WIDGET_VISIBLE = ( new RawContextKey("exceptionWidgetVisible", false, {
    type: "boolean",
    description: ( localize(10048, "True when the exception widget is visible."))
}));
const CONTEXT_MULTI_SESSION_REPL = ( new RawContextKey("multiSessionRepl", false, {
    type: "boolean",
    description: ( localize(10049, "True when there is more than 1 debug console."))
}));
const CONTEXT_MULTI_SESSION_DEBUG = ( new RawContextKey("multiSessionDebug", false, {
    type: "boolean",
    description: ( localize(10050, "True when there is more than 1 active debug session."))
}));
const CONTEXT_DISASSEMBLE_REQUEST_SUPPORTED = ( new RawContextKey("disassembleRequestSupported", false, {
    type: "boolean",
    description: ( localize(10051, "True when the focused sessions supports disassemble request."))
}));
const CONTEXT_DISASSEMBLY_VIEW_FOCUS = ( new RawContextKey("disassemblyViewFocus", false, {
    type: "boolean",
    description: ( localize(10052, "True when the Disassembly View is focused."))
}));
...
const INTERNAL_CONSOLE_OPTIONS_SCHEMA = {
    ...
    description: ( localize(10056, "Controls when the internal Debug Console should open."))
};
var State;
(function(State) {
