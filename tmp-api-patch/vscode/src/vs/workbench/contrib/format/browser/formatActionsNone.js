...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(11283, "Format Document")),
    ...
    async run(accessor, editor) {
        ...
        if (formatterCount > 1) {
            ...
            notificationService.warn(( localize(11284, "This file cannot be formatted because it is too large")));
        } else {
            const langName = model.getLanguageId();
            const message = ( localize(11285, "There is no formatter for '{0}' files installed.", langName));
            const {
                confirmed
            } = await dialogService.confirm({
                message,
                primaryButton: ( localize(11286, "&&Install Formatter..."))
            });
            if (confirmed) {
