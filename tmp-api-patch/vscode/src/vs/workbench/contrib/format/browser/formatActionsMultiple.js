...
import { FormattingConflicts, FormattingKind, FormattingMode, getRealAndSyntheticDocumentFormattersOrdered, formatDocumentWithProvider, formatDocumentRangesWithProvider } from '../../../../editor/contrib/format/browser/format.js';
...
let DefaultFormatter = class DefaultFormatter extends Disposable {
    ...
    constructor(
        ...
        this._store.add(
            this._extensionService.onDidChangeExtensions(this._updateConfigValues, this)
        );
        this._store.add(FormattingConflicts.setFormatterSelector(
        ...
        this._store.add(
            _languageFeaturesService.documentFormattingEditProvider.onDidChange(this._updateStatus, this)
        );
        this._store.add(
            _languageFeaturesService.documentRangeFormattingEditProvider.onDidChange(this._updateStatus, this)
        );
        this._store.add(
            _languageFeaturesService.documentFormattingEditProvider.onDidChange(this._updateConfigValues, this)
        );
        this._store.add(
            _languageFeaturesService.documentRangeFormattingEditProvider.onDidChange(this._updateConfigValues, this)
        );
        this._store.add(_configService.onDidChangeConfiguration(
            e => e.affectsConfiguration(DefaultFormatter_1.configName) && this._updateStatus()
        ));
    ...
    async _updateConfigValues() {
        ...
        DefaultFormatter_1.extensionItemLabels.push(( localize(11264, "None")));
        DefaultFormatter_1.extensionDescriptions.push(( localize(11265, "None")));
    ...
    async _analyzeFormatter(kind, formatter, document) {
        ...
        if (defaultFormatterId) {
            const defaultFormatter = formatter.find(
                formatter => ExtensionIdentifier.equals(formatter.extensionId, defaultFormatterId)
            );
            if (defaultFormatter) {
            ...
            if (extension && this._extensionEnablementService.isEnabled(toExtension(extension))) {
                ...
                const detail = kind === FormattingKind.File ? ( localize(
                    11266,
        ...
        const langName = this._languageService.getLanguageName(document.getLanguageId()) || document.getLanguageId();
        const message = !defaultFormatterId ? ( localize(
            11268,
    ...
    async _selectFormatter(formatter, document, mode, kind) {
        ...
        if (mode !== FormattingMode.Silent) {
            const {
                ...
                message: ( localize(11270, "Configure Default Formatter")),
                detail: formatterOrMessage,
                primaryButton: ( localize(11271, "&&Configure..."))
            });
            if (confirmed) {
                return this._pickAndPersistDefaultFormatter(formatter, document);
            }
        } else {
            this._notificationService.prompt(Severity$1.Info, formatterOrMessage, [{
                label: ( localize(11272, "Configure...")),
    ...
    _updateStatus() {
        ...
        this._analyzeFormatter(FormattingKind.File, formatter, document).then(result => {
            ...
            const command = {
                ...
                title: ( localize(11274, "Configure..."))
            };
            this._languageStatusStore.add(CommandsRegistry.registerCommand(
                command.id,
                () => this._pickAndPersistDefaultFormatter(formatter, document)
            ));
            this._languageStatusStore.add(this._languageStatusService.addStatus({
                id: "formatter.conflict",
                name: ( localize(11275, "Formatter Conflicts")),
                selector: {
                    language: document.getLanguageId(),
                    pattern: document.uri.fsPath
                },
                severity: Severity$1.Error,
                label: ( localize(11276, "Formatting")),
                detail: result,
                busy: false,
                source: "",
                command,
...
DefaultFormatter = DefaultFormatter_1 = ( __decorate([( __param(0, IExtensionService)), ( __param(1, IWorkbenchExtensionEnablementService)), ( __param(2, IConfigurationService)), ( __param(3, INotificationService)), ( __param(4, IDialogService)), ( __param(5, IQuickInputService)), ( __param(6, ILanguageService)), ( __param(7, ILanguageFeaturesService)), ( __param(8, ILanguageStatusService)), ( __param(9, IEditorService))], DefaultFormatter));
...
async function showFormatterPick(accessor, model, formatters) {
    ...
    const picks = ( formatters.map((provider, index) => {
        ...
        const pick = {
            ...
            description: isDefault ? ( localize(11278, "(default)")) : undefined
        };
        if (isDefault) {
    ...
    const configurePick = {
        label: ( localize(11279, "Configure Default Formatter..."))
    };
    const pick = await quickPickService.pick([...picks, {
        type: "separator"
    }, configurePick], {
        placeHolder: ( localize(11280, "Select a formatter")),
...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(11281, "Format Document With...")),
...
registerEditorAction(class FormatSelectionMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(11282, "Format Selection With...")),
            alias: "Format Code...",
