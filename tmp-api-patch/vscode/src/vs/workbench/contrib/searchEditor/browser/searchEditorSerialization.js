...
function fileMatchToSearchResultFormat(fileMatch, labelFormatter) {
    ...
    const cellSerializations = (isNotebookFileMatch(fileMatch)) ? ( fileMatch.cellMatches().sort((a, b) => a.cellIndex - b.cellIndex).sort().filter(cellMatch => cellMatch.contentMatches.length > 0).map(
...
function matchesToSearchResultFormat(
    ...
    const context = [];
    matchContext.forEach((line, lineNumber) => context.push({
        line,
        lineNumber
    }));
        ...
        for (let i = 0; i < str.length; i++) {
            if (str[i] === "\\") {
                ...
                if (escaped === "n") {
                    ...
                    throw Error(( localize(14190, "All backslashes in Query string must be escaped (\\\\)")));
                }
            } else {
                out += str[i];
            }
...
const serializeSearchResultForEditor = (
    searchResult,
    rawIncludePattern,
    rawExcludePattern,
    contextLines,
    ...
    const filecount = searchResult.fileCount() > 1 ? ( localize(14191, "{0} files", searchResult.fileCount())) : ( localize(14192, "1 file"));
    const resultcount = searchResult.count() > 1 ? ( localize(14193, "{0} results", searchResult.count())) : ( localize(14194, "1 result"));
    const info = [searchResult.count() ? `${resultcount} - ${filecount}` : ( localize(14195, "No Results"))];
    if (limitHit) {
        info.push(( localize(
            14196,
    ...
    const allResults = flattenSearchResultSerializations(( searchResult.folderMatches().sort(matchComparer).map(
        folderMatch => folderMatch.allDownstreamFileMatches().sort(matchComparer).flatMap(fileMatch => fileMatchToSearchResultFormat(fileMatch, labelFormatter))
    )).flat());
    return {
