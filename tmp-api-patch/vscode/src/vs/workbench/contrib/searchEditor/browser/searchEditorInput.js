...
import * as searchEditor from './media/searchEditor.css';
import { Emitter } from '../../../../base/common/event.js';
import { basename } from '../../../../base/common/path.js';
import { isEqual, extname, joinPath } from '../../../../base/common/resources.js';
...
import { EditorInputCapabilities, EditorResourceAccessor } from '../../../common/editor.js';
...
registerCss(searchEditor);
const SEARCH_EDITOR_EXT = ".code-search";
const SearchEditorIcon = registerIcon("search-editor-label-icon", Codicon.search, ( localize(14187, "Icon of the search editor label.")));
let SearchEditorInput = class SearchEditorInput extends EditorInput {
    ...
    getName(maxLength = 12) {
        ...
        if (query) {
            return localize(14188, "Search: {0}", trimToMax(query));
        }
        return localize(14189, "Search");
    }
    setDirty(dirty) {
    ...
    getMatchRanges() {
        return ( (this._cachedResultsModel?.getAllDecorations() ?? []).filter(decoration => decoration.options.className === SearchEditorFindMatchClass).filter((
            {
                range
            }
        ) => !(range.startColumn === 1 && range.endColumn === 1)).map((
            {
                range
            }
        ) => range));
    ...
    async revert(group, options) {
        ...
        if (this.backingUri) {
            const {
                config,
                text
            } = await this.instantiationService.invokeFunction(parseSavedSearchEditor, this.backingUri);
            const {
                resultsModel,
                configurationModel
            } = await this.resolveModels();
    ...
    async suggestFileName() {
        ...
        return joinPath(
            await this.fileDialogService.defaultFilePath(this.pathService.defaultUriScheme),
            searchFileName
        );
    }
    toUntyped() {
...
SearchEditorInput = SearchEditorInput_1 = ( __decorate([( __param(2, IModelService)), ( __param(3, ITextFileService)), ( __param(4, IFileDialogService)), ( __param(5, IInstantiationService)), ( __param(6, IWorkingCopyService)), ( __param(7, ITelemetryService)), ( __param(8, IPathService)), ( __param(9, IStorageService))], SearchEditorInput));
const getOrMakeSearchEditorInput = (accessor, existingData) => {
    ...
    if (!( searchEditorModelFactory.models.has(modelUri))) {
        if (existingData.from === "existingFile") {
            instantiationService.invokeFunction(
                accessor => searchEditorModelFactory.initializeModelFromExistingFile(accessor, modelUri, existingData.fileUri)
            );
        } else {
            ...
            if (existingData.from === "rawData") {
                ...
                instantiationService.invokeFunction(
                    accessor => searchEditorModelFactory.initializeModelFromRawData(accessor, modelUri, config, existingData.resultsContents)
                );
            } else {
                instantiationService.invokeFunction(
                    accessor => searchEditorModelFactory.initializeModelFromExistingModel(accessor, modelUri, config)
                );
            }
        }
    }
    return instantiationService.createInstance(
        SearchEditorInput,
        modelUri,
        existingData.from === "existingFile" ? existingData.fileUri : existingData.from === "model" ? existingData.backupOf : undefined
    );
};
