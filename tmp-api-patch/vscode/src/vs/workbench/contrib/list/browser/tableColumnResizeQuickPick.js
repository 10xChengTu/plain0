...
let TableColumnResizeQuickPick = class TableColumnResizeQuickPick extends Disposable {
    ...
    async show() {
        ...
        const column = await this._quickInputService.pick(items, {
            placeHolder: ( localize(11527, "Select the column to resize, type to filter."))
        });
        if (!column) {
            return;
        }
        const value = await this._quickInputService.input({
            placeHolder: ( localize(11528, "i.e. 20, 60, 100...")),
            prompt: ( localize(
                11529,
    ...
    async _validateColumnResizeValue(input) {
        ...
        if (input && !Number.isInteger(percentage)) {
            return localize(11530, "Please enter an integer.");
...
TableColumnResizeQuickPick = ( __decorate([( __param(1, IQuickInputService))], TableColumnResizeQuickPick));
