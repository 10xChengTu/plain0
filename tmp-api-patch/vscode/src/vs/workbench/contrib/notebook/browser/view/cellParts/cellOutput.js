...
import { CellUri, RENDERER_NOT_AVAILABLE, NotebookCellExecutionState } from '../../../common/notebookCommon.js';
...
let CellOutputElement = class CellOutputElement extends Disposable {
    ...
    rerender() {
        ...
        if (!this.innerContainer) {
            ...
            const previousSibling = currOutputIndex > 0 && !!(this.cellOutputContainer.renderedOutputEntries[currOutputIndex - 1].element.innerContainer?.parentElement) ? this.cellOutputContainer.renderedOutputEntries[currOutputIndex - 1].element.innerContainer : undefined;
    ...
    _renderMissingRenderer(viewModel, preferredMimeType) {
        if (!viewModel.model.outputs.length) {
            return this._renderMessage(viewModel, ( localize(12838, "Cell has no output")));
        }
    ...
    async _attachToolbar(
        ...
        const toolbar = this.toolbarDisposables.add(
            this.instantiationService.createInstance(WorkbenchToolBar, mimeTypePicker, {
        ...
        const pickAction = this.toolbarDisposables.add(( new Action("notebook.output.pickMimetype", ( localize(12840, "Change Presentation")), ThemeIcon.asClassName(mimetypeIcon), undefined, async _context => this._pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, this.output))));
        ...
        const menu = this.toolbarDisposables.add(
            this.menuService.createMenu(MenuId.NotebookOutputToolbar, menuContextKeyService)
        );
    ...
    async _pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, viewModel) {
        ...
        mimeTypes.forEach((mimeType, index) => {
            if (mimeType.isTrusted) {
                ...
                arr.push({
                    ...
                    description: index === currIndex ? ( localize(12841, "Currently Active")) : undefined
                });
            }
        });
        if (( unsupportedItems.some(m => JUPYTER_RENDERER_MIMETYPES.includes(m.id)))) {
            unsupportedItems.push({
                label: ( localize(12842, "Install additional renderers from the marketplace")),
                id: "installRenderers",
        ...
        picker.placeholder = items.length !== mimeTypes.length ? ( localize(12843, "Select mimetype to render for current output")) : ( localize(12844, "Select mimetype to render for current output"));
    ...
    _generateRendererInfo(renderId) {
        ...
        return localize(12845, "renderer not available");
    }
...
CellOutputElement = ( __decorate([( __param(5, INotebookService)), ( __param(6, IQuickInputService)), ( __param(7, IContextKeyService)), ( __param(8, IMenuService)), ( __param(9, IExtensionsWorkbenchService)), ( __param(10, IInstantiationService))], CellOutputElement));
...
let CellOutputContainer = class CellOutputContainer extends CellContentPart {
    ...
    _renderNow(splice, context) {
        ...
        if (firstGroupEntries.length + newlyInserted.length + secondGroupEntries.length > this.options.limit) {
            if (firstGroupEntries.length + newlyInserted.length > this.options.limit) {
                ...
                const newlyInsertedEntries = ( newlyInserted.map(insert => {
                    return ( new OutputEntryViewHandler(insert, this.instantiationService.createInstance(
                        CellOutputElement,
                        this.notebookEditor,
                        this.viewCell,
                        this,
            ...
            if (firstGroupEntries.length + newlyInsertedEntries.length + secondGroupEntries.length < this.viewCell.outputsViewModels.length) {
                ...
                outputsNewlyAvailable = ( this.viewCell.outputsViewModels.slice(
                    ...
                    return ( new OutputEntryViewHandler(output, this.instantiationService.createInstance(
                        CellOutputElement,
                        this.notebookEditor,
                        this.viewCell,
                        this,
        ...
        this._validateFinalOutputHeight(
            context === CellOutputUpdateContext.Other && this.viewCell.outputsViewModels.length === 0
        );
    }
...
CellOutputContainer = ( __decorate([( __param(4, IOpenerService)), ( __param(5, INotebookExecutionStateService)), ( __param(6, IInstantiationService))], CellOutputContainer));
