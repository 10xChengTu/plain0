...
import { Disposable, DisposableStore, combinedDisposable, dispose, MutableDisposable } from '../../../../base/common/lifecycle.js';
...
let NotebookEditorWidget = class NotebookEditorWidget extends Disposable {
    ...
    _createBody(parent) {
        ...
        this._register(
            this.instantiationService.createInstance(NotebookHorizontalTracker, this, this._list.scrollableElement)
        );
    ...
    _createLayoutStyles() {
        ...
        styleSheets.push(
            `.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .view-zones .code-cell-row div.cell.code { margin-left: ${getCellEditorContainerLeftMargin}px; }`
        );
        styleSheets.push(
            `.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .view-zones .code-cell-row div.cell { margin-right: ${cellRightMargin}px; }`
        );
        styleSheets.push(
    ...
    _createCellList() {
        ...
        this._editorPool = this._register(
            this.instantiationService.createInstance(NotebookCellEditorPool, this, getScopedContextKeyService)
        );
        const renderers = [this.instantiationService.createInstance(
            CodeCellRenderer,
            this,
        ...
        this._listTopCellToolbar = this._register(
            this.instantiationService.createInstance(ListTopCellToolbar, this, this.notebookOptions)
        );
        this._webviewTransparentCover = append(this._list.rowsContainer, $(".webview-cover"));
        this._webviewTransparentCover.style.display = "none";
        this._register(
            addStandardDisposableGenericMouseDownListener(this._overlayContainer, e => {
        ...
        this._register(
            addStandardDisposableGenericMouseUpListener(this._overlayContainer, () => {
    ...
    _registerNotebookOverviewRuler() {
        this._notebookOverviewRuler = this._register(
            this.instantiationService.createInstance(NotebookOverviewRuler, this, this._notebookOverviewRulerContainer)
        );
    }
    _registerNotebookActionsToolbar() {
        this._notebookTopToolbar = this._register(this.instantiationService.createInstance(
    ...
    _registerNotebookStickyScroll() {
        this._notebookStickyScroll = this._register(this.instantiationService.createInstance(
        ...
        if (focused) {
            if (!this._cellContextKeyManager) {
                this._cellContextKeyManager = this._localStore.add(
                    this.instantiationService.createInstance(CellContextKeyManager, this, focused)
                );
            }
        ...
        if (options?.cellSelections) {
            ...
            if (focusedCell) {
                this.viewModel.updateSelectionsState({
                    kind: SelectionStateType.Index,
                    focus: {
                        start: focusCellIndex,
                        end: focusCellIndex + 1
                    },
    ...
    async _attachModel(textModel, viewType, viewState, perf) {
        ...
        this._viewContext.eventDispatcher.emit([( new NotebookLayoutChangedEvent({
            width: true,
            fontInfo: true
        }, this.getLayoutInfo()))]);
        this.notebookOptions.updateOptions(this._readOnly);
        ...
        this._localStore.add(this._list.onDidChangeContentHeight(() => {
            ...
            renderScrollHeightDisposable.value = scheduleAtNextAnimationFrame(getWindow(this.getDomNode()), () => {
    ...
    getBodyHeight(dimensionHeight) {
        return Math.max(
            dimensionHeight - (this._notebookTopToolbar?.useGlobalToolbar ?  26 : 0),
            0
        );
    }
    ...
    layoutContainerOverShadowElement(anchorElement) {
        ...
        const isModal = isHTMLElement(modalEditorContainer) && modalEditorContainer.contains(anchorElement);
        const clippingContainer = isModal ? undefined : this.layoutService.getContainer(getWindow(this.getDomNode()), Parts.EDITOR_PART);
...
}, ( localize(12738, "The border color for notebook cells.")));
registerColor("notebook.focusedEditorBorder", focusBorder, ( localize(12739, "The color of the notebook cell editor border.")));
const cellStatusIconSuccess = registerColor(
    "notebookStatusSuccessIcon.foreground",
    debugIconStartForeground,
    ( localize(12740, "The error icon color of notebook cells in the cell status bar."))
);
...
const cellStatusIconError = registerColor("notebookStatusErrorIcon.foreground", errorForeground, ( localize(12742, "The error icon color of notebook cells in the cell status bar.")));
registerColor("notebookStatusRunningIcon.foreground", foreground, ( localize(12743, "The running icon color of notebook cells in the cell status bar.")));
registerColor("notebook.outputContainerBorderColor", null, ( localize(12744, "The border color of the notebook output container.")));
registerColor("notebook.outputContainerBackgroundColor", null, ( localize(12745, "The color of the notebook output container background.")));
registerColor("notebook.cellToolbarSeparator", {
    dark: ( ( Color.fromHex("#808080")).transparent(0.35)),
    light: ( ( Color.fromHex("#808080")).transparent(0.35)),
    hcDark: contrastBorder,
    hcLight: contrastBorder
}, ( localize(12746, "The color of the separator in the cell bottom toolbar")));
const focusedCellBackground = registerColor("notebook.focusedCellBackground", null, ( localize(12747, "The background color of a cell when the cell is focused.")));
registerColor("notebook.selectedCellBackground", {
    dark: listInactiveSelectionBackground,
    light: listInactiveSelectionBackground,
    hcDark: null,
    hcLight: null
}, ( localize(12748, "The background color of a cell when the cell is selected.")));
registerColor("notebook.cellHoverBackground", {
    ...
    hcDark: null,
    hcLight: null
}, ( localize(12749, "The background color of a cell when the cell is hovered.")));
...
registerColor("notebook.inactiveSelectedCellBorder", {
    ...
    hcDark: focusBorder,
    hcLight: focusBorder
}, ( localize(12751, "The color of the cell's borders when multiple cells are selected.")));
...
}, ( localize(12754, "The background color of notebook cell status bar items.")));
registerColor("notebook.cellInsertionIndicator", focusBorder, ( localize(12755, "The color of the notebook cell insertion indicator.")));
registerColor(
    "notebookScrollbarSlider.background",
    scrollbarSliderBackground,
    ( localize(12756, "Notebook scrollbar slider background color."))
);
registerColor(
    "notebookScrollbarSlider.hoverBackground",
    scrollbarSliderHoverBackground,
    ( localize(12757, "Notebook scrollbar slider background color when hovering."))
);
registerColor(
    "notebookScrollbarSlider.activeBackground",
    scrollbarSliderActiveBackground,
    ( localize(12758, "Notebook scrollbar slider background color when clicked on."))
);
registerColor("notebook.symbolHighlightBackground", {
    ...
    hcDark: null,
    hcLight: null
}, ( localize(12759, "Background color of highlighted cell")));
registerColor("notebook.cellEditorBackground", {
    light: SIDE_BAR_BACKGROUND,
    dark: SIDE_BAR_BACKGROUND,
    hcDark: null,
    hcLight: null
}, ( localize(12760, "Cell editor background color.")));
const notebookEditorBackground = registerColor("notebook.editorBackground", {
    ...
    hcDark: null,
    hcLight: null
}, ( localize(12761, "Notebook background color.")));
