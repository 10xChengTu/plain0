...
import { $ as $$1, append, hide, addDisposableListener, EventType } from '../../../../../../base/browser/dom.js';
...
let CollapsedCellOutput = class CollapsedCellOutput extends CellContentPart {
    constructor(notebookEditor, cellOutputCollapseContainer, keybindingService) {
        ...
        placeholder.textContent = ( localize(12853, "Outputs are collapsed"));
        ...
        if (keybinding) {
            placeholder.title = ( localize(12854, "Double-click to expand cell output ({0})", keybinding.getLabel()));
            cellOutputCollapseContainer.title = ( localize(12855, "Expand Cell Output (${0})", keybinding.getLabel()));
        }
        hide(cellOutputCollapseContainer);
        this._register(
            addDisposableListener(expandIcon, EventType.CLICK, () => this.expand())
        );
        this._register(
            addDisposableListener(cellOutputCollapseContainer, EventType.DBLCLICK, () => this.expand())
        );
    }
    expand() {
        if (!this.currentCell) {
