...
import { TEXT_BASED_MIMETYPES, getOutputStreamText, getOutputText } from '../../viewModel/cellOutputTextHelper.js';
...
let BackLayerWebView = class BackLayerWebView extends Themable {
    ...
    _generateStyles() {
        return {
            ...
            "notebook-cell-markup-empty-content": ( localize(12860, "Empty markdown cell, double-click or press enter to edit.")),
            "notebook-cell-renderer-not-found-error": ( localize(12861, "No renderer found for '$0'")),
            "notebook-cell-renderer-fallbacks-exhausted": ( localize(12862, "Could not render content for '$0'")),
    ...
    generateContent(baseUrl) {
        ...
        const preloadScript = preloadsScriptStr({
            ...
            dragAndDropEnabled: this.options.dragAndDropEnabled
        }, renderOptions, renderersData, preloadsData, this.workspaceTrustManagementService.isWorkspaceTrusted(), this.nonce);
    ...
    getStaticPreloadsData() {
        return Array.from(
            ...
            preload => {
                return {
                    entrypoint: ( ( this.asWebviewUri(preload.entrypoint, preload.extensionLocation).toString()).toString())
                };
            }
        );
    }
    asWebviewUri(uri, fromExtension) {
        return asWebviewUri(uri, fromExtension?.scheme === Schemas.vscodeRemote ? {
            isRemote: true,
            authority: fromExtension.authority
        } : undefined);
    }
    ...
    _handleNotebookCellResource(uri) {
        ...
        if (fragmentLineMatch) {
            ...
            if (!isNaN(parsedLineNumber)) {
                ...
                return this.openerService.open(notebookResource.with({
                    fragment
                }), {
                    fromUserGesture: true,
                    fromWorkspace: true,
    ...
    _createInset(webviewService, content) {
        ...
        const webview = webviewService.createWebviewElement({
            ...
            title: ( localize(12863, "Notebook webview content")),
            options: {
    ...
    _getResourceRootsCache() {
        ...
        return [this.notebookService.getNotebookProviderResourceRoots(), ( this.notebookService.getRenderers().map(x => dirname(x.entrypoint.path))), ...Array.from(
            this.notebookService.getStaticPreloads(this.notebookViewType),
            x => [dirname(x.entrypoint), ...x.localResourceRoots]
        ), workspaceFolders, notebookDir, this.getBuiltinLocalResourceRoots()].flat();
    }
    ...
    requestCreateOutputWhenWebviewIdle(cellInfo, content, cellTop, offset) {
        ...
        this.pendingWebviewIdleCreationRequest.set(content.source, runWhenGlobalIdle(() => {
            ...
            } = this._createOutputCreationMessage(cellInfo, content, cellTop, offset, true, true);
    ...
    createOutput(cellInfo, content, cellTop, offset) {
        ...
        const {
            ...
            renderer,
            transfer: transferable
        } = this._createOutputCreationMessage(cellInfo, content, cellTop, offset, false, false);
        this._sendMessageToWebview(message, transferable);
    ...
    _updatePreloadsFromKernel(kernel) {
        ...
        for (const preload of kernel.preloadUris) {
            const uri = this.environmentService.isExtensionDevelopment && (preload.scheme === "http" || preload.scheme === "https") ? preload : this.asWebviewUri(preload, undefined);
...
BackLayerWebView = BackLayerWebView_1 = ( __decorate([( __param(6, IWebviewService)), ( __param(7, IOpenerService)), ( __param(8, INotebookService)), ( __param(9, IWorkspaceContextService)), ( __param(10, IWorkbenchEnvironmentService)), ( __param(11, IFileDialogService)), ( __param(12, IFileService)), ( __param(13, IContextMenuService)), ( __param(14, IContextKeyService)), ( __param(15, IWorkspaceTrustManagementService)), ( __param(16, IConfigurationService)), ( __param(17, ILanguageService)), ( __param(18, IWorkspaceContextService)), ( __param(19, IEditorGroupsService)), ( __param(20, IStorageService)), ( __param(21, IPathService)), ( __param(22, INotebookLoggingService)), ( __param(23, IThemeService)), ( __param(24, ITelemetryService))], BackLayerWebView));
