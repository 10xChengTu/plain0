...
import { KERNEL_RECOMMENDATIONS, JUPYTER_EXTENSION_ID } from '../notebookBrowser.js';
...
function toKernelQuickPick(kernel, selected) {
    ...
    if (kernel.id === selected?.id) {
        if (!res.description) {
            res.description = ( localize(12866, "Currently Selected"));
        } else {
            res.description = ( localize(12867, "{0} - Currently Selected", res.description));
        }
    }
    return res;
}
class KernelPickerStrategyBase {
    ...
    async showQuickPick(editor, wantedId, skipAutoRun) {
        ...
        const quickPick = localDisposableStore.add(this._quickInputService.createQuickPick({
            useSeparators: true
        }));
        ...
        const extensionRecommendataionPromise = quickPickItems.length === 0 ? createCancelablePromise(
            token => this._showInstallKernelExtensionRecommendation(notebook, quickPick, this._extensionWorkbenchService, token)
        ) : undefined;
        const kernelChangeEventListener = Event.debounce(Event.any(
        ...
        ), (last, _current) => last, KERNEL_PICKER_UPDATE_DEBOUNCE)(async () => {
    ...
    async _showKernelExtension(
        ...
        for (const extId of extIds) {
            ...
            if (extension.enablementState === EnablementState.DisabledGlobally || extension.enablementState === EnablementState.DisabledWorkspace || extension.enablementState === EnablementState.DisabledByEnvironment) {
                ...
                if (( extensionWorkbenchService.installed.some(
                    e => areSameExtensions(e.identifier, extension.identifier) && e.server === extensionManagementServerService.remoteExtensionManagementServer
                ))) {
                    continue;
                } else {
                    const canInstall = await extensionWorkbenchService.canInstall(extension);
        ...
        if (extensionsToInstall.length || extensionsToEnable.length || extensionsToInstallOnRemote.length) {
            await Promise.all([...( extensionsToInstall.map(async extension => {
                ...
                await extensionWorkbenchService.installInServer(
                    extension,
                    this._extensionManagementServerService.remoteExtensionManagementServer
                );
            }))]);
    ...
    async _getKernelRecommendationsQuickPickItems(notebookTextModel, extensionWorkbenchService) {
        ...
        if (suggestedExtension) {
            ...
            quickPickItems.push({
                ...
                label: `$(${Codicon.lightbulb.id}) ` + ( localize(12870, "Install/Enable suggested extensions")),
                extensionIds: suggestedExtension.extensionIds
            });
        }
        quickPickItems.push({
            id: "install",
            label: ( localize(12871, "Browse marketplace for kernel extensions"))
        });
...
let KernelPickerMRUStrategy = class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
    ...
    _getKernelPickerQuickPickItems(
        notebookTextModel,
        matchResult,
        ...
        quickPickItems.push({
            id: "selectAnother",
            label: ( localize(12872, "Select Another Kernel...")),
    ...
    async displaySelectAnotherQuickPick(editor, kernelListEmpty) {
        ...
        const quickPickItem = await ( new Promise(resolve => {
            quickPick.title = kernelListEmpty ? ( localize(12873, "Select Kernel")) : ( localize(12874, "Select Another Kernel"));
            quickPick.placeholder = ( localize(12875, "Type to choose a kernel source"));
            quickPick.busy = true;
            ...
            if (isKernelSourceQuickPickItem(selectedKernelPickItem)) {
                try {
                    ...
                    if (selectedKernelId) {
                        const {
                            all
                        } = await this._getMatchingResult(notebook);
    ...
    async _calculdateKernelSources(editor) {
        ...
        quickPickItems.push(...( validActions.map(action => {
            const buttons = action.documentation ? [{
                ...
                tooltip: ( localize(12876, "Learn More"))
            }] : [];
            return {
    ...
    async _selectOneKernel(notebook, source, kernels) {
        ...
        quickPick.items = quickPickItems;
        quickPick.canSelectMany = false;
        quickPick.title = ( localize(12877, "Select Kernel from {0}", source));
    ...
    static updateKernelStatusAction(notebook, action, notebookKernelService, notebookKernelHistoryService) {
        ...
        if (detectionTasks.length) {
            ...
            if (info.selected) {
                ...
                action.tooltip = kernelInfo ? ( localize(12878, "Selected Kernel: {0} (Kernel Detection Tasks Running)", kernelInfo)) : ( localize(12879, "Detecting Kernels"));
            } else {
                action.label = ( localize(12879, "Detecting Kernels"));
            }
            return;
        }
        ...
        if (selected) {
            ...
            action.label = ( localize(12873, "Select Kernel"));
            action.class = ThemeIcon.asClassName(selectKernelIcon);
            action.tooltip = "";
        }
    }
