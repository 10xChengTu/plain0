...
let MarkupCell = class MarkupCell extends Disposable {
    ...
    updateMarkupCellOptions() {
        this.updateEditorOptions(
            this.cellEditorOptions.getUpdatedValue(this.viewCell.internalMetadata, this.viewCell.uri)
        );
        if (this.editor) {
    ...
    viewUpdateCollapsed() {
        ...
        if (keybinding) {
            element.title = ( localize(12858, "Double-click to expand cell input ({0})", keybinding.getLabel()));
            expandIcon.title = ( localize(12859, "Expand Cell Input ({0})", keybinding.getLabel()));
        }
    ...
    viewUpdateEditing() {
        ...
        if (this.editor && this.editor.hasModel()) {
            ...
            this.editor = this.editorDisposables.add(
                editorInstaService.createInstance(CodeEditorWidget, this.templateData.editorContainer, {
                    ...this.editorOptions,
                    dimension: {
                        width: width,
                        height: editorHeight
    ...
    focusEditorIfNeeded() {
        if (this.viewCell.focusMode === CellFocusMode.Editor && (this.notebookEditor.hasEditorFocus() || getActiveElement(this.notebookEditor.getDomNode().ownerDocument) === getRootContainer(this.notebookEditor.getDomNode()))) {
            if (!this.editor) {
                return;
            }
            this.editor.focus();
...
MarkupCell = ( __decorate([( __param(4, IAccessibilityService)), ( __param(5, IContextKeyService)), ( __param(6, IInstantiationService)), ( __param(7, ILanguageService)), ( __param(8, IConfigurationService)), ( __param(9, IKeybindingService))], MarkupCell));
