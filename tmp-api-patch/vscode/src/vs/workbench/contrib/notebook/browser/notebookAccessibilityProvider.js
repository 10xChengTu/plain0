...
let NotebookAccessibilityProvider = class NotebookAccessibilityProvider extends Disposable {
    ...
    shouldReadCellOutputs(state) {
        return (
            state === undefined && this.isReplHistory && this.accessibilityService.isScreenReaderOptimized() && this.configurationService.getValue("accessibility.replEditor.readLastExecutionOutput")
        );
    }
    ...
    get widgetAriaLabelName() {
        return this.isReplHistory ? ( localize(12721, "REPL Editor History")) : ( localize(12722, "Notebook"));
    }
    getWidgetAriaLabel() {
    ...
    mergeEvents(last, e) {
        ...
        if (viewModel && e.type === NotebookExecutionType.cell && e.affectsNotebook(viewModel.uri)) {
            ...
            result.push({
                cellHandle: e.cellHandle,
                state: e.changed?.state
            });
        }
        return result;
    }
};
NotebookAccessibilityProvider = ( __decorate([( __param(2, INotebookExecutionStateService)), ( __param(3, IKeybindingService)), ( __param(4, IConfigurationService)), ( __param(5, IAccessibilityService))], NotebookAccessibilityProvider));
