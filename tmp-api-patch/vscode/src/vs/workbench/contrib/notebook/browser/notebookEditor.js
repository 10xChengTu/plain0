...
import { createEditorOpenError, DEFAULT_EDITOR_ASSOCIATION, EditorPaneSelectionChangeReason, isEditorOpenError, createTooLargeFileError, EditorResourceAccessor, EditorPaneSelectionCompareResult } from '../../../common/editor.js';
...
const confirmedWebHostNotebooks = ( new Set());
let NotebookEditor = class NotebookEditor extends EditorPane {
    ...
    constructor(
        ...
        this._register(
            this._fileService.onDidChangeFileSystemProviderCapabilities(e => this._onDidChangeFileSystemProvider(e.scheme))
        );
        this._register(
            this._fileService.onDidChangeFileSystemProviderRegistrations(e => this._onDidChangeFileSystemProvider(e.scheme))
        );
    }
    ...
    getActionViewItem(action, options) {
        if (action.id === SELECT_KERNEL_ID) {
            return this._register(
                this._instantiationService.createInstance(NotebooKernelActionViewItem, action, this, options)
            );
        }
        return undefined;
    }
    ...
    hasFocus() {
        ...
        return !!value && (isAncestorOfActiveElement(
            value.getDomNode() || isAncestorOfActiveElement(value.getOverflowContainerDomNode())
        ));
    }
    async _confirmOpenOnWebHost(input) {
        ...
        const {
            ...
            type: "warning",
            message: ( localize(12725, "Do you trust the authors of this notebook?")),
            detail: ( localize(
                12726,
            ...
            primaryButton: ( localize(12727, "Open Notebook")),
            checkbox: {
                label: ( localize(12728, "Don't ask me again"))
            }
        });
        if (!confirmed) {
            throw createEditorOpenError(( localize(12729, "The notebook was not opened because its authors are not trusted.")), [toAction({
                id: "workbench.notebook.action.openAsText",
                label: ( localize(12730, "Open As Text")),
                run: async () => {
        ...
        if (checkboxChecked) {
            this._storageService.store(
                NOTEBOOK_WEB_HOST_OPEN_CONFIRMED_KEY,
                true,
    ...
    async setInput(input, options, context, token, noRetry) {
        ...
        try {
            ...
            this._widgetDisposableStore.add(
                this._widget.value.onDidChangeActiveCell(() => this._onDidChangeSelection.fire({
            ...
            if (model === null) {
                ...
                throw createEditorOpenError(( new Error(( localize(
                    ...
                    label: extensionInfo ? ( localize(12733, "Enable extension for '{0}'", input.viewType)) : ( localize(12734, "Install extension for '{0}'", input.viewType)),
                    run: async () => {
                        ...
                        try {
                            if (extensionInfo) {
                                await this._extensionsWorkbenchService.setEnablement(
                                    extensionInfo,
                                    extensionInfo.enablementState === EnablementState.DisabledWorkspace ? EnablementState.EnabledWorkspace : EnablementState.EnabledGlobally
                                );
                            } else {
            ...
            await this._widget.value.setOptions({
                ...options,
                isReadOnly
            });
            ...
            this._widgetDisposableStore.add(
                this._editorGroupService.createEditorDropTarget(this._widget.value.getDomNode(), {
            ...
            const error = createEditorOpenError(e instanceof Error ? e : ( new Error((e ? e.message : ""))), [toAction({
                ...
                label: ( localize(12737, "Open in Text Editor")),
                run: async () => {
            ...
            for (const cell of notebook.cells) {
                if (cell.cellKind === CellKind.Code) {
                    ...
                    outputBytes = (outputBytes || 0) + cell.outputs.reduce(
                        (prev, cur) => prev + cur.outputs.reduce((size, item) => size + item.data.byteLength, 0),
                        0
                    );
                } else {
...
NotebookEditor = NotebookEditor_1 = ( __decorate([( __param(1, ITelemetryService)), ( __param(2, IThemeService)), ( __param(3, IInstantiationService)), ( __param(4, IStorageService)), ( __param(5, IEditorService)), ( __param(6, IEditorGroupsService)), ( __param(7, INotebookEditorService)), ( __param(8, IContextKeyService)), ( __param(9, IFileService)), ( __param(10, ITextResourceConfigurationService)), ( __param(11, IEditorProgressService)), ( __param(12, INotebookService)), ( __param(13, IExtensionsWorkbenchService)), ( __param(14, IWorkingCopyBackupService)), ( __param(15, ILogService)), ( __param(16, IPreferencesService)), ( __param(17, IDialogService)), ( __param(18, IWorkbenchEnvironmentService))], NotebookEditor));
...
export { NotebookEditor, isNotebookContainingCellEditor };
