...
let TerminalEditorInput = class TerminalEditorInput extends EditorInput {
    ...
    async confirm(terminals) {
        const {
            ...
            message: ( localize(14815, "Do you want to terminate running processes?")),
            primaryButton: ( localize(14816, "&&Terminate")),
            detail: terminals.length > 1 ? ( terminals.map(terminal => terminal.editor.getName())).join("\n") + "\n\n" + ( localize(14817, "Closing will terminate the running processes in the terminals.")) : ( localize(14818, "Closing will terminate the running processes in this terminal."))
        });
    ...
    _setupInstanceListeners() {
        ...
        const disposeListeners = [instance.onExit(e => {
            if (!instance.waitOnExit) {
                this.dispose();
            }
        }), instance.onDisposed(() => this.dispose()), instance.onTitleChanged(() => this._onDidChangeLabel.fire()), instance.onIconChanged(() => this._onDidChangeLabel.fire()), instanceOnDidFocusListener, instanceOnDidBlurListener, instance.statusList.onDidChangePrimaryStatus(() => this._onDidChangeLabel.fire())];
...
TerminalEditorInput = TerminalEditorInput_1 = ( __decorate([( __param(2, IThemeService)), ( __param(3, ITerminalInstanceService)), ( __param(4, IInstantiationService)), ( __param(5, IConfigurationService)), ( __param(6, ILifecycleService)), ( __param(7, IContextKeyService)), ( __param(8, IDialogService))], TerminalEditorInput));
