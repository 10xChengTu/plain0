...
function getTerminalDecorationHoverContent(command, hoverMessage, showCommandActions) {
    let hoverContent = showCommandActions ? `${( localize(14948, "Show Command Actions"))}\n\n---\n\n` : "";
    if (!command) {
        if (hoverMessage) {
        ...
        if (isNumber(command.duration)) {
            ...
            if (command.exitCode) {
                if (command.exitCode === -1) {
                    hoverContent += ( localize(14952, "Command executed {0} and failed", fromNow(command.timestamp, true)));
                } else {
                    hoverContent += ( localize(
                        14953,
                ...
                hoverContent += ( localize(14954, "Command executed {0} now"));
            }
        }
    }
    return hoverContent;
...
const unknownText = ( localize(14955, "Unknown"));
const runningText = ( localize(14956, "Running"));
function getTerminalCommandDecorationTooltip(command, storedState) {
    ...
    if (typeof duration === "number") {
        ...
        if (exitCode) {
            ...
            hoverContent += ( localize(14951, "Command executed {0} and took {1}", fromNowText, durationText));
        }
    } else {
        if (exitCode) {
            if (exitCode === -1) {
                hoverContent += ( localize(14952, "Command executed {0} and failed", fromNowText));
            } else {
                hoverContent += ( localize(
                    14953,
                    "Command executed {0} and failed (Exit Code {1})",
            ...
            hoverContent += ( localize(14957, "Command executed {0} ", fromNowText));
        }
    }
    return hoverContent;
}
