...
import { IAgentHostCustomizationService } from '../../chat/browser/agentSessions/agentHost/agentHostCustomizationService.service.js';
...
const AssistedTypes = {
    [AddConfigurationType.NpmPackage]: {
        title: ( localize(11845, "Enter NPM Package Name")),
        placeholder: ( localize(11846, "Package name (e.g., @org/package)")),
        pickLabel: ( localize(11847, "NPM Package")),
        pickDescription: ( localize(11848, "Install from an NPM package name")),
        enabledConfigKey: null
    },
    [AddConfigurationType.PipPackage]: {
        title: ( localize(11849, "Enter Pip Package Name")),
        placeholder: ( localize(11850, "Package name (e.g., package-name)")),
        pickLabel: ( localize(11851, "Pip Package")),
        pickDescription: ( localize(11852, "Install from a Pip package name")),
        enabledConfigKey: null
    },
    [AddConfigurationType.NuGetPackage]: {
        title: ( localize(11853, "Enter NuGet Package Name")),
        placeholder: ( localize(11854, "Package name (e.g., Package.Name)")),
        pickLabel: ( localize(11855, "NuGet Package")),
        pickDescription: ( localize(11856, "Install from a NuGet package name")),
        enabledConfigKey: "chat.mcp.assisted.nuget.enabled"
    },
    [AddConfigurationType.DockerImage]: {
        title: ( localize(11857, "Enter Docker Image Name")),
        placeholder: ( localize(11858, "Image name (e.g., mcp/imagename)")),
        pickLabel: ( localize(11859, "Docker Image")),
        pickDescription: ( localize(11860, "Install from a Docker image")),
        enabledConfigKey: null
    }
};
...
let McpAddConfigurationCommand = class McpAddConfigurationCommand {
    ...
    async getServerType() {
        const items = [{
            ...
            label: ( localize(11861, "Command (stdio)")),
            description: ( localize(11862, "Run a local command that implements the MCP protocol"))
        }, {
            kind: AddConfigurationType.HTTP,
            label: ( localize(11863, "HTTP (HTTP or Server-Sent Events)")),
            description: ( localize(11864, "Connect to a remote HTTP server that implements the MCP protocol"))
        }];
        let aiSupported;
        try {
        ...
        if (aiSupported) {
            items.unshift({
                type: "separator",
                label: ( localize(11865, "Manual Install"))
            });
            ...
            items.push({
                type: "separator",
                label: ( localize(11866, "Model-Assisted"))
            }, ...elligableTypes);
        }
        items.push({
            type: "separator"
        ...
        if (discovery && typeof discovery === "object" && ( allDiscoverySources.some(d => !discovery[d]))) {
            items.push({
                kind: "discovery",
                label: ( localize(11867, "Add from another application..."))
            });
        }
        items.push({
            kind: "browse",
            label: ( localize(11868, "Browse MCP Servers..."))
        });
        const result = await this._quickInputService.pick(items, {
            placeHolder: ( localize(11869, "Choose the type of MCP server to add"))
        });
    ...
    async getStdioConfig() {
        const command = await this._quickInputService.input({
            title: ( localize(11870, "Enter Command")),
            placeHolder: ( localize(11871, "Command to run (with optional arguments)")),
    ...
    async getSSEConfig() {
        const url = await this._quickInputService.input({
            title: ( localize(11872, "Enter Server URL")),
            placeHolder: ( localize(11873, "URL of the MCP server (e.g., http://localhost:3000)")),
            ignoreFocusLost: true
        });
        if (!url) {
            return undefined;
    ...
    async getServerId(suggestion = `my-mcp-server-${generateUuid().split("-")[0]}`) {
        const id = await this._quickInputService.input({
            title: ( localize(11874, "Enter Server ID")),
            placeHolder: ( localize(11875, "Unique identifier for this server")),
            value: suggestion,
            ignoreFocusLost: true
        });
        return id;
    ...
    async getConfigurationTarget() {
        const options = [{
            ...
            label: ( localize(11876, "Global")),
            description: ( localize(11877, "Available in all workspaces, runs locally"))
        }];
        ...
        if (raLabel) {
            options.push({
                ...
                label: ( localize(11878, "Remote")),
                description: ( localize(11879, "Available on this remote machine, runs on {0}", raLabel))
            });
        }
        ...
        if (workbenchState !== WorkbenchState.EMPTY) {
            ...
            if (this._environmentService.remoteAuthority) {
                options.push({
                    target,
                    label: ( localize(11880, "Workspace")),
                    description: ( localize(11881, "Available in this workspace, runs on {0}", raLabel))
                });
            } else {
                options.push({
                    target,
                    label: ( localize(11880, "Workspace")),
                    description: ( localize(11882, "Available in this workspace, runs locally"))
                });
            }
        }
        if (options.length === 1) {
        ...
        const targetPick = await this._quickInputService.pick(options, {
            title: ( localize(11883, "Add MCP Server")),
            placeHolder: ( localize(11884, "Select the configuration target"))
        });
    ...
    async getInstallTarget() {
        ...
        if (session && hasAgentHostSession) {
            ...
            const items = [{
                id: AGENT_HOST_ID,
                label: ( localize(11885, "Add to Current Agent Session")),
                alwaysShow: true
            }, {
                type: "separator"
            }, {
                id: LOCAL_ID,
                label: ( localize(11886, "Install Server Locally...")),
    ...
    async getAssistedConfig(type) {
        ...
        loadingQuickPick.title = ( localize(11887, "Loading package details..."));
            ...
            if (!result || result.state === "error") {
                ...
                if (result?.helpUri) {
                    items.push({
                        ...
                        label: result.helpUriLabel ?? ( localize(11888, "Open help URL")),
                        helpUri: ( URI.parse(result.helpUri))
                    });
                }
                items.push({
                    id: LoadAction.Retry,
                    label: ( localize(11889, "Try a different package"))
                }, {
                    id: LoadAction.Cancel,
                    label: ( localize(11890, "Cancel"))
                });
    ...
    async run() {
        ...
        await this._mcpManagementService.install({
            name,
            config,
            inputs
        }, {
        ...
        if (inputValues) {
            for (const [key, value] of Object.entries(inputValues)) {
                await this._mcpRegistry.setSavedInput(
                    key,
                    (isWorkspaceFolder(target) ? ConfigurationTarget.WORKSPACE_FOLDER : target) ?? ConfigurationTarget.WORKSPACE,
                    value
                );
            }
        }
    ...
    async pickForUrlHandler(resource, showIsPrimary = false) {
        ...
        const placeHolder = ( localize(11893, "Install MCP server {0}", name));
        const items = [{
            id: "install",
            label: ( localize(11894, "Install Server"))
        }, {
            id: "show",
            label: ( localize(11895, "Show Configuration", name))
        }, {
            id: "rename",
            label: ( localize(11896, "Rename \"{0}\"", name))
        }, {
            id: "cancel",
            label: ( localize(11890, "Cancel"))
        }];
        ...
        const pick = await this._quickInputService.pick(items, {
            placeHolder,
            ignoreFocusLost: true
        });
        ...
        case "install":
            ...
            try {
                ...
                this._notificationService.error(( localize(11897, "Error installing MCP server {0}: {1}", name, e.message)));
                await this._editorService.openEditor({
                    resource
                });
            }
        ...
        case "rename":
            {
                const newName = await this._quickInputService.input({
                    placeHolder: ( localize(11898, "Enter new name")),
                    value: name
                });
                if (newName) {
                    const newURI = resource.with({
                        path: `/${encodeURIComponent(newName)}.json`
                    });
                    await this._editorService.save(getEditors());
...
McpAddConfigurationCommand = ( __decorate([( __param(1, IQuickInputService)), ( __param(2, IWorkbenchMcpManagementService)), ( __param(3, IWorkspaceContextService)), ( __param(4, IWorkbenchEnvironmentService)), ( __param(5, ICommandService)), ( __param(6, IMcpRegistry)), ( __param(7, IOpenerService)), ( __param(8, IEditorService)), ( __param(9, IFileService)), ( __param(10, INotificationService)), ( __param(11, ITelemetryService)), ( __param(12, IMcpService)), ( __param(13, ILabelService)), ( __param(14, IConfigurationService)), ( __param(15, IAgentHostCustomizationService)), ( __param(16, IChatWidgetService))], McpAddConfigurationCommand));
let McpInstallFromManifestCommand = class McpInstallFromManifestCommand {
    ...
    async run() {
        const result = await this._fileDialogService.showOpenDialog({
            title: ( localize(11899, "Select MCP Server Manifest")),
            filters: [{
                name: ( localize(11900, "MCP Manifest")),
                extensions: ["json"]
            }],
            canSelectFiles: true,
            canSelectMany: false,
            openLabel: mnemonicButtonLabel(( localize(11901, "&&Install")))
        });
        if (!result?.[0]) {
            return;
        }
        ...
        try {
            ...
            this._notificationService.error(( localize(11902, "Failed to read manifest file: {0}", e.message)));
            return;
        }
        if (!manifest || typeof manifest !== "object") {
            this._notificationService.error(( localize(11903, "Invalid manifest file: expected a JSON object")));
            return;
        }
        ...
        try {
            const {
                ...
                notices
            } = this._mcpManagementService.getMcpServerConfigurationFromManifest(galleryManifest, packageType);
            config = mcpServerConfiguration.config;
            inputs = mcpServerConfiguration.inputs;
            if (notices.length > 0) {
                this._logService.warn(
                    `MCP Management Service: Warnings while installing the MCP server from ${manifestUri.path}`,
                    notices
                );
            }
        } catch (e) {
            this._notificationService.error(( localize(11905, "Failed to parse manifest: {0}", e.message)));
            return;
        }
        let name = galleryManifest.name;
        if (!name) {
            name = await this._quickInputService.input({
                title: ( localize(11906, "Enter Server ID")),
                placeHolder: ( localize(11907, "Unique identifier for this server")),
        ...
        try {
            ...
            this._notificationService.info(( localize(11908, "MCP server '{0}' installed successfully", name)));
        } catch (e) {
            this._notificationService.error(( localize(11909, "Failed to install MCP server: {0}", e.message)));
        }
    }
};
McpInstallFromManifestCommand = ( __decorate([( __param(0, IFileDialogService)), ( __param(1, IFileService)), ( __param(2, IQuickInputService)), ( __param(3, INotificationService)), ( __param(4, IWorkbenchMcpManagementService)), ( __param(5, ILogService))], McpInstallFromManifestCommand));

export { AddConfigurationType, AssistedTypes, McpAddConfigurationCommand, McpInstallFromManifestCommand };
