...
import { isContributionDisabled, ContributionEnablementState, isContributionEnabled } from '../../chat/common/enablement.js';
...
let InstallAction = class InstallAction extends McpServerAction {
    ...
    constructor(open, mcpWorkbenchService, telemetryService, mcpService) {
        super("extensions.install", ( localize(11985, "Install")), InstallAction_1.CLASS, false);
        this.open = open;
...
let InstallInWorkspaceAction = class InstallInWorkspaceAction extends McpServerAction {
    ...
    constructor(
        ...
        super("extensions.installWorkspace", ( localize(11987, "Install in Workspace")), InstallAction.CLASS, false);
        this.open = open;
        this.mcpWorkbenchService = mcpWorkbenchService;
    ...
    async getConfigurationTarget() {
        ...
        for (const folder of this.workspaceService.getWorkspace().folders) {
            options.push({
                target: folder,
                label: folder.name,
                description: ( localize(11988, "Workspace Folder"))
            });
        }
        if (this.workspaceService.getWorkbenchState() === WorkbenchState.WORKSPACE) {
            ...
            options.push({
                ...
                label: ( localize(11989, "Workspace"))
            });
        }
        if (options.length === 1) {
        ...
        const targetPick = await this.quickInputService.pick(options, {
            title: ( localize(11990, "Choose where to install the MCP server"))
        });
...
let InstallInRemoteAction = class InstallInRemoteAction extends McpServerAction {
    ...
    constructor(
        ...
        super("extensions.installRemote", ( localize(11991, "Install (Remote)")), InstallAction.CLASS, false);
        this.open = open;
        this.mcpWorkbenchService = mcpWorkbenchService;
        ...
        this.label = ( localize(11992, "Install in {0}", remoteLabel));
        this.update();
    }
    update() {
        this.enabled = false;
...
class InstallingLabelAction extends McpServerAction {
    static {
        this.LABEL = ( localize(11993, "Installing"));
    }
    static {
...
let UninstallAction = class UninstallAction extends McpServerAction {
    ...
    constructor(mcpWorkbenchService) {
        super("extensions.uninstall", ( localize(11994, "Uninstall")), UninstallAction_1.CLASS, false);
        this.mcpWorkbenchService = mcpWorkbenchService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(11994, "Uninstall"));
    }
    async run() {
        if (!this.mcpServer) {
            return;
...
let EnableMcpServerGloballyAction = class EnableMcpServerGloballyAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super(EnableMcpServerGloballyAction_1.ID, ( localize(11995, "Enable")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.tooltip = ( localize(11996, "Enable this MCP server"));
        this.update();
    }
    update() {
        this.enabled = false;
...
EnableMcpServerGloballyAction = EnableMcpServerGloballyAction_1 = ( __decorate([( __param(0, IMcpService))], EnableMcpServerGloballyAction));
let EnableMcpServerForWorkspaceAction = class EnableMcpServerForWorkspaceAction extends McpServerAction {
    ...
    constructor(mcpService, workspaceService) {
        super(EnableMcpServerForWorkspaceAction_1.ID, ( localize(11997, "Enable (Workspace)")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.workspaceService = workspaceService;
        this.tooltip = ( localize(11998, "Enable this MCP server only in this workspace"));
        this.update();
    }
    update() {
        this.enabled = false;
...
EnableMcpServerForWorkspaceAction = EnableMcpServerForWorkspaceAction_1 = ( __decorate([( __param(0, IMcpService)), ( __param(1, IWorkspaceContextService))], EnableMcpServerForWorkspaceAction));
let DisableMcpServerGloballyAction = class DisableMcpServerGloballyAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super(DisableMcpServerGloballyAction_1.ID, ( localize(11999, "Disable")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.tooltip = ( localize(12000, "Disable this MCP server"));
        this.update();
    }
    update() {
        this.enabled = false;
...
DisableMcpServerGloballyAction = DisableMcpServerGloballyAction_1 = ( __decorate([( __param(0, IMcpService))], DisableMcpServerGloballyAction));
let DisableMcpServerForWorkspaceAction = class DisableMcpServerForWorkspaceAction extends McpServerAction {
    ...
    constructor(mcpService, workspaceService) {
        super(DisableMcpServerForWorkspaceAction_1.ID, ( localize(12001, "Disable (Workspace)")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.workspaceService = workspaceService;
        this.tooltip = ( localize(12002, "Disable this MCP server only in this workspace"));
        this.update();
    }
    update() {
        this.enabled = false;
...
DisableMcpServerForWorkspaceAction = DisableMcpServerForWorkspaceAction_1 = ( __decorate([( __param(0, IMcpService)), ( __param(1, IWorkspaceContextService))], DisableMcpServerForWorkspaceAction));
...
EnableMcpDropDownAction = ( __decorate([( __param(0, IInstantiationService))], EnableMcpDropDownAction));
...
DisableMcpDropDownAction = ( __decorate([( __param(0, IInstantiationService))], DisableMcpDropDownAction));
...
let ManageMcpServerAction = class ManageMcpServerAction extends DropDownAction {
    ...
    constructor(isEditorAction, instantiationService) {
        ...
        this.tooltip = ( localize(12003, "Manage"));
        this.update();
    }
    async run() {
        return super.run(
...
let StartServerAction = class StartServerAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.start", ( localize(12004, "Start Server")), StartServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(12004, "Start Server"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let StopServerAction = class StopServerAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.stop", ( localize(12005, "Stop Server")), StopServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(12005, "Stop Server"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let RestartServerAction = class RestartServerAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.restart", ( localize(12006, "Restart Server")), RestartServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(12006, "Restart Server"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let AuthServerAction = class AuthServerAction extends McpServerAction {
    ...
    static {
        this.SIGN_OUT = ( localize(12007, "Sign Out"));
    }
    static {
        this.DISCONNECT = ( localize(12008, "Disconnect Account"));
    }
    constructor(mcpService, _authenticationQueryService, _authenticationService) {
        super("extensions.restart", ( localize(12006, "Restart Server")), RestartServerAction.CLASS, false);
        this.mcpService = mcpService;
...
let ShowServerOutputAction = class ShowServerOutputAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.output", ( localize(12009, "Show Output")), ShowServerOutputAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(12009, "Show Output"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let ShowServerConfigurationAction = class ShowServerConfigurationAction extends McpServerAction {
    ...
    constructor(mcpWorkbenchService) {
        super("extensions.config", ( localize(12010, "Show Configuration")), ShowServerConfigurationAction_1.CLASS, false);
        this.mcpWorkbenchService = mcpWorkbenchService;
        this.update();
    }
    update() {
...
let ShowServerJsonConfigurationAction = class ShowServerJsonConfigurationAction extends McpServerAction {
    ...
    constructor(mcpService, mcpRegistry, editorService) {
        super("extensions.jsonConfig", ( localize(12011, "Show Configuration (JSON)")), ShowServerJsonConfigurationAction_1.CLASS, false);
        this.mcpService = mcpService;
...
let ConfigureModelAccessAction = class ConfigureModelAccessAction extends McpServerAction {
    ...
    constructor(mcpService, commandService) {
        super("extensions.config", ( localize(12012, "Configure Model Access")), ConfigureModelAccessAction_1.CLASS, false);
        this.mcpService = mcpService;
    ...
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(12012, "Configure Model Access"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let ShowSamplingRequestsAction = class ShowSamplingRequestsAction extends McpServerAction {
    ...
    constructor(mcpService, samplingService, editorService) {
        super("extensions.config", ( localize(12013, "Show Sampling Requests")), ShowSamplingRequestsAction_1.CLASS, false);
        this.mcpService = mcpService;
    ...
    async run() {
        ...
        this.editorService.openEditor({
            ...
            label: ( localize(12014, "MCP Sampling: {0}", server.definition.label))
        });
    }
    getServer() {
        if (!this.mcpServer) {
...
let BrowseResourcesAction = class BrowseResourcesAction extends McpServerAction {
    ...
    constructor(mcpService, commandService) {
        super("extensions.config", ( localize(12015, "Browse Resources")), BrowseResourcesAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.commandService = commandService;
        this.update();
    }
...
export { AuthServerAction, BrowseResourcesAction, ButtonWithDropDownExtensionAction, ButtonWithDropdownExtensionActionViewItem, ConfigureModelAccessAction, DisableMcpDropDownAction, DisableMcpServerForWorkspaceAction, DisableMcpServerGloballyAction, DropDownAction, DropDownExtensionActionViewItem, EnableMcpDropDownAction, EnableMcpServerForWorkspaceAction, EnableMcpServerGloballyAction, InstallAction, InstallInRemoteAction, InstallInWorkspaceAction, InstallingLabelAction, ManageMcpServerAction, McpServerAction, McpServerStatusAction, RestartServerAction, ShowSamplingRequestsAction, ShowServerConfigurationAction, ShowServerJsonConfigurationAction, ShowServerOutputAction, StartServerAction, StopServerAction, UninstallAction, getContextMenuActions };
