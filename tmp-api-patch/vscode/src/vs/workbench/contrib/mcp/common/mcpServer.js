...
import { raceCancellationError, Sequencer, AsyncIterableProducer } from '../../../../base/common/async.js';
import { CancellationToken, CancellationTokenSource } from '../../../../base/common/cancellation.js';
import { Iterable } from '../../../../base/common/iterator.js';
import { parseTree, findNodeAtOffset, getNodePath } from '../../../../base/common/json.js';
...
import { StorageTarget } from '../../../../platform/storage/common/storage.js';
...
import { IMcpSandboxService } from './mcpSandboxService.service.js';
import { McpTaskManager } from './mcpTaskManager.js';
import { McpToolName, McpConnectionState, McpConnectionFailedError, McpServerCacheState, McpServerStaticToolAvailability, extensionMcpCollectionPrefix, UserInteractionRequiredError, McpServerTransportType, McpToolVisibility, mcpPromptReplaceSpecialChars, McpCapability, MpcResponseError, ElicitationKind, McpResourceURI } from './mcpTypes.js';
import { IMcpSamplingService, IMcpElicitationService } from './mcpTypes.service.js';
import { MCP } from '../../../../platform/mcp/common/modelContextProtocol.js';
import { UriTemplate } from '../../../../base/common/uriTemplate.js';
import { observableValue, disposableObservableValue } from '../../../../base/common/observableInternal/observables/observableValue.js';
...
McpServerMetadataCache = ( __decorate([( __param(1, IStorageService))], McpServerMetadataCache));
...
class CachedPrimitive {
    constructor(
        ...
        this.fromServer = derived(
            reader => this.fromServerPromise.read(reader)?.promiseResult.read(reader)?.data
        );
...
let McpServer = McpServer_1 = class McpServer extends Disposable {
    ...
    constructor(
        ...
        const workspaces = explicitRoots ? observableValue(this, ( explicitRoots.map(uri => ({
            uri,
            name: basename(uri)
        })))) : observableFromEvent(
            this,
        ...
        this._serverMetadata = ( new CachedPrimitive(this.definition.id, this._primitiveCache, ( staticMetadata.map(
            m => m ? this._toStoredMetadata(m?.serverInfo, m?.instructions) : undefined
        )), entry => ({
            serverName: entry.serverName,
            serverInstructions: entry.serverInstructions,
            serverIcons: entry.serverIcons
        }), entry => ({
            serverName: entry?.serverName,
            serverInstructions: entry?.serverInstructions,
            icons: McpIcons.fromStored(entry?.serverIcons)
        }), undefined));
        const preferredName = derived(
            reader => this._serverMetadata.value.read(reader)?.serverName || this.definition.label
        );
        ...
        })).map((o, reader) => o?.promiseResult.read(reader)?.data)), entry => entry.tools, (entry, reader) => ( entry.map(
            def => this._instantiationService.createInstance(McpTool, this, toolPrefix.read(reader), def)
        ...
        this._capabilities = ( new CachedPrimitive(this.definition.id, this._primitiveCache, ( staticMetadata.map(
            m => m?.capabilities !== undefined ? encodeCapabilities(m.capabilities) : undefined
        )), entry => entry.capabilities, entry => entry, undefined));
    ...
    start(
        ...
        interaction?.participants.set(this.definition.id, {
            s: "unknown"
        });
        return this._connectionSequencer.queue(async () => {
            ...
            let state = await connection.start({
                ...
                elicitationRequestHandler: async (req, token) => {
                    ...
                    const r = await this._elicitationService.elicit(
                        this,
                        Iterable.first(this.runningToolCalls),
                        req,
    ...
    showInteractiveError(cnx, error, debug) {
        ...
        if (error.code === "ENOENT" && cnx.launchDefinition.type === McpServerTransportType.Stdio) {
            ...
            const options = [{
                label: ( localize(12169, "Show Output")),
                run: () => this.showOutput()
            }];
            if (cnx.definition.devMode?.debug?.type === "debugpy" && debug) {
                this._notificationService.prompt(Severity$1.Error, ( localize(
                    ...
                    label: ( localize(12171, "View Docs")),
            ...
            if (docsLink) {
                options.push({
                    label: ( localize(12172, "Install {0}", cnx.launchDefinition.command)),
    ...
    _confirmAndApplySandboxConfigSuggestion(cnx, potentialBlocks, suggestion) {
        ...
        void this._dialogService.confirm({
            ...
            message: ( localize(
                12174,
                "Update sandbox configuration in mcp.json for {0}?",
                cnx.definition.label
            )),
            detail: suggestion.message,
            primaryButton: ( localize(12175, "Yes")),
            cancelButton: ( localize(12176, "No"))
        }).then(async result => {
            if (!result.confirmed) {
                return;
            }
            if (!mcpResource || configTarget === undefined) {
                this._notificationService.warn(( localize(
                    12177,
                    "Couldn't determine where to update sandbox configuration for {0}.",
                    cnx.definition.label
                )));
                return;
            }
            try {
                const updated = await this._mcpSandboxService.applySandboxConfigSuggestion(
                    cnx.definition,
                    mcpResource,
                    configTarget,
                    potentialBlocks,
                    suggestion.sandboxConfig
                );
                if (updated) {
                    this._removePotentialSandboxBlocks(potentialBlocks);
                    this._notificationService.info(( localize(
                        12178,
                        "Updated sandbox configuration for {0} in mcp.json. Restart server.",
                        cnx.definition.label
                    )));
                }
            } catch (e) {
                this._notificationService.error(( localize(
                    12179,
                    "Failed to update sandbox configuration for {0}: {1}",
                    cnx.definition.label,
    ...
    async _getValidatedTools(tools) {
        ...
        for (const [i, result] of validations.entries()) {
            if ("error" in result) {
                error += ( localize(12180, "Tool `{0}` has invalid JSON parameters:", tools[i].name)) + "\n";
    ...
    _setServerMetadata(
        ...
        this._serverMetadata.fromServerPromise.set(ObservablePromise.resolved({
            nonce,
            data: serverMetadata
        }), tx);
        const capabilitiesEncoded = encodeCapabilities(capabilities);
        this._capabilities.fromServerPromise.set(ObservablePromise.resolved({
            data: capabilitiesEncoded,
            nonce
        }), tx);
        this._primitiveCache.store(this.definition.id, {
            ...serverMetadata,
            nonce,
            capabilities: capabilitiesEncoded
        });
    }
    _populateLiveData(handler, cacheNonce, store) {
        ...
        transaction(tx => {
            this._setServerMetadata(cacheNonce, {
                serverInfo: handler.serverInfo,
                instructions: handler.serverInstructions,
...
McpServer = McpServer_1 = ( __decorate([( __param(7, IMcpRegistry)), ( __param(8, IWorkspaceContextService)), ( __param(9, IExtensionService)), ( __param(10, ILoggerService)), ( __param(11, IOutputService)), ( __param(12, ITelemetryService)), ( __param(13, ICommandService)), ( __param(14, IInstantiationService)), ( __param(15, IDialogService)), ( __param(16, INotificationService)), ( __param(17, IOpenerService)), ( __param(18, IMcpSamplingService)), ( __param(19, IMcpElicitationService)), ( __param(20, IMcpSandboxService)), ( __param(21, IWorkbenchEnvironmentService))], McpServer));
class McpPrompt {
    ...
    async resolve(args, token) {
        const result = await McpServer.callOn(this._server, h => h.getPrompt({
        ...
        return McpServer.callOn(this._server, async h => {
            ...
            try {
                const result = await h.callTool({
                    ...
                    _meta: meta
                }, token, progress ? message => progress.report({
                    message
                }) : undefined);
...
McpTool = ( __decorate([( __param(3, IMcpElicitationService))], McpTool));
...
class McpResourceTemplate {
    ...
    async complete(templatePart, prefix, alreadyResolved, token) {
        const result = await McpServer.callOn(this._server, h => h.complete({
            ref: {
                type: "ref/resource",
                uri: this._definition.uriTemplate
            },
            argument: {
                name: templatePart,
                value: prefix
...
export { McpPrefixGenerator, McpServer, McpServerMetadataCache, McpTool };
