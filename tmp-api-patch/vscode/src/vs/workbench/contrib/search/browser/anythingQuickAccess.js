...
import * as anythingQuickAccess from './media/anythingQuickAccess.css';
...
import { IQuickChatService, IChatWidgetService } from '../../chat/browser/chat.service.js';
...
registerCss(anythingQuickAccess);
...
let AnythingQuickAccessProvider = class AnythingQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    static {
        this.NO_RESULTS_PICK = {
            label: ( localize(13841, "No matching results"))
        };
    }
    static {
    ...
    doGetPicks(filter, options, disposables, token) {
        ...
        if (options.additionPicks) {
            for (const pick of options.additionPicks) {
                ...
                const {
                    ...
                    labelMatch,
                    descriptionMatch
                } = scoreItemFuzzy(pick, query, true, quickPickItemScorerAccessor, this.pickState.scorerCache);
                if (!score) {
                    continue;
                }
                pick.highlights = {
        ...
        if (this.pickState.isQuickNavigating) {
            if (picks.length > 0) {
                picks.push({
                    type: "separator",
                    label: ( localize(13842, "recently opened"))
                });
            }
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                let additionalPicks = await this.getAdditionalPicks(
                    ...
                    Boolean(this.configuration?.includeSymbols),
                    token
                );
                if (options.filter) {
                ...
                return additionalPicks.length > 0 ? [{
                    type: "separator",
                    label: this.configuration.includeSymbols ? ( localize(13843, "file and symbol results")) : ( localize(13844, "file results"))
                }, ...additionalPicks] : [];
            })(),
    ...
    async getAdditionalPicks(query, excludes, includeSymbols, token) {
        ...
        const sortedAnythingPicks = top(
            ...
            AnythingQuickAccessProvider_1.MAX_RESULTS
        );
    ...
    getEditorHistoryPicks(query) {
        ...
        return editorHistoryPicks.sort((editorA, editorB) => compareItemsByFuzzyScore(
            editorA,
            editorB,
            query,
            false,
    ...
    async getFilePicks(query, excludes, token) {
        ...
        if (absolutePathResult) {
            ...
            absolutePathPick.highlights = {
                label: [{
                    start: 0,
                    end: absolutePathPick.label.length
                }],
                description: absolutePathPick.description ? [{
                    start: 0,
                    end: absolutePathPick.description.length
                }] : undefined
            };
    ...
    async doFileSearch(query, token) {
        ...
        return [...fileSearchResults.filter(result => !( relativePathFileResultsMap.has(result))), ...relativePathFileResults];
    }
    ...
    doGetFileSearchResults(filePattern, token) {
        ...
        return this.searchService.fileSearch(
            this.fileQueryBuilder.file(this.contextService.getWorkspace().folders, this.getFileQueryOptions({
                filePattern,
    ...
    getHelpPicks(query, token, runOptions) {
        ...
        const providers = this.lazyRegistry.value.getQuickAccessProviders(this.contextKeyService).filter(p => ( p.helpEntries.some(h => h.commandCenterOrder !== undefined))).flatMap(provider => ( provider.helpEntries.filter(h => h.commandCenterOrder !== undefined).map(helpEntry => {
            ...
            return {
                ...
                ariaLabel: ( localize(13845, "{0}, {1}", label, helpEntry.description)),
                accept: () => {
        ...
        if (this.quickChatService.enabled) {
            providers.push({
                label: ( localize(13846, "Open Quick Chat")),
    ...
    async getWorkspaceSymbolPicks(query, includeSymbols, token) {
        if (
        !query.normalized || !includeSymbols || this.pickState.lastRange) {
            return [];
        }
    ...
    async doGetEditorSymbolPicks(activeGlobalPick, activeGlobalResource, filter, disposables, token) {
        ...
        const editorSymbolPicks = (await this.editorSymbolsQuickAccess.getSymbolPicks(model, filter, {
    ...
    createAnythingPick(resourceOrEditor, configuration) {
        ...
        const buttonsValue = ( new Lazy(() => {
            ...
            buttons.push({
                ...
                tooltip: openSideBySideDirection === "right" ? ( localize(13847, "Open to the Side")) : ( localize(13848, "Open to the Bottom"))
            });
            if (isEditorHistoryEntry) {
                buttons.push({
                    iconClass: isDirty ? ("dirty-anything " + ThemeIcon.asClassName(Codicon.circleFilled)) : ThemeIcon.asClassName(Codicon.close),
                    tooltip: ( localize(13849, "Remove from Recently Opened")),
        ...
        return {
            resource,
            label,
            ariaLabel: isDirty ? ( localize(13850, "{0} unsaved changes", labelAndDescription)) : labelAndDescription,
            description,
            iconPath: URI.isUri(icon) ? {
                dark: icon
            } : undefined,
            get iconClasses() {
...
AnythingQuickAccessProvider = AnythingQuickAccessProvider_1 = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, ISearchService)), ( __param(2, IWorkspaceContextService)), ( __param(3, IPathService)), ( __param(4, IWorkbenchEnvironmentService)), ( __param(5, IFileService)), ( __param(6, ILabelService)), ( __param(7, IModelService)), ( __param(8, ILanguageService)), ( __param(9, IWorkingCopyService)), ( __param(10, IConfigurationService)), ( __param(11, IEditorService)), ( __param(12, IHistoryService)), ( __param(13, IFilesConfigurationService)), ( __param(14, ITextModelService)), ( __param(15, IUriIdentityService)), ( __param(16, IQuickInputService)), ( __param(17, IKeybindingService)), ( __param(18, IContextKeyService)), ( __param(19, IQuickChatService)), ( __param(20, ILogService)), ( __param(21, ICustomEditorLabelService)), ( __param(22, IChatWidgetService))], AnythingQuickAccessProvider));
