...
import { isSearchTreeFolderMatch, isSearchTreeFileMatch, isSearchTreeMatch, isTextSearchHeading, isPlainTextSearchHeading, isSearchTreeFolderMatchWorkspaceRoot, isSearchTreeFolderMatchNoRoot } from './searchTreeModel/searchTreeCommon.js';
...
let TextSearchResultRenderer = class TextSearchResultRenderer extends Disposable {
    ...
    renderTemplate(container) {
        ...
        const textSearchResultElement = append(container, $(".textsearchresult"));
        ...
        const actionBarContainer = append(textSearchResultElement, $(".actionBarContainer"));
        ...
        const actions = disposables.add(
            instantiationService.createInstance(MenuWorkbenchToolBar, actionBarContainer, MenuId.SearchActionMenu, {
                menuOptions: {
    ...
    async renderElement(node, index, templateData) {
        if (isPlainTextSearchHeading(node.element)) {
            templateData.label.setLabel(( localize(14063, "Text Results")));
            ...
            const localizedLabel = ( localize(14064, "AI-assisted results"));
...
TextSearchResultRenderer = TextSearchResultRenderer_1 = ( __decorate([( __param(1, IWorkspaceContextService)), ( __param(2, IInstantiationService)), ( __param(3, IContextKeyService))], TextSearchResultRenderer));
let FolderMatchRenderer = class FolderMatchRenderer extends Disposable {
    ...
    renderCompressedElements(node, index, templateData) {
        ...
        if (folder.resource) {
            ...
            templateData.label.setLabel(( localize(14065, "Other files")));
        }
    ...
    renderFolderDetails(folder, templateData) {
        ...
        templateData.badge.setTitleFormat(count > 1 ? ( localize(14066, "{0} files found", count)) : ( localize(14067, "{0} file found", count)));
...
FolderMatchRenderer = FolderMatchRenderer_1 = ( __decorate([( __param(2, IWorkspaceContextService)), ( __param(3, ILabelService)), ( __param(4, IInstantiationService)), ( __param(5, IContextKeyService))], FolderMatchRenderer));
let FileMatchRenderer = class FileMatchRenderer extends Disposable {
    ...
    renderElement(node, index, templateData) {
        ...
        templateData.badge.setCount(count);
        templateData.badge.setTitleFormat(count > 1 ? ( localize(14068, "{0} matches found", count)) : ( localize(14069, "{0} match found", count)));
        templateData.actions.context = {
            viewer: this.searchView.getControl(),
            element: fileMatch
        };
...
FileMatchRenderer = FileMatchRenderer_1 = ( __decorate([( __param(2, IWorkspaceContextService)), ( __param(3, IConfigurationService)), ( __param(4, IInstantiationService)), ( __param(5, IContextKeyService))], FileMatchRenderer));
let MatchRenderer = class MatchRenderer extends Disposable {
    ...
    renderTemplate(container) {
        ...
        const parentHover = disposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), parent, "")
        );
        const lineNumberHover = disposables.add(
            this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), lineNumber, "")
        );
        return {
            parent,
            before,
    ...
    getMatchTitle(match, showLineNumbers) {
        ...
        const numLines = match.range().endLineNumber - match.range().startLineNumber;
        const lineNumStr = showLineNumbers ? ( localize(14070, "From line {0}", startLine, numLines)) + " " : "";
        const numLinesStr = numLines > 0 ? "+ " + ( localize(14071, "{0} more lines", numLines)) : "";
        return lineNumStr + numLinesStr;
    }
};
MatchRenderer = MatchRenderer_1 = ( __decorate([( __param(1, IWorkspaceContextService)), ( __param(2, IConfigurationService)), ( __param(3, IInstantiationService)), ( __param(4, IContextKeyService)), ( __param(5, IHoverService))], MatchRenderer));
    ...
    getAriaLabel(element) {
        if (isSearchTreeFolderMatch(element)) {
            ...
            return element.resource ? ( localize(
                ...
                count,
                element.name()
            )) : ( localize(14074, "{0} matches outside of the workspace, Search result", count));
        }
...
SearchAccessibilityProvider = ( __decorate([( __param(1, ILabelService))], SearchAccessibilityProvider));

export { FileMatchRenderer, FolderMatchRenderer, MatchRenderer, SearchAccessibilityProvider, SearchDelegate, TextSearchResultRenderer };
