...
import { $, append, createTextNode } from '../../../../base/browser/dom.js';
...
const renderSearchMessage = (
    ...
    append(div, $("." + SeverityIcon.className(
        message.type === TextSearchCompleteMessageType.Information ? Severity$1.Info : Severity$1.Warning
    ).split(" ").join(".")));
    for (const node of linkedText.nodes) {
        if (typeof node === "string") {
            append(div, createTextNode(node));
        } else {
            const link = instantiationService.createInstance(Link, div, node, {
                opener: async href => {
                    ...
                    if (parsed.scheme === Schemas.command && message.trusted) {
                        ...
                        if (parsed.scheme === Schemas.command && !message.trusted) {
                            notificationService.error(( localize(14057, "Unable to open command link from untrusted source: {0}", href)));
                        } else {
                            notificationService.error(( localize(14058, "Unable to open unknown link: {0}", href)));
                        }
                    }
                }
            });
