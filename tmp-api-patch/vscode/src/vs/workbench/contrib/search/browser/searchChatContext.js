...
import { VIEW_ID, getExcludes, QueryType } from '../../../services/search/common/search.js';
...
import { picksWithPromiseFn } from '../../chat/browser/attachments/chatContextPickService.js';
import { IChatContextPickService } from '../../chat/browser/attachments/chatContextPickService.service.js';
import { SearchContext } from '../common/constants.js';
import { SearchView } from './searchView.js';
import { basename, dirname, relativePath, joinPath } from '../../../../base/common/resources.js';
...
let SearchChatContextContribution = class SearchChatContextContribution extends Disposable {
    ...
    constructor(instantiationService, chatContextPickService) {
        super();
        this._store.add(
            chatContextPickService.registerChatContextItem(instantiationService.createInstance(SearchViewResultChatContextPick))
        );
        this._store.add(
            chatContextPickService.registerChatContextItem(instantiationService.createInstance(FilesAndFoldersPickerPick))
        );
        this._store.add(chatContextPickService.registerChatContextItem(
            this._store.add(instantiationService.createInstance(SymbolsContextPickerPick))
        ));
    }
};
SearchChatContextContribution = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, IChatContextPickService))], SearchChatContextContribution));
let SearchViewResultChatContextPick = class SearchViewResultChatContextPick {
    constructor(_contextKeyService, _viewsService, _labelService) {
        ...
        this.type = "valuePick";
        this.label = ( localize(14031, "Search Results"));
...
SearchViewResultChatContextPick = ( __decorate([( __param(0, IContextKeyService)), ( __param(1, IViewsService)), ( __param(2, ILabelService))], SearchViewResultChatContextPick));
let SymbolsContextPickerPick = class SymbolsContextPickerPick {
    constructor(_instantiationService) {
        this._instantiationService = _instantiationService;
        this.type = "pickerPick";
        this.label = ( localize(14032, "Symbols..."));
    ...
    asPicker() {
        return {
            placeholder: ( localize(14033, "Select a symbol")),
...
SymbolsContextPickerPick = ( __decorate([( __param(0, IInstantiationService))], SymbolsContextPickerPick));
let FilesAndFoldersPickerPick = class FilesAndFoldersPickerPick {
    constructor(
        ...
        this._instantiationService = _instantiationService;
        this.type = "pickerPick";
        this.label = ( localize(14034, "Files & Folders..."));
        this.icon = Codicon.folder;
        this.ordinal = 600;
    }
    asPicker() {
        return {
            placeholder: ( localize(14035, "Search file or folder by name")),
            picks: picksWithPromiseFn(async (value, token) => {
                ...
                this._historyService.getHistory().filter(
                    a => a.resource && this._instantiationService.invokeFunction(accessor => isSupportedChatFileScheme(accessor, a.resource.scheme))
                ).slice(0, 30).forEach(
                    uri => defaultItems.push(this._createPickItem(uri.resource, FileKind.FILE))
                );
                if (value === "") {
                    return defaultItems;
                }
    ...
    _createPickItem(resource, kind) {
        return {
            ...
            description: this._labelService.getUriLabel(dirname(resource), {
                relative: true
            }),
...
FilesAndFoldersPickerPick = ( __decorate([( __param(0, ISearchService)), ( __param(1, ILabelService)), ( __param(2, IModelService)), ( __param(3, ILanguageService)), ( __param(4, IConfigurationService)), ( __param(5, IWorkspaceContextService)), ( __param(6, IFileService)), ( __param(7, IHistoryService)), ( __param(8, IInstantiationService))], FilesAndFoldersPickerPick));
async function searchFilesAndFolders(
    ...
    const segmentMatchPattern = fuzzyMatchingGlobPattern(pattern) ;
    const searchExcludePattern = getExcludes(configurationService.getValue({
        resource: workspace
    })) || {};
    const searchOptions = {
        folderQueries: [{
...
export { SearchChatContextContribution, getTopLevelFolders, searchFilesAndFolders };
