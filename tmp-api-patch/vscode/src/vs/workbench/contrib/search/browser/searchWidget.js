...
let SearchWidget = class SearchWidget extends Widget {
    ...
    static {
        this.REPLACE_ALL_DISABLED_LABEL = ( localize(14135, "Replace All (Submit Search to Enable)"));
    }
    static {
        this.REPLACE_ALL_ENABLED_LABEL = keyBindingService2 => {
            return keyBindingService2.appendKeybinding(( localize(14136, "Replace All")), ReplaceAllAction.ID);
        };
    }
    constructor(
        container,
    ...
    _updateSearchInputAriaLabel(includeHint) {
        ...
        let searchLabel = ( localize(14137, "Search: Type Search Term and press Enter to search"));
        if (includeHint && !this._accessibilityHelpHintAnnounced && this.configurationService.getValue("accessibility.verbosity.find") && this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (keybinding) {
                searchLabel += ", " + ( localize(14138, "Press {0} for accessibility help", keybinding));
                this._accessibilityHelpHintAnnounced = true;
                this._labelResetTimeout?.dispose();
                this._labelResetTimeout = disposableTimeout(() => {
                    if (this.searchInput) {
                        this.searchInput.inputBox.setAriaLabel(( localize(14137, "Search: Type Search Term and press Enter to search")));
                    }
                }, 1000);
            }
        }
    ...
    renderToggleReplaceButton(parent) {
        const opts = {
            ...
            title: ( localize(14139, "Toggle Replace")),
    ...
    renderSearchInput(parent, options) {
        ...
        const inputOptions = {
            label: ( localize(14137, "Search: Type Search Term and press Enter to search")),
            validation: value => this.validateSearchInput(value),
            placeholder: ( localize(14140, "Search")),
            appendCaseSensitiveLabel: this.keybindingService.appendKeybinding("", SearchCommandIds.ToggleCaseSensitiveCommandId),
            appendWholeWordsLabel: this.keybindingService.appendKeybinding("", SearchCommandIds.ToggleWholeWordCommandId),
            appendRegexLabel: this.keybindingService.appendKeybinding("", SearchCommandIds.ToggleRegexCommandId),
        ...
        this._register(
            this.searchInput.onCaseSensitiveKeyDown(keyboardEvent => this.onCaseSensitiveKeyDown(keyboardEvent))
        );
        this._register(
            this.searchInput.onRegexKeyDown(keyboardEvent => this.onRegexKeyDown(keyboardEvent))
        );
        this._register(this.searchInput.inputBox.onDidChange(() => this.onSearchInputChanged()));
        this._register(
            this.searchInput.inputBox.onDidHeightChange(() => this._onDidHeightChange.fire())
        );
        ...
        this._register(
            this.searchInputFocusTracker.onDidBlur(() => this.searchInputBoxFocused.set(false))
        );
        this.showContextToggle = ( new Toggle({
            isChecked: false,
            title: this.keybindingService.appendKeybinding(( localize(14141, "Toggle Context Lines")), ToggleSearchEditorContextLinesCommandId),
    ...
    renderReplaceInput(parent, options) {
        ...
        this.replaceInput = this._register(( new ContextScopedReplaceInput(replaceBox, this.contextViewService, {
            label: ( localize(14142, "Replace: Type replace term and press Enter to preview")),
            placeholder: ( localize(14143, "Replace")),
            appendPreserveCaseLabel: this.keybindingService.appendKeybinding("", SearchCommandIds.TogglePreserveCaseId),
        ...
        this._register(
            this.replaceInput.inputBox.onDidChange(() => this._onReplaceValueChanged.fire())
        );
        this._register(
            this.replaceInput.inputBox.onDidHeightChange(() => this._onDidHeightChange.fire())
        );
        ...
        this._register(
            this.replaceInputFocusTracker.onDidFocus(() => this.replaceInputBoxFocused.set(true))
        );
        this._register(
            this.replaceInputFocusTracker.onDidBlur(() => this.replaceInputBoxFocused.set(false))
        );
        this._register(
            this.replaceInput.onPreserveCaseKeyDown(keyboardEvent => this.onPreserveCaseKeyDown(keyboardEvent))
        );
    }
    triggerReplaceAll() {
        ...
        if (this.searchConfiguration.searchOnType) {
            if (this.searchInput?.getRegex()) {
                try {
                    ...
                    const delayMultiplier = matchienessHeuristic < 50 ? 1 : matchienessHeuristic < 100 ?
                    5 : 10;
                    this.submitSearch(
                        true,
                        this.searchConfiguration.searchOnTypeDebouncePeriod * delayMultiplier
                    );
                } catch {}
            } else {
    ...
    onCaseSensitiveKeyDown(keyboardEvent) {
        if (keyboardEvent.equals(KeyMod$1.Shift | KeyCode.Tab)) {
            if (this.isReplaceShown()) {
                this.replaceInput?.focus();
                keyboardEvent.preventDefault();
            }
    ...
    onPreserveCaseKeyDown(keyboardEvent) {
        if (keyboardEvent.equals(KeyCode.Tab)) {
            ...
            keyboardEvent.preventDefault();
        } else if (keyboardEvent.equals(KeyMod$1.Shift | KeyCode.Tab)) {
...
SearchWidget = SearchWidget_1 = ( __decorate([( __param(2, IContextViewService)), ( __param(3, IContextKeyService)), ( __param(4, IKeybindingService)), ( __param(5, IClipboardService)), ( __param(6, IConfigurationService)), ( __param(7, IAccessibilityService)), ( __param(8, IContextMenuService)), ( __param(9, IInstantiationService)), ( __param(10, IEditorService))], SearchWidget));
function registerContributions() {
    KeybindingsRegistry.registerCommandAndKeybindingRule({
        ...
        primary: KeyMod$1.Alt | KeyMod$1.CtrlCmd | KeyCode.Enter,
        handler: accessor => {
