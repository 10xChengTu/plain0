...
let ExtensionsListView = class ExtensionsListView extends AbstractExtensionsListView {
        ...
        try {
            ...
            if (localExtensions.length) {
                ...
                return {
                    ...
                    disposables: ( new DisposableStore()),
                    message: {
                        text: ( localize(10734, "{0} Showing local extensions.", message.text)),
        ...
        if (this.bodyTemplate) {
            ...
            if (this.isBodyVisible()) {
                if (message) {
                    ...
                    this.bodyTemplate.messageBox.textContent = ( localize(10735, "No extensions found."));
                }
    ...
    getMessage(error) {
        if (this.isOfflineError(error)) {
            ...
            return {
                text: ( localize(10737, "Error while fetching extensions. {0}", getErrorMessage(error))),
    ...
    async installWorkspaceRecommendations() {
        ...
        if (installableRecommendations.length) {
            ...
            this.notificationService.notify({
                severity: Severity$1.Info,
                message: ( localize(10738, "There are no extensions to install."))
            });
        }
    }
}
