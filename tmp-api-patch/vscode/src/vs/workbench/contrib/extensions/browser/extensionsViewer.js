...
import { Disposable, MutableDisposable, DisposableStore, isDisposable, toDisposable, dispose } from '../../../../base/common/lifecycle.js';
...
function getAriaLabelForExtension(extension) {
    ...
    const publisher = extension.publisherDomain?.verified ? ( localize(10678, "Verified Publisher {0}", extension.publisherDisplayName)) : ( localize(10679, "Publisher {0}", extension.publisherDisplayName));
    const deprecated = extension?.deprecationInfo ? ( localize(10680, "Deprecated")) : "";
        ...
        this.list = instantiationService.createInstance(
            ...
            [renderer],
            {
                ...
                accessibilityProvider: {
                    ...
                    getWidgetAriaLabel() {
                        return localize(10682, "Extensions");
                    }
                },
    ...
    openExtension(extension, options) {
        ...
        this.extensionsWorkbenchService.open(extension, {
            ...
            modal: options.sideByside ? undefined : buildModalNavigationForPagedList(
                ...
                (ext, modal) => this.extensionsWorkbenchService.open(ext, {
                    pinned: false,
                    modal
                }),
    ...
    async onContextMenu(e) {
        if (e.element) {
            ...
            const extension = e.element ? this.extensionsWorkbenchService.local.find(
                local => areSameExtensions(local.identifier, e.element.identifier) && (!e.element.server || e.element.server === local.server)
            ) || e.element : e.element;
...
ExtensionsList = ( __decorate([( __param(4, IExtensionsWorkbenchService)), ( __param(5, IViewDescriptorService)), ( __param(6, IWorkbenchLayoutService)), ( __param(7, INotificationService)), ( __param(8, IContextMenuService)), ( __param(9, IContextKeyService)), ( __param(10, IInstantiationService)), ( __param(11, ILogService))], ExtensionsList));
let ExtensionsGridView = class ExtensionsGridView extends Disposable {
    constructor(parent, delegate, instantiationService) {
        ...
        this.renderer = this.instantiationService.createInstance(Renderer, {
            onFocus: Event.None,
            onBlur: Event.None,
            filters: {}
        }, {
    ...
    renderExtension(extension, index) {
        ...
        this.disposableStore.add(
            addDisposableListener(template.name, EventType.CLICK, e => handleEvent(( new StandardMouseEvent(getWindow(template.name), e))))
        );
        this.disposableStore.add(
            addDisposableListener(template.name, EventType.KEY_DOWN, e => handleEvent(( new StandardKeyboardEvent(e))))
        );
        this.disposableStore.add(addDisposableListener(
            extensionContainer,
            EventType.KEY_DOWN,
...
ExtensionsGridView = ( __decorate([( __param(2, IInstantiationService))], ExtensionsGridView));
...
ExtensionRenderer = ExtensionRenderer_1 = ( __decorate([( __param(0, IInstantiationService))], ExtensionRenderer));
class UnknownExtensionRenderer {
    ...
    renderTemplate(container) {
        ...
        append(messageContainer, $("span.error-marker")).textContent = ( localize(10683, "Error"));
        append(messageContainer, $("span.message")).textContent = ( localize(10684, "Unknown Extension:"));
...
OpenExtensionAction = ( __decorate([( __param(0, IExtensionsWorkbenchService))], OpenExtensionAction));
    ...
    constructor(
        ...
        this.disposables.add(this.onDidChangeSelection(event => {
            if (isKeyboardEvent(event.browserEvent)) {
                extensionsWorkdbenchService.open(event.elements[0].extension, {
                    sideByside: false
                });
            }
        }));
...
ExtensionsTree = ( __decorate([( __param(3, IContextKeyService)), ( __param(4, IListService)), ( __param(5, IInstantiationService)), ( __param(6, IConfigurationService)), ( __param(7, IExtensionsWorkbenchService))], ExtensionsTree));
class ExtensionData {
...
async function getExtensions(extensions, extensionsWorkbenchService) {
    ...
    if (toQuery.length) {
        const galleryResult = await extensionsWorkbenchService.getExtensions(( toQuery.map(id => ({
            id
...
export { ExtensionData, ExtensionsGridView, ExtensionsList, ExtensionsTree, buildModalNavigationForPagedList, getExtensions };
