...
import { ThrottledDelayer, Promises, disposableTimeout, createCancelablePromise } from '../../../../base/common/async.js';
...
import { isUndefined, isDefined, isString } from '../../../../base/common/types.js';
...
import { ExtensionGalleryServiceUrlConfigKey, getExtensionGalleryManifestResourceUri, ExtensionGalleryResourceType } from '../../../../platform/extensionManagement/common/extensionGalleryManifest.js';
...
import { ExtensionState, HasOutdatedExtensionsContext, AutoUpdateConfigurationKey, AutoUpdateDelayConfigurationKey, AutoCheckUpdatesConfigurationKey, AutoRestartConfigurationKey, ExtensionRuntimeActionType, VIEWLET_ID } from '../common/extensions.js';
        ...
        if (this.gallery) {
            ...
            if (!cache) {
                if (this.gallery.assets.manifest) {
                    ...
                    this.logService.error(( localize(10768, "Manifest is not found")), this.identifier.id);
                }
            }
            return cache;
        }
...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends Disposable {
    ...
    async initialize() {
        ...
        this.extensionGalleryManifestService.getExtensionGalleryManifest().then(manifest => {
            ...
            this._register(
                this.extensionGalleryManifestService.onDidChangeExtensionGalleryManifest(manifest => this.updateExtensionGalleryManifest(manifest))
            );
        }).catch(
            e => this.logService.error("Error while fetching extension gallery manifest", e)
        );
    ...
    async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled) {
        ...
        const result = await this.dialogService.confirm({
            title: ( localize(10769, "Auto Update Extensions")),
            message: isAutoUpdateEnabled ? ( localize(10770, "Do you want to enable auto update for extensions?")) : ( localize(10771, "Do you want to disable auto update for extensions?")),
            detail: ( localize(
                10772,
    ...
    updateExtensionsPinnedState(pinned) {
        return this.progressService.withProgress({
            ...
            title: ( localize(10773, "Updating Extensions Auto Update State"))
    ...
    computeExtensionsNotifications() {
        ...
        if (invalidExtensions.length) {
            if (( invalidExtensions.some(e => e.local && e.local.manifest.engines?.vscode && !isEngineValid(
                ...
                computedNotificiations.push({
                    message: ( localize(10777, "Invalid extensions detected. Review them.")),
                    severity: Severity$1.Warning,
                    extensions: invalidExtensions,
        ...
        if (!this.configurationService.getValue(AutoRestartConfigurationKey)) {
            ...
            if (restartRequiredExtensions.length) {
                ...
                computedNotificiations.push({
                    message: needsReload ? ( localize(10778, "Extensions require a window reload to apply updates.")) : ( localize(10779, "All extensions require a restart to apply updates.")),
                    severity: Severity$1.Info,
                    extensions: restartRequiredExtensions,
                    query: "@restartrequired",
                    action: {
                        label: needsReload ? ( localize(10780, "Reload Window")) : ( localize(10781, "Restart Extensions")),
                        run: () => {
                            if (needsReload) {
        ...
        if (privateMarketplaceUrl) {
            ...
            let linkUri = this.extensionGalleryManifest ? getExtensionGalleryManifestResourceUri(
    ...
    async updateRunningExtensions(
        message = ( localize(10784, "Changing extension enablement")),
        auto = false
    ) {
        const toAdd = [];
        const toRemove = [];
            ...
            if (await this.extensionService.stopExtensionHosts(message, auto)) {
                ...
                if (auto) {
                    this.notificationService.notify({
                        severity: Severity$1.Info,
                        message: ( localize(10785, "Extensions were auto restarted to enable updates.")),
    ...
    getRuntimeState(extension) {
        ...
        const reloadActionLabel = reloadAction === ExtensionRuntimeActionType.ReloadWindow ? ( localize(10786, "reload window")) : ( localize(10787, "restart extensions"));
        if (isUninstalled) {
                ...
                if (isEnabled) {
                    ...
                    if (isSameExtensionRunning) {
                        if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
                            ...
                            return {
                                action: reloadAction,
                                reason: ( localize(10792, "Please {0} to enable the updated extension.", reloadActionLabel))
                            };
                        }
                        if (this.extensionsServers.length > 1) {
                            ...
                            if (extensionInOtherServer) {
                                if (runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.localExtensionManagementServer) {
                                    return {
                                        action: reloadAction,
                                        reason: ( localize(10793, "Please {0} to enable this extension locally.", reloadActionLabel))
                                    };
                                }
                        ...
                        if (extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                            if (this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest)) {
                                return {
                                    action: reloadAction,
                                    reason: ( localize(10795, "Please {0} to enable this extension.", reloadActionLabel))
                                };
                            }
                        }
                    ...
                    if (isSameExtensionRunning && !runningExtension.isUnderDevelopment) {
                        return {
                            action: reloadAction,
                            reason: ( localize(10796, "Please {0} to disable this extension.", reloadActionLabel))
                        };
                    }
                }
                return undefined;
    ...
    async downloadVSIX(extensionId, versionKind) {
        ...
        if (!galleryExtension) {
            throw ( new Error(( localize(10797, "Extension '{0}' not found.", extensionId))));
        }
        ...
        for (const targetPlatform of version?.targetPlatforms ?? galleryExtension.allTargetPlatforms) {
            if (targetPlatform !== TargetPlatform.UNKNOWN && targetPlatform !== TargetPlatform.UNIVERSAL) {
                options.push({
                    label: targetPlatform === TargetPlatform.UNDEFINED ? ( localize(10798, "All Platforms")) : TargetPlatformToString(targetPlatform),
                    id: targetPlatform
                });
            }
        }
        ...
        const result = await this.fileDialogService.showOpenDialog({
            title: ( localize(10800, "Select folder to download the VSIX")),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: ( localize(10801, "Download"))
        });
        if (!result?.[0]) {
            return;
        }
        this.progressService.withProgress({
            ...
            try {
                progress.report({
                    message: ( localize(10802, "Downloading VSIX..."))
                });
                ...
                this.notificationService.info(( localize(10803, "Successfully downloaded the VSIX")));
            } catch (error) {
                this.notificationService.error(( localize(10804, "Error while downloading the VSIX: {0}", getErrorMessage(error))));
            }
        });
    }
    async pickVersionToDownload(extensionId) {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(( localize(10805, "This extension has no other versions.")));
            return;
        }
        const picks = ( allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(( new Date(Date.parse(v.date))), true)}${v.isPreReleaseVersion ? ` (${( localize(10806, "pre-release"))})` : ""}`,
                ariaLabel: `${v.isPreReleaseVersion ? "Pre-Release version" : "Release version"} ${v.version}`,
                data: v
            };
        }));
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(10807, "Select Version to Download")),
            matchOnDetail: true
        });
        return pick?.data;
    }
    ...
    async canInstall(extension) {
        if (!(extension instanceof Extension)) {
            return ( new MarkdownString()).appendText(( localize(10810, "The provided object is not an extension.")));
        }
        if (extension.isMalicious) {
            return ( new MarkdownString()).appendText(( localize(10811, "This extension is reported to be problematic.")));
        }
        if (extension.deprecationInfo?.disallowInstall) {
            return ( new MarkdownString()).appendText(( localize(10812, "This extension is disallowed to be installed.")));
        }
        if (extension.gallery) {
            if (!extension.gallery.isSigned && shouldRequireRepositorySignatureFor(
                ...
                return ( new MarkdownString()).appendText(( localize(10813, "This extension is not signed.")));
            }
    ...
    async install(arg, installOptions = {}, progressLocation) {
        ...
        if (!extension) {
            throw ( new Error(( localize(10815, "Unable to install extension"))));
        }
        if (installOptions.enable) {
            if (extension.enablementState === EnablementState.DisabledWorkspace || extension.enablementState === EnablementState.DisabledGlobally) {
                if (installOptions.justification) {
                    const result = await this.dialogService.confirm({
                        title: ( localize(10816, "Enable Extension")),
                        message: ( localize(10817, "Would you like to enable '{0}' extension?", extension.displayName)),
                        detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                        primaryButton: isString(installOptions.justification) ? ( localize(10818, "&&Enable Extension")) : ( localize(10819, "&&Enable Extension and {0}", installOptions.justification.action))
                    });
                    if (!result.confirmed) {
    ...
    async _install(arg, installOptions = {}, progressLocation) {
        ...
        if (arg instanceof URI) {
            ...
            if (extension?.isMalicious) {
                throw ( new Error(( localize(10811, "This extension is reported to be problematic."))));
            }
            if (gallery) {
        ...
        if (installable) {
            if (installOptions.justification) {
                ...
                const buttons = [];
                buttons.push({
                    label: isString(installOptions.justification) || !installOptions.justification.action ? ( localize(10823, "&&Install Extension")) : ( localize(10824, "&&Install Extension and {0}", installOptions.justification.action)),
                    run: () => true
                });
                if (!extension) {
                    buttons.push({
                        label: ( localize(10825, "Open Extension")),
                        run: () => {
                            this.open(extension);
                            return false;
                        }
                ...
                const result = await this.dialogService.prompt({
                    title: ( localize(10826, "Install Extension")),
                    message: extension ? ( localize(
                        10827,
                        "Would you like to install '{0}' extension from '{1}'?",
                        extension.displayName,
                        extension.publisherDisplayName
                    )) : ( localize(10828, "Would you like to install the extension?")),
                    detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                    cancelButton: true,
                    buttons,
                    checkbox: syncCheck ? {
                        label: ( localize(10829, "Sync this extension")),
                        checked: true
                    } : undefined
                });
                if (!result.result) {
    ...
    async uninstall(e) {
        ...
        if (extension.local.isApplicationScoped && this.userDataProfilesService.profiles.length > 1) {
            const {
                ...
                title: ( localize(10831, "Uninstall Extension")),
                ...
                primaryButton: ( localize(10833, "Uninstall (All Profiles)"))
            });
            if (!confirmed) {
                throw ( new CancellationError());
            }
        ...
        return this.withProgress({
            location: ProgressLocation.Extensions,
            title: ( localize(10836, "Uninstalling extension...")),
    ...
    doInstall(extension, installTask, progressLocation) {
        const title = extension ? ( localize(10840, "Installing '{0}' extension...", extension.displayName)) : ( localize(10841, "Installing extension..."));
        return this.withProgress({
