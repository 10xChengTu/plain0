...
let InstallExtensionQuickAccessProvider = class InstallExtensionQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    _getPicks(filter, disposables, token) {
        if (!filter) {
            return [{
                label: ( localize(10674, "Type an extension name to install or search."))
            }];
        }
        const genericSearchPickItem = {
            label: ( localize(10675, "Press Enter to search for extension '{0}'.", filter)),
    ...
    async getPicksForExtensionId(filter, fallback, token) {
        try {
            ...
            return [{
                label: ( localize(10676, "Press Enter to install extension '{0}'.", filter)),
...
InstallExtensionQuickAccessProvider = InstallExtensionQuickAccessProvider_1 = ( __decorate([( __param(0, IExtensionsWorkbenchService)), ( __param(1, IExtensionGalleryService)), ( __param(2, IExtensionManagementService)), ( __param(3, INotificationService)), ( __param(4, ILogService))], InstallExtensionQuickAccessProvider));
let ManageExtensionsQuickAccessProvider = class ManageExtensionsQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    _getPicks() {
        return [{
            label: ( localize(10677, "Press Enter to manage your extensions.")),
            accept: () => this.extensionsWorkbenchService.openSearch("")
        }];
    }
};
ManageExtensionsQuickAccessProvider = ManageExtensionsQuickAccessProvider_1 = ( __decorate([( __param(0, IExtensionsWorkbenchService))], ManageExtensionsQuickAccessProvider));
