...
import { errorIcon, warningIcon, infoIcon } from './extensionsIcons.js';
...
import { chartAxis, chartGuide, chartLine } from '../../../../platform/theme/common/colors/miscColors.js';
...
let RuntimeStatusMarkdownRenderer = class RuntimeStatusMarkdownRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        disposables.add(
            this.extensionFeaturesManagementService.onDidChangeAccessData(e => emitter.fire(this.createElement(manifest, disposables)))
        );
        return {
    ...
    createElement(manifest, disposables) {
        ...
        if (( this.extensionService.extensions.some(extension => ExtensionIdentifier.equals(extension.identifier, extensionId)))) {
            ...
            data.appendMarkdown(`### ${( localize(10297, "Activation"))}\n\n`);
        ...
        if (status.runtimeErrors.length || status.messages.length) {
            ...
            if (status.runtimeErrors.length) {
                data.appendMarkdown(`\n ### ${( localize(10300, "Uncaught Errors ({0})", status.runtimeErrors.length))}\n`);
                for (const error of status.runtimeErrors) {
                    data.appendMarkdown(`$(${Codicon.error.id})&nbsp;${getErrorMessage(error)}\n\n`);
                }
            }
            if (status.messages.length) {
                data.appendMarkdown(`\n ### ${( localize(10301, "Messages ({0})", status.messages.length))}\n`);
                for (const message of status.messages) {
                    data.appendMarkdown(
                        `$(${(message.type === Severity$1.Error ? Codicon.error : message.type === Severity$1.Warning ? Codicon.warning : Codicon.info).id})&nbsp;${message.message}\n\n`
                    );
                }
            }
            if (data.value) {
    ...
    renderMarkdown(markdown, container, disposables) {
        const {
            element
        } = disposables.add(this.markdownRendererService.render({
    ...
    renderRequestsChart(container, accessTimes, disposables) {
        ...
        for (const accessTime of accessTimes) {
            const day = `${accessTime.getDate()} ${accessTime.toLocaleString("default", {
    month: "short"
})}`;
...
RuntimeStatusMarkdownRenderer = ( __decorate([( __param(0, IExtensionService)), ( __param(1, IHoverService)), ( __param(2, IExtensionFeaturesManagementService)), ( __param(3, IMarkdownRendererService))], RuntimeStatusMarkdownRenderer));
const runtimeStatusFeature = {
    id: RuntimeStatusMarkdownRenderer.ID,
    label: ( localize(10302, "Runtime Status")),
    access: {
        canToggle: false
    },
...
let ExtensionFeaturesTab = class ExtensionFeaturesTab extends Themable {
    ...
    create() {
        ...
        if (features.length === 0) {
            append($(".no-features"), this.domNode).textContent = ( localize(10303, "No features contributed."));
            return;
        }
        ...
        const list = this.instantiationService.createInstance(
            ...
            [renderer],
            {
                ...
                accessibilityProvider: {
                    ...
                    getWidgetAriaLabel() {
                        return localize(10304, "Extension Features");
                    }
                },
...
ExtensionFeaturesTab = ( __decorate([( __param(2, IThemeService)), ( __param(3, IInstantiationService))], ExtensionFeaturesTab));
...
let ExtensionFeatureItemRenderer = class ExtensionFeatureItemRenderer {
    ...
    renderTemplate(container) {
        ...
        disabledElement.textContent = ( localize(10305, "No Access"));
    ...
    renderElement(element, index, templateData) {
        ...
        templateData.disposables.add(this.extensionFeaturesManagementService.onDidChangeEnablement((
            {
                extension,
                featureId,
                enabled
        ...
        updateStatus();
        templateData.disposables.add(this.extensionFeaturesManagementService.onDidChangeAccessData((
            {
                extension,
                featureId
            }
...
ExtensionFeatureItemRenderer = ( __decorate([( __param(1, IExtensionFeaturesManagementService))], ExtensionFeatureItemRenderer));
let ExtensionFeatureView = class ExtensionFeatureView extends Disposable {
    ...
    create(content) {
        ...
        if (this.feature.access.canToggle) {
            ...
            this._register(this.extensionFeaturesManagementService.onDidChangeEnablement((
                {
                    extension,
                    featureId
                }
            ...
            this._register(button.onDidClick(async () => {
                ...
                const confirmationResult = await this.dialogService.confirm({
                    title: ( localize(10306, "Enable '{0}' Feature", this.feature.label)),
                    ...
                    custom: true,
                    primaryButton: enabled ? ( localize(10309, "Revoke Access")) : ( localize(10310, "Allow Access")),
                    cancelButton: ( localize(10311, "Cancel"))
                });
        ...
        if (accessData?.current?.status) {
            append(
                bodyContent,
                $(".feature-status", undefined, $(`span${ThemeIcon.asCSSSelector(
    accessData.current.status.severity === Severity$1.Error ? errorIcon : accessData.current.status.severity === Severity$1.Warning ? warningIcon : infoIcon
)}`, undefined), $("span", undefined, accessData.current.status.message))
            );
        }
    ...
    updateButtonLabel(button) {
        button.label = this.extensionFeaturesManagementService.isEnabled(this.extensionId, this.feature.id) ? ( localize(10309, "Revoke Access")) : ( localize(10312, "Allow Access"));
    }
    ...
    renderMarkdown(markdown, container) {
        const {
            element
        } = this._register(this.markdownRendererService.render({
            value: markdown.value,
            isTrusted: markdown.isTrusted,
            supportThemeIcons: true
        }));
...
ExtensionFeatureView = ( __decorate([( __param(3, IInstantiationService)), ( __param(4, IExtensionFeaturesManagementService)), ( __param(5, IDialogService)), ( __param(6, IMarkdownRendererService))], ExtensionFeatureView));
