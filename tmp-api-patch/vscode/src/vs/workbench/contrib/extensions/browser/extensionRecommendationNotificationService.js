...
import { raceCancellablePromises, createCancelablePromise, Promises, raceCancellation, timeout } from '../../../../base/common/async.js';
...
import { RecommendationsNotificationResult, RecommendationSourceToString, RecommendationSource } from '../../../../platform/extensionRecommendations/common/extensionRecommendations.js';
...
import { IWorkbenchExtensionManagementService, IWorkbenchExtensionEnablementService } from '../../../services/extensionManagement/common/extensionManagement.service.js';
    ...
    show() {
        if (!this.notificationHandle) {
            this.updateNotificationHandle(
                this.notificationService.prompt(this.severity, this.message, this.choices, {
                    sticky: true,
...
let ExtensionRecommendationNotificationService = class ExtensionRecommendationNotificationService extends Disposable {
    get ignoredRecommendations() {
        return distinct(( [...JSON.parse(this.storageService.get(
    ...
    async promptImportantExtensionsInstallNotification(extensionRecommendations) {
        ...
        return this.promptRecommendationsNotification({
            ...
            onDidInstallRecommendedExtensions: extensions => extensions.forEach(
            ...
            onDidShowRecommendedExtensions: extensions => extensions.forEach(
                extension => this.telemetryService.publicLog2("extensionRecommendations:popup", {
                    userReaction: "show",
                    extensionId: extension.identifier.id,
                    source: RecommendationSourceToString(extensionRecommendations.source)
            ...
            onDidCancelRecommendedExtensions: extensions => extensions.forEach(
                extension => this.telemetryService.publicLog2("extensionRecommendations:popup", {
            ...
            onDidNeverShowRecommendedExtensionsAgain: extensions => {
                ...
                this.notificationService.prompt(Severity$1.Info, ( localize(10313, "Do you want to ignore all extension recommendations?")), [{
                    label: ( localize(10314, "Yes, Ignore All")),
                    run: () => this.setIgnoreRecommendationsConfig(true)
                }, {
                    label: ( localize(10315, "No")),
    ...
    async promptWorkspaceRecommendations(recommendations) {
        ...
        recommendations = recommendations.filter(
            recommendation => installed.every(local => isString(recommendation) ? !areSameExtensions({
                id: recommendation
            }, local.identifier) : !this.uriIdentityService.extUri.isEqual(recommendation, local.location))
        );
        ...
        await this.promptRecommendationsNotification({
            ...
            name: ( localize(10316, "this repository"))
        }, {
    ...
    async promptRecommendationsNotification(
        ...
        if (extensions.length === 1) {
            ...
            const publishers = [...extensions.reduce(
                (result, extension) => result.add(extension.publisherDisplayName),
                ( new Set())
            )];
            if (publishers.length > 2) {
                extensionsMessage = ( localize(10318, "extensions from {0}, {1} and others", publishers[0], publishers[1]));
            } else if (publishers.length === 2) {
                extensionsMessage = ( localize(10319, "extensions from {0} and {1}", publishers[0], publishers[1]));
            } else {
                extensionsMessage = ( localize(10320, "extensions from {0}", publishers[0]));
            }
        }
        ...
        const donotShowAgainLabel = source === RecommendationSource.WORKSPACE ? ( localize(10323, "Don't Show Again for this Repository")) : extensions.length > 1 ? ( localize(10324, "Don't Show Again for these Extensions")) : ( localize(10325, "Don't Show Again for this Extension"));
        return raceCancellablePromises([this._registerP(this.showRecommendationsNotification(
            extensions,
            message,
            searchValue,
            donotShowAgainLabel,
    ...
    showRecommendationsNotification(
        ...
        return createCancelablePromise(async token => {
            ...
            const installExtensions = async isMachineScoped => {
                ...
                await Promises.settled([Promises.settled(( extensions.map(extension => this.extensionsWorkbenchService.open(extension, {
                    pinned: true
                })))), galleryExtensions.length ? this.extensionManagementService.installGalleryExtensions(( galleryExtensions.map(e => ({
                    extension: e,
                    options: {
                        isMachineScoped
                    }
                })))) : Promise.resolve(), resourceExtensions.length ? Promise.allSettled(( resourceExtensions.map(r => this.extensionsWorkbenchService.install(r)))) : Promise.resolve()]);
            };
            choices.push({
                label: ( localize(10326, "Install")),
                ...
                menu: this.userDataSyncEnablementService.isEnabled() && this.userDataSyncEnablementService.isResourceEnabled(SyncResource.Extensions) ? [{
                    label: ( localize(10327, "Install (Do not sync)")),
                    run: () => installExtensions(true)
                }] : undefined
            });
            choices.push(...[{
                label: ( localize(10328, "Show Recommendations")),
                run: async () => {
            ...
            try {
                accepted = await this.doShowRecommendationsNotification(Severity$1.Info, message, choices, source, token);
            } catch (error) {
    ...
    async doShowRecommendationsNotification(severity, message, choices, source, token) {
        ...
        try {
            ...
            if (this.visibleNotification) {
                ...
                disposables.add(
                    token.onCancellationRequested(() => this.pendingNotificaitons.splice(index, 1))
                );
            ...
            await raceCancellation(( new Promise(
                c => disposables.add(Event.once(recommendationsNotification.onDidClose)(c))
            )), token);
