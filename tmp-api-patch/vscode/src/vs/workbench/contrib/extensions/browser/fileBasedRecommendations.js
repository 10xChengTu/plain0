...
let FileBasedRecommendations = class FileBasedRecommendations extends ExtensionRecommendations {
    ...
    promptImportantRecommendations(uri, model, extensionRecommendations) {
        ...
        for (const [extensionId, conditions] of extensionRecommendationEntries) {
            ...
            for (const condition of conditions) {
                ...
                if (pathGlob) {
                    if (processedPathGlobs.get(pathGlob) ?? match(pathGlob, ( uri.with({
                        fragment: ""
                    }).toString()), {
                        ignoreCase: true
                    })) {
                ...
                if (matched && condition.whenInstalled) {
                    if (!condition.whenInstalled.every(id => ( installed.some(local => areSameExtensions({
                        id
                    }, local.identifier))))) {
                        matched = false;
                    }
                ...
                if (matched && condition.whenNotInstalled) {
                    if (( installed.some(local => condition.whenNotInstalled?.some(id => areSameExtensions({
                        id
                    }, local.identifier))))) {
                        matched = false;
                    }
    ...
    promptFromRecommendations(uri, model, extensionRecommendations) {
        ...
        if (importantRecommendations.size && this.promptRecommendedExtensionForFileType(
            languageName && isImportantRecommendationForLanguage && language !== PLAINTEXT_LANGUAGE_ID ? ( localize(10848, "the {0} language", languageName)) : basename(uri),
            language,
    ...
    async promptImportantExtensionsInstallNotification(extensions, name, language) {
        try {
            const result = await this.extensionRecommendationNotificationService.promptImportantExtensionsInstallNotification({
                extensions,
                name,
                source: RecommendationSource.FILE
            });
    ...
    getPromptedRecommendations() {
        return JSON.parse(
            this.storageService.get(promptedRecommendationsStorageKey, StorageScope.PROFILE, "{}")
        );
    }
    ...
    getCachedRecommendations() {
        let storedRecommendations = JSON.parse(
            this.storageService.get(recommendationsStorageKey, StorageScope.PROFILE, "[]")
        );
...
FileBasedRecommendations = ( __decorate([( __param(0, IExtensionsWorkbenchService)), ( __param(1, IModelService)), ( __param(2, ILanguageService)), ( __param(3, IProductService)), ( __param(4, IStorageService)), ( __param(5, IExtensionRecommendationNotificationService)), ( __param(6, IExtensionIgnoredRecommendationsService)), ( __param(7, IWorkspaceContextService)), ( __param(8, IUntitledTextEditorService))], FileBasedRecommendations));
