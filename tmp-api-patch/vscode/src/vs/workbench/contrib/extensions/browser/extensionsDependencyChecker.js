...
let ExtensionDependencyChecker = class ExtensionDependencyChecker extends Disposable {
    constructor(
        ...
        MenuRegistry.appendMenuItem(MenuId.CommandPalette, {
            command: {
                ...
                category: ( localize(10644, "Extensions")),
                title: ( localize(10645, "Install Missing Dependencies"))
            }
        });
    }
    async getUninstalledMissingDependencies() {
        ...
        return allMissingDependencies.filter(id => localExtensions.every(l => !areSameExtensions(l.identifier, {
            id
        })));
    }
    ...
    async installMissingDependencies() {
        ...
        if (missingDependencies.length) {
            const extensions = await this.extensionsWorkbenchService.getExtensions(( missingDependencies.map(id => ({
                id
            ...
            if (extensions.length) {
                ...
                this.notificationService.notify({
                    ...
                    actions: {
                        primary: [( new Action("realod", ( localize(10647, "Reload Window")), "", true, () => this.hostService.reload()))]
                    }
                });
            }
        } else {
            this.notificationService.info(( localize(10648, "There are no missing dependencies to install.")));
        }
    }
};
ExtensionDependencyChecker = ( __decorate([( __param(0, IExtensionService)), ( __param(1, IExtensionsWorkbenchService)), ( __param(2, INotificationService)), ( __param(3, IHostService))], ExtensionDependencyChecker));
