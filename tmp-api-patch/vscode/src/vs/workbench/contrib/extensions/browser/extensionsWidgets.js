...
import * as extensionsWidgets from './media/extensionsWidgets.css';
import { semverExports } from '../../../../../../../_virtual/semver.js';
import { Disposable, DisposableStore, MutableDisposable, toDisposable } from '../../../../base/common/lifecycle.js';
import { ExtensionState, ExtensionEditorTab } from '../common/extensions.js';
import { IExtensionsWorkbenchService } from '../common/extensions.service.js';
import { addDisposableListener, EventType, finalHandler, append, $, reset } from '../../../../base/browser/dom.js';
...
import { installCountIcon, starFullIcon, starHalfIcon, starEmptyIcon, sponsorIcon, ratingIcon, preReleaseIcon, remoteIcon, privateExtensionIcon, syncIgnoredIcon, restartRequiredIcon, activationTimeIcon, errorIcon, warningIcon, infoIcon } from './extensionsIcons.js';
import { registerColor } from '../../../../platform/theme/common/colorUtils.js';
import { textLinkForeground } from '../../../../platform/theme/common/colors/baseColors.js';
...
import { MarkdownString, createCommandUri } from '../../../../base/common/htmlContent.js';
...
import { extensionDefaultIcon, verifiedPublisherIcon, extensionVerifiedPublisherIconColor } from '../../../services/extensionManagement/common/extensionsIcons.js';
...
let InstallCountWidget = InstallCountWidget_1 = class InstallCountWidget extends ExtensionWidget {
    ...
    render() {
        ...
        const parent = this.small ? this.container : append(this.container, $("span.install", {
            tabIndex: 0
        }));
        ...
        if (!this.small) {
            this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.container, ( localize(10739, "Install count")))
            );
        }
    }
...
InstallCountWidget = InstallCountWidget_1 = ( __decorate([( __param(2, IHoverService))], InstallCountWidget));
let RatingsWidget = class RatingsWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.small) {
            ...
            const element = append(this.container, $("span.rating.clickable", {
                tabIndex: 0
            }));
            for (let i = 1; i <= 5; i++) {
                if (rating >= i) {
            ...
            this.containerHover.update(( localize(10740, "Average rating: {0} out of 5", rating)));
...
RatingsWidget = ( __decorate([( __param(2, IHoverService)), ( __param(3, IOpenerService))], RatingsWidget));
    ...
    render() {
        ...
        if (this.small) {
            ...
            this.containerHover = this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.element, ( localize(10741, "Publisher ({0})", this.extension.publisherDisplayName)))
            );
            append(this.element, publisherDisplayName);
            if (this.extension.publisherDomain?.verified) {
                append(this.element, verifiedPublisher);
            ...
            if (this.extension.url) {
                this.disposables.add(onClick(
                    this.element,
                    () => this.extensionsWorkbenchService.openSearch(`publisher:"${this.extension?.publisherDisplayName}"`)
                ));
            }
        }
    }
};
PublisherWidget = ( __decorate([( __param(2, IExtensionsWorkbenchService)), ( __param(3, IHoverService)), ( __param(4, IOpenerService))], PublisherWidget));
let SponsorWidget = class SponsorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        const sponsor = append(this.container, $("span.sponsor.clickable", {
            tabIndex: 0
        }));
        ...
        const label = $("span", undefined, ( localize(10743, "Sponsor")));
...
SponsorWidget = ( __decorate([( __param(1, IHoverService)), ( __param(2, IOpenerService))], SponsorWidget));
let RecommendationWidget = class RecommendationWidget extends ExtensionWidget {
    constructor(parent, extensionRecommendationsService) {
        ...
        this._register(toDisposable(() => this.clear()));
        this._register(
            this.extensionRecommendationsService.onDidChangeRecommendations(() => this.render())
        );
    }
    clear() {
        this.element?.remove();
...
RecommendationWidget = ( __decorate([( __param(1, IExtensionRecommendationsService))], RecommendationWidget));
...
RemoteBadgeWidget = ( __decorate([( __param(2, IExtensionManagementServerService)), ( __param(3, IInstantiationService))], RemoteBadgeWidget));
let ExtensionIconBadge = class ExtensionIconBadge extends Disposable {
    constructor(icon, tooltip, hoverService, labelService, themeService) {
        ...
        this.elementHover = this._register(
            hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.element, "")
        );
        this.render();
    }
    render() {
...
ExtensionIconBadge = ( __decorate([( __param(2, IHoverService)), ( __param(3, ILabelService)), ( __param(4, IThemeService))], ExtensionIconBadge));
...
let ExtensionKindIndicatorWidget = class ExtensionKindIndicatorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension?.private) {
            ...
            if (!this.small) {
                append(this.element, $("span.private-extension-label", undefined, ( localize(10745, "Private Extension"))));
            }
            return;
        }
        if (!this.small) {
        ...
        if (workspaceFolder && this.extension.isWorkspaceScoped) {
            this.element.textContent = ( localize(10746, "Workspace Extension"));
            this.element.classList.add("clickable");
            this.element.setAttribute("role", "button");
            this.disposables.add(this.hoverService.setupManagedHover(
                getDefaultHoverDelegate("mouse"),
                this.element,
                this.uriIdentityService.extUri.relativePath(workspaceFolder.uri, location)
            ));
            ...
            this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.element, location.path)
            );
            this.element.textContent = ( localize(10747, "Local Extension"));
        }
    }
};
ExtensionKindIndicatorWidget = ( __decorate([( __param(2, IHoverService)), ( __param(3, IWorkspaceContextService)), ( __param(4, IUriIdentityService)), ( __param(5, IExplorerService)), ( __param(6, IViewsService)), ( __param(7, IExtensionGalleryManifestService))], ExtensionKindIndicatorWidget));
let SyncIgnoredWidget = class SyncIgnoredWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension && this.extension.state === ExtensionState.Installed && this.userDataSyncEnablementService.isEnabled() && this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension)) {
            ...
            this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), element, ( localize(10748, "This extension is ignored during sync.")))
            );
            element.classList.add(...ThemeIcon.asClassNameArray(syncIgnoredIcon));
        }
    }
};
SyncIgnoredWidget = ( __decorate([( __param(1, IConfigurationService)), ( __param(2, IExtensionsWorkbenchService)), ( __param(3, IHoverService)), ( __param(4, IUserDataSyncEnablementService))], SyncIgnoredWidget));
let ExtensionRestartRequiredWidget = class ExtensionRestartRequiredWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (runtimeState && /restart|reload/i.test(reason)) {
            const element = append(this.container, $(
                "span.extension-restart-required" + ThemeIcon.asCSSSelector(restartRequiredIcon)
            ));
            append(
                this.container,
                $("span.extension-restart-required-label", undefined, ( localize(10749, "Restart Required")))
            );
            this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), element, reason)
            );
        }
    }
};
ExtensionRestartRequiredWidget = ( __decorate([( __param(1, IHoverService))], ExtensionRestartRequiredWidget));
let ExtensionRuntimeStatusWidget = class ExtensionRuntimeStatusWidget extends ExtensionWidget {
    constructor(
        ...
        this._register(extensionService.onDidChangeExtensionsStatus(extensions => {
            if (this.extension && ( extensions.some(e => areSameExtensions({
                id: e.value
    ...
    render() {
        ...
        if (this.extensionViewState.filters.featureId && this.extension.state === ExtensionState.Installed) {
            ...
            if (feature?.icon && accessData) {
                ...
                featureAccessTimeElement.textContent = ( localize(10750, "{0} reqs", accessData.accessTimes.length));
...
ExtensionRuntimeStatusWidget = ( __decorate([( __param(2, IExtensionService)), ( __param(3, IExtensionFeaturesManagementService)), ( __param(4, IExtensionsWorkbenchService))], ExtensionRuntimeStatusWidget));
let ExtensionHoverWidget = ExtensionHoverWidget_1 = class ExtensionHoverWidget extends ExtensionWidget {
    ...
    getHoverMarkdown() {
        ...
        if (semverExports.valid(this.extension.version)) {
        ...
        if (this.extension.private) {
            markdown.appendMarkdown(`$(${privateExtensionIcon.id}) ${( localize(10745, "Private Extension"))}`);
            addSeparator = true;
        }
        if (this.extension.state === ExtensionState.Installed) {
            ...
            if (this.extension.publisherSponsorLink) {
                ...
                markdown.appendMarkdown(`$(${sponsorIcon.id}) [${( localize(10743, "Sponsor"))}](${this.extension.publisherSponsorLink})`);
                addSeparator = true;
            }
        }
        if (addSeparator) {
        ...
        if (location) {
            if (this.extension.isWorkspaceScoped && this.contextService.isInsideWorkspace(location)) {
                markdown.appendMarkdown(( localize(10746, "Workspace Extension")));
            } else {
                markdown.appendMarkdown(( localize(10747, "Local Extension")));
            }
            markdown.appendText(`\n`);
        }
        ...
        if (this.extension.outdated) {
            markdown.appendMarkdown(( localize(10752, "Latest version:")));
            markdown.appendMarkdown(
        ...
        if (extensionRuntimeStatus || extensionFeaturesAccessData.size || extensionStatus.length || runtimeState || recommendationMessage || preReleaseMessage) {
            ...
            if (extensionRuntimeStatus) {
                if (extensionRuntimeStatus.activationTimes) {
                    ...
                    markdown.appendMarkdown(`${( localize(10753, "Activation time"))}${extensionRuntimeStatus.activationTimes.activationReason.startup ? ` (${( localize(10754, "Startup"))})` : ""}: \`${activationTime}ms\``);
                    markdown.appendText(`\n`);
                }
                if (extensionRuntimeStatus.runtimeErrors.length || extensionRuntimeStatus.messages.length) {
                    ...
                    const errorsLink = extensionRuntimeStatus.runtimeErrors.length ? `[${extensionRuntimeStatus.runtimeErrors.length === 1 ? ( localize(10755, "1 uncaught error")) : ( localize(10756, "{0} uncaught errors", extensionRuntimeStatus.runtimeErrors.length))}](${createCommandUri(
    "extension.open",
    this.extension.identifier.id,
    ExtensionEditorTab.Features
)})` : undefined;
                    const messageLink = extensionRuntimeStatus.messages.length ? `[${extensionRuntimeStatus.messages.length === 1 ? ( localize(10757, "1 message")) : ( localize(10758, "{0} messages", extensionRuntimeStatus.messages.length))}](${createCommandUri(
    "extension.open",
    this.extension.identifier.id,
    ExtensionEditorTab.Features
)})` : undefined;
            ...
            if (extensionFeaturesAccessData.size) {
                ...
                for (const [featureId, accessData] of extensionFeaturesAccessData) {
                    if (accessData?.accessTimes.length) {
                        ...
                        if (feature) {
                            markdown.appendMarkdown(( localize(10759, "{0} usage", feature.label)));
    ...
    static getPreReleaseMessage(extension) {
        ...
        const preReleaseVersionLink = `[${( localize(10761, "Pre-Release version"))}](${createCommandUri(
    "workbench.extensions.action.showPreReleaseVersion",
    extension.identifier.id
)})`;
        return localize(10762, "This extension has a {0} available", preReleaseVersionLink);
    }
};
ExtensionHoverWidget = ExtensionHoverWidget_1 = ( __decorate([( __param(2, IExtensionsWorkbenchService)), ( __param(3, IExtensionFeaturesManagementService)), ( __param(4, IHoverService)), ( __param(5, IConfigurationService)), ( __param(6, IExtensionRecommendationsService)), ( __param(7, IThemeService)), ( __param(8, IWorkspaceContextService))], ExtensionHoverWidget));
...
ExtensionStatusWidget = ( __decorate([( __param(2, IMarkdownRendererService))], ExtensionStatusWidget));
let ExtensionRecommendationWidget = class ExtensionRecommendationWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (recommendationStatus) {
            ...
            append(
                this.container,
                $(`div.recommendation-text`, undefined, recommendationStatus.message)
            );
        }
        this._onDidRender.fire();
    }
...
ExtensionRecommendationWidget = ( __decorate([( __param(1, IExtensionRecommendationsService)), ( __param(2, IExtensionIgnoredRecommendationsService))], ExtensionRecommendationWidget));
const extensionRatingIconColor = registerColor("extensionIcon.starForeground", {
    light: "#DF6100",
    dark: "#FF8E00",
    hcDark: "#FF8E00",
    hcLight: textLinkForeground
}, ( localize(10764, "The icon color for extension ratings.")), false);
const extensionPreReleaseIconColor = registerColor("extensionIcon.preReleaseForeground", {
    dark: "#1d9271",
    light: "#1d9271",
    hcDark: "#1d9271",
    hcLight: textLinkForeground
}, ( localize(10765, "The icon color for pre-release extension.")), false);
registerColor("extensionIcon.sponsorForeground", {
    light: "#B51E78",
    dark: "#D758B3",
    hcDark: null,
    hcLight: "#B51E78"
}, ( localize(10766, "The icon color for extension sponsor.")), false);
const extensionPrivateBadgeBackground = registerColor("extensionIcon.privateForeground", {
    dark: "#ffffff60",
    light: "#00000060",
    hcDark: "#ffffff60",
    hcLight: "#00000060"
}, ( localize(10767, "The icon color for private extensions.")));
...
export { ExtensionHoverWidget, ExtensionIconBadge, ExtensionIconWidget, ExtensionKindIndicatorWidget, ExtensionPackCountWidget, ExtensionRecommendationWidget, ExtensionRestartRequiredWidget, ExtensionRuntimeStatusWidget, ExtensionStatusWidget, ExtensionWidget, InstallCountWidget, PreReleaseBookmarkWidget, PublisherWidget, RatingsWidget, RecommendationWidget, RemoteBadgeWidget, SponsorWidget, SyncIgnoredWidget, extensionPreReleaseIconColor, extensionPrivateBadgeBackground, extensionRatingIconColor, onClick };
