...
let WorkspaceRecommendations = class WorkspaceRecommendations extends ExtensionRecommendations {
    ...
    async doActivate() {
        ...
        await this.fetch();
        this._register(
            this.workspaceExtensionsConfigService.onDidChangeExtensionsConfigs(() => this.onDidChangeExtensionsConfigs())
        );
        for (const folder of this.contextService.getWorkspace().folders) {
            this._register(this.fileService.watch(
                this.uriIdentityService.extUri.joinPath(folder.uri, WORKSPACE_EXTENSIONS_FOLDER)
            ));
        }
        this._register(
            this.contextService.onDidChangeWorkspaceFolders(() => this.onDidChangeWorkspaceExtensionsScheduler.schedule())
        );
                    ...
                    if (invalidRecommendations.indexOf(extensionId) === -1) {
                        this._recommendations.push({
                            ...
                            reason: {
                                ...
                                reasonText: ( localize(10850, "This extension is recommended by users of the current workspace."))
                            }
                        });
                    }
                }
    ...
    async validateExtensions(contents) {
        ...
        let message = "";
        const allRecommendations = distinct(contents.flatMap((
            {
                recommendations
            }
        ) => recommendations || []));
...
WorkspaceRecommendations = ( __decorate([( __param(0, IWorkspaceExtensionsConfigService)), ( __param(1, IWorkspaceContextService)), ( __param(2, IUriIdentityService)), ( __param(3, IFileService)), ( __param(4, IWorkbenchExtensionManagementService)), ( __param(5, INotificationService))], WorkspaceRecommendations));
