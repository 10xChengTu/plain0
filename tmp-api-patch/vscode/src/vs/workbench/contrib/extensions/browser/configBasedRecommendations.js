...
import { IExtensionTipsService } from '../../../../platform/extensionManagement/common/extensionManagement.service.js';
...
let ConfigBasedRecommendations = class ConfigBasedRecommendations extends ExtensionRecommendations {
    ...
    async doActivate() {
        await this.fetch();
        this._register(
            this.workspaceContextService.onDidChangeWorkspaceFolders(e => this.onWorkspaceFoldersChanged(e))
        );
    }
    async fetch() {
    ...
    async onWorkspaceFoldersChanged(event) {
        if (event.added.length) {
            ...
            if (( this.importantTips.some(
                current => oldImportantRecommended.every(old => current.extensionId !== old.extensionId)
            ))) {
...
ConfigBasedRecommendations = ( __decorate([( __param(0, IExtensionTipsService)), ( __param(1, IWorkspaceContextService))], ConfigBasedRecommendations));
