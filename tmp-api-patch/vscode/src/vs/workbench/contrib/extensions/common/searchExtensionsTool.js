...
const SearchExtensionsToolData = {
    ...
    displayName: ( localize(10864, "Search Extensions")),
    ...
    userDescription: ( localize(10865, "Search for VS Code extensions")),
    ...
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        if (!params.keywords?.length && !params.category && !params.ids?.length) {
            return {
                content: [{
                    kind: "text",
                    value: ( localize(10866, "Please provide a category or keywords or ids to search for."))
                }]
            };
        }
        ...
        if (params.ids?.length) {
            const extensions = await this.extensionWorkbenchService.getExtensions(( params.ids.map(id => ({
                id
            }))), token);
        ...
        return {
            ...
            toolResultDetails: {
                ...
                output: [{
                    type: "embed",
                    isText: true,
                    value: JSON.stringify(( result.map(extension => extension.id)))
                }]
            }
        };
    }
};
SearchExtensionsTool = ( __decorate([( __param(0, IExtensionsWorkbenchService))], SearchExtensionsTool));
