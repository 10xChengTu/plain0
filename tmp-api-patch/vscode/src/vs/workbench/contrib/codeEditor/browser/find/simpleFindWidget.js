...
import * as simpleFindWidget from './simpleFindWidget.css';
import { localize } from '../../../../../nls.js';
import { createElement, trackFocus, addDisposableListener, getComputedStyle, createTextNode } from '../../../../../base/browser/dom.js';
...
registerCss(simpleFindWidget);
const NLS_FIND_INPUT_LABEL = ( localize(9065, "Find"));
const NLS_FIND_INPUT_PLACEHOLDER = ( localize(9066, "Find"));
const NLS_PREVIOUS_MATCH_BTN_LABEL = ( localize(9067, "Previous Match"));
const NLS_NEXT_MATCH_BTN_LABEL = ( localize(9068, "Next Match"));
const NLS_CLOSE_BTN_LABEL = ( localize(9069, "Close"));
...
class SimpleFindWidget extends Widget {
    constructor(
        ...
        this._innerDomNode = createElement("div");
        ...
        this._register(
            this._findInputFocusTracker.onDidFocus(this._onFindInputFocusTrackerFocus.bind(this))
        );
        this._register(
            this._findInputFocusTracker.onDidBlur(this._onFindInputFocusTrackerBlur.bind(this))
        );
        ...
        if (options?.enableSash) {
            ...
            const resizeSash = this._register(( new Sash(this._innerDomNode, this, {
    ...
    _updateFindInputAriaLabel() {
        ...
        if (!this._accessibilityHelpHintAnnounced && this._configurationService.getValue("accessibility.verbosity.find") && this._accessibilityService.isScreenReaderOptimized()) {
            ...
            if (keybinding) {
                findLabel += ", " + ( localize(9070, "Press {0} for accessibility help", keybinding));
    ...
    _announceSearchResults(label, searchString) {
        if (!searchString) {
            return localize(9071, "Enter search input");
        }
        if (label === NLS_NO_RESULTS) {
            return searchString === "" ? ( localize(9072, "{0} found", label)) : ( localize(9073, "{0} found for '{1}'", label, searchString));
        }
        return localize(9074, "{0} found for '{1}'", label, searchString);
    }
}
const simpleFindWidgetSashBorder = registerColor("simpleFindWidget.sashBorder", {
    dark: "#454545",
    light: "#C8C8C8",
    hcDark: "#6FC3DF",
    hcLight: "#0F4A85"
}, ( localize(9075, "Border color of the sash border.")));
