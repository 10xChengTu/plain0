...
import { HasSpeechProvider, SpeechToTextInProgress, SpeechToTextStatus } from '../../../speech/common/speechService.js';
    ...
    constructor() {
        super({
            ...
            keybinding: {
                ...
                secondary: isWindows ? [KeyMod$1.Alt | KeyCode.Backquote] : undefined
            }
        });
    }
...
class EditorDictationStopAction extends EditorAction2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(9048, "Stop Dictation in Editor")),
            category: VOICE_CATEGORY,
...
class DictationWidget extends Disposable {
    constructor(editor, keybindingService) {
        ...
        actionBar.push(toAction({
            id: EditorDictationStopAction.ID,
            label: stopActionKeybinding ? ( localize(9049, "Stop Dictation ({0})", stopActionKeybinding)) : ( localize(9050, "Stop Dictation")),
...
let EditorDictation = class EditorDictation extends Disposable {
    ...
    async start() {
        ...
        const replaceText = (text, isPreview) => {
            ...
            this.editor.executeEdits(EditorDictation_1.ID, [EditOperation.replace(Range.fromPositions(
                previewStart,
                previewStart.with(undefined, previewStart.column + lastReplaceTextLength)
            ), text)], [Selection.fromPositions(endPosition)]);
            if (isPreview) {
                collection.set([{
