...
import { UndoRedoGroup, UndoRedoElementType } from '../../../../../platform/undoRedo/common/undoRedo.js';
...
import { keepObserved } from '../../../../../base/common/observableInternal/utils/utils.js';
...
let MergeEditorModel = class MergeEditorModel extends EditorModel {
    constructor(
        ...
        this.modifiedBaseRangeResultStates = derived(this, reader => {
            const map = ( new Map(( this.modifiedBaseRanges.read(reader).map(s => [s, ( new ModifiedBaseRangeData(s))]))));
            return map;
        });
        ...
        this.baseResultMapping = derived(this, reader => {
            ...
            m.inputRange.deltaStart(-1), m.outputRange.deltaStart(-1))) : m)), map.inputLineCount));
        });
        ...
        this.isUpToDate = derived(
            this,
            reader => this.diffComputingState.read(reader) === MergeEditorModelState.upToDate
        );
        this.firstRun = true;
            ...
            this._register(autorunHandleChanges({
                changeTracker: {
                    ...
                    handleChange: ctx => {
                        ...
                        return (
                            ctx.didChange(this.resultTextModelDiffs.diffs) ? ctx.change === TextModelDiffChangeReason.textChange : true
                        );
                    }
                }
            }, reader => {
    ...
    computeAutoMergedResult() {
        ...
        for (const baseRange of baseRanges) {
            appendLinesToResult(
                baseLines,
                MergeEditorLineRange.fromLineNumbers(baseStartLineNumber, baseRange.baseRange.startLineNumber)
            );
        ...
        appendLinesToResult(
            baseLines,
            MergeEditorLineRange.fromLineNumbers(baseStartLineNumber, baseLines.length + 1)
        );
    ...
    updateBaseRangeAcceptedState(resultDiffs, states, tx) {
        const baseRangeWithStoreAndTouchingDiffs = leftJoin(
            states,
            resultDiffs,
            (baseRange, diff) => baseRange[0].baseRange.intersectsOrTouches(diff.inputRange) ? CompareResult.neitherLessOrGreaterThan : MergeEditorLineRange.compareByStart(baseRange[0].baseRange, diff.inputRange)
        );
    ...
    setState(baseRange, state, _markInputAsHandled, tx, _pushStackElement = false) {
        ...
        const {
            edit,
            effectiveState
        } = baseRange.getEditForBase(state);
    ...
    setInputHandled(baseRange, inputNumber, handled, tx) {
        ...
        this.undoRedoService.pushElement({
            ...
            label: ( localize(12270, "Set Input Handled")),
            redo() {
    ...
    async getResultValueWithConflictMarkers() {
        ...
        for (const [range, state] of states) {
            ...
            appendLinesToResult(resultLines, MergeEditorLineRange.fromLineNumbers(
        ...
        appendLinesToResult(
            resultLines,
            MergeEditorLineRange.fromLineNumbers(resultStartLineNumber, resultLines.length + 1)
        );
...
MergeEditorModel = ( __decorate([( __param(7, ILanguageService)), ( __param(8, IUndoRedoService))], MergeEditorModel));
...
class MarkAsHandledUndoRedoElement {
    constructor(resource, mergeEditorModelRef, stateRef, input1Handled, input2Handled) {
        ...
        this.label = ( localize(12271, "Undo Mark As Handled"));
