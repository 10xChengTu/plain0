...
let TempFileMergeEditorInputModel = class TempFileMergeEditorInputModel extends EditorModel {
    ...
    async confirmClose(inputModels) {
        ...
        if (someDirty) {
            ...
            const buttons = [{
                label: hasUnhandledConflicts ? ( localize(12243, "&&Save With Conflicts")) : ( localize(12244, "&&Save")),
                run: () => ConfirmResult.SAVE
            }, {
                label: ( localize(12245, "Do&&n't Save")),
            ...
            choice = (await this.dialogService.prompt({
                ...
                )) : isMany ? ( localize(12248, "The merge results will be lost if you don't save them.")) : ( localize(12249, "The merge result will be lost if you don't save it.")),
                buttons,
                cancelButton: {
    ...
    async save(options) {
        ...
        (async () => {
            const {
                ...
                message: ( localize(12250, "Do you want to accept the merge result?")),
                detail: ( localize(
                    12251,
                    "This will write the merge result to the original file and close the merge editor."
                )),
                primaryButton: ( localize(12252, "&&Accept Merge"))
            });
            if (confirmed) {
                await this.accept();
...
let WorkspaceMergeEditorModeFactory = class WorkspaceMergeEditorModeFactory {
    ...
    static {
        this.FILE_SAVED_SOURCE = SaveSourceRegistry.registerSource("merge-editor.source", ( localize(12253, "Before Resolving Conflicts In Merge Editor")));
    }
    async createInputModel(args) {
        const store = ( new DisposableStore());
        let [base, result, input1Data, input2Data] = await Promise.all([this._textModelService.createModelReference(args.base).then(v => ({
            ...
            console.error(e);
            return undefined;
        }), this._textModelService.createModelReference(args.result), toInputData(args.input1, this._textModelService, store), toInputData(args.input2, this._textModelService, store)]);
        if (base === undefined) {
...
WorkspaceMergeEditorModeFactory = WorkspaceMergeEditorModeFactory_1 = ( __decorate([( __param(1, IInstantiationService)), ( __param(2, ITextModelService)), ( __param(3, ITextFileService)), ( __param(4, IModelService)), ( __param(5, ILanguageService))], WorkspaceMergeEditorModeFactory));
let WorkspaceMergeEditorInputModel = class WorkspaceMergeEditorInputModel extends EditorModel {
    constructor(
        ...
        this.isDirty = observableFromEvent(this, Event.any(
    ...
    async confirmClose(inputModels) {
        ...
        if (someDirty) {
            ...
            const {
                ...
                )) : isMany ? ( localize(12258, "Your changes will be lost if you don't save them.")) : ( localize(12259, "Your changes will be lost if you don't save them.")),
                buttons: [{
                    label: someUnhandledConflicts ? ( localize(12260, "&&Save with Conflicts")) : ( localize(12261, "&&Save")),
                    run: () => ConfirmResult.SAVE
                }, {
                    label: ( localize(12262, "Do&&n't Save")),
                    run: () => ConfirmResult.DONT_SAVE
                }],
                cancelButton: {
                    run: () => ConfirmResult.CANCEL
            ...
            const {
                ...
                message: isMany ? ( localize(12263, "Do you want to close {0} merge editors?", inputModels.length)) : ( localize(
                    12264,
                    "Do you want to close the merge editor for {0}?",
                    basename(inputModels[0].resultUri)
                )),
                detail: someUnhandledConflicts ? isMany ? ( localize(12265, "The files contain unhandled conflicts.")) : ( localize(12266, "The file contains unhandled conflicts.")) : undefined,
                primaryButton: someUnhandledConflicts ? ( localize(12267, "&&Close with Conflicts")) : ( localize(12268, "&&Close")),
                checkbox: {
                    label: ( localize(12269, "Do not ask me again"))
                }
            });
            if (checkboxChecked) {
