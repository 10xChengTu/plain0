...
let ExternalUriOpenerService = class ExternalUriOpenerService extends Disposable {
    ...
    async showOpenerPrompt(openers, targetUri, ctx, token) {
        ...
        items.push({
            label: isWeb ? ( localize(10873, "Open in new browser window")) : ( localize(10874, "Open in default browser")),
            opener: undefined
        }, {
            type: "separator"
        }, {
            label: ( localize(10875, "Configure default opener...")),
            opener: "configureDefault"
        });
        const picked = await this.quickInputService.pick(items, {
            placeHolder: ( localize(10876, "How would you like to open: {0}", (targetUri.toString())))
        });
        if (!picked) {
            return true;
        }
...
ExternalUriOpenerService = ( __decorate([( __param(0, IOpenerService)), ( __param(1, IConfigurationService)), ( __param(2, ILogService)), ( __param(3, IPreferencesService)), ( __param(4, IQuickInputService))], ExternalUriOpenerService));
