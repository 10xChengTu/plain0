...
class CommentsModel extends Disposable {
        ...
        changed.forEach(thread => {
            ...
            if (index >= 0) {
                ...
                matchingResourceData.commentThreads.push(
                    ResourceWithCommentThreads.createCommentNode(uniqueOwner, owner, ( URI.parse(matchingResourceData.id)), thread)
                );
            }
        });
            ...
            if (existingResource.length) {
                ...
                if (thread.comments && thread.comments.length) {
                    resource.commentThreads.push(
                        ResourceWithCommentThreads.createCommentNode(uniqueOwner, owner, resource.resource, thread)
                    );
                }
            } else {
    ...
    getMessage() {
        if (!this._resourceCommentThreads.length) {
            return localize(9283, "There are no comments in this workspace yet.");
        } else {
            return "";
        }
    }
...
export { CommentsModel, threadHasMeaningfulComments };
