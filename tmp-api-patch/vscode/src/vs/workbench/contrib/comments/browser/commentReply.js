...
let CommentReply = class CommentReply extends Disposable {
    constructor(
        ...
        this.commentEditor = this._register(this._scopedInstatiationService.createInstance(
            SimpleCommentEditor,
            this._form,
    ...
    async initialize(focus) {
        ...
        if (commentController) {
            resource = resource.with({
                authority: commentController.id
            });
        }
    ...
    setCommentEditorDecorations() {
        const hasExistingComments = this._commentThread.comments && this._commentThread.comments.length > 0;
        const placeholder = hasExistingComments ? (this._commentOptions?.placeHolder || ( localize(9218, "Reply..."))) : (this._commentOptions?.placeHolder || ( localize(9219, "Type a new comment")));
    ...
    createReplyButton(commentEditor, commentForm) {
        this._reviewThreadReplyButton = append(
            commentForm,
        ...
        this._register(this.hoverService.setupManagedHover(
            ...
            this._commentOptions?.prompt || ( localize(9218, "Reply..."))
        ));
        this._reviewThreadReplyButton.textContent = this._commentOptions?.prompt || ( localize(9218, "Reply..."));
...
CommentReply = ( __decorate([( __param(12, ICommentService)), ( __param(13, IConfigurationService)), ( __param(14, IKeybindingService)), ( __param(15, IContextMenuService)), ( __param(16, IHoverService)), ( __param(17, ITextModelService))], CommentReply));
