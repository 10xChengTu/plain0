...
import { $, trackFocus, addDisposableListener, EventType } from '../../../../base/browser/dom.js';
...
let CommentThreadWidget = class CommentThreadWidget extends Disposable {
    ...
    constructor(
        ...
        this._register(
            this._header = this._scopedInstantiationService.createInstance(CommentThreadHeader, container, {
    ...
    _setAriaLabel() {
        let ariaLabel = ( localize(9226, "Comment"));
        let keybinding;
        ...
        if (keybinding) {
            ariaLabel = ( localize(9227, "{0}, use ({1}) for accessibility help", ariaLabel, keybinding));
        } else if (verbose) {
            ariaLabel = ( localize(
                9228,
    ...
    currentThreadListeners() {
        ...
        let hasFocus = false;
        this._register(
            addDisposableListener(this.container, EventType.MOUSE_ENTER, e => {
    ...
    async updateCommentThread(commentThread) {
        const shouldCollapse = (this._commentThread.collapsibleState === CommentThreadCollapsibleState.Expanded) && (this._commentThreadState === CommentThreadState.Unresolved) && (commentThread.state === CommentThreadState.Resolved);
...
CommentThreadWidget = ( __decorate([( __param(12, ICommentService)), ( __param(13, IConfigurationService)), ( __param(14, IKeybindingService))], CommentThreadWidget));
