...
import * as review from './media/review.css';
...
class CommentingRangeDecorator {
    ...
    _doUpdate(
        ...
        commentInfos,
        emphasisLine = -1,
        selectionRange = this._lastSelection
    ) {
        ...
        for (const info of commentInfos) {
            info.commentingRanges.ranges.forEach(range => {
                ...
                if (
                (selectionRange && (emphasisLine >= 0) && intersectingSelectionRange) && !((intersectingSelectionRange.startLineNumber === intersectingSelectionRange.endLineNumber) && (emphasisLine === intersectingSelectionRange.startLineNumber))) {
...
let CommentController = class CommentController extends Disposable {
    constructor(
        ...
        this._register(
            this.commentService.onDidSetDataProvider(_ => this.beginComputeAndHandleEditorChange())
        );
        this._register(
            this.commentService.onDidUpdateCommentingRanges(_ => this.beginComputeAndHandleEditorChange())
        );
    ...
    registerEditorListeners() {
        ...
        this._editorDisposables.push(
            this.editor.onDidChangeCursorPosition(e => this.onEditorChangeCursorPosition(e.position))
        );
        this._editorDisposables.push(this.editor.onDidFocusEditorWidget(
            () => this.onEditorChangeCursorPosition(this.editor?.getPosition() ?? null)
        ));
        this._editorDisposables.push(
            this.editor.onDidChangeCursorSelection(e => this.onEditorChangeCursorSelection(e))
        );
        this._editorDisposables.push(
            this.editor.onDidBlurEditorWidget(() => this.onEditorChangeCursorSelection())
        );
    }
    ...
    onEditorChangeCursorPosition(e) {
        ...
        const range = Range.fromPositions(e, {
            column: -1,
    ...
    _findNearestCommentThread(focusThread, reverse) {
        ...
        if (nextWidget !== undefined) {
            ...
            nextWidget.reveal(
                undefined,
                focusThread ? CommentWidgetFocus.Widget : CommentWidgetFocus.None
            );
        }
    }
    ...
    _findNearestCommentingRange(reverse) {
        ...
        if (this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (commentRangeStart && commentRangeEnd) {
                ...
                oneLine ? status(( localize(9263, "Line {0}", commentRangeStart))) : status(( localize(9264, "Lines {0} to {1}", commentRangeStart, commentRangeEnd)));
            }
        }
    }
    nextCommentingRange() {
    ...
    onModelChanged() {
        ...
        this.localToDispose.add(this.commentService.onDidUpdateCommentThreads(async e => {
            ...
            const added = e.added.filter(
                thread => thread.resource && this.uriIdentityService.extUri.isEqual(( URI.parse(thread.resource)), editorURI)
            );
    ...
    async resumePendingComment(editorURI, thread) {
        const matchedZones = this._commentWidgets.filter(
            zoneWidget => zoneWidget.uniqueOwner === thread.uniqueOwner && Range.lift(zoneWidget.commentThread.range)?.equalsRange(thread.range)
        );
            ...
            if (!this._hasRespondedToEditorChange) {
                if (( this._commentInfos.some(
                    ...
                    if (verbose) {
                        ...
                        status(( localize(9267, "Editor has commenting ranges.")));
                    }
                }
            }
        });
    ...
    async displayCommentThread(uniqueOwner, thread, shouldReveal, pendingComment, pendingEdits) {
        ...
        const zoneWidget = this.instantiationService.createInstance(
            ...
            uniqueOwner,
            thread,
            pendingComment ?? continueOnCommentReply?.comment,
            pendingEdits
        );
    ...
    getCommentsAtLine(commentRange) {
        return this._commentWidgets.filter(
        ...
        if (!this._addInProgress) {
            ...
            if (existingCommentsAtLine.length) {
                ...
                existingCommentsAtLine.forEach(
                    allExpanded ? widget => widget.collapse(true) : widget => widget.expand(true)
                );
    ...
    addCommentAtLine(range, e) {
        ...
        if (!newCommentInfos.length || !this.editor?.hasModel()) {
            ...
            if (!newCommentInfos.length) {
                if (range) {
                    this.notificationService.error(( localize(9268, "The cursor must be within a commenting range to add a comment.")));
                } else {
                    this.notificationService.error(( localize(9269, "File comments are not allowed on this file.")));
                }
            }
            return Promise.resolve();
        }
        if (newCommentInfos.length > 1) {
            if (e && range) {
                ...
                return this.quickInputService.pick(picks, {
                    placeHolder: ( localize(9270, "Select Comment Provider")),
