...
import { CommentState, CommentThreadCollapsibleState } from '../../../../editor/common/languages.js';
...
import { getCommentThreadStateBorderColor, commentThreadStateColorVar, commentThreadStateBackgroundColorVar } from './commentColors.js';
...
function commentThreadHasDraft(commentThread) {
    ...
    return ( comments.some(comment => comment.state === CommentState.Draft));
}
    ...
    async confirmCollapse() {
        ...
        if (confirmSetting === "whenHasUnsubmittedComments" && this._commentThreadWidget.hasUnsubmittedComments) {
            const result = await this.dialogService.confirm({
                ...
                primaryButton: ( localize(9230, "Discard")),
                type: Severity$1.Warning,
                checkbox: {
                    label: ( localize(9231, "Never ask me again")),
                    checked: false
                }
            });
...
ReviewZoneWidget = ( __decorate([( __param(5, IInstantiationService)), ( __param(6, IThemeService)), ( __param(7, ICommentService)), ( __param(8, IContextKeyService)), ( __param(9, IConfigurationService)), ( __param(10, IDialogService))], ReviewZoneWidget));
