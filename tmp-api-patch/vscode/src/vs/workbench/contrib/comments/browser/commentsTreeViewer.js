...
const COMMENTS_VIEW_TITLE = ( localize2(9284, "Comments"));
...
CommentsMenus = ( __decorate([( __param(0, IMenuService))], CommentsMenus));
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return localize(9285, "{0} replies", commentCount - 1);
        } else if (commentCount === 2) {
            return localize(9286, "1 reply");
        } else {
            return localize(9287, "1 comment");
        }
    }
    getRenderedComment(commentBody) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? ( localize(9288, "Image: {0}", image.alt)) : ( localize(9289, "Image"));
    ...
    renderElement(node, index, templateData) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = ( localize(9290, "Outdated"));
        ...
        templateData.threadMetadata.icon.classList.add(
            ...ThemeIcon.asClassNameArray(this.getIcon(node.element.threadState, hasDraft))
        );
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = ( localize(9291, "[Ln {0}]", node.element.range.startLineNumber));
            } else {
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = ( localize(9293, "Last reply from {0}", lastComment.userName));
...
CommentNodeRenderer = ( __decorate([( __param(2, IConfigurationService)), ( __param(3, IHoverService)), ( __param(4, IThemeService))], CommentNodeRenderer));
var FilterDataType;
...
class Filter {
    ...
    filterCommentNode(comment, parentVisibility) {
        const matchesResolvedState = (comment.threadState === undefined) || (this.options.showResolved && CommentThreadState.Resolved === comment.threadState) || (this.options.showUnresolved && CommentThreadState.Unresolved === comment.threadState);
        ...
        ) || FilterOptions._messageFilter(this.options.textFilter.text, comment.comment.userName) || ( comment.replies.map(reply => {
            return (
                FilterOptions._messageFilter(this.options.textFilter.text, reply.comment.userName) || FilterOptions._messageFilter(
                    this.options.textFilter.text,
...
CommentsList = ( __decorate([( __param(3, IContextKeyService)), ( __param(4, IListService)), ( __param(5, IInstantiationService)), ( __param(6, IConfigurationService)), ( __param(7, IContextMenuService)), ( __param(8, IKeybindingService))], CommentsList));
