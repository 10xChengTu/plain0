...
class ToggleReactionsAction extends Action {
    ...
    constructor(toggleDropdownMenu, title) {
        super(ToggleReactionsAction.ID, title || ( localize(9321, "Pick Reactions...")), "toggle-reactions", true);
        this._menuActions = [];
...
class ReactionActionViewItem extends ActionViewItem {
    ...
    getTooltip() {
        const action = this.action;
        const toggleMessage = action.enabled ? ( localize(9322, "Toggle reaction, ")) : "";
        if (action.count === undefined) {
            return localize(9323, "{0}{1} reaction", toggleMessage, action.label);
        } else if (action.reactors === undefined || action.reactors.length === 0) {
            if (action.count === 1) {
                return localize(9324, "{0}1 reaction with {1}", toggleMessage, action.label);
