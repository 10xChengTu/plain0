...
async function maybeEnableAuthExtension(extensionsWorkbenchService, logService) {
    ...
    if (providerExtension.enablementState === EnablementState.DisabledGlobally || providerExtension.enablementState === EnablementState.DisabledWorkspace) {
        ...
        try {
            ...
            await extensionsWorkbenchService.updateRunningExtensions(( localize(7225, "Enabling GitHub Authentication")));
            return true;
        } catch (error) {
            logService.error(
...
export { ChatSetupAnonymous, ChatSetupStep, ChatSetupStrategy, buildUpgradeUrlWithRedirect, maybeEnableAuthExtension, refreshTokens };
