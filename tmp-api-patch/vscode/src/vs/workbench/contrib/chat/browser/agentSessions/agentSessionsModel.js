...
import { Extensions } from '../../../../services/output/common/output.js';
import { IOutputService } from '../../../../services/output/common/output.service.js';
import { isAgentHostTarget, ChatSessionStatus, isSessionInProgressStatus } from '../../common/chatSessionsService.js';
...
import { AgentSessionProviders, getAgentSessionProvider, getAgentSessionProviderName, getAgentSessionProviderIcon, isBuiltInAgentSessionProvider } from './agentSessions.js';
...
const agentSessionsOutputChannelLabel = ( localize(5603, "Agent Sessions"));
...
AgentSessionsLogger = ( __decorate([( __param(1, ILogService)), ( __param(2, IOutputService)), ( __param(3, IChatEntitlementService))], AgentSessionsLogger));
let AgentSessionsModel = class AgentSessionsModel extends Disposable {
    ...
    registerListeners() {
        ...
        this._register(
            this.chatSessionsService.onDidChangeAvailability(() => this.resolve(undefined))
        );
        ...
        this._register(
            this.workspaceContextService.onDidChangeWorkspaceFolders(() => this.resolve(undefined))
        );
        this._register(
            this.workspaceTrustManagementService.onDidChangeTrust(() => this.resolve(undefined))
        );
    ...
    observeSession(resource) {
        if (!( this._resolvedResources.has(resource))) {
            ...
            this.chatSessionsService.resolveChatSessionItem(sessionType, resource, CancellationToken.None).catch(
                error => this.logger.logIfTrace(`observeSession: resolve failed for ${( resource.toString())}: ${error instanceof Error ? error.message : String(error)}`)
            );
        }
    ...
    async resolve(provider) {
        const providers = Array.isArray(provider) ? provider : provider !== undefined ? [provider] : this.chatSessionsService.getRegisteredChatSessionItemProviders();
        await Promise.all(( providers.map(provider => this.resolveProvider(provider, {
    ...
    resolveStateEntry(session) {
        ...
        this.sessionStates.set(session.resource, {
            ...prev
        });
    ...
    isMarkedUnread(session) {
        return this.resolveStateEntry(session)?.read === AgentSessionsModel_1.UNREAD_MARKER;
    }
    isRead(session) {
    ...
    resolveReadDateBaseline() {
        let readDateBaseline = this.storageService.getNumber(AgentSessionsModel_1.READ_DATE_BASELINE_KEY, StorageScope.WORKSPACE, 0);
        if (readDateBaseline > 0) {
            return readDateBaseline;
        }
        readDateBaseline = this.productService.quality === "stable" ? Date.now() - (7 * 24 * 60 * 60 * 1000) : Date.now();
...
AgentSessionsModel = AgentSessionsModel_1 = ( __decorate([( __param(0, IChatSessionsService)), ( __param(1, ILifecycleService)), ( __param(2, IInstantiationService)), ( __param(3, IStorageService)), ( __param(4, IProductService)), ( __param(5, IChatWidgetService)), ( __param(6, IWorkspaceContextService)), ( __param(7, IWorkspaceTrustManagementService)), ( __param(8, IChatEntitlementService))], AgentSessionsModel));
let AgentSessionsCache = class AgentSessionsCache {
    ...
    saveCachedSessions(sessions) {
        const serialized = ( sessions.map(session => ({
            ...
            badge: session.badge,
            tooltip: session.tooltip,
            status: isSessionInProgressStatus(session.status) ? ChatSessionStatus.Completed : session.status,
            archived: session.archived,
            timing: session.timing,
...
AgentSessionsCache = AgentSessionsCache_1 = ( __decorate([( __param(0, IStorageService))], AgentSessionsCache));

export { AgentSessionSection, ChatSessionStatus as AgentSessionStatus, AgentSessionsModel, getAgentChangesSummary, hasValidDiff, isAgentHostAgentSessionItem, isAgentSession, isAgentSessionSection, isAgentSessionShowLess, isAgentSessionShowMore, isAgentSessionsModel, isLocalAgentSessionItem, isMarshalledAgentSessionContext, isSessionInProgressStatus };
