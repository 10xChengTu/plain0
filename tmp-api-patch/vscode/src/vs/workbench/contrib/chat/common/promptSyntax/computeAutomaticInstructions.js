...
let ComputeAutomaticInstructions = class ComputeAutomaticInstructions {
    ...
    async _parseInstructionsFile(uri, token) {
        ...
        try {
            ...
            this._logService.error(
                `[InstructionsContextComputer] Failed to parse instruction file: ${uri}`,
                error
            );
            return undefined;
        }
    ...
    async addApplyingInstructions(instructionFiles, context, variables, telemetryEvent, debugInfo, token) {
        ...
        if (!includeApplyingInstructions && this._modeKind !== ChatModeKind.Edit) {
            this._logService.trace(
                `[InstructionsContextComputer] includeApplyingInstructions is disabled and agent kind is not Edit. No applying instructions will be added.`
            );
            return;
        }
        ...
        for (const instructionFile of instructionFiles) {
            ...
            if (!pattern) {
                ...
                debugInfo.debugDetails.push({
                    ...
                    uri,
                    reason: ( localize(8620, "no applyTo pattern"))
                });
                continue;
            }
            ...
            if (( context.instructions.has(uri))) {
                ...
                debugInfo.debugDetails.push({
                    ...
                    name: ( basename(uri).toString()),
                    uri,
                    reason: ( localize(8621, "already processed"))
                });
                continue;
            }
            ...
            if (match) {
                ...
                const reason = !match.file ? ( localize(8622, "automatically attached as pattern is **")) : ( localize(
                    8623,
                ...
                debugInfo.debugDetails.push({
                    ...
                    name: ( basename(uri).toString()),
                    uri,
                    reason: ( localize(8624, "applyTo '{0}' did not match any attached files", pattern))
                });
            }
        }
    }
    ...
    async _addAgentInstructions(variables, telemetryEvent, debugInfo, token) {
        ...
        for (const {
            ...
            debugInfo.debugDetails.push({
                ...
                name: ( basename(uri).toString()),
                uri,
                reason: ( localize(8625, "always added"))
            });
    ...
    _getTool(referenceName) {
        ...
        if (tool && this._enabledTools[tool.id]) {
            return {
                tool,
                variable: `#tool:${this._languageModelToolsService.getFullReferenceName(tool)}`
            };
        }
        return undefined;
    }
    async _getCustomizationsIndex(instructionFiles, _existingVariables, telemetryEvent, debugInfo, token) {
        ...
        if (fileReadTool) {
            ...
            entries.push(
                `If the file content is not already included in the context, use the ${fileReadTool.variable} tool to read it before proceeding. Use the exact value from the <file> element as-is with the tool; do not add or remove prefixes or otherwise modify it.`
            );
            entries.push(
            ...
            for (const {
                ...
                const description = folderName.trim().length === 0 ? ( localize(8626, "Instructions for the workspace")) : ( localize(8627, "Instructions for folder '{0}'", folderName));
                entries.push("<instruction>");
            ...
            const modelInvocableSkills = agentSkills?.filter(skill => {
                if (!skill.description) {
                    debugInfo.debugDetails.push({
                        ...
                        name: skill.name,
                        uri: skill.uri,
                        reason: ( localize(8628, "no description for model invocation"))
                    });
                    return false;
                }
                if (skill.disableModelInvocation) {
                    debugInfo.debugDetails.push({
                        ...
                        name: skill.name,
                        uri: skill.uri,
                        reason: ( localize(8629, "model invocation disabled"))
                    });
                    return false;
                }
                if (!matchesSessionType(skill.sessionTypes, currentSessionType)) {
                    debugInfo.debugDetails.push({
                        ...
                        name: skill.name,
                        uri: skill.uri,
                        reason: ( localize(8630, "session type not matched"))
                    });
                    return false;
                }
                if (!isFileLoggingEnabled && skill.uri.path.includes(TROUBLESHOOT_SKILL_PATH)) {
                    debugInfo.debugDetails.push({
                        ...
                        name: skill.name,
                        uri: skill.uri,
                        reason: ( localize(8631, "debug logging disabled"))
                    });
                    return false;
                }
                return true;
            ...
            if (modelInvocableSkills && modelInvocableSkills.length > 0) {
                ...
                if (useSkillAdherencePrompt) {
                    ...
                    if (skillTool) {
                        ...
                        entries.push(
                            `BLOCKING REQUIREMENT: When a skill applies to the user's request, you MUST load and read the SKILL.md file IMMEDIATELY as your first action, BEFORE generating any other response or taking action on the task. Use ${fileReadTool.variable} to load the relevant skill(s).`
                        );
                    }
                    entries.push(
                    ...
                    if (skillTool) {
                        ...
                        entries.push(
                            `When a user asks you to perform a task that falls within the domain of a skill, use the ${skillTool.variable} tool with the skill name to load it.`
                        );
                    } else {
                        entries.push(
                            "Here is a list of skills that contain domain specific knowledge on a variety of topics."
                        ...
                        entries.push(
                            `When a user asks you to perform a task that falls within the domain of a skill, use the ${fileReadTool.variable} tool to acquire the full instructions from the file URI.`
                        );
                    }
                }
                ...
                if (truncatedAtIndex < modelInvocableSkills.length) {
                    ...
                    entries.push(
                        remaining > 0 ? `Additional skills available (invoke by name): ${nameList}... and ${remaining} more` : `Additional skills available (invoke by name): ${nameList}`
                    );
                }
                entries.push("</skills>", "", "");
            }
                ...
                for (const agent of agents) {
                    if (canUseAgent(agent)) {
                        ...
                        debugInfo.debugDetails.push({
                            ...
                            name: agent.name,
                            uri: agent.uri,
                            reason: ( localize(8632, "not invocable by model"))
                        });
                    }
                }
                entries.push("</agents>", "", "");
    ...
    async _addReferencedInstructions(attachedContext, telemetryEvent, debugInfo, token) {
        ...
        if (!includeReferencedInstructions && this._modeKind !== ChatModeKind.Edit) {
            this._logService.trace(
                `[InstructionsContextComputer] includeReferencedInstructions is disabled and agent kind is not Edit. No referenced instructions will be added.`
            );
            return;
        }
                ...
                if (refsToCheck.length > 0) {
                    ...
                    for (let i = 0; i < stats.length; i++) {
                        ...
                        if (stat.success && stat.stat?.isFile) {
                            ...
                            const reason = ( localize(8633, "Referenced by {0}", basename(next)));
                            attachedContext.add(
