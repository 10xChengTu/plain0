...
let ChatAgentService = class ChatAgentService extends Disposable {
        ...
        data = {
            ...
            get slashCommands() {
                return commands.filter(
                    c => !c.when || that.contextKeyService.contextMatchesRules(ContextKeyExpr.deserialize(c.when))
                );
            }
        };
        const entry = {
    ...
    getContributedDefaultAgent(location) {
        return this._preferExtensionAgent(
            this.getAgents().filter(a => !!a.isDefault && a.locations.includes(location))
        );
    }
    ...
    async detectAgentOrCommand(request, history, options, token) {
        ...
        const result = await provider.provideParticipantDetection(request, history, {
            ...options,
            participants
        }, token);
        if (!result) {
...
ChatAgentService = ( __decorate([( __param(0, IContextKeyService)), ( __param(1, IConfigurationService))], ChatAgentService));
...
let ChatAgentNameService = class ChatAgentNameService {
    ...
    checkAgentNameRestriction(name, chatAgentData) {
        ...
        return ( allowList.map(allowList => {
            ...
            return ( allowList.some(id => equalsIgnoreCase(
                id,
...
export { ChatAgentNameService, ChatAgentService, MergedChatAgent, getFullyQualifiedId, reviveSerializedAgent };
