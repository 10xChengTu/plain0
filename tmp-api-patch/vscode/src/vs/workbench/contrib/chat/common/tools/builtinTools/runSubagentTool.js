...
import { ChatConfiguration, ChatAgentLocation, ChatModeKind, GeneralPurposeAgentName } from '../../constants.js';
import { ILanguageModelChatMetadata, COPILOT_VENDOR_ID } from '../../languageModels.js';
...
import { ToolDataSource, VSCodeToolReference, isToolSet } from '../languageModelToolsService.js';
...
let RunSubagentTool = class RunSubagentTool extends Disposable {
    ...
    constructor(
        ...
        this._register(Event.filter(
            ...
            e => e.affectsConfiguration(ChatConfiguration.GeneralPurposeAgentEnabled)
    ...
    getToolData() {
        ...
        properties.agentName = {
            type: "string",
            description: generalPurposeAgentEnabled ? "Name of the agent to invoke." : "Optional name of a specific agent to invoke. If not provided, uses the current agent."
        };
        properties.model = {
            type: "string",
        ...
        const runSubagentToolData = {
            ...
            displayName: ( localize(8971, "Run Subagent")),
    ...
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        this.logService.debug(
            `RunSubagentTool: Invoking with prompt: ${args.prompt.substring(0, 100)}...`
        );
        ...
        try {
            ...
            if (existingRunSubagentEnablement !== false) {
                modeTools[RunSubagentTool_1.Id] = depthAllowed;
            }
            modeTools[ManageTodoListToolToolId] = false;
            modeTools["copilot_askQuestions"] = false;
            if (maxDepth > 0) {
                this.logService.debug(
                    `RunSubagentTool: Nested subagents enabling ${modeTools[RunSubagentTool_1.Id]}: session ${sessionKey}, currentDepth: ${currentDepth}, maxDepth: ${maxDepth}, allowInvocationsFromSubagents: ${allowInvocationsFromSubagents}`
                );
            }
            ...
            if (this.configurationService.getValue(ChatConfiguration.CollectInstructionsInExtension) !== true) {
                const computer = this.instantiationService.createInstance(
                    ...
                    modeTools,
                    undefined,
                    getChatSessionType(invocation.context.sessionResource)
                );
    ...
    getAvailableModelsInfo(mainModelId) {
        const models = ( this.languageModelsService.getLanguageModelIds().map(id => ({
            id,
            metadata: this.languageModelsService.lookupLanguageModel(id)
        }))).filter(
        ...
        if (unavailableDueToMultiplier.length > 0) {
            parts.push(
                `Unavailable (exceeds current model's cost tier): ${unavailableDueToMultiplier.join(", ")}`
            );
        }
    ...
    resolveSubagentModel(subagent, mainModelId, explicitModelQualifiedName) {
        ...
        if (explicitModelQualifiedName) {
            ...
            if (lm?.identifier) {
                ...
                throw ( new Error(
                    `Requested model '${explicitModelQualifiedName}' not found. ${this.getAvailableModelsInfo(mainModelId)}`
                ));
            }
        }
        ...
        if (modeModelId) {
            ...
            if (check.exceeds) {
                ...
                throw ( new Error(
                    `Requested model '${modelMetadata?.name}' ${check.reason}. ${this.getAvailableModelsInfo(mainModelId)}`
                ));
            }
        }
...
RunSubagentTool = RunSubagentTool_1 = ( __decorate([( __param(0, IChatAgentService)), ( __param(1, IChatService)), ( __param(2, ILanguageModelToolsService)), ( __param(3, ILanguageModelsService)), ( __param(4, ILogService)), ( __param(5, IConfigurationService)), ( __param(6, IPromptsService)), ( __param(7, IInstantiationService)), ( __param(8, IProductService))], RunSubagentTool));

export { RUN_SUBAGENT_MAX_NESTING_DEPTH, RunSubagentTool };
