...
function createReviewPlanToolData() {
    ...
    return {
        ...
        displayName: ( localize(8965, "Review Plan")),
        userDescription: ( localize(8966, "Ask the user to review and approve a plan before proceeding.")),
...
let ReviewPlanTool = class ReviewPlanTool extends Disposable {
    ...
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        if (!actions || actions.length === 0) {
            throw ( new Error(( localize(8967, "At least one approval action must be provided."))));
        }
        const {
            request
        } = this.getRequest(invocation.context?.sessionResource, invocation.chatRequestId);
        if (!request) {
        ...
        const reviewData = ( new ChatPlanReviewData(title ?? ( localize(8968, "Review plan")), content, actions, canProvideFeedback, planUri?.toJSON(), generateUuid()));
    ...
    async prepareToolInvocation(context, _token) {
        ...
        return {
            invocationMessage: ( new MarkdownString(( localize(8969, "Asking you to review the plan")))),
            pastTenseMessage: ( new MarkdownString(( localize(8970, "Asked you to review the plan"))))
        };
    }
    toResult(result) {
        return {
...
ReviewPlanTool = ( __decorate([( __param(0, IChatService)), ( __param(1, ILogService))], ReviewPlanTool));

export { ReviewPlanTool, ReviewPlanToolData, ReviewPlanToolId, createReviewPlanToolData };
