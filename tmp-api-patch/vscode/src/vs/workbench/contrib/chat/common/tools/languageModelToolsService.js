...
(function(ToolDataSource) {
    ...
    function classify(source) {
        if (source.type === "internal") {
            ...
            return {
                ordinal: 0,
                label: ( localize(9008, "User Defined"))
            };
        } else {
            return {
                ordinal: 3,
...
class ToolSet {
    ...
    getTools(r) {
        return Iterable.concat(Iterable.filter(
            this._tools.observable.read(r),
            toolData => this._contextKeyService.contextMatchesRules(toolData.when)
...
class ToolSetForModel {
    ...
    getTools(r) {
        return Iterable.filter(
            this._toolSet.getTools(r),
            toolData => toolMatchesModel(toolData, this.model) && (!this.toolFilter || this.toolFilter(toolData))
        );
    }
}
...
export { SpecedToolAliases, ToolAndToolSetEnablementMap, ToolDataSource, ToolInvocationPresentation, ToolSet, ToolSetForModel, VSCodeToolReference, createToolSchemaUri, isToolInvocationContext, isToolResultInputOutputDetails, isToolResultOutputDetails, isToolSet, stringifyPromptTsxPart, toolContentToA11yString, toolMatchesModel, toolResultHasBuffers };
