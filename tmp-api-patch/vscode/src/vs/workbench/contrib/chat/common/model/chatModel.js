...
import { registerAutorunSelfDisposable, autorun } from '../../../../../base/common/observableInternal/reactions/autorun.js';
...
(function(IChatRequestVariableData) {
    function toExport(data) {
        return {
            variables: ( data.variables.map(IChatRequestVariableEntry.toExport))
        };
    }
    ...
    partsToRepr(parts) {
        ...
        for (const part of parts) {
            ...
            case "inlineReference":
                segment = {
                    text: this.inlineRefToRepr(part)
                };
                break;
            case "command":
                segment = {
                    text: part.command.title,
                    isBlock: true
                };
                break;
            case "textEditGroup":
        ...
        if (hasEditGroupsAfterLastClear) {
            blocks.push(( localize(8573, "Made changes.")));
        }
        return blocks.join("\n\n");
    }
...
class Response extends AbstractResponse {
    ...
    constructor(value) {
        super(( asArray(value).map(v => ("kind" in v ? v : isMarkdownString(v) ? {
            content: v,
    ...
    updateContent(progress, quiet) {
        if (progress.kind === "clearToPreviousToolInvocation") {
            ...
            if (progress.kind === "textEdit" && !notebookUri) {
                ...
                const cellEdits = ( progress.edits.map(edit => ({
                    uri: progress.uri,
                    edit
                })));
    ...
    _handleExternalToolInvocationUpdate(progress) {
        ...
        }, toolData, progress.toolCallId, progress.subagentInvocationId,
        undefined, {},
        undefined));
        if (progress.isComplete) {
            if (progress.toolSpecificData !== undefined) {
...
class ChatResponseModel extends Disposable {
    ...
    constructor(params) {
            ...
            for (const part of this._response.value) {
                if (part.kind === "toolInvocation") {
                    ...
                    if (state.type === IChatToolInvocation.StateKind.WaitingForPostApproval) {
                        return localize(8576, "Approve tool result?");
                    }
                }
                ...
                if (part.kind === "questionCarousel" && !part.isUsed) {
                    return localize(8577, "Answer questions to continue...");
                }
                if (part.kind === "planReview" && !part.isUsed) {
                    return localize(8578, "Review the plan to continue...");
                }
        ...
        this.isPendingConfirmation = ( _startedWaitingAt.map((waiting, r) => waiting ? {
            startedWaitingAt: waiting,
            detail: _pendingInfo.read(r)
        } : undefined));
    ...
    isSameUsage(usage) {
        ...
        return !!currentUsage && currentUsage.promptTokens === usage.promptTokens && currentUsage.completionTokens === usage.completionTokens && currentUsage.outputBuffer === usage.outputBuffer && currentUsage.copilotCredits === usage.copilotCredits && equals(currentUsage.promptTokenDetails, usage.promptTokenDetails);
    }
    complete() {
        if (this.isComplete) {
            return;
    ...
    toJSON() {
        ...
        return {
            responseId: this.id,
            result: this.result,
            responseMarkdownInfo: this.codeBlockInfos?.map(info => ({
                suggestionId: info.suggestionId
            })),
            followups: this.followups,
            modelState: modelState.value === ResponseModelState.Pending || modelState.value === ResponseModelState.NeedsInput ? {
                value: ResponseModelState.Cancelled,
                completedAt: Date.now()
            } : modelState,
            vote: this.vote,
...
let ChatModel = ChatModel_1 = class ChatModel extends Disposable {
    ...
    constructor(
        ...
        this.inputModel = ( new InputModel(serializedInputState && {
            ...
            modelConfiguration: serializedInputState.selectedModel ? (serializedInputState.selectedModel.modelConfiguration ?? serializedInputState.modelConfiguration) : undefined,
    ...
    _deserialize(obj) {
        const requests = hasKey(obj, {
            serializer: true
        }) ? obj.value.requests : obj.requests;
    ...
    getParsedRequestFromString(message) {
        const parts = [( new ChatRequestTextPart(( new OffsetRange(0, message.length)), {
            startColumn: 1,
            startLineNumber: 1,
            endColumn: 1,
            endLineNumber: 1
    ...
    acceptResponseProgress(request, progress, quiet) {
        ...
        if (progress.kind === "usage") {
            ...
            request.response.addUndoStop({
                id: progress.undoStopId ?? generateUuid(),
                kind: "undoStop"
            });
    ...
    toExport() {
        return {
            ...
            requests: ( this._requests.map(r => {
                ...
                const agentJson = agent && "toJSON" in agent ? agent.toJSON() : agent ? {
                    ...agent
                } : undefined;
                return {
                    requestId: r.id,
...
function getCodeCitationsMessage(citations) {
    ...
    const label = licenseTypes.size === 1 ? ( localize(8579, "Similar code found with 1 license type", licenseTypes.size)) : ( localize(8580, "Similar code found with {0} license types", licenseTypes.size));
    return label;
}
...
export { CHAT_ATTACHABLE_IMAGE_MIME_TYPES, ChatModel, ChatRequestEditedFileEventKind, ChatRequestModel, ChatRequestRemovalReason, ChatResponseModel, ChatResponseResource, IChatRequestVariableData, Response, appendMarkdownString, canMergeMarkdownStrings, defaultChatResponseModelChangeReason, getAttachableImageExtension, getCodeCitationsMessage, isCellTextEditOperation, isCellTextEditOperationArray, isExportableSessionData, isSerializableSessionData, logChangesToStateModel, normalizeSerializableChatData, serializeSendOptions, toChatHistoryContent, updateRanges };
