...
(function(ChatRequestHooks) {
    ...
    ChatRequestHooks.isEquals = isEquals;
...
const HOOK_COMMAND_FIELD_DESCRIPTIONS = {
    type: ( localize(8634, "Must be \"command\".")),
    command: ( localize(
        8635,
    ...
    bash: ( localize(8639, "Bash command for Linux and macOS.")),
    powershell: ( localize(8640, "PowerShell command for Windows.")),
    cwd: ( localize(8641, "Working directory for the script (relative to repository root).")),
    env: ( localize(
        8642,
        "Additional environment variables that are merged with the existing environment."
    )),
    timeout: ( localize(8643, "Maximum execution time in seconds (default: 30).")),
    timeoutSec: ( localize(8644, "Maximum execution time in seconds (default: 10)."))
};
...
function buildHookProperties(target, arraySchema) {
    return Object.fromEntries(( Object.entries(HOOKS_BY_TARGET[target]).map(([key, hookType]) => [key, {
        ...arraySchema,
        description: HOOK_METADATA[hookType]?.description
    }])));
}
...
const copilotCliHookCommandSchema = {
    ...
    errorMessage: ( localize(8646, "At least one of \"bash\" or \"powershell\" must be specified.")),
    properties: {
        type: {
            type: "string",
            enum: ["command"],
...
const hookFileSchema = {
    ...
    properties: {
        hooks: {
            type: "object",
            description: ( localize(8648, "Hook definitions organized by type.")),
            additionalProperties: true
        }
    },
    if: {
    ...
    then: {
        properties: {
            version: {
                type: "number",
                description: ( localize(8649, "Hook configuration format version."))
            },
            hooks: {
    ...
    defaultSnippets: [{
        label: ( localize(8650, "Basic hook configuration")),
        description: ( localize(8651, "A basic hook configuration with common hooks")),
        body: {
            hooks: {
                SessionStart: [{
                    type: "command",
...
export { ChatRequestHooks, HOOK_COMMAND_FIELD_DESCRIPTIONS, HOOK_SCHEMA_URI, extractHookCommandsFromItem, formatHookCommandLabel, getEffectiveCommandFieldKey, hookFileSchema, mergeHooks, parseSubagentHooksFromYaml, resolveEffectiveCommand, resolveHookCommand, toHookType };
