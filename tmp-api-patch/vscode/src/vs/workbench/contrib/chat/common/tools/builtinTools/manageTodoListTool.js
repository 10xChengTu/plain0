...
function createManageTodoListToolData() {
    ...
    return {
        ...
        displayName: ( localize(8953, "Manage and track todo items for task planning")),
        userDescription: ( localize(8954, "Manage and track todo items for task planning")),
...
let ManageTodoListTool = class ManageTodoListTool extends Disposable {
    ...
    async prepareToolInvocation(context, _token) {
        ...
        let message;
        if (args.operation === "read") {
            message = ( localize(8955, "Read todo list"));
        ...
        const invocationLabel = message?.replace(/^(Starting|Completed): /i, "") ?? ( localize(8956, "Updating todo list"));
        ...
        return {
            ...
            pastTenseMessage: ( new MarkdownString(message ?? ( localize(8957, "Updated todo list")))),
            toolSpecificData: {
                kind: "todoList",
                todoList: todoList
            }
    ...
    generatePastTenseMessage(currentTodos, newTodos) {
        if (currentTodos.length === 0 && newTodos.length > 0) {
            return newTodos.length === 1 ? ( localize(8958, "Created 1 todo")) : ( localize(8959, "Created {0} todos", newTodos.length));
        }
        ...
        if (addedTodos.length > 0) {
            return addedTodos.length === 1 ? ( localize(8962, "Added 1 todo")) : ( localize(8963, "Added {0} todos", addedTodos.length));
        }
        return localize(8964, "Updated todo list");
    }
...
ManageTodoListTool = ( __decorate([( __param(0, IChatTodoListService)), ( __param(1, ILogService)), ( __param(2, ITelemetryService))], ManageTodoListTool));

export { ManageTodoListTool, ManageTodoListToolToolId, createManageTodoListToolData };
