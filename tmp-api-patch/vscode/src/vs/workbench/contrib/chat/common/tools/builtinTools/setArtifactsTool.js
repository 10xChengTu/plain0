...
const SetArtifactsToolData = {
    ...
    displayName: ( localize(8976, "Set Session Artifacts")),
...
let SetArtifactsTool = class SetArtifactsTool {
    ...
    async prepareToolInvocation(_context, _token) {
        return {
            pastTenseMessage: ( new MarkdownString(( localize(8977, "Updated session artifacts")))),
    ...
    async invoke(invocation, _countTokens, _progress, _token) {
            ...
            if (uri) {
                ...
                if (parsed.scheme !== "http" && parsed.scheme !== "https") {
                    if (!(await this._fileService.exists(parsed))) {
                        throw ( new Error(( localize(8978, "Artifact URI does not exist: {0}", uri))));
                    }
                }
            }
            artifacts.push({
        ...
        return {
            content: [{
                kind: "text",
                value: ( localize(8979, "Set {0} artifact(s)", artifacts.length))
            }]
        };
    }
