...
(function(PromptHeaderAttributes) {
    ...
    PromptHeaderAttributes.context = "context";
...
class PromptHeader {
    ...
    get _parsedHeader() {
        if (this._parsed === undefined) {
            ...
            const attributes = [];
            const errors = ( yamlErrors.map(err => ({
                message: err.message,
                range: asRange(err),
                code: err.code
            })));
    ...
    getParsedBody() {
        if (this._parsed === undefined) {
            ...
            const bodyOffset = Iterable.reduce(
                Iterable.slice(this.linesWithEOL, 0, this.range.startLineNumber - 1),
            ...
            this._parsed = {
                fileReferences: fileReferences.sort((a, b) => Range.compareRangesUsingStarts(a.range, b.range)),
                variableReferences,
                bodyOffset
            };
        }
...
export { ParsedPromptFile, PromptBody, PromptFileParser, PromptHeader, PromptHeaderAttributes, evaluateApplyToPattern, parseCommaSeparatedList };
