...
import { ChatPermissionLevel, ChatConfiguration } from '../../constants.js';
...
function createAskQuestionsToolData() {
    ...
    return {
        ...
        displayName: ( localize(8944, "Ask Clarifying Questions")),
...
let AskQuestionsTool = class AskQuestionsTool extends Disposable {
    ...
    async invoke(invocation, _countTokens, progress, token) {
        ...
        const {
            request,
            sessionResource
        } = this.getRequest(chatSessionResource, chatRequestId);
        if (!sessionResource || !request) {
            this.logService.warn(
                "[AskQuestionsTool] Missing chat context; marking all questions as skipped."
            );
        ...
        if (request.modeInfo?.permissionLevel === ChatPermissionLevel.Autopilot || this.configService.getValue(ChatConfiguration.AutoReply)) {
            ...
            const {
                carousel,
                idToHeaderMap
            } = this.toQuestionCarousel(questions, resolveId);
        ...
        this.logService.trace(
            `[AskQuestionsTool] request=${request.id} terminalExecutionId=${request.terminalExecutionId ?? "undefined"} carousel.terminalId=${carousel.terminalId ?? "undefined"}`
        );
        this.chatService.appendProgress(request, carousel);
        ...
        if (carousel.dismissedByTerminalInput && carousel.terminalId) {
            this.logService.info(
                `[AskQuestionsTool] Carousel dismissed because user typed directly in terminal ${carousel.terminalId}`
            );
            return {
                content: [{
                    kind: "text",
        ...
        progress.report({
            message: ( localize(8947, "Analyzing your answers..."))
        });
        ...
        } = this.collectMetrics(questions, converted);
        this.sendTelemetry(
        ...
        this.logService.trace(
            `[AskQuestionsTool] Returning tool result with metrics: questions=${questions.length}, answered=${answeredCount}, skipped=${skippedCount}, freeText=${freeTextCount}, recommendedAvailable=${recommendedAvailableCount}, recommendedSelected=${recommendedSelectedCount}`
        );
        return {
            content: [{
                kind: "text",
    ...
    async prepareToolInvocation(context, _token) {
        ...
        const message = questionCount === 1 ? ( localize(8949, "Asking a question ({0})", headers)) : ( localize(8950, "Asking {0} questions ({1})", questionCount, headers));
        const pastMessage = questionCount === 1 ? ( localize(8951, "Asked a question ({0})", headers)) : ( localize(8952, "Asked {0} questions ({1})", questionCount, headers));
        return {
                ...
                if (part.kind === "toolInvocation" && part.toolId === TerminalToolId.RunInTerminal) {
                    ...
                    if (state.type === IChatToolInvocation.StateKind.Completed && state.contentForModel) {
                        for (const item of state.contentForModel) {
                            if (item.kind === "text") {
                                const idMatch = item.value.match(
                                    /(?:running in terminal ID|may still be running in terminal ID) ([0-9a-fA-F-]+)/
                                );
                                if (idMatch) {
                                    return idMatch[1];
                                }
    ...
    toQuestionCarousel(questions, resolveId) {
        ...
        const mappedQuestions = ( questions.map(
            (question, index) => this.toChatQuestion(question, idToHeaderMap, carouselResolveId, index)
        ));
        return {
    ...
    convertCarouselAnswers(questions, carouselAnswers, idToHeaderMap) {
        ...
        for (const question of questions) {
            ...
            this.logService.trace(
                `[AskQuestionsTool] Processing question "${question.header}" (internal ID: ${internalId}), raw answer: ${JSON.stringify(answer)}, type: ${typeof answer}`
            );
            if (answer === undefined) {
                result.answers[question.header] = {
                    selected: [],
            ...
            } else if (typeof answer === "object" && hasKey(answer, {
                selectedValues: true
            })) {
                const {
                    selectedValues,
                ...
                this.logService.warn(
                    `[AskQuestionsTool] Unknown answer format for "${question.header}": ${JSON.stringify(answer)}`
                );
                result.answers[question.header] = {
                    selected: [],
                    freeText: null,
    ...
    buildAutopilotCarouselAnswers(questions, carousel, idToHeaderMap) {
        ...
        for (const question of questions) {
            ...
            if (chatQuestion.type === "multiSelect") {
                data[internalId] = {
                    selectedValues: [],
                    freeformValue: AUTOPILOT_ASK_USER_RESPONSE
                };
...
AskQuestionsTool = ( __decorate([( __param(0, IChatService)), ( __param(1, ITelemetryService)), ( __param(2, ILogService)), ( __param(3, IConfigurationService))], AskQuestionsTool));

export { AUTOPILOT_ASK_USER_RESPONSE, AskQuestionsTool, AskQuestionsToolData, AskQuestionsToolId, createAskQuestionsToolData };
