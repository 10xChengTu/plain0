...
let ChatRequestParser = class ChatRequestParser {
    ...
    parseChatRequestWithReferences(
        references,
        selectedToolAndToolSets,
        message,
    ...
    tryToParseSlashCommand(remainingMessage, fullMessage, offset, position, parts, location, context) {
        ...
        const usedAgent = parts.find(p => p instanceof ChatRequestAgentPart)?.agent ?? (context?.forcedAgent ? context.forcedAgent : undefined);
        if (usedAgent) {
        ...
        const slashCommand = slashCommands.find(
            c => c.command === command && matchesSessionType(c.sessionTypes, context?.sessionType)
        );
        if (!usedAgent || slashCommand?.silent || capabilities?.supportsPromptAttachments) {
            ...
            if (subMatch) {
                ...
                if (this.promptsService.hasPromptSlashCommand(candidate)) {
                    ...
                    const extendedEditorRange = ( new Range(
                        position.lineNumber,
                        position.column,
                        position.lineNumber,
                        position.column + consumedLength
                    ));
    ...
    tryToParseDynamicVariable(message, offset, position, references) {
        ...
        if (refAtThisPosition) {
            ...
            return ( new ChatRequestDynamicVariablePart(
                ...
                refAtThisPosition._meta
            ));
        }
        return;
    }
};
ChatRequestParser = ( __decorate([( __param(0, IChatAgentService)), ( __param(1, IChatVariablesService)), ( __param(2, IChatSlashCommandService)), ( __param(3, IPromptsService))], ChatRequestParser));

export { ChatRequestParser, agentReg, slashReg, variableReg };
