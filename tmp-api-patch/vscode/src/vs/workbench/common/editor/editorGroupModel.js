...
import { GroupModelChangeKind, EditorsOrder, EditorCloseContext, SideBySideEditor, EditorExtensions } from '../editor.js';
...
let EditorGroupModel = class EditorGroupModel extends Disposable {
    ...
    setIndex(index) {
        this._onDidModelChange.fire({
            kind: GroupModelChangeKind.GROUP_INDEX
        });
    }
    setLabel(label) {
        this._onDidModelChange.fire({
            kind: GroupModelChangeKind.GROUP_LABEL
        });
    }
    pin(candidate) {
        const res = this.findEditor(candidate);
...
EditorGroupModel = EditorGroupModel_1 = ( __decorate([( __param(1, IInstantiationService)), ( __param(2, IConfigurationService))], EditorGroupModel));
