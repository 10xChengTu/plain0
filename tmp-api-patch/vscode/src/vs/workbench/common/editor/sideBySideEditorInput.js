...
let SideBySideEditorInput = class SideBySideEditorInput extends EditorInput {
    ...
    registerListeners() {
        this._register(
            Event.once(Event.any(this.primary.onWillDispose, this.secondary.onWillDispose))(() => {
                if (!this.isDisposed()) {
                    this.dispose();
                }
            })
        ...
        this._register(
            this.primary.onDidChangeCapabilities(() => this._onDidChangeCapabilities.fire())
        );
        this._register(
            this.secondary.onDidChangeCapabilities(() => this._onDidChangeCapabilities.fire())
        );
    ...
    getName() {
        ...
        return localize(4379, "{0} - {1}", this.secondary.getName(), this.primary.getName());
    }
    getPreferredName() {
...
SideBySideEditorInput = SideBySideEditorInput_1 = ( __decorate([( __param(4, IEditorService))], SideBySideEditorInput));
