...
let ConfigurationMigrationWorkbenchContribution = class ConfigurationMigrationWorkbenchContribution extends Disposable {
    ...
    constructor(configurationService, workspaceService) {
        ...
        this._register(
            configurationMigrationRegistry.onDidRegisterConfigurationMigration(migration => this.migrateConfigurations(migration))
        );
    }
        ...
        for (const [dataKey, target] of targetPairs) {
            ...
            if (migrationValues.length) {
                await Promise.allSettled(( migrationValues.map(
                    async ([[key, value], overrideIdentifiers]) => this.configurationService.updateValue(key, value.value, {
                        resource,
                        overrideIdentifiers
                    }, target)
                )));
    ...
    async runMigration(migration, dataKey, value, resource, overrideIdentifiers) {
        const valueAccessor = key => {
            ...
            ) => equals(identifiers, overrideIdentifiers))?.value;
        };
        const result = await migration.migrateFn(value, valueAccessor);
        return Array.isArray(result) ? result : [[migration.key, result]];
    }
};
ConfigurationMigrationWorkbenchContribution = ( __decorate([( __param(0, IConfigurationService)), ( __param(1, IWorkspaceContextService))], ConfigurationMigrationWorkbenchContribution));
            ...
            "properties": {
                "security.allowedUNCHosts": {
                    ...
                    "items": {
                        "type": "string",
                        "pattern": "^[^\\\\]+$",
                        "patternErrorMessage": ( localize(4286, "UNC host names must not contain backslashes."))
                    },
                    "default": [],
...
DynamicWorkbenchSecurityConfiguration = ( __decorate([( __param(0, IRemoteAgentService))], DynamicWorkbenchSecurityConfiguration));
...
let DynamicWindowConfiguration = class DynamicWindowConfiguration extends Disposable {
    ...
    constructor(userDataProfilesService, configurationService) {
        ...
        this._register(
            this.userDataProfilesService.onDidChangeProfiles(e => this.registerNewWindowProfileConfiguration())
        );
        ...
        const configurationNode = {
            ...
            "properties": {
                [CONFIG_NEW_WINDOW_PROFILE]: {
                    ...
                    "enumItemLabels": [...( this.userDataProfilesService.profiles.map(() => "")), ( localize(4289, "Active Window"))],
...
DynamicWindowConfiguration = ( __decorate([( __param(0, IUserDataProfilesService)), ( __param(1, IConfigurationService))], DynamicWindowConfiguration));
