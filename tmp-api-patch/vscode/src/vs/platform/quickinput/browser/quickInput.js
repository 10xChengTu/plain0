...
import { quickInputButtonsToActionArrays, quickInputButtonToAction, renderQuickInputDescription } from './quickInputUtils.js';
...
const InQuickInputContextKey = ( new RawContextKey(inQuickInputContextKeyValue, false, ( localize(2201, "Whether keyboard focus is inside the quick input control"))));
...
const QuickInputAlignmentContextKey = ( new RawContextKey(quickInputAlignmentContextKeyValue, "top", ( localize(2202, "The alignment of the quick input"))));
const quickInputTypeContextKeyValue = "quickInputType";
const QuickInputTypeContextKey = ( new RawContextKey(quickInputTypeContextKeyValue, undefined, ( localize(2203, "The type of the currently visible quick input"))));
...
const backButton = {
    ...
    tooltip: ( localize(2205, "Back")),
    handle: -1
};
class QuickInput extends Disposable {
    static {
        this.noPromptMessage = ( localize(2206, "Press 'Enter' to confirm your input or 'Escape' to cancel"));
    }
    constructor(ui) {
        super();
        this.ui = ui;
    ...
    getSteps() {
        if (this.step && this.totalSteps) {
            return localize(2207, "{0}/{1}", this.step, this.totalSteps);
        }
        if (this.step) {
...
class QuickPick extends QuickInput {
    static {
        this.DEFAULT_ARIA_LABEL = ( localize(2208, "Type to narrow down results."));
    }
    constructor(ui) {
        super(ui);
        this._value = "";
    ...
    get okLabel() {
        return this._okLabel ?? ( localize(2209, "OK"));
    }
    set okLabel(okLabel) {
    ...
    show() {
        if (!this.visible) {
            ...
            this.visibleDisposables.add(
                this._focusEventBufferer.wrapEvent(this.ui.list.onDidChangeFocus,
                (_, e) => e)(focusedItems => {
            ...
            this.visibleDisposables.add(
                this.ui.list.onButtonTriggered(event => this.onDidTriggerItemButtonEmitter.fire(event))
            );
            this.visibleDisposables.add(
...
class InputBox extends QuickInput {
    ...
    set prompt(prompt) {
        this._prompt = prompt;
        this.noValidationMessage = prompt ? ( localize(2210, "{0} (Press 'Enter' to confirm or 'Escape' to cancel)", prompt)) : QuickInput.noPromptMessage;
        this.update();
    }
    show() {
        if (!this.visible) {
...
let QuickInputHoverDelegate = class QuickInputHoverDelegate extends WorkbenchHoverDelegate {
    ...
    getOverrideOptions(options) {
        const showHoverHint = (isHTMLElement(options.content) ? options.content.textContent ?? "" : typeof options.content === "string" ? options.content : options.content.value).includes("\n");
        return {
            persistence: {
                hideOnKeyDown: false
            },
...
export { EndOfQuickInputBoxContextKey, InQuickInputContextKey, InputBox, QuickInput, QuickInputAlignmentContextKey, QuickInputHoverDelegate, QuickInputTypeContextKey, QuickPick, QuickWidget, backButton, endOfQuickInputBoxContext, endOfQuickInputBoxContextKeyValue, inQuickInputContext, inQuickInputContextKeyValue, quickInputAlignmentContextKeyValue, quickInputTypeContextKeyValue };
