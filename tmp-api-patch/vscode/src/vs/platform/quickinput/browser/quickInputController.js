...
import { AnchorPosition, layout2d, AnchorAlignment } from '../../../base/common/layout.js';
...
let QuickInputController = class QuickInputController extends Disposable {
    ...
    getUI(showInActiveContainer) {
        ...
        const checkAll = this._register(( new TriStateCheckbox(( localize(2216, "Toggle all checkboxes")), false, {
        ...
        const visibleCount = this._register(( new CountBadge(visibleCountContainer, {
            countFormat: ( localize(2217, "{0} Results"))
        ...
        const count = this._register(( new CountBadge(countContainer, {
            countFormat: ( localize(2218, "{0} Selected"))
        }, this.styles.countBadge)));
        ...
        customButton.label = ( localize(2220, "Custom"));
        ...
        this.dndController = this._register(
            this.instantiationService.createInstance(QuickInputDragAndDropController, this._container, container, [{
                node: titleBar,
    ...
    input(options = {}, token = CancellationToken.None) {
        return ( new Promise(resolve => {
            ...
            const disposables = [input, onDidValueChange(value => {
    ...
    show(controller) {
        ...
        backButton.tooltip = backKeybindingLabel ? ( localize(2221, "Back ({0})", backKeybindingLabel)) : ( localize(2222, "Back"));
    ...
    updateLayout() {
        if (this.ui && this.isVisible()) {
            ...
            let width = Math.min(
                ...
                QuickInputController_1.MAX_WIDTH
            );
            ...
            if (this.controller?.anchor) {
                ...
                const {
                    ...
                    anchorPosition: preferredAnchorPosition
                });
    ...
    (this.dimension.width * (this.viewState?.left ?? 0.5) ) - (width / 2)
)}px`;
                style.right = "";
                style.bottom = "";
                style.height = "";
    ...
    loadViewState() {
        try {
            const data = JSON.parse(
                this.storageService.get(VIEWSTATE_STORAGE_KEY, StorageScope.APPLICATION, "{}")
            );
...
QuickInputController = QuickInputController_1 = ( __decorate([( __param(1, ILayoutService)), ( __param(2, IInstantiationService)), ( __param(3, IContextKeyService)), ( __param(4, IStorageService)), ( __param(5, IContextMenuService)), ( __param(6, IKeybindingService))], QuickInputController));
let QuickInputDragAndDropController = class QuickInputDragAndDropController extends Disposable {
    constructor(
        _container,
        _quickInputContainer,
        _quickInputDragAreas,
        initialViewState,
        _layoutService,
        contextKeyService,
    ...
    registerMouseListeners() {
        ...
        this._register(addDisposableGenericMouseUpListener(dragArea, event => {
            ...
            const area = this._quickInputDragAreas.find((
                {
                    node,
                    includeChildren
                }
            ) => includeChildren ? isAncestor(originEvent.target, node) : originEvent.target === node);
    ...
    _layout(topCoordinate, leftCoordinate) {
        ...
        topCoordinate = Math.max(0, Math.min(
            topCoordinate,
        ...
        if (topCoordinate < this._layoutService.activeContainerOffset.top) {
            if (this._controlsOnLeft) {
                ...
                leftCoordinate = Math.min(
                    leftCoordinate,
                    this._container.clientWidth - this._quickInputContainer.clientWidth - (140 / getZoomFactor(getActiveWindow()))
                );
            }
        }
        ...
        leftCoordinate = Math.max(0, Math.min(
            leftCoordinate,
...
QuickInputDragAndDropController = ( __decorate([( __param(4, ILayoutService)), ( __param(5, IContextKeyService)), ( __param(6, IConfigurationService))], QuickInputDragAndDropController));
