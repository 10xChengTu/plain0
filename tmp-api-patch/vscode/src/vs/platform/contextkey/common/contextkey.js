...
const errorEmptyString = ( localize(1957, "Empty context key expression"));
...
const errorNoInAfterNot = ( localize(1959, "'in' after 'not'."));
const errorClosingParenthesis = ( localize(1960, "closing parenthesis ')'"));
const errorUnexpectedToken = ( localize(1961, "Unexpected token"));
const hintUnexpectedToken = ( localize(1962, "Did you forget to put && or || before the token?"));
const errorUnexpectedEOF = ( localize(1963, "Unexpected end of expression"));
const hintUnexpectedEOF = ( localize(1964, "Did you forget to put a context key?"));
class Parser {
    static {
    ...
    _errExpectedButGot(expected, got, additionalInfo) {
        const message = ( localize(1965, "Expected: {0}\nReceived: '{1}'.", expected, Scanner.getLexeme(got)));
    ...
    return (whenClauses.map(whenClause => {
        ...
        if (parser.lexingErrors.length > 0) {
            return (parser.lexingErrors.map(se => ({
                errorMessage: se.additionalInfo ? ( localize(1966, "Unexpected token. Hint: {0}", se.additionalInfo)) : ( localize(1967, "Unexpected token.")),
                offset: se.offset,
