...
(function(StorageScope) {
    StorageScope[StorageScope["APPLICATION_SHARED"] = -2] = "APPLICATION_SHARED";
...
class AbstractStorageService extends Disposable {
    ...
    async log() {
        ...
        const applicationSharedItems = this.getStorage(StorageScope.APPLICATION_SHARED)?.items ?? ( new Map());
...
class InMemoryStorageService extends AbstractStorageService {
    constructor() {
        ...
        this.applicationSharedStorage = this._register(( new Storage(( new InMemoryStorageDatabase()), {
            hint: StorageHint.STORAGE_IN_MEMORY
        })));
...
async function logStorage(
    ...
    const applicationSharedItems = ( new Map());
    const applicationSharedItemsParsed = ( new Map());
