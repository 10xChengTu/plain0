...
class RemovedResources {
    ...
    createMessage() {
        ...
        for (const [, element] of this.elements) {
            const dest = (element.reason === RemovedResourceReason.ExternalRemoval ? externalRemoval : noParallelUniverses);
...
let UndoRedoService = class UndoRedoService {
    ...
    async _confirmAndExecuteWorkspaceUndo(strResource, element, editStackSnapshot, undoConfirmed) {
        if (element.canSplit() && !this._isPartOfUndoGroup(element)) {
            ...
            const {
                ...
                message: ( localize(2639, "Would you like to undo '{0}' across all files?", element.label)),
                buttons: [{
                    label: ( localize(2640, "&&Undo in {0} Files", editStackSnapshot.editStacks.length)),
                    run: () => UndoChoice.All
                }, {
                    label: ( localize(2641, "Undo this &&File")),
                    run: () => UndoChoice.This
                }],
                cancelButton: {
    ...
    async _confirmAndContinueUndo(strResource, sourceId, element) {
        const result = await this._dialogService.confirm({
            message: ( localize(2643, "Would you like to undo '{0}'?", element.label)),
            primaryButton: ( localize(2644, "&&Yes")),
            cancelButton: ( localize(2645, "No"))
        });
        if (!result.confirmed) {
            return;
        }
