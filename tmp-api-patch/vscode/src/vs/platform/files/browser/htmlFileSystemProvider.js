...
import { FileSystemProviderCapabilities, FileSystemProviderErrorCode, FileType, FileChangeType, FileSystemProviderError, createFileSystemProviderError } from '../common/files.js';
...
class HTMLFileSystemProvider extends Disposable {
    ...
    async writeFile(resource, content, opts) {
        try {
            ...
            if (!handle) {
                ...
                handle = await parent.getFileHandle(this.extUri.basename(resource), {
                    create: true
                });
                if (!handle) {
                    throw this.createFileSystemProviderError(
    ...
    async rename(from, to, opts) {
        try {
            ...
            if (fileHandle) {
                ...
                throw this.createFileSystemProviderError(from, ( localize(2083, "Rename is only supported for files.")), FileSystemProviderErrorCode.Unavailable);
            }
        } catch (error) {
            throw this.toFileSystemProviderError(error);
        }
    ...
    watch(resource, opts) {
        ...
        this.doWatch(resource, opts, disposables).catch(
            error => this.logService.error(`[File Watcher ('FileSystemObserver')] Error: ${error} (${resource})`)
        );
        return disposables;
    }
    async doWatch(resource, opts, disposables) {
        ...
        try {
            await observer.observe(handle, opts.recursive ? {
                recursive: true
            } : undefined);
        } finally {
            if (disposables.isDisposed) {
    ...
    toFileSystemProviderError(error) {
        ...
        if (error.name === "NotAllowedError") {
            error = ( new Error(( localize(2084, "Insufficient permissions. Please retry and allow the operation."))));
