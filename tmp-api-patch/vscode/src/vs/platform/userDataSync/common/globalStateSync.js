...
import { StorageTarget, StorageScope } from '../../storage/common/storage.js';
...
let GlobalStateSynchroniser = class GlobalStateSynchroniser extends AbstractSynchroniser {
    constructor(
        ...
        super({
            ...
            profile
        }, collection, fileService, environmentService, storageService, userDataSyncStoreService, userDataSyncLocalStoreService, userDataSyncEnablementService, telemetryService, logService, configurationService, uriIdentityService);
        ...
        this._register(
            fileService.watch(this.extUri.dirname(this.environmentService.argvResource))
        );
        ...
        Event.filter(
            ...
            ) => this.syncResource.profile.id === profile.id && ( changes.some(change => change.target === StorageTarget.USER))))) {
                return true;
            }
            return false;
    ...
    async hasLocalData() {
        try {
            const {
                storage
            } = await this.localGlobalStateProvider.getLocalGlobalState(this.syncResource.profile);
    ...
    async getStorageKeys(lastSyncGlobalState) {
        ...
        const unregistered = lastSyncGlobalState?.storage ? ( Object.keys(lastSyncGlobalState.storage)).filter(
            key => !key.startsWith(argvStoragePrefx) && !registered.includes(key) && storageData.get(key) !== undefined
        ) : [];
        if (!isWeb) {
...
let LocalGlobalStateProvider = class LocalGlobalStateProvider {
    ...
    async writeLocalGlobalState(
        ...
        const argv = {};
        const updatedProfileStorage = ( new Map());
        const updatedSharedStorage = profile.isDefault ? ( new Map()) : undefined;
        const storageData = await this.userDataProfileStorageService.readStorageData(profile);
        ...
        if (updatedSharedStorage?.size) {
            ...
            await this.userDataProfileStorageService.updateStorageData(
                profile,
                updatedSharedStorage,
                StorageTarget.USER,
                StorageScope.APPLICATION_SHARED
            );
            this.logService.info(
                `${syncResourceLogLabel}: Updated application shared state`,
                [...( updatedSharedStorage.keys())]
            );
        }
    }
};
LocalGlobalStateProvider = ( __decorate([( __param(0, IFileService)), ( __param(1, IEnvironmentService)), ( __param(2, IUserDataProfileStorageService)), ( __param(3, IUserDataSyncLogService))], LocalGlobalStateProvider));
        ...
        for (const key of ( Object.keys(remoteGlobalState.storage))) {
            if (key.startsWith(argvStoragePrefx)) {
                ...
                if (this.storageService.get(key, scope) === undefined) {
                    storage[key] = {
                        value: remoteGlobalState.storage[key].value,
                        scope
                    };
                }
            }
...
GlobalStateInitializer = ( __decorate([( __param(0, IStorageService)), ( __param(1, IFileService)), ( __param(2, IUserDataProfilesService)), ( __param(3, IEnvironmentService)), ( __param(4, IUserDataSyncLogService)), ( __param(5, IUriIdentityService))], GlobalStateInitializer));
let UserDataSyncStoreTypeSynchronizer = class UserDataSyncStoreTypeSynchronizer {
    ...
    async doSync(userDataSyncStoreType, syncHeaders) {
        ...
        const remoteGlobalState = this.parseGlobalState(globalStateUserData) || {
            storage: {}
        };
...
UserDataSyncStoreTypeSynchronizer = ( __decorate([( __param(1, IStorageService)), ( __param(2, IEnvironmentService)), ( __param(3, IFileService)), ( __param(4, ILogService))], UserDataSyncStoreTypeSynchronizer));
