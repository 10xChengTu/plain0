...
import { Builder, inline, className, stringValue, identValue, asCSSUrl } from '../../../base/browser/cssValue.js';
        ...
        getCSS() {
            ...
            for (const contribution of iconRegistry.getIcons()) {
                ...
                if (fontContribution) {
                    ...
                    rootAttribs.push(
                        inline`${fontFamilyVar}: ${stringValue(fontContribution.id)};`,
                        inline`${contentVar}: ${stringValue(definition.fontCharacter)};`
                    );
                    rules.push(
                        inline`.codicon-${className(contribution.id)}:before { content: ${stringValue(definition.fontCharacter)}; font-family: ${stringValue(fontContribution.id)}; }`
                    );
                } else {
                    rootAttribs.push(
                        inline`${contentVar}: ${stringValue(definition.fontCharacter)}; ${fontFamilyVar}: 'codicon';`
                    );
                    rules.push(
                        inline`.codicon-${className(contribution.id)}:before { content: ${stringValue(definition.fontCharacter)}; }`
                    );
                }
            }
            for (const id in usedFontIds) {
                ...
                for (const l of definition.src) {
                    src.push(
                        inline`${asCSSUrl(l.location)} format(${stringValue(l.format)})`
                    );
                }
                rules.push(
                    inline`@font-face { src: ${src.join(", ")}; font-family: ${stringValue(id)};${fontWeight}${fontStyle} font-display: block; }`
                );
            }
            rules.push(inline`:root, :host { ${rootAttribs.join(" ")} }`);
            return rules.join("\n");
        }
    };
}
