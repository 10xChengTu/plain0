...
let AbstractExtensionsScannerService = class AbstractExtensionsScannerService extends Disposable {
    constructor(
        ...
        this.systemExtensionsCachedScanner = this._register(
            this.instantiationService.createInstance(CachedExtensionsScanner, currentProfile)
        );
        this.userExtensionsCachedScanner = this._register(
            this.instantiationService.createInstance(CachedExtensionsScanner, currentProfile)
        );
        this.extensionsScanner = this._register(this.instantiationService.createInstance(ExtensionsScanner));
        this._register(
            this.systemExtensionsCachedScanner.onDidChangeCache(() => this._onDidChangeCache.fire(ExtensionType.System))
        );
        this._register(
            this.userExtensionsCachedScanner.onDidChangeCache(() => this._onDidChangeCache.fire(ExtensionType.User))
        );
    }
    getTargetPlatform() {
    ...
    async scanSystemExtensions(scanOptions) {
        ...
        promises.push(
            this.scanDevSystemExtensions(scanOptions.language, !!scanOptions.checkControlFile)
        );
        ...
        return this.applyScanOptions(allSystemExtensions, ExtensionType.System, {
            pickLatest: false
        });
    }
    async scanUserExtensions(scanOptions) {
        ...
        extensions = await this.applyScanOptions(extensions, ExtensionType.User, {
    ...
    async scanAllUserExtensions(
        ...
        return this.applyScanOptions(extensions, ExtensionType.User, {
    ...
    async initializeDefaultProfileExtensions() {
        try {
            await this.extensionsProfileScannerService.scanProfileExtensions(this.userDataProfilesService.defaultProfile.extensionsResource, {
                bailOutWhenFileNotFound: true
            });
        } catch (error) {
            if (error instanceof ExtensionsProfileScanningError && error.code === ExtensionsProfileScanningErrorCode.ERROR_PROFILE_NOT_FOUND) {
        ...
        if (!this.initializeDefaultProfileExtensionsPromise) {
            this.initializeDefaultProfileExtensionsPromise = (async () => {
                try {
                    ...
                    const userExtensions = await this.scanAllUserExtensions({
                        includeInvalid: true
                    });
    ...
    dedupExtensions(system, user, development, targetPlatform, pickLatest) {
        ...
        user?.forEach(extension => {
            ...
            if (( productBuiltInExtensionsEnabledWithAutoUpdates.has(extension.identifier.id.toLowerCase())) && !extension.forceAutoUpdate) {
                this.logService.info(
                    `Skipping user installed builtin extension ${extension.identifier.id} with version ${extension.manifest.version} because it is not allowed to in the current product quality ${this.productService.quality}`
                );
                return;
            }
            if (!existing || pick(existing, extension, false)) {
    ...
    async scanDevSystemExtensions(language, checkControlFile) {
        ...
        const result = await Promise.all(( devSystemExtensionsLocations.map(
            async location => this.extensionsScanner.scanExtension((await this.createExtensionScannerInput(
                location,
                false,
                ExtensionType.System,
                language,
    ...
    async createExtensionScannerInput(
        location,
        profile,
        type,
        language$1,
...
AbstractExtensionsScannerService = ( __decorate([( __param(4, IUserDataProfilesService)), ( __param(5, IExtensionsProfileScannerService)), ( __param(6, IExtensionResourceLoaderService)), ( __param(7, IFileService)), ( __param(8, ILogService)), ( __param(9, IEnvironmentService)), ( __param(10, IProductService)), ( __param(11, IUriIdentityService)), ( __param(12, IInstantiationService))], AbstractExtensionsScannerService));
...
let ExtensionManifestTranslator = class ExtensionManifestTranslator extends Disposable {
    ...
    async getLocalizedMessages(extensionLocation, extensionManifest, nlsConfiguration) {
        ...
        const reportInvalidFormat = localized => {
            this.logService.error(this.formatMessage(extensionLocation, ( localize(2060, "Invalid format {0}: JSON object expected.", localized?.path))));
        };
...
ExtensionManifestTranslator = ( __decorate([( __param(0, IExtensionResourceLoaderService)), ( __param(1, IFileService)), ( __param(2, ILogService))], ExtensionManifestTranslator));
let ExtensionsScanner = class ExtensionsScanner extends ExtensionManifestTranslator {
    ...
    async scanExtensionsFromLocation(input) {
        ...
        return (
            coalesce(extensions).sort((a, b) => a.location.path < b.location.path ? -1 : 1)
        );
    }
    ...
    async scanExtensionManifest(extensionLocation) {
        ...
        try {
            ...
            if (toFileOperationResult(error) !== FileOperationResult.FILE_NOT_FOUND) {
                this.logService.error(this.formatMessage(extensionLocation, ( localize(2061, "Cannot read file {0}: {1}.", manifestLocation.path, error.message))));
            }
            throw error;
        }
        let manifest;
...
ExtensionsScanner = ( __decorate([( __param(0, IExtensionsProfileScannerService)), ( __param(1, IUriIdentityService)), ( __param(2, IExtensionResourceLoaderService)), ( __param(3, IFileService)), ( __param(4, IProductService)), ( __param(5, IEnvironmentService)), ( __param(6, ILogService))], ExtensionsScanner));
let CachedExtensionsScanner = class CachedExtensionsScanner extends ExtensionsScanner {
    ...
    async scanExtensions(input) {
        ...
        await this.writeExtensionCache(cacheFile, {
            input,
            result
        });
        return result;
    ...
    getProfile(input) {
        ...
        return this.userDataProfilesService.profiles.find(
            p => this.uriIdentityService.extUri.isEqual(input.location, p.extensionsResource)
        ) ?? this.currentProfile;
    }
};
CachedExtensionsScanner = ( __decorate([( __param(1, IUserDataProfilesService)), ( __param(2, IExtensionsProfileScannerService)), ( __param(3, IUriIdentityService)), ( __param(4, IExtensionResourceLoaderService)), ( __param(5, IFileService)), ( __param(6, IProductService)), ( __param(7, IEnvironmentService)), ( __param(8, ILogService))], CachedExtensionsScanner));
