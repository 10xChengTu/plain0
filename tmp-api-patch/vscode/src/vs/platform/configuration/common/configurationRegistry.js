...
const applicationMachineSettings = {
    properties: {},
    patternProperties: {}
};
...
class ConfigurationRegistry extends Disposable {
    constructor() {
        ...
        this.defaultLanguageConfigurationOverridesNode = {
            id: "defaultOverrides",
            title: ( localize(1947, "Default Language Configuration Overrides")),
            properties: {}
        };
        ...
        this.policyReferenceConfigurations = ( new Map());
        ...
        for (const {
            ...
            for (const key in overrides) {
                ...
                const index = configurationDefaultOverridesForKey.configurationDefaultOverrides.findIndex(
                    configurationDefaultOverride => source ? isConfigurationDefaultSourceEquals(configurationDefaultOverride.source, source) : configurationDefaultOverride.value === overrides[key]
                );
                if (index === -1) {
                    continue;
                }
    ...
    mergeDefaultConfigurationsForOverrideIdentifier(
        overrideIdentifier,
    ...
    mergeDefaultConfigurationsForConfigurationProperty(propertyKey, value, valuesSource, existingDefaultOverride) {
        ...
        const isObjectSetting = isObject(value) && (property !== undefined && property.type === "object" || property === undefined && (isUndefined(existingDefaultValue) || isObject(existingDefaultValue)));
        if (isObjectSetting) {
            ...
            value = {
                ...(isObject(existingDefaultValue) ? existingDefaultValue : {}),
                ...value
            };
        }
        return {
        ...
        if (properties) {
            for (const key in properties) {
                ...
                if (property.experiment) {
                    ...
                    console.error(
                        `Invalid tag 'onExP' found for property '${key}'. Please use 'experiment' property instead.`
                    );
                    property.experiment = {
                        mode: "startup"
                    };
    ...
    updateOverridePropertyPatternKey() {
        for (const overrideIdentifier of ( this.overrideIdentifiers.values())) {
            ...
            const resourceLanguagePropertiesSchema = {
                type: "object",
                description: ( localize(1949, "Configure editor settings to be overridden for a language.")),
                errorMessage: ( localize(1950, "This setting does not support per-language configuration.")),
                $ref: resourceLanguageSettingsSchemaId
            };
    ...
    updatePropertyDefaultValue(key, property) {
        ...
        if (
        configurationdefaultOverride && (!property.disallowConfigurationDefault || !configurationdefaultOverride.source)) {
...
function validateProperty(property, schema, extensionId) {
    if (!property.trim()) {
        return localize(1951, "Cannot register an empty property");
    }
...
const EXTENSION_UNIFICATION_EXTENSION_IDS = ( new Set(product.defaultChatAgent ? ( [
    product.defaultChatAgent.extensionId,
    product.defaultChatAgent.chatExtensionId
].map(id => id.toLowerCase())) : []));

export { ConfigurationScope, EXTENSION_UNIFICATION_EXTENSION_IDS, EditPresentationTypes, Extensions, OVERRIDE_PROPERTY_PATTERN, OVERRIDE_PROPERTY_REGEX, allSettings, applicationMachineSettings, applicationSettings, configurationDefaultsSchemaId, getAllConfigurationProperties, getDefaultValue, getScopes, isConfigurationDefaultSourceEquals, keyFromOverrideIdentifiers, machineOverridableSettings, machineSettings, overrideIdentifiersFromKey, parseScope, resourceLanguageSettingsSchemaId, resourceSettings, validateProperty, windowSettings };
