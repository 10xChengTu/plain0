...
(function(ProfileResourceType) {
    ...
    ProfileResourceType["LanguageModels"] = "languageModels";
...
function reviveProfile(profile, scheme) {
    return {
        ...
        name: profile.name,
        icon: profile.icon,
        location: URI.revive(profile.location).with({
            scheme
        }),
        globalStorageHome: URI.revive(profile.globalStorageHome).with({
            scheme
        }),
        settingsResource: URI.revive(profile.settingsResource).with({
            scheme
        }),
        keybindingsResource: URI.revive(profile.keybindingsResource).with({
            scheme
        }),
        tasksResource: URI.revive(profile.tasksResource).with({
            scheme
        }),
        snippetsHome: URI.revive(profile.snippetsHome).with({
            scheme
        }),
        promptsHome: URI.revive(profile.promptsHome).with({
            scheme
        }),
        extensionsResource: URI.revive(profile.extensionsResource).with({
            scheme
        }),
        mcpResource: URI.revive(profile.mcpResource).with({
            scheme
        }),
        languageModelsResource: URI.revive(profile.languageModelsResource).with({
            scheme
        }),
        agentPluginsHome: URI.revive(profile.agentPluginsHome),
        cacheHome: URI.revive(profile.cacheHome).with({
            scheme
        }),
...
let UserDataProfilesService = class UserDataProfilesService extends Disposable {
        ...
        if (!this._profilesObject) {
            ...
            try {
                for (const storedProfile of this.getStoredProfiles()) {
                    ...
                    profiles.push(
                        toUserDataProfile(id, storedProfile.name, storedProfile.location, this.profilesCacheHome, {
    ...
    createDefaultProfile() {
        const defaultProfile = toUserDataProfile("__default__profile__", ( localize(2663, "Default")), this.environmentService.userRoamingDataHome, this.profilesCacheHome);
        return {
            ...defaultProfile,
    ...
    async createTransientProfile(workspaceIdentifier) {
        ...
        return this.createProfile(( hash(generateUuid()).toString(16)), name, {
            transient: true
        }, workspaceIdentifier);
    }
            ...
            profileCreationPromise = (async () => {
                try {
                    const existing = this.profiles.find(
                        p => p.id === id || (id !== AGENTS_WINDOW_PROFILE_ID && !p.isTransient && !options?.transient && p.name === name)
                    );
                    if (existing) {
                    ...
                    const profile = toUserDataProfile(id, name, this.uriIdentityService.extUri.joinPath(
                        this.profilesHome,
                        ...(id === AGENTS_WINDOW_PROFILE_ID ? [SYSTEM_PROFILES_HOME, id] : [id])
                    ), this.profilesCacheHome, id === AGENTS_WINDOW_PROFILE_ID ? {} : options, this.defaultProfile);
    ...
    async removeProfile(profileToRemove) {
        ...
        this.updateProfiles([], [profile], []);
        try {
            await this.fileService.del(profile.cacheHome, {
                recursive: true
            });
        } catch (error) {
    ...
    async cleanUp() {
        try {
            if (await this.fileService.exists(this.profilesHome)) {
                ...
                await Promise.all(( (stat.children || []).filter(
                    child => child.isDirectory && child.name !== SYSTEM_PROFILES_HOME && this.profiles.every(p => !this.uriIdentityService.extUri.isEqual(p.location, child.resource))
    ...
    getProfileForWorkspace(workspaceIdentifier) {
        ...
        return URI.isUri(workspace) ? this.profiles.find(
            p => p.workspaces?.some(w => this.uriIdentityService.extUri.isEqual(w, workspace))
...
UserDataProfilesService = ( __decorate([( __param(0, IEnvironmentService)), ( __param(1, IFileService)), ( __param(2, IUriIdentityService)), ( __param(3, ILogService))], UserDataProfilesService));

export { AGENTS_WINDOW_PROFILE_ID, ProfileResourceType, UserDataProfilesService, isUserDataProfile, reviveProfile, toUserDataProfile };
