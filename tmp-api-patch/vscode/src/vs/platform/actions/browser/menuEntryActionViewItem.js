...
import * as menuEntryActionViewItem from './menuEntryActionViewItem.css';

registerCss(menuEntryActionViewItem);
    ...
    async onClick(event) {
        ...
        if (this._options?.onClickAnimation && this.element && !this._accessibilityService.isMotionReduced()) {
            ...
            triggerClickAnimation(
                this.element,
                this._options.onClickAnimation,
                ThemeIcon.isThemeIcon(icon) ? icon : undefined
            );
        }
        try {
    ...
    updateLabel() {
        ...
        if (this.label) {
            ...
            if (this._options?.conversational) {
                this.label.textContent = ( localize(1818, "{1} to {0}", this._action.label, kb2));
            } else {
                this.label.textContent = ( localize(1819, "{0} ({1})", this._action.label, kb2));
            }
        }
    }
...
let DropdownWithDefaultActionViewItem = class DropdownWithDefaultActionViewItem extends BaseActionViewItem {
    ...
    constructor(
        ...
        this._defaultAction = this._defaultActionDisposables.add(
            this._instaService.createInstance(MenuEntryActionViewItem, defaultAction, {
        ...
        this._dropdown = this._register(( new DropdownMenuActionViewItem(
            submenuAction,
...
let SubmenuEntrySelectActionViewItem = class SubmenuEntrySelectActionViewItem extends SelectActionViewItem {
    constructor(action, contextViewService, configurationService) {
        super(null, action, ( action.actions.map(a => (a.id === Separator.ID ? SeparatorSelectOption : {
            text: a.label,
...
SubmenuEntrySelectActionViewItem = ( __decorate([( __param(1, IContextViewService)), ( __param(2, IConfigurationService))], SubmenuEntrySelectActionViewItem));
...
export { DropdownWithDefaultActionViewItem, MenuEntryActionViewItem, SubmenuEntryActionViewItem, TextOnlyMenuEntryActionViewItem, createActionViewItem, fillInActionBarActions, getActionBarActions, getContextMenuActions, getFlatActionBarActions, getFlatContextMenuActions };
