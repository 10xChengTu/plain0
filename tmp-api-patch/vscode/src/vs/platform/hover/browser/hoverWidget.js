...
import { $ as $$1, getWindow, addDisposableListener, isHTMLElement, DisposableResizeObserver, prepend, append, getActiveElement, getDomNodeZoomLevel, EventType } from '../../../base/browser/dom.js';
...
import { HoverPosition, HoverWidget as HoverWidget$1, HoverAction, getHoverAccessibleViewHint } from '../../../base/browser/ui/hover/hoverWidget.js';
import { Widget } from '../../../base/browser/ui/widget.js';
import '../../../base/browser/ui/contextview/contextview.js';
import { IMarkdownRendererService } from '../../markdown/browser/markdownRenderer.service.js';
...
let HoverWidget = class HoverWidget extends Widget {
    ...
    constructor(
        ...
        this._hoverPosition = options.position?.hoverPosition === undefined ? HoverPosition.ABOVE : isNumber(options.position.hoverPosition) ? options.position.hoverPosition : HoverPosition.BELOW;
        ...
        this._register(
            addDisposableListener(this._targetWindow, "blur", () => this.dispose())
        );
        ...
        if (typeof options.content === "string") {
            ...
            const resizeObserver = new DisposableResizeObserver("HoverWidget", () => {
                this.layout();
                this._onRequestLayout.fire();
            });
            resizeObserver.observe(contentsElement);
            this._register(resizeObserver);
        } else {
            const markdown = options.content;
            const {
                element
            } = this._register(this._markdownRenderer.render(markdown, {
        ...
        if (options.appearance?.showHoverHint) {
            ...
            infoElement.textContent = ( localize(2126, "Hold {0} key to mouse over", isMacintosh ? "Option" : "Alt"));
    ...
    computeXCordinate(target) {
        ...
        if (this._target.x !== undefined) {
            ...
            if (this._x + hoverWidth >= this._targetDocumentElement.clientWidth) {
                ...
                this._x = Math.max(
                    this._targetDocumentElement.clientWidth - hoverWidth - Constants.HoverWindowEdgeMargin,
...
HoverWidget = ( __decorate([( __param(1, IKeybindingService)), ( __param(2, IConfigurationService)), ( __param(3, IMarkdownRendererService)), ( __param(4, IAccessibilityService))], HoverWidget));
class CompositeMouseTracker extends Widget {
    ...
    addElement(element) {
        ...
        store.add(
            addDisposableListener(element, EventType.MOUSE_OVER, () => this._onTargetMouseOver())
        );
        store.add(
            addDisposableListener(element, EventType.MOUSE_LEAVE, () => this._onTargetMouseLeave())
        );
