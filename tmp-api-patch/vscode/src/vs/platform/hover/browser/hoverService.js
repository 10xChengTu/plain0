...
let HoverService = class HoverService extends Disposable {
    ...
    _createHover(options, skipLastFocusedUpdate) {
        ...
        if (!options.persistence?.sticky) {
            if ("targetElements" in options.target) {
                ...
                hoverDisposables.add(addDisposableListener(
                    options.target,
                    EventType.CLICK,
                    () => this._hideHoverAndDescendants(hover)
                ));
            ...
            if (focusedElement) {
                ...
                hoverDisposables.add(
                    addDisposableListener(focusedElement, EventType.KEY_UP, e => this._keyUp(e, hover))
                );
                hoverDisposables.add(
    ...
    _showHover(result, options, focus) {
        ...
        const container = options.container ?? this._layoutService.getContainer(getWindow(
            isHTMLElement(options.target) ? options.target : options.target.targetElements[0]
        ));
        ...
        if (focus || options.persistence?.sticky) {
            ...
            store.add(
                addDisposableListener(targetWindow, EventType.RESIZE, () => contextView.layout())
            );
        }
    ...
    setupManagedHover(hoverDelegate, targetElement, content, options) {
        ...
        store.add(addDisposableListener(targetElement, EventType.MOUSE_OVER, e => {
            ...
            if (hoverDelegate.placement === undefined || hoverDelegate.placement === "mouse") {
                ...
                mouseOverStore.add(
                    addDisposableListener(targetElement, EventType.MOUSE_MOVE, onMouseMove, true)
                );
            }
            ...
            mouseOverStore.add(triggerShowHover(
        ...
        const onFocus = e => {
            ...
            toDispose.add(triggerShowHover(
                typeof hoverDelegate.delay === "function" ? hoverDelegate.delay(content) : hoverDelegate.delay,
                false,
                target
            ));
...
HoverService = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, IConfigurationService)), ( __param(2, IContextMenuService)), ( __param(3, IKeybindingService)), ( __param(4, ILayoutService)), ( __param(5, IAccessibilityService))], HoverService));
