...
import { getDefaultHoverDelegate, createInstantHoverDelegate } from '../hover/hoverDelegateFactory.js';
...
class Button extends Disposable {
    ...
    set label(value) {
        ...
        if (isMarkdownString(value)) {
            const rendered = renderMarkdown(value, buttonMarkdownRenderOptions, createElement("span"));
            rendered.dispose();
    ...
    setTitle(title) {
        if (!this._hover && title !== "") {
            this._hover = this._register(getBaseLayerHoverDelegate().setupManagedHover(
...
class ButtonBar {
    ...
    pushButton(button) {
        ...
        this._buttonStore.add(addDisposableListener(button.element, EventType.KEY_DOWN, e => {
            ...
            if (event.equals(
                this.options?.alignment === ButtonBarAlignment.Vertical ? KeyCode.UpArrow : KeyCode.LeftArrow
            )) {
                buttonIndexToFocus = index > 0 ? index - 1 : this._buttons.length - 1;
            } else if (event.equals(
                this.options?.alignment === ButtonBarAlignment.Vertical ? KeyCode.DownArrow : KeyCode.RightArrow
            )) {
...
export { Button, ButtonBar, ButtonBarAlignment, ButtonWithDescription, ButtonWithDropdown, ButtonWithIcon, unthemedButtonStyles };
