...
import { $ as $$1, append, isShadowRoot, getTotalHeight, isActiveElement, getTotalWidth } from '../../dom.js';
...
import { AnchorAlignment } from '../../../common/layout.js';
...
class InputBox extends Widget {
        ...
        if (this.options.flexibleHeight) {
            ...
            const onAnchoredSelectionChange = Event.filter(onSelectionChange.event, () => {
                ...
                if (isShadowRoot(rootNode)) {
                    selection = rootNode.getSelection?.() ?? selection;
                }
    ...
    _showMessage() {
        ...
        if (this.message.type === MessageType.ERROR) {
            alertText = ( localize(20, "Error: {0}", this.message.content));
        } else if (this.message.type === MessageType.WARNING) {
            alertText = ( localize(21, "Warning: {0}", this.message.content));
        } else {
...
class HistoryInputBox extends InputBox {
    constructor(container, contextViewProvider, options) {
        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_NO_PARENS = ( localize(23, " or {0} for history", `\u21C5`));
        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_IN_PARENS = ( localize(24, " ({0} for history)", `\u21C5`));
    ...
    showNextValue() {
        ...
        this.value = next ?? "";
        status(this.value ? this.value : ( localize(25, "Cleared Input")));
    }
    showPreviousValue() {
        if (!( this.history.has(this.value))) {
            this.addToHistory();
