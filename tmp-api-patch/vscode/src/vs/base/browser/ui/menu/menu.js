...
class Menu extends ActionBar {
    constructor(container, actions, options, menuStyles) {
        ...
        super(menuElement, {
            ...
            triggerKeys: {
                keys: [KeyCode.Enter, ...(isMacintosh || isLinux ? [KeyCode.Space] : [])],
                keyDown: true
            }
        });
        ...
        if (this.options.label) {
            ...
            if (label) {
                ...
                if (matches) {
                    ...
                    if (escMatch) {
                        this.label.append(
                            ltrim(replaceDoubleEscapes(label.substr(0, escMatch.index)), " "),
                            $("u", {
                                "aria-hidden": "true"
                            }, escMatch[3]),
                            rtrim(
                                replaceDoubleEscapes(label.substr(escMatch.index + escMatch[0].length)),
                                " "
                            )
                        );
                    } else {
...
class SubmenuMenuActionViewItem extends BaseMenuActionViewItem {
    ...
    calculateSubmenuMenuLayout(windowDimensions, submenu, entry, expandDirection) {
        ...
        ret.left = layout(windowDimensions.width, submenu.width, {
            position: expandDirection.horizontal === HorizontalDirection.Right ? LayoutAnchorPosition.Before : LayoutAnchorPosition.After,
            offset: entry.left,
            size: entry.width
        }).position;
...
export { HorizontalDirection, MENU_ESCAPED_MNEMONIC_REGEX, MENU_MNEMONIC_REGEX, Menu, VerticalDirection, cleanMnemonic, formatRule, getMenuWidgetCSS, unthemedMenuStyles };
