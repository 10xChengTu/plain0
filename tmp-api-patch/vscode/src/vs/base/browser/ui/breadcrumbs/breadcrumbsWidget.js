...
import { createElement, addStandardDisposableListener, trackFocus, Dimension, modify, getWindow, measure, isAncestorOfActiveElement, clearNode, $ } from '../../dom.js';
...
import * as breadcrumbsWidget from './breadcrumbsWidget.css';

registerCss(breadcrumbsWidget);
...
class BreadcrumbsWidget {
    constructor(
        ...
        this._disposables.add(
            addStandardDisposableListener(this._domNode, "click", e => this._onClick(e))
        );
    ...
    _render(start) {
        ...
        for (; start < this._items.length; start++) {
            const item = this._items[start];
            const node = this._freeNodes.length > 0 ? this._freeNodes.pop() : createElement("div");
            if (node) {
                this._renderItem(item, node);
