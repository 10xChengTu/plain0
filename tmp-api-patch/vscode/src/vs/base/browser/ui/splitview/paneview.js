...
import { getWindow, $, append, trackFocus, EventType, addDisposableListener, scheduleAtNextAnimationFrame, EventHelper, isHTMLElement, clearNode, getActiveElement } from '../../dom.js';
...
class Pane extends Disposable {
    ...
    resolveHeaderSize() {
        if (this._headerSize === undefined) {
            const size = parseInt(
                getWindow(this.element).getComputedStyle(this.element).getPropertyValue("--pane-header-size"),
                10
            );
    ...
    getAriaHeaderLabel(title) {
        return localize(32, "{0} Section", title);
    }
    constructor(options) {
        super();
    ...
    render() {
        ...
        this._register(
            focusTracker.onDidBlur(() => this.header?.classList.remove("focused"), null)
        );
        this.updateHeader();
        ...
        this._register(
            Event.filter(onHeaderKeyDown, e => e.keyCode === KeyCode.LeftArrow, eventDisposables)(() => this.setExpanded(false), null)
        );
        this._register(
            Event.filter(onHeaderKeyDown, e => e.keyCode === KeyCode.RightArrow, eventDisposables)(() => this.setExpanded(true), null)
        );
...
class PaneDraggable extends Disposable {
    ...
    constructor(pane, dnd, context) {
        ...
        this._register(
            addDisposableListener(pane.draggableElement, "dragstart", e => this.onDragStart(e))
        );
        this._register(
            addDisposableListener(pane.dropTargetElement, "dragenter", e => this.onDragEnter(e))
        );
        this._register(
            addDisposableListener(pane.dropTargetElement, "dragleave", e => this.onDragLeave(e))
        );
        this._register(
            addDisposableListener(pane.dropTargetElement, "dragend", e => this.onDragEnd(e))
        );
...
class PaneView extends Disposable {
    constructor(container, options = {}) {
        ...
        this.splitview = this._register(( new SplitView(this.element, {
        ...
        const onHeaderKeyDown = ( Event.map(Event.filter(
            onKeyDown.event,
            e => isHTMLElement(e.target) && e.target.classList.contains("pane-header"),
            eventDisposables
        ), e => ( new StandardKeyboardEvent(e)), eventDisposables));
        this._register(
    ...
    focusPrevious() {
        ...
        const index = headers.indexOf(getActiveElement(this.element.ownerDocument));
        if (index === -1) {
            return;
        }
