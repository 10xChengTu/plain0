...
import { DisposableStore, Disposable, toDisposable, MutableDisposable } from '../common/lifecycle.js';
...
class ActiveElementTracker extends Disposable {
    get onDidChangeActiveElement() {
        return this._onDidChangeActiveElement.event;
    }
    constructor(window) {
        super();
        this.window = window;
        ...
        this._onFocusIn = e => {
            ...
            if (isElement(e.target) && e.target.shadowRoot) {
                ...
                disposableStore.add(
                    addDisposableListener(e.target.shadowRoot, EventType.FOCUS_IN, this._onFocusIn)
                );
...
function addDisposableGenericMouseDownListener(node, handler, useCapture) {
    return addDisposableListener(
        node,
        isIOS && BrowserFeatures.pointerEvents ? EventType.POINTER_DOWN : EventType.MOUSE_DOWN,
        handler,
        useCapture
    );
}
function addDisposableGenericMouseMoveListener(node, handler, useCapture) {
    return addDisposableListener(
        node,
        isIOS && BrowserFeatures.pointerEvents ? EventType.POINTER_MOVE : EventType.MOUSE_MOVE,
        handler,
        useCapture
    );
}
function addDisposableGenericMouseUpListener(node, handler, useCapture) {
    return addDisposableListener(
        node,
        isIOS && BrowserFeatures.pointerEvents ? EventType.POINTER_UP : EventType.MOUSE_UP,
        handler,
        useCapture
    );
}
...
function getTopLeftOffset(element) {
    ...
    while ((element = element.parentNode) !== null && element !== element.getRootNode()) {
...
function getFullscreenElement(_document = getActiveDocument()) {
    ...
    node.webkitFullscreenElement ?? null;
...
class ModifierKeyEmitter extends Emitter {
    constructor() {
        ...
        this._subscriptions.add(Event.runAndSubscribe(onDidRegisterWindow, (
            {
                window,
                disposables
            }
...
let _lastInvokedDisposableResizeObserver;
function getRecentDisposableResizeObserverAttributionForLoopError(message) {
            ...
            if (key === "style") {
                for (const [cssKey, cssValue] of Object.entries(value)) {
                    ...
                    if (isObservable(cssValue)) {
                        this._deriveds.push(derivedOpts({
                            owner: this,
                            debugName: () => `set.style.${key}`
                        }, reader => {
