...
function renderMarkdown(markdown, options = {}, target) {
    ...
    const {
        ...
        codeBlocks,
        syncCodeBlocks
    } = createMarkdownRenderer(markedInstance, options, markdown);
    ...
    if (options.fillInIncompleteTokens) {
        ...
        renderedMarkdown = markedInstance.parse(value, {
            ...options?.markedOptions,
            renderer,
            async: false
        });
...
function massageHref(markdown, href, isDomUri) {
    ...
    if (uri.query) {
        uri = uri.with({
            query: uriMassage(markdown, uri.query)
        });
    }
    return ( uri.toString());
}
...
function renderAsPlaintext(str, options) {
    ...
    const html = parse$1(value, {
        async: false,
        renderer
    });
    return ( sanitizeRenderedMarkdown(html, {
        isTrusted: false
    }, {}).toString()).replace(/&(#\d+|[a-zA-Z]+);/g, m => unescapeInfo.get(m) ?? m).trim();
}
