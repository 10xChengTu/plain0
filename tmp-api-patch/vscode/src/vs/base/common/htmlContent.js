...
class MarkdownString {
    ...
    appendText(value, newlineStyle = MarkdownStringTextNewlineStyle.Paragraph) {
        this.value +=
        escapeMarkdownSyntaxTokens(this.supportThemeIcons ? escapeIcons(value) : value).replace(/([ \t]+)/g, (_match, g1) => "&nbsp;".repeat(g1.length)).replace(/\>/gm, "\\>").replace(
            /\n/g,
...
function isMarkdownString(thing) {
    if (thing instanceof MarkdownString) {
        ...
        return typeof thing.value === "string" && (typeof thing.isTrusted === "boolean" || typeof thing.isTrusted === "object" || thing.isTrusted === undefined) && (typeof thing.supportThemeIcons === "boolean" || thing.supportThemeIcons === undefined) && (typeof thing.supportAlertSyntax === "boolean" || thing.supportAlertSyntax === undefined);
    }
    return false;
}
...
export { MarkdownString, MarkdownStringTextNewlineStyle, appendEscapedMarkdownCodeBlockFence, appendEscapedMarkdownInlineCode, createCommandUri, createMarkdownCommandLink, createMarkdownLink, escapeDoubleQuotes, escapeMarkdownSyntaxTokens, isEmptyMarkdownString, isMarkdownString, markdownStringEqual, parseHrefAndDimensions, removeMarkdownEscapes };
