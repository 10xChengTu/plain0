    ...
    if (parsed && parsed.type === "sequence") {
        for (const item of parsed.items) {
            if (item.type === "scalar") {
                items.push({
                    ...item,
                    startOffset: item.startOffset + shift,
                    endOffset: item.endOffset + shift
                });
            }
        }
    } else {
...
class YamlParser {
    ...
    parseBlockMapping(baseIndent, inlineFirstEntry = false) {
        ...
        while (this.currentToken().type !== TokenType.EOF) {
            ...
            if (indent !== baseIndent) {
                if (indent > baseIndent) {
                    this.emitError(( localize(137, "Unexpected indentation (expected {0}, got {1})", baseIndent, indent)), this.currentToken().startOffset, this.currentToken().endOffset, "unexpected-indentation");
                } else {
                    break;
                }
            }
            ...
            if (!this.options.allowDuplicateKeys && ( seenKeys.has(entry.key.value))) {
                this.emitError(( localize(138, "Duplicate key: \"{0}\"", entry.key.value)), entry.key.startOffset, entry.key.endOffset, "duplicate-key");
            }
    ...
    parseMappingEntry(baseIndent) {
        ...
        if (colon.type !== TokenType.Colon) {
            this.emitError(( localize(139, "Expected \":\"")), colon.startOffset, colon.endOffset, "expected-colon");
            return undefined;
        }
    ...
    parseMappingValue(baseIndent, colonToken) {
        ...
        const afterNewline = this.currentToken();
        if (afterNewline.type === TokenType.EOF) {
            this.emitError(( localize(140, "Missing value")), colonToken.startOffset, colonToken.endOffset, "missing-value");
    ...
    parseSequenceItemValue(baseIndent, dashToken) {
        ...
        this.skipNewlinesAndComments();
        if (this.currentToken().type === TokenType.EOF) {
            this.emitError(( localize(141, "Missing sequence item value")), dashToken.startOffset, dashToken.endOffset, "missing-value");
    ...
    parseFlowMap() {
        ...
        while (this.currentToken().type !== TokenType.FlowMapEnd && this.currentToken().type !== TokenType.EOF) {
            ...
            if (this.currentToken().type === TokenType.Scalar) {
                ...
                this.emitError(( localize(142, "Expected mapping key")), this.currentToken().startOffset, this.currentToken().endOffset, "expected-key");
                break;
            }
            this.skipFlowWhitespace();
            let value;
        ...
        if (endToken.type === TokenType.FlowMapEnd) {
            ...
            this.emitError(( localize(143, "Expected \"}\"")), endToken.startOffset, endToken.endOffset, "expected-flow-map-end");
        }
        return {
            type: "map",
            properties,
    ...
    parseFlowSeq() {
        ...
        while (this.currentToken().type !== TokenType.FlowSeqEnd && this.currentToken().type !== TokenType.EOF) {
            ...
            if (this.currentToken().type === TokenType.FlowMapStart) {
                ...
                this.emitError(( localize(144, "Unexpected token in flow sequence")), this.currentToken().startOffset, this.currentToken().endOffset, "unexpected-token");
                this.advance();
                continue;
            }
            items.push(item);
        ...
        if (endToken.type === TokenType.FlowSeqEnd) {
            ...
            this.emitError(( localize(145, "Expected \"]\"")), endToken.startOffset, endToken.endOffset, "expected-flow-seq-end");
        }
        return {
            type: "sequence",
            items,
...
export { MarkdownNode, parse, parseCommaSeparatedList, parseFrontMatter };
