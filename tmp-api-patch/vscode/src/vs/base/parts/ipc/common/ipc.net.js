...
(function(SocketDiagnostics) {
    ...
    function traceSocketEvent(nativeObject, socketDebugLabel, type, data) {
        ...
        if (data instanceof VSBuffer || data instanceof Uint8Array || data instanceof ArrayBuffer || ArrayBuffer.isView(data)) {
            ...
            copiedData.set(data);
            SocketDiagnostics.records.push({
                timestamp: Date.now(),
                id,
...
var SocketTimeoutReason;
(function(SocketTimeoutReason) {
    SocketTimeoutReason["UNACKNOWLEDGED_MESSAGE"] = "unacknowledgedMessage";
    SocketTimeoutReason["KEEP_ALIVE"] = "keepAlive";
})(SocketTimeoutReason || (SocketTimeoutReason = {}));
...
class ProtocolWriter {
    ...
    _writeNow() {
        ...
        this._socket.traceSocketEvent(SocketDiagnosticsEventType.ProtocolWrite, {
...
class PersistentProtocol {
    ...
    _keepAliveTimeoutCheck() {
        ...
        if (timeSinceLastReceivedSomeData >= ProtocolConstants.TimeoutTime && timeSinceLastTimeout >= ProtocolConstants.TimeoutTime) {
            if (!this._loadEstimator.hasHighLoad()) {
...
export { BufferedEmitter, ChunkStream, Client, LoadEstimator, PersistentProtocol, Protocol, ProtocolConstants, SocketCloseEventType, SocketDiagnostics, SocketDiagnosticsEventType, SocketTimeoutReason };
