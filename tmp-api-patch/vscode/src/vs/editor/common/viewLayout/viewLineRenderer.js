...
class RenderLineInput {
    ...
    constructor(
        ...
        this.renderWhitespace = (renderWhitespace === "all" ? RenderWhitespace.All : renderWhitespace === "boundary" ? RenderWhitespace.Boundary : renderWhitespace === "selection" ? RenderWhitespace.Selection : renderWhitespace === "trailing" ? RenderWhitespace.Trailing : RenderWhitespace.None);
...
function renderViewLine(input, sb) {
    if (input.lineContent.length === 0) {
        ...
        return ( new RenderLineOutput(( new CharacterMapping(0, 0)), ForeignElementType.None));
    }
...
function resolveRenderLineInput(input) {
    ...
    if (input.renderWhitespace === RenderWhitespace.All || input.renderWhitespace === RenderWhitespace.Boundary || (input.renderWhitespace === RenderWhitespace.Selection && !!input.selectionsOnLine) || (input.renderWhitespace === RenderWhitespace.Trailing && !input.continuesWithWrappedLine)) {
...
function _renderLine(input, sb) {
    ...
    if (isOverflowing) {
        ...
        sb.appendString(( localize(945, "Show more ({0})", renderOverflowingCharCount(overflowingCharCount))));
        sb.appendString("</span>");
    }
    sb.appendString("</span>");
    return ( new RenderLineOutput(characterMapping, containsForeignElements));
...
export { CharacterMapping, DomPosition, ForeignElementType, RenderLineInput, RenderLineOutput, RenderLineOutput2, RenderWhitespace, renderViewLine, renderViewLine2 };
