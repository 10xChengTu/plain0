...
import { MovedText, LinesDiff } from './linesDiffComputer.js';
import { RangeMapping, lineRangeMappingFromRangeMappings, LineRangeMapping } from './rangeMapping.js';
...
function loadExternalModule() {
    if (!externalModulePromise) {
        externalModulePromise = import('@vscode/diff');
    }
...
function loadExternalComputer(useWasm) {
    if (useWasm) {
        if (!externalWasmDiffComputerPromise) {
            externalWasmDiffComputerPromise = loadExternalModule().then(m => m.createDiffComputer({
                useWasm: true
            }));
        }
    ...
    if (!externalDiffComputerPromise) {
        externalDiffComputerPromise = loadExternalModule().then(m => m.createDiffComputer({
            useWasm: false
        }));
    }
