...
let MultiDiffEditorWidgetImpl = class MultiDiffEditorWidgetImpl extends Disposable {
    constructor(
        ...
        const placeholderMessage = derived(reader => {
            ...
            const vm = this._viewModel.read(reader);
            return (!vm || vm.isLoading.read(reader)) ? ( localize(238, "Loading...")) : ( localize(239, "No Changed Files"));
        });
        this._register(autorun(reader => {
    ...
    _goToFile(index, position, focusEditor = true) {
        ...
        this.reveal({
            original: item.viewModel.originalUri,
            modified: item.viewModel.modifiedUri
        });
        const editor = item.template.get()?.editor;
        if (editor?.getDiffComputationResult()?.changes2?.length) {
            if (position === "first") {
                ...
                modifiedEditor.setPosition({
                    lineNumber: lastChange.modified.startLineNumber,
                    column: 1
                });
