...
import { RunOnceScheduler, rejectIfNotCanceled } from '../../../../base/common/async.js';
import { CancellationTokenSource } from '../../../../base/common/cancellation.js';
import { Disposable, toDisposable } from '../../../../base/common/lifecycle.js';
import '../../../../base/common/observableInternal/index.js';
import { IDiffProviderFactoryService } from './diffProviderFactoryService.service.js';
...
import '../../../common/diff/defaultLinesDiffComputer/linesSliceCharSequence.js';
    ...
    constructor(model, _options, _diffProviderFactoryService) {
        ...
        this._register(autorun(reader => {
            ...
            const updatedLastUnchangedRegions = ( lastUnchangedRegions.regions.map(
                (r, idx) => (!lastUnchangedRegionsOrigRanges[idx] || !lastUnchangedRegionsModRanges[idx]) ? undefined : ( new UnchangedRegion(
        ...
        this._register(autorun(async reader => {
            ...
            transaction(tx => {
                ...
                this.movedTextToCompare.set(currentSyncedMovedText ? this._lastDiff.moves.find(
                    m => m.lineRangeMapping.modified.intersect(currentSyncedMovedText.lineRangeMapping.modified)
                ) : undefined, tx);
            });
        }));
    }
...
class UnchangedRegion {
    ...
    showMoreAbove(count = 10, tx) {
        ...
        this._visibleLineCountTop.set(
            Math.min(this._visibleLineCountTop.get() + count, maxVisibleLineCountTop),
            tx
        );
    }
    showMoreBelow(count = 10, tx) {
        ...
        this._visibleLineCountBottom.set(
            Math.min(this._visibleLineCountBottom.get() + count, maxVisibleLineCountBottom),
            tx
        );
    }
    showAll(tx) {
    ...
    showOriginalLine(lineNumber, preference, tx) {
        ...
        if (preference === RevealPreference.FromCloserSide && top < bottom || preference === RevealPreference.FromTop) {
            this._visibleLineCountTop.set(Math.min(
                this._visibleLineCountTop.get() + bottom - top,
                this.getMaxVisibleLineCountTop()
            ), tx);
        } else {
            this._visibleLineCountBottom.set(Math.min(
