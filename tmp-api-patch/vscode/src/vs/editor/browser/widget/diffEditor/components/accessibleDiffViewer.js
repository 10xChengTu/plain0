...
import { createElement, reset, addStandardDisposableListener, createTextNode, addDisposableListener } from '../../../../../base/browser/dom.js';
...
const accessibleDiffViewerInsertIcon = registerIcon("diff-review-insert", Codicon.add, ( localize(191, "Icon for 'Insert' in accessible diff viewer.")));
const accessibleDiffViewerRemoveIcon = registerIcon("diff-review-remove", Codicon.remove, ( localize(192, "Icon for 'Remove' in accessible diff viewer.")));
const accessibleDiffViewerCloseIcon = registerIcon("diff-review-close", Codicon.close, ( localize(193, "Icon for 'Close' in accessible diff viewer.")));
let AccessibleDiffViewer = class AccessibleDiffViewer extends Disposable {
    ...
    constructor(
        ...
        this._state = derived(this, reader => {
            ...
            const model = reader.store.add(
                this._instantiationService.createInstance(ViewModel, this._diffs, this._models, this._setVisible, this._canClose)
            );
            const view = reader.store.add(
                this._instantiationService.createInstance(View, this._parentNode, model, this._width, this._height, this._models)
            );
            return {
                model,
                view
    ...
    _goToGroupDelta(delta, tx) {
        ...
        subtransaction(tx, tx => {
            this._currentGroupIdx.set(
                OffsetRange.ofLength(groups.length).clipCyclic(this._currentGroupIdx.get() + delta),
                tx
            );
    ...
    _goToLineDelta(delta) {
        ...
        transaction(tx => {
            this._currentElementIdx.set(
                OffsetRange.ofLength(group.lines.length).clip(this._currentElementIdx.get() + delta),
                tx
            );
        });
    }
...
let View = class View extends Disposable {
    constructor(_element, _model, _width, _height, _models, _languageService) {
        ...
        this._register(addStandardDisposableListener(this.domNode, "keydown", e => {
            if (e.equals(KeyCode.DownArrow) || e.equals(KeyMod$1.CtrlCmd | KeyCode.DownArrow) || e.equals(KeyMod$1.Alt | KeyCode.DownArrow)) {
                e.preventDefault();
                this._model.goToNextLine();
            }
            if (e.equals(KeyCode.UpArrow) || e.equals(KeyMod$1.CtrlCmd | KeyCode.UpArrow) || e.equals(KeyMod$1.Alt | KeyCode.UpArrow)) {
                e.preventDefault();
                this._model.goToPreviousLine();
            }
            if (e.equals(KeyCode.Escape) || e.equals(KeyMod$1.CtrlCmd | KeyCode.Escape) || e.equals(KeyMod$1.Alt | KeyCode.Escape) || e.equals(KeyMod$1.Shift | KeyCode.Escape)) {
                e.preventDefault();
                this._model.close();
            }
    ...
    _render(store) {
        ...
        container.setAttribute("aria-label", ( localize(195, "Accessible Diff Viewer. Use arrow up and down to navigate.")));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = lines => lines === 0 ? ( localize(196, "no lines changed")) : lines === 1 ? ( localize(197, "1 line changed")) : ( localize(198, "{0} lines changed", lines));
            ...
            const isSelectedObs = derived(
                reader =>  this._model.currentElement.read(reader) === viewItem
            );
    ...
    _createRow(
        ...
        if (item.originalLineNumber !== undefined) {
            originalLineNumber.appendChild(createTextNode(String(item.originalLineNumber)));
        } else {
        ...
        if (item.modifiedLineNumber !== undefined) {
            modifiedLineNumber.appendChild(createTextNode(String(item.modifiedLineNumber)));
        } else {
        ...
        if (lineContent.length === 0) {
            lineContent = ( localize(200, "blank"));
        }
        let ariaLabel = "";
        switch (item.type) {
        case LineType.Unchanged:
            if (item.originalLineNumber === item.modifiedLineNumber) {
                ariaLabel = ( localize(201, "{0} unchanged line {1}", lineContent, item.originalLineNumber));
            } else {
                ariaLabel = ( localize(
                    202,
            ...
            break;
        case LineType.Added:
            ariaLabel = ( localize(203, "+ {0} modified line {1}", lineContent, item.modifiedLineNumber));
            break;
        case LineType.Deleted:
            ariaLabel = ( localize(204, "- {0} original line {1}", lineContent, item.originalLineNumber));
            break;
        }
