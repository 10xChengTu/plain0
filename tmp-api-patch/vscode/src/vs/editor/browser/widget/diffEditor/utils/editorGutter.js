...
class EditorGutter extends Disposable {
    constructor(_editor, _domNode, itemProvider) {
        ...
        const o = this._register(( new DisposableResizeObserver("EditorGutter.domNodeSizeChanged", () => {
            transaction(tx => {
        ...
        this._register(
            autorun(reader =>  this.render(reader))
        );
    }
    dispose() {
        super.dispose();
                ...
                transaction(tx => {
                    for (const gutterItem of gutterItems) {
                        ...
                        const bottom = gutterItem.range.endLineNumberExclusive === 1 ? Math.max(
                            top,
                            this._editor.getTopForLineNumber(gutterItem.range.startLineNumber, false) - scrollTop
                        ) : Math.max(
                            top,
