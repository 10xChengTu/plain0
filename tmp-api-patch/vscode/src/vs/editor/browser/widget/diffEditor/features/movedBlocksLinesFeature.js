...
class MovedBlocksLinesFeature extends Disposable {
    ...
    constructor(
        _rootElement,
        _diffModel,
        _originalEditorLayoutInfo,
        _modifiedEditorLayoutInfo,
        _editors
    ) {
        super();
        ...
        this._modifiedViewZonesChangedSignal = observableSignalFromEvent(
            "modified.onDidChangeViewZones",
            this._editors.modified.onDidChangeViewZones
        );
        this._originalViewZonesChangedSignal = observableSignalFromEvent(
        ...
        for (const line of lines) {
            ...
            if (trackIdx === -1) {
                ...
                if (setsPerTrack.length >= maxTrackCount) {
                    trackIdx = findMaxIdx(
                        setsPerTrack,
                        compareBy(set => set.intersectWithRangeLength(line), numberComparator)
                    );
                } else {
        ...
        const actionCompare = ( new Action(
            ...
            () => {
                ...
                this._diffModel.movedTextToCompare.set(
                    this._diffModel.movedTextToCompare.get() === _move ? undefined : this._move,
                    undefined
                );
            }
        ));
