...
let HideUnchangedRegionsFeature = class HideUnchangedRegionsFeature extends Disposable {
    static {
        HideUnchangedRegionsFeature_1 = this;
    }
    static {
        this._breadcrumbsSourceFactory = observableValue(this, () => ({
            dispose() {},
    ...
    constructor(_editors, _diffModel, _options, _instantiationService) {
                ...
                if (compactMode) {
                    {
                        const d = derived(
                            this,
                            reader =>  r.getHiddenOriginalRange(reader).startLineNumber - 1
                        );
                    ...
                    }
                    {
                        const d = derived(
                            this,
                            reader =>  r.getHiddenModifiedRange(reader).startLineNumber - 1
                        );
        ...
        const unchangedLinesDecorationShow = {
            ...
            })).appendMarkdown(( localize(218, "Fold Unchanged Region"))),
            glyphMarginClassName: "fold-unchanged " + ThemeIcon.asClassName(Codicon.fold),
            zIndex: 10001
        };
        this._register(
            applyObservableDecorations(this._editors.original, derived(this, reader => {
                const curUnchangedRegions = unchangedRegions.read(reader);
        ...
        this._register(
            applyObservableDecorations(this._editors.modified, derived(this, reader => {
                const curUnchangedRegions = unchangedRegions.read(reader);
                const result = ( curUnchangedRegions.map(r => ({
...
HideUnchangedRegionsFeature = HideUnchangedRegionsFeature_1 = ( __decorate([( __param(3, IInstantiationService))], HideUnchangedRegionsFeature));
class CompactCollapsedCodeOverlayWidget extends ViewZoneOverlayWidget {
    constructor(editor, _viewZone, _unchangedRegion, _hide = false) {
        ...
        this._hide = _hide;
        this._nodes = h(
            "div.diff-hidden-lines-compact",
            [h("div.line-left", []), h("div.text@text", []), h("div.line-right", [])]
        );
        ...
        this._register(autorun(reader => {
            if (!this._hide) {
                ...
                const linesHiddenText = ( localize(219, "{0} hidden lines", lineCount));
...
class CollapsedCodeOverlayWidget extends ViewZoneOverlayWidget {
    constructor(
        ...
        this._nodes = h("div.diff-hidden-lines", [h("div.top@top", {
            title: ( localize(220, "Click or drag to show more above"))
            ...
            title: ( localize(221, "Show Unchanged Region")),
            role: "button",
            onclick: () => {
            ...
            title: ( localize(222, "Click or drag to show more below")),
            role: "button"
        })]);
        root.root.appendChild(this._nodes.root);
        if (!this._hide) {
        ...
        this._register(addDisposableListener(this._nodes.top, "mousedown", e => {
            ...
            const mouseMoveListener = addDisposableListener(window, "mousemove", e => {
                ...
                const newVal = Math.max(
                    0,
                    Math.min(cur + lineDelta, this._unchangedRegion.getMaxVisibleLineCountTop())
                );
        ...
        this._register(addDisposableListener(this._nodes.bottom, "mousedown", e => {
            ...
            const mouseMoveListener = addDisposableListener(window, "mousemove", e => {
                ...
                const newVal = Math.max(
                    0,
                    Math.min(cur - lineDelta, this._unchangedRegion.getMaxVisibleLineCountBottom())
                );
        ...
        this._register(autorun(reader => {
            ...
            if (!this._hide) {
                ...
                const linesHiddenText = ( localize(219, "{0} hidden lines", lineCount));
                const span = $("span", {
                    title: ( localize(223, "Double click to unfold"))
                }, linesHiddenText);
