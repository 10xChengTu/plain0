...
import { IWebWorkerService } from '../../../platform/webWorker/browser/webWorkerService.service.js';
...
import { onUnexpectedError, canceled } from '../../../base/common/errors.js';
...
let EditorWorkerService = class EditorWorkerService extends Disposable {
    ...
    constructor(
        ...
        this._workerManager = this._register(( new WorkerManager(
            EditorWorkerService_1.workerDescriptor,
            this._modelService,
    ...
    async computeDiff(original, modified, options, algorithm) {
        const worker = await this._workerWithResources([original, modified],  true);
    ...
    async computeMoreMinimalEdits(resource, edits, pretty = false) {
        if (isNonEmptyArray(edits)) {
            ...
            result.finally(
                () => this._logService.trace("FORMAT#computeMoreMinimalEdits", ( resource.toString(true)), sw.elapsed())
            );
    ...
    computeHumanReadableDiff(resource, edits) {
        if (isNonEmptyArray(edits)) {
            ...
            result.finally(
                () => this._logService.trace("FORMAT#computeHumanReadableDiff", ( resource.toString(true)), sw.elapsed())
            );
            return result;
        } else {
            return Promise.resolve(undefined);
...
EditorWorkerService = EditorWorkerService_1 = ( __decorate([( __param(0, IModelService)), ( __param(1, ITextResourceConfigurationService)), ( __param(2, ILogService)), ( __param(3, ILanguageConfigurationService)), ( __param(4, ILanguageFeaturesService)), ( __param(5, IWebWorkerService))], EditorWorkerService));
class WordBasedCompletionItemProvider {
    ...
    async provideCompletionItems(model, position) {
        ...
        this.logService.trace(
            ...
            `word: "${word?.word || ""}", wordDef: "${wordDefRegExp}", replace: [${( replace.toString())}], insert: [${( insert.toString())}]`
        );
...
WorkerManager = ( __decorate([( __param(1, IModelService)), ( __param(2, IWebWorkerService))], WorkerManager));
...
EditorWorkerClient = ( __decorate([( __param(2, IModelService)), ( __param(3, IWebWorkerService))], EditorWorkerClient));
