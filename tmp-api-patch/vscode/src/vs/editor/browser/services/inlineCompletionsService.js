...
const InlineCompletionsSnoozing = ( new RawContextKey("inlineCompletions.snoozed", false, ( localize(161, "Whether inline completions are currently snoozed"))));
let InlineCompletionsService = class InlineCompletionsService extends Disposable {
    static {
        InlineCompletionsService_1 = this;
    }
    static {
        this.SNOOZE_DURATION = 300_000;
    }
    get snoozeTimeLeft() {
...
InlineCompletionsService = InlineCompletionsService_1 = ( __decorate([( __param(0, IContextKeyService)), ( __param(1, ITelemetryService))], InlineCompletionsService));
...
class SnoozeInlineCompletion extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(162, "Snooze Inline Suggestions")),
    ...
    async getDurationFromUser(quickInputService, storageService) {
        ...
        const predefinedItems = [{
            label: ( localize(163, "1 minute")),
            id: "1",
            value: 60_000
        }, {
            label: ( localize(164, "5 minutes")),
            id: "5",
            value: 300_000
        }, {
            label: ( localize(165, "10 minutes")),
            id: "10",
            value: 600_000
        }, {
            label: ( localize(166, "15 minutes")),
            id: "15",
            value: 900_000
        }, {
            label: ( localize(167, "30 minutes")),
            id: "30",
            value: 1_800_000
        }, {
            label: ( localize(168, "60 minutes")),
            id: "60",
            value: 3_600_000
        }];
        ...
        if (lastSelectedDuration > 0 && !( predefinedItems.some(item => item.value === lastSelectedDuration))) {
            ...
            const customItem = {
                label: ( localize(169, "{0} minutes (Last used)", minutes)),
                id: "last-custom",
                value: lastSelectedDuration,
                description: ( localize(170, "Last used custom duration"))
            };
            ...
            if (index === -1) {
                ...
                items = [
                    ...predefinedItems.slice(0, index),
                    customItem,
                    ...predefinedItems.slice(index)
                ];
            }
        }
        items.push({
            label: ( localize(171, "Custom...")),
            id: "custom",
            value: -1
        });
        const picked = await quickInputService.pick(items, {
            placeHolder: ( localize(172, "Select snooze duration for Inline Suggestions")),
    ...
    async getCustomDurationFromUser(quickInputService, storageService) {
        const customMinutes = await quickInputService.input({
            placeHolder: ( localize(173, "Duration in minutes (e.g. 90)")),
            prompt: ( localize(174, "Enter snooze duration in minutes")),
            validateInput: async value => {
                const n = Number(value);
                if (isNaN(n) || n <= 0 || !Number.isFinite(n)) {
                    return localize(175, "Please enter a positive number");
                }
                return undefined;
            }
        });
...
class CancelSnoozeInlineCompletion extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(176, "Cancel Snooze Inline Suggestions")),
