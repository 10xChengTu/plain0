...
let HoverColorPickerParticipant = class HoverColorPickerParticipant {
        ...
        for (const d of lineDecorations) {
            ...
            if (colorData) {
                const colorHover = ColorHover.fromBaseColor(
                    this,
                    await createColorHover(this._editor.getModel(), colorData.colorInfo, colorData.provider)
                );
                return [colorHover];
            }
        }
    ...
    getAccessibleContent(hoverPart) {
        return localize(1028, "There is a color picker here.");
    }
    handleResize() {
...
HoverColorPickerParticipant = ( __decorate([( __param(1, IThemeService))], HoverColorPickerParticipant));
