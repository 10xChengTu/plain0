...
registerEditorAction(class PasteAsAction extends EditorAction {
    ...
    run(_accessor, editor, args) {
        ...
        if (args) {
            if ("kind" in args) {
                preference = {
                    only: ( new HierarchicalKind(args.kind))
                };
            } else if ("preferences" in args) {
                preference = {
                    preferences: ( args.preferences.map(kind => ( new HierarchicalKind(kind))))
                };
            }
        }
...
registerEditorAction(class extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1060, "Paste as Text")),
            precondition: EditorContextKeys.writable,
            canTriggerInlineEdits: true
        });
    }
