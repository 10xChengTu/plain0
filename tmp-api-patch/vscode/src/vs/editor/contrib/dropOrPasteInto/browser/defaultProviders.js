...
class DefaultTextPasteOrDropEditProvider extends SimplePasteAndDropProvider {
    ...
    async getEdit(dataTransfer, _token) {
        ...
        return {
            ...
            title: ( localize(1070, "Insert Plain Text")),
            insertText,
            kind: this.kind
        };
    }
...
class PathProvider extends SimplePasteAndDropProvider {
    ...
    async getEdit(dataTransfer, token) {
        ...
        let label;
        if (uriCount > 0) {
            label = entries.length > 1 ? ( localize(1071, "Insert Uris")) : ( localize(1072, "Insert Uri"));
        } else {
            label = entries.length > 1 ? ( localize(1073, "Insert Paths")) : ( localize(1074, "Insert Path"));
        }
        return {
...
let RelativePathProvider = class RelativePathProvider extends SimplePasteAndDropProvider {
    ...
    async getEdit(dataTransfer, token) {
        ...
        return {
            ...
            title: entries.length > 1 ? ( localize(1075, "Insert Relative Paths")) : ( localize(1076, "Insert Relative Path")),
            kind: this.kind
        };
    }
};
    ...
    async provideDocumentPasteEdits(_model, _ranges, dataTransfer, context, token) {
        ...
        return {
            ...
            edits: [{
                ...
                title: ( localize(1077, "Insert HTML")),
                kind: this.kind
            }]
        };
    }
...
let DefaultDropProvidersFeature = class DefaultDropProvidersFeature extends Disposable {
    constructor(languageFeaturesService, workspaceContextService) {
        super();
        this._register(
            languageFeaturesService.documentDropEditProvider.register(genericLanguageSelector, ( new DefaultTextPasteOrDropEditProvider()))
        );
        this._register(
            languageFeaturesService.documentDropEditProvider.register(genericLanguageSelector, ( new PathProvider()))
        );
        this._register(
            languageFeaturesService.documentDropEditProvider.register(genericLanguageSelector, ( new RelativePathProvider(workspaceContextService)))
        );
    }
};
...
let DefaultPasteProvidersFeature = class DefaultPasteProvidersFeature extends Disposable {
    constructor(languageFeaturesService, workspaceContextService) {
        super();
        this._register(
            languageFeaturesService.documentPasteEditProvider.register(genericLanguageSelector, ( new DefaultTextPasteOrDropEditProvider()))
        );
        this._register(
            languageFeaturesService.documentPasteEditProvider.register(genericLanguageSelector, ( new PathProvider()))
        );
        this._register(
            languageFeaturesService.documentPasteEditProvider.register(genericLanguageSelector, ( new RelativePathProvider(workspaceContextService)))
        );
        this._register(
            languageFeaturesService.documentPasteEditProvider.register(genericLanguageSelector, ( new PasteHtmlProvider()))
        );
    }
};
