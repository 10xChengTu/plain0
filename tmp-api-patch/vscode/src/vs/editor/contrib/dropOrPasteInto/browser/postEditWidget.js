...
import * as postEditWidget from './postEditWidget.css';

var PostEditWidget_1;
registerCss(postEditWidget);
let PostEditWidget = class PostEditWidget extends Disposable {
    ...
    create() {
        ...
        this._register(
            addDisposableListener(this.domNode, EventType.CLICK, () => this.showSelector())
        );
    }
    getId() {
...
PostEditWidget = PostEditWidget_1 = ( __decorate([( __param(8, IContextKeyService)), ( __param(9, IKeybindingService)), ( __param(10, IActionWidgetService))], PostEditWidget));
let PostEditWidgetManager = class PostEditWidgetManager extends Disposable {
    ...
    async applyEditAndShowIfNeeded(ranges, edits, canShowWidget, resolve, token) {
        ...
        try {
            ...
            return handleError(e, ( localize(1081, "Error resolving edit '{0}':\n{1}", edit.title, toErrorMessage(e))));
        } finally {
        ...
        try {
            ...
            return handleError(e, ( localize(1082, "Error applying edit '{0}':\n{1}", edit.title, toErrorMessage(e))));
        } finally {
...
PostEditWidgetManager = ( __decorate([( __param(5, IInstantiationService)), ( __param(6, IBulkEditService)), ( __param(7, INotificationService))], PostEditWidgetManager));
