...
import { ITreeViewsDnDService } from '../../../common/services/treeViewsDndService.service.js';
...
const dropWidgetVisibleCtx = ( new RawContextKey("dropWidgetVisible", false, ( localize(1078, "Whether the drop widget is showing"))));
let DropIntoEditorController = class DropIntoEditorController extends Disposable {
    ...
    constructor(
        ...
        this._postDropWidgetManager = this._register(
            instantiationService.createInstance(PostEditWidgetManager, "dropIntoEditor", editor, dropWidgetVisibleCtx, {
                id: changeDropTypeCommandId,
                label: ( localize(1079, "Show drop options..."))
            }, () => DropIntoEditorController_1._configureDefaultAction ? [DropIntoEditorController_1._configureDefaultAction] : [])
        );
        this._register(
    ...
    async onDropIntoEditor(editor, position, dragEvent) {
        ...
        this._dropProgressManager.showWhile(position, ( localize(1080, "Running drop handlers. Click to cancel")), p, {
            cancel: () => p.cancel()
        });
