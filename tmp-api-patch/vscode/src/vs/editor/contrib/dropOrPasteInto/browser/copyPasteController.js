...
const pasteWidgetVisibleCtx = ( new RawContextKey("pasteWidgetVisible", false, ( localize(1061, "Whether the paste widget is showing"))));
...
let CopyPasteController = class CopyPasteController extends Disposable {
    ...
    constructor(
        ...
        this._postPasteWidgetManager = this._register(
            instantiationService.createInstance(PostEditWidgetManager, "pasteIntoEditor", editor, pasteWidgetVisibleCtx, {
                id: changePasteTypeCommandId,
                label: ( localize(1062, "Show paste options..."))
            }, () => CopyPasteController_1._configureDefaultAction ? [CopyPasteController_1._configureDefaultAction] : [])
        );
    }
    changePasteType() {
    ...
    handleCopy(e) {
        ...
        const operations = ( providers.map(provider => {
            return {
                ...
                operation: createCancelablePromise(
                    token => provider.prepareDocumentPaste(model, e.dataToCopy.sourceRanges, dataTransfer, token).catch(err => {
                        console.error(err);
                        return undefined;
                    })
                )
    ...
    async handlePaste(e) {
        ...
        if (
        this._editor.getOption(EditorOption.readOnly) || (!this.isPasteAsEnabled() && !this._pasteAsActionContext)) {
            return;
        }
        const metadata = this.fetchCopyMetadata(e);
        this._logService.trace(
            "CopyPasteController#handlePaste with metadata : ",
            metadata?.id,
            " and text.length : ",
            e.clipboardData.getData("text/plain").length
        );
    ...
    showPasteAsNoEditMessage(selections, preference) {
        const kindLabel = "only" in preference ? preference.only.value : "preferences" in preference ? (preference.preferences.length ? ( preference.preferences.map(preference => preference.value)).join(", ") : ( localize(1063, "empty"))) : preference.providerId;
        MessageController.get(this._editor)?.showMessage(( localize(1064, "No paste edits for '{0}' found", kindLabel)), selections[0].getStartPosition());
    }
    doPasteInline(allProviders, selections, dataTransfer, metadata, clipboardEvent) {
        ...
        const p = createCancelablePromise(async pToken => {
            ...
            try {
                ...
                if (
                !supportedProviders.length || (supportedProviders.length === 1 && supportedProviders[0] instanceof DefaultTextPasteOrDropEditProvider)) {
                ...
                if (editSession.edits.length) {
                    ...
                    return this._postPasteWidgetManager.applyEditAndShowIfNeeded(selections, {
                        ...
                        const resolved = await this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(1065, "Resolving paste edit for '{0}'. Click to cancel", edit.title)), raceCancellation(Promise.race([showP.p, resolveP]), resolveToken), {
                            cancel: () => showP.cancel()
                        }, 0);
                        if (resolved) {
        ...
        this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(1066, "Running paste handlers. Click to cancel and do basic paste")), p, {
            cancel: async () => {
                p.cancel();
    ...
    showPasteAsPick(preference, allProviders, selections, dataTransfer, metadata) {
        ...
        const p = createCancelablePromise(async token => {
            ...
            try {
                ...
                let supportedProviders = allProviders.filter(
                    provider => this.isSupportedPasteProvider(provider, dataTransfer, preference)
                );
                if (preference) {
                ...
                let editSession = disposables.add(await this.getPasteEdits(
                    supportedProviders,
                    dataTransfer,
                    model,
                    selections,
                ...
                if (preference) {
                    ...
                    const configureDefaultItem = {
                        ...
                        label: ( localize(1067, "Configure default paste action")),
                        edit: undefined
                    };
                    const selected = await this._quickInputService.pick([...( editSession.edits.map(edit => ({
                        ...
                        edit
                    }))), ...(CopyPasteController_1._configureDefaultAction ? [upcast({
                        type: "separator"
                    }), {
                        label: CopyPasteController_1._configureDefaultAction.label,
                        edit: undefined
                    }] : [])], {
                        placeHolder: ( localize(1068, "Select Paste Action"))
                    });
        ...
        this._progressService.withProgress({
            ...
            title: ( localize(1069, "Running paste handlers"))
        }, () => p);
    }
...
CopyPasteController = CopyPasteController_1 = ( __decorate([( __param(1, IInstantiationService)), ( __param(2, ILogService)), ( __param(3, IBulkEditService)), ( __param(4, IClipboardService)), ( __param(5, ICommandService)), ( __param(6, IConfigurationService)), ( __param(7, ILanguageFeaturesService)), ( __param(8, IQuickInputService)), ( __param(9, IProgressService))], CopyPasteController));
