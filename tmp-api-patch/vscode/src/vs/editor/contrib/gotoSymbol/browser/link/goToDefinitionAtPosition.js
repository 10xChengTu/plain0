...
import * as goToDefinitionAtPosition from './goToDefinitionAtPosition.css';
...
registerCss(goToDefinitionAtPosition);
let GotoDefinitionAtPositionEditorContribution = class GotoDefinitionAtPositionEditorContribution {
    ...
    async startFindDefinition(position) {
        ...
        if (results.length > 1) {
            ...
            this.addDecoration(combinedRange, ( new MarkdownString()).appendText(( localize(1220, "Click to show {0} definitions.", results.length))));
        } else {
    ...
    getPreviewRangeBasedOnIndentation(textEditorModel, startLineNumber) {
        ...
        const maxLineNumber = Math.min(
            ...
            startLineNumber + GotoDefinitionAtPositionEditorContribution_1.MAX_SOURCE_PREVIEW_LINES
        );
