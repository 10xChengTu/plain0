...
class DebugEditorGpuRendererAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1241, "Developer: Debug Editor GPU Renderer")),
    ...
    async run(accessor, editor) {
        ...
        const choice = await quickInputService.pick([{
            label: ( localize(1242, "Log Texture Atlas Stats")),
            id: "logTextureAtlasStats"
        }, {
            label: ( localize(1243, "Save Texture Atlas")),
            id: "saveTextureAtlas"
        }, {
            label: ( localize(1244, "Draw Glyph")),
            id: "drawGlyph"
        }], {
            canPickMany: false
        });
            ...
            instantiationService.invokeFunction(async accessor => {
                ...
                if (folders.length > 0) {
                    ...
                    for (const [layerIndex, page] of atlas.pages.entries()) {
                        promises.push(...[fileService.writeFile(
        ...
        case "drawGlyph":
            instantiationService.invokeFunction(async accessor => {
                ...
                const resource = URI.joinPath(
                    folders[0].uri,
                    `glyph_${chars}_${tokenMetadata}_${fontSize}px_${fontFamily.replaceAll(/[,\\\/\.'\s]/g, "_")}.png`
                );
