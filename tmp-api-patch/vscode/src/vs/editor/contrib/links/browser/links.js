...
import * as links from './links.css';
    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === "invalid") {
                ...
                this.notificationService.warn(( localize(1426, "Failed to open this link because its target is missing.")));
            } else {
                onUnexpectedError(err);
            }
        });
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip ? link.tooltip : executeCmd ? ( localize(1427, "Execute command")) : ( localize(1428, "Follow link"));
    const kb = useMetaKey ? isMacintosh ? ( localize(1429, "cmd + click")) : ( localize(1430, "ctrl + click")) : isMacintosh ? ( localize(1431, "option + click")) : ( localize(1432, "alt + click"));
    if (link.url) {
        let nativeLabel = "";
        if (/^command:/i.test(( link.url.toString()))) {
            ...
            if (match) {
                ...
                nativeLabel = ( localize(1433, "Execute command {0}", commandId));
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1434, "Open Link")),
