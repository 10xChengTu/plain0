...
import { $, addStandardDisposableGenericMouseDownListener, getDomNodePagePosition, addDisposableListener } from '../../../../base/browser/dom.js';
...
import * as lightBulbWidget from './lightBulbWidget.css';
...
function computeLightBulbInfo(actions, trigger, preferredKbLabel, quickFixKbLabel, forGutter = false) {
    ...
    let title;
    if (autoRun) {
        title = ( localize(1020, "Run: {0}", actions.validActions[0].action.title));
        ...
        title = ( localize(1022, "Show Code Actions ({0})", quickFixKbLabel));
    } else {
        title = ( localize(1023, "Show Code Actions"));
    }
    return {
        actions,
        trigger,
...
let LightBulbWidget = class LightBulbWidget extends Disposable {
    ...
    static {
        this._posPref = [ContentWidgetPositionPreference.EXACT];
    }
    ...
    constructor(_editor, _keybindingService) {
        ...
        this._combinedInfo = derived(this, reader => {
            ...
            if (gutterState.type === LightBulbState.Type.Showing) {
                return LightBulbWidget_1._computeLightBulbInfo(
                    gutterState,
                    true,
                    this._preferredKbLabel.read(reader),
                    this._quickFixKbLabel.read(reader)
                );
            }
        ...
        this._register(addStandardDisposableGenericMouseDownListener(this._domNode, e => {
            ...
            const {
                top,
                height
            } = getDomNodePagePosition(this._domNode);
        ...
        this._register(this._editor.onMouseDown(async e => {
            ...
            const {
                top,
                height
            } = getDomNodePagePosition(e.target.element);
            const lineHeight = this._editor.getOption(EditorOption.lineHeight);
            let pad = Math.floor(lineHeight / 3);
