...
import { isNonEmptyArray, equals, coalesce } from '../../../../base/common/arrays.js';
import { CancellationToken } from '../../../../base/common/cancellation.js';
import { isCancellationError, onUnexpectedExternalError, illegalArgument } from '../../../../base/common/errors.js';
...
import { CodeActionTriggerType, ProviderId } from '../../../common/languages.js';
...
import { CodeActionKind, filtersAction, CodeActionItem, mayIncludeActionsOfKind, CodeActionTriggerSource } from '../common/types.js';
...
function getCodeActionProviders(registry, model, filter) {
    return (
        registry.all(model).filter(provider => {
...
async function applyCodeAction(accessor, item, codeActionReason, options, token = CancellationToken.None) {
    ...
    if (item.action.edit?.edits.length) {
        const result = await bulkEditService.apply(item.action.edit, {
            ...
            reason: EditSources.codeAction({
                ...
                providerId: ProviderId.fromExtensionId(item.provider?.extensionId)
            })
        });
        if (!result.isApplied) {
            return;
    ...
    if (item.action.command) {
        try {
            ...
            notificationService.error(typeof message === "string" ? message : ( localize(971, "An unknown error occurred while applying the code action")));
        }
    }
    setTimeout(
