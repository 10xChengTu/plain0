...
const warningIcon = registerIcon("extensions-warning-message", Codicon.warning, ( localize(1681, "Icon shown with a warning message in the extensions editor.")));
        ...
        this._updateState = state => {
            if (state && state.hasMore) {
                ...
                if (state.nonBasicAsciiCharacterCount >= max) {
                    data = {
                        message: ( localize(1682, "This document contains many non-basic ASCII unicode characters")),
                        command: ( new DisableHighlightingOfNonBasicAsciiCharactersAction())
                    };
                } else if (state.ambiguousCharacterCount >= max) {
                    data = {
                        message: ( localize(1683, "This document contains many ambiguous unicode characters")),
                        command: ( new DisableHighlightingOfAmbiguousCharactersAction())
                    };
                } else if (state.invisibleCharacterCount >= max) {
                    data = {
                        message: ( localize(1684, "This document contains many invisible unicode characters")),
...
const configureUnicodeHighlightOptionsStr = ( localize(1685, "Configure Unicode Highlight Options"));
    ...
    computeSync(anchor, lineDecorations) {
        ...
        for (const d of lineDecorations) {
            ...
            case UnicodeHighlighterReasonKind.Invisible:
                reason = ( localize(1688, "The character {0} is invisible.", codePointStr));
                break;
            case UnicodeHighlighterReasonKind.NonBasicAscii:
                reason = ( localize(1689, "The character {0} is not a basic ASCII character.", codePointStr));
                break;
            }
            ...
            const adjustSettings = ( localize(1690, "Adjust settings"));
...
UnicodeHighlighterHoverParticipant = ( __decorate([( __param(1, IMarkdownRendererService))], UnicodeHighlighterHoverParticipant));
...
class DisableHighlightingInCommentsAction extends EditorAction {
    ...
    constructor() {
        super({
            ...
            label: ( localize2(1691, "Disable highlighting of characters in comments")),
            precondition: undefined
        });
        this.shortLabel = ( localize(1692, "Disable Highlight In Comments"));
    }
...
class DisableHighlightingInStringsAction extends EditorAction {
    ...
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            label: ( localize2(1693, "Disable highlighting of characters in strings")),
            precondition: undefined
        });
        this.shortLabel = ( localize(1694, "Disable Highlight In Strings"));
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
class DisableHighlightingOfAmbiguousCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            title: ( localize2(1695, "Disable highlighting of ambiguous characters")),
            precondition: undefined,
            f1: false
        });
        this.shortLabel = ( localize(1696, "Disable Ambiguous Highlight"));
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
class DisableHighlightingOfInvisibleCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(1697, "Disable highlighting of invisible characters")),
            precondition: undefined,
            f1: false
        });
        this.shortLabel = ( localize(1698, "Disable Invisible Highlight"));
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
class DisableHighlightingOfNonBasicAsciiCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(1699, "Disable highlighting of non basic ASCII characters")),
            precondition: undefined,
            f1: false
        });
        this.shortLabel = ( localize(1700, "Disable Non ASCII Highlight"));
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
class ShowExcludeOptions extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(1701, "Show Exclude Options")),
            precondition: undefined,
            f1: false
        });
    }
