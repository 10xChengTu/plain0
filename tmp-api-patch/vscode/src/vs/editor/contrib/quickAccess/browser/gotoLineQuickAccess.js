...
class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    provideWithoutTextEditor(picker) {
        const label = ( localize(1484, "Open a text editor first to go to a line or an offset."));
        picker.items = [{
            label
        }];
    ...
    provideWithTextEditor(context, picker, token) {
        ...
        const offsetButton = {
            ...
            tooltip: ( localize(1486, "Toggle Zero-Based Offset")),
            location: QuickInputButtonLocation.Input,
            toggle: {
                checked: this.useZeroBasedOffset
            }
        };
        const updatePickerAndEditor = () => {
            ...
            const {
                ...
                column,
                label
            } = this.parsePosition(editor, inputText);
            picker.buttons = inOffsetMode ? [offsetButton] : [];
            picker.items = [{
                lineNumber,
                column,
                label,
                ariaLabel: lineNumber ? ( localize(
                    1487,
    ...
    parsePosition(editor, value) {
        ...
        if (!model) {
            return {
                label: ( localize(1484, "Open a text editor first to go to a line or an offset."))
            };
        }
        if (value.startsWith(":")) {
            ...
            if (isNaN(offset)) {
                return {
                    inOffsetMode: true,
                    label: this.useZeroBasedOffset ? ( localize(1488, "Type a character position to go to (from 0 to {0}).", maxOffset - 1)) : ( localize(1489, "Type a character position to go to (from 1 to {0}).", maxOffset))
                };
            } else {
            ...
            if (parts.length < 1 || isNaN(lineNumber)) {
                return {
                    label: ( localize(1491, "Type a line number to go to (from 1 to {0}).", maxLine))
                };
            }
            ...
            return {
                lineNumber,
                column: realColumn,
                label: ( localize(1490, "Press 'Enter' to go to line {0} at column {1}.", lineNumber, column))
            };
        }
    }
}
