...
import { Disposable, DisposableStore, toDisposable, MutableDisposable } from '../../../../base/common/lifecycle.js';
...
import { IOutlineModelService } from '../../documentSymbols/browser/outlineModel.service.js';
...
let AbstractGotoSymbolQuickAccessProvider = class AbstractGotoSymbolQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    doProvideWithoutEditorSymbols(context, model, picker, token) {
        ...
        this.provideLabelPick(picker, ( localize(1495, "The active text editor does not provide symbol information.")));
        (async () => {
        ...
        const updatePickerItems = async positionToEnclose => {
            ...
            try {
                const query = prepareQuery(
                    picker.value.substr(AbstractGotoSymbolQuickAccessProvider_1.PREFIX.length).trim()
                );
                ...
                if (items.length > 0) {
                    ...
                    if (positionToEnclose && query.original.length === 0) {
                        const candidate = findLast(items, item => Boolean(
                    ...
                    if (query.original.length > 0) {
                        this.provideLabelPick(picker, ( localize(1496, "No matching editor symbols")));
                    } else {
                        this.provideLabelPick(picker, ( localize(1497, "No editor symbols")));
                    }
                }
            } finally {
                if (!token.isCancellationRequested) {
    ...
    async doGetSymbolPicks(symbolsPromise, query, options, token, model) {
        ...
        if (openSideBySideDirection) {
            buttons = [{
                ...
                tooltip: openSideBySideDirection === "right" ? ( localize(1498, "Open to the Side")) : ( localize(1499, "Open to the Bottom"))
            }];
        }
        ...
        if (filterBySymbolKind) {
            ...
            symbolPicks = [{
                label: ( localize(1500, "symbols ({0})", filteredSymbolPicks.length)),
                type: "separator"
...
const FALLBACK_NLS_SYMBOL_KIND = ( localize(1501, "properties ({0})"));
const NLS_SYMBOL_KIND_CACHE = {
    [SymbolKind.Method]: ( localize(1502, "methods ({0})")),
    [SymbolKind.Function]: ( localize(1503, "functions ({0})")),
    [SymbolKind.Constructor]: ( localize(1504, "constructors ({0})")),
    [SymbolKind.Variable]: ( localize(1505, "variables ({0})")),
    [SymbolKind.Class]: ( localize(1506, "classes ({0})")),
    [SymbolKind.Struct]: ( localize(1507, "structs ({0})")),
    [SymbolKind.Event]: ( localize(1508, "events ({0})")),
    [SymbolKind.Operator]: ( localize(1509, "operators ({0})")),
    [SymbolKind.Interface]: ( localize(1510, "interfaces ({0})")),
    [SymbolKind.Namespace]: ( localize(1511, "namespaces ({0})")),
    [SymbolKind.Package]: ( localize(1512, "packages ({0})")),
    [SymbolKind.TypeParameter]: ( localize(1513, "type parameters ({0})")),
    [SymbolKind.Module]: ( localize(1514, "modules ({0})")),
    [SymbolKind.Property]: ( localize(1501, "properties ({0})")),
    [SymbolKind.Enum]: ( localize(1515, "enumerations ({0})")),
    [SymbolKind.EnumMember]: ( localize(1516, "enumeration members ({0})")),
    [SymbolKind.String]: ( localize(1517, "strings ({0})")),
    [SymbolKind.File]: ( localize(1518, "files ({0})")),
    [SymbolKind.Array]: ( localize(1519, "arrays ({0})")),
    [SymbolKind.Number]: ( localize(1520, "numbers ({0})")),
    [SymbolKind.Boolean]: ( localize(1521, "booleans ({0})")),
    [SymbolKind.Object]: ( localize(1522, "objects ({0})")),
    [SymbolKind.Key]: ( localize(1523, "keys ({0})")),
    [SymbolKind.Field]: ( localize(1524, "fields ({0})")),
    [SymbolKind.Constant]: ( localize(1525, "constants ({0})"))
};
