...
class RenameSkeleton {
    ...
    async _provideRenameEdits(newName, i, rejects, token) {
        ...
        if (!result) {
            return this._provideRenameEdits(newName, i + 1, rejects.concat(( localize(1528, "No result."))), token);
...
let RenameController = class RenameController {
    ...
    constructor(
        ...
        this._renameWidget = this._disposableStore.add(this._instaService.createInstance(
            RenameWidget,
            this.editor,
            ["acceptRenameInput", "acceptRenameInputWithPreview"]
        ));
    }
    dispose() {
    ...
    async run() {
        ...
        try {
            ...
            if (e instanceof CancellationError) {
                ...
                if (typeof e === "string" || isMarkdownString(e)) {
                    MessageController.get(this.editor)?.showMessage(e || ( localize(1529, "An unknown error occurred while resolving rename location")), position);
                }
            }
            return undefined;
        } finally {
        ...
        const renameOperation = raceCancellation(
            ...
            this._bulkEditService.apply(renameResult, {
                ...
                label: ( localize(1530, "Renaming '{0}' to '{1}'", loc?.text, inputFieldResult.newName)),
                code: "undoredo.rename",
                quotableLabel: ( localize(1531, "Renaming {0} to {1}", loc?.text, inputFieldResult.newName)),
                ...
                this._notificationService.error(( localize(1533, "Rename failed to apply edits")));
                this._logService.error(err);
            });
        }, err => {
            trace("error when providing rename edits", JSON.stringify(err, null, "\t"));
            this._notificationService.error(( localize(1534, "Rename failed to compute edits")));
            this._logService.error(err);
        }).finally(() => {
            cts2.dispose();
        });
...
class RenameAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1535, "Rename Symbol")),
...
registerAction2(class FocusNextRenameSuggestion extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(1536, "Focus Next Rename Suggestion"))
            },
            precondition: CONTEXT_RENAME_INPUT_VISIBLE,
            keybinding: [{
...
registerAction2(class FocusPreviousRenameSuggestion extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...( localize2(1537, "Focus Previous Rename Suggestion"))
            },
            precondition: CONTEXT_RENAME_INPUT_VISIBLE,
            keybinding: [{
...
( Registry.as(Extensions.Configuration)).registerConfiguration({
    ...
    properties: {
        "editor.rename.enablePreview": {
            ...
            description: ( localize(1538, "Enable/disable the ability to preview changes before renaming")),
            default: true,
            type: "boolean"
        }
    }
...
export { RenameAction, hasProvider, rawRename, rename };
