...
import * as renameWidget from './renameWidget.css';
...
const CONTEXT_RENAME_INPUT_VISIBLE = ( new RawContextKey("renameInputVisible", false, ( localize(1539, "Whether the rename input widget is visible"))));
let RenameWidget = class RenameWidget {
    ...
    getPosition() {
        ...
        if (
        !this._editor.hasModel() || !this._editor.getDomNode()) {
            return null;
        }
    ...
    getInput(where, currentName, supportPreview, requestRenameCandidates, cts) {
        ...
        if (requestRenameCandidates === undefined) {
            ...
            disposeOnDone.add(addDisposableListener(
        ...
        disposeOnDone.add(
            cts.token.onCancellationRequested(() => this.cancelInput(true, "cts.token.onCancellationRequested"))
        );
        {
            disposeOnDone.add(this._editor.onDidBlurEditorWidget(
                () => this.cancelInput(!this._domNode?.ownerDocument.hasFocus(), "editor.onDidBlurEditorWidget")
            ));
        }
        this._show();
...
class RenameCandidateListView {
    ...
    setCandidates(candidates) {
        ...
        status(( localize(1541, "Received {0} rename suggestions", candidates.length)));
    }
    clearCandidates() {
    ...
    _pickListWidth(candidates) {
        ...
        const width = Math.max(this._minimumWidth,
...
class InputWithButton {
    ...
    get domNode() {
        if (!this._domNode) {
            ...
            this._inputNode.setAttribute("aria-label", ( localize(1542, "Rename input. Type new name and press Enter to commit.")));
            this._domNode.appendChild(this._inputNode);
            this._buttonNode = createElement("div");
            this._buttonNode.className = "rename-suggestions-button";
            this._buttonNode.setAttribute("tabindex", "0");
            this._buttonGenHoverText = ( localize(1543, "Generate New Name Suggestions"));
            this._buttonCancelHoverText = ( localize(1544, "Cancel"));
            ...
            this._disposables.add(
                addDisposableListener(this.input, EventType.INPUT, () => this._onDidInputChange.fire())
            );
            ...
            this._disposables.add(
                addDisposableListener(this.input, EventType.CLICK, () => this._onDidInputChange.fire())
            );
