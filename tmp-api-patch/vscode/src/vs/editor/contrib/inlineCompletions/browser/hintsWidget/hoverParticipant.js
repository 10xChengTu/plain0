...
let InlineCompletionsHoverParticipant = class InlineCompletionsHoverParticipant {
    ...
    suggestHoverAnchor(mouseEvent) {
        ...
        if (target.type === MouseTargetType.CONTENT_VIEW_ZONE) {
            ...
            if (controller.shouldShowHoverAtViewZone(viewZoneData.viewZoneId)) {
                return ( new HoverForeignElementAnchor(1000, this, Range.fromPositions(
                    this._editor.getModel().validatePosition(viewZoneData.positionBefore || viewZoneData.position)
                ), mouseEvent.event.posx, mouseEvent.event.posy, false));
            }
        }
    ...
    getAccessibleContent(hoverPart) {
        return localize(1353, "There are inline completions here");
    }
    renderScreenReaderText(context, part) {
        ...
        const render = code => {
            const inlineSuggestionAvailable = ( localize(1354, "Suggestion:"));
...
InlineCompletionsHoverParticipant = ( __decorate([( __param(1, IAccessibilityService)), ( __param(2, IInstantiationService)), ( __param(3, ITelemetryService)), ( __param(4, IMarkdownRendererService))], InlineCompletionsHoverParticipant));
