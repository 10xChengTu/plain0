...
import { onUnexpectedExternalError, BugIndicatingError, onUnexpectedError } from '../../../../../base/common/errors.js';
...
function provideInlineCompletions(
    ...
    if (foundCycles.length > 0) {
        onUnexpectedExternalError(( new Error(
            `Inline completions: cyclic yield-to dependency detected.` + ` Path: ${( foundCycles.map(s => s.toString ? ( s.toString()) : ("" + s))).join(" -> ")}`
        )));
    }
    let runningCount = 0;
...
class InlineSuggestData {
    ...
    reportPartialAccept(acceptedCharacters, info, partialAcceptance) {
        ...
        this._partiallyAcceptedSinceOriginal.ratio = Math.min(
            this._partiallyAcceptedSinceOriginal.ratio + (1 - this._partiallyAcceptedSinceOriginal.ratio) * partialAcceptance.ratio,
            1
        );
