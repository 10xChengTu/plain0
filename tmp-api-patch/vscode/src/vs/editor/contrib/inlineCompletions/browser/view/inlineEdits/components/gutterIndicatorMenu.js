    ...
    _createHoverContent() {
        ...
        const createOptionArgs = options => {
            return {
                title: options.title,
                icon: options.icon,
                keybinding: typeof options.commandId === "string" ? this._getKeybinding(options.commandArgs ? undefined : options.commandId) : derived(
                    this,
                    reader => typeof options.commandId === "string" ? undefined : this._getKeybinding(options.commandArgs ? undefined : options.commandId.read(reader)).read(reader)
                ),
        ...
        const gotoAndAccept = option(createOptionArgs({
            id: "gotoAndAccept",
            title: ( localize(1360, "Go To / Accept")),
            icon: Codicon.check,
            commandId: inlineSuggestCommitId
        }));
        const reject = option(createOptionArgs({
            id: "reject",
            title: ( localize(1361, "Reject")),
            icon: Codicon.close,
...
GutterIndicatorMenuContent = ( __decorate([( __param(3, IContextKeyService)), ( __param(4, IKeybindingService)), ( __param(5, ICommandService))], GutterIndicatorMenuContent));
