...
import { NextMarkerAction, PrevMarkerAction, NextMarkerInFilesAction, PrevMarkerInFilesAction } from '../../../gotoError/browser/gotoError.js';
...
let InlineCompletionsController = class InlineCompletionsController extends Disposable {
    ...
    constructor(
        ...
        this._view = derived(reader => reader.store.add(this._instantiationService.createInstance(
            InlineSuggestionsView.hot.read(reader),
            this.editor,
            this.model,
            this._focusIsInMenu
        )));
        ...
        this._isScreenReaderEnabled = observableFromEvent(
            this,
        ...
        this._register(runOnChangeWithStore(derived(reader => {
            ...
            this._accessibilitySignalService.playSignal(
                state.kind === "ghostText" ? AccessibilitySignal.inlineSuggestion : AccessibilitySignal.nextEditSuggestion
            );
        ...
        this._register(
            contextKeySvcObs.bind(InlineCompletionContextKeys.cursorAtInlineEdit, ( this.model.map(
                (m, reader) => m?.inlineEditState?.read(reader)?.cursorAtInlineEdit.read(reader)
            )))
        );
        this._register(
            contextKeySvcObs.bind(InlineCompletionContextKeys.tabShouldAcceptInlineEdit, ( this.model.map((m, r) => !!m?.tabShouldAcceptInlineEdit.read(r))))
        );
        this._register(
            contextKeySvcObs.bind(InlineCompletionContextKeys.tabShouldJumpToInlineEdit, ( this.model.map((m, r) => !!m?.tabShouldJumpToInlineEdit.read(r))))
        );
        this._register(contextKeySvcObs.bind(
        ...
        this._register(
            contextKeySvcObs.bind(InlineCompletionContextKeys.suppressSuggestions, reader => {
                const model = this.model.read(reader);
        ...
        this._register(
            contextKeySvcObs.bind(InlineCompletionContextKeys.inlineSuggestionVisible, reader => {
                const model = this.model.read(reader);
                const state = model?.inlineCompletionState.read(reader);
        ...
        this._register(
            contextKeySvcObs.bind(InlineCompletionContextKeys.cursorBeforeGhostText, reader => {
        ...
        this._register(
            this._instantiationService.createInstance(TextModelChangeRecorder, this.editor)
        );
    }
