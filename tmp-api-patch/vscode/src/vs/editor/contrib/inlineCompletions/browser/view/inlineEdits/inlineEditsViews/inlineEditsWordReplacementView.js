...
import { editorHoverForeground } from '../../../../../../../platform/theme/common/colors/editorColors.js';
...
import { getOriginalBorderColor, getModifiedBorderColor, observeColor, modifiedChangedTextOverlayColor, inlineEditIndicatorPrimaryForeground, inlineEditIndicatorPrimaryBackground, inlineEditIndicatorPrimaryBorder, getEditorBackgroundColor, INLINE_EDITS_BORDER_RADIUS, originalChangedTextOverlayColor } from '../theme.js';
...
import { observableFromPromise } from '../../../../../../../base/common/observableInternal/utils/utils.js';
...
let InlineEditsWordReplacementView = class InlineEditsWordReplacementView extends Disposable {
    ...
    constructor(
        ...
        this._renderTextEffect = derived(this, _reader => {
            ...
            const edit = StringReplacement.replace(( new OffsetRange(
                this._viewData.edit.range.startColumn - 1,
                this._viewData.edit.range.endColumn - 1
            )), this._viewData.edit.text);
        ...
        const altModifierActive = derived(
            this,
            reader => this._userInteractionService.readModifierKeyStatus(this._editor.editor.getDomNode(), reader).shiftKey
        );
        this._layout = derived(this, reader => {
            ...
            if (this._viewData.alternativeAction) {
                ...
                const occurrencesLabel = count !== undefined ? count === 1 ? ( localize(1367, "{0} 1 occurrence", label)) : ( localize(1368, "{0} {1} occurrences", label, count)) : label;
                const keybindingTooltip = ( localize(1369, "{0} show occurrences", "[shift]"));
                alternativeAction = {
            ...
            const modifiedLine = codeLine.withWidth(
                codeLine.width + (alternativeAction ? alternativeAction.label.length * w + 8 + 4 + 12 : 0)
            );
        ...
        this._root = n.div({
            ...
            const alternativeActionActive = derived(
                reader => (alternativeAction.read(reader)?.active.read(reader) ?? false) || secondaryElementHovered.read(reader)
            );
            ...
            const primaryActionStyles = derived(
                this,
                r => alternativeActionActive.read(r) ? primaryActiveStyles : primaryActiveStyles
            );
            const secondaryActionStyles = derived(
                this,
                r => alternativeActionActive.read(r) ? secondaryActiveStyles : passiveStyles
            );
            return [n.div({
                id: DOM_ID_OVERLAY,
                style: {
                ...
                style: {
                    position: "absolute",
                    ...rectToProps(
                        reader => layout.read(reader).lowerBackground.withMargin(BORDER_WIDTH, 2 * BORDER_WIDTH, BORDER_WIDTH, 0)
                    ),
                ...
                style: {
                    position: "absolute",
                    ...rectToProps(
                        reader => layout.read(reader).modifiedLine.withMargin(BORDER_WIDTH, 2 * BORDER_WIDTH)
                    ),
                    width: undefined,
                    pointerEvents: "auto",
                ...
                style: {
                    ...
                    width: rectToProps(
                        reader => layout.read(reader).codeLine.withMargin(BORDER_WIDTH, 2 * BORDER_WIDTH)
                    ).width,
                    borderRadius: `${INLINE_EDITS_BORDER_RADIUS}px`,
            ...
            }, [this._line]), derived(this, reader => {
                const altAction = alternativeAction.read(reader);
                if (!altAction) {
                    return undefined;
                }
                const keybinding = createElement("div");
                const keybindingLabel = reader.store.add(( new KeybindingLabel(keybinding, OS, {
                ...
                return n.div({
                    ...
                    ref: elem => {
                        if (altAction.tooltip) {
                            reader.store.add(this._hoverService.setupDelayedHoverAtMouse(elem, {
                                content: altAction.tooltip,
                                appearance: {
                                    compact: true
                                }
                            }));
...
InlineEditsWordReplacementView = ( __decorate([( __param(3, ILanguageService)), ( __param(4, IThemeService)), ( __param(5, IKeybindingService)), ( __param(6, IHoverService)), ( __param(7, IUserInteractionService))], InlineEditsWordReplacementView));
...
export { InlineEditsWordReplacementView, WordReplacementsViewData };
