...
class RenameInferenceEngine {
    ...
    inferRename(textModel, editRange, insertText, wordDefinition) {
        ...
        ))) + insertText + textModel.getValueInRange(( new Range(
        ...
        } = ( new LcsDiff(( new StringDiffSequence(originalText)), ( new StringDiffSequence(modifiedText)))).ComputeDiff(true);
...
let RenameSymbolProcessor = class RenameSymbolProcessor extends Disposable {
    constructor(
        ...
        this._register(
            CommandsRegistry.registerCommand(renameSymbolCommandId, async (_, source, renameRunnable) => {
                ...
                try {
                    ...
                    bulkEditService.apply(workspaceEdit, {
                        reason: source
                    });
                } finally {
    ...
    async proposeRenameRefactoring(textModel, suggestItem, context) {
        ...
        let timedOut = false;
        const check = await raceTimeout(
            this.checkRenamePrecondition(suggestItem, textModel, position, oldName, newName, lastSymbolRename),
            100,
            () => {
                timedOut = true;
            }
        ...
        if (this._renameRunnable === undefined) {
            this._renameRunnable = ( new RenameSymbolRunnable(
                ...
                newName,
                lastSymbolRename,
                lastSymbolRename !== undefined ? oldName : undefined
            ));
        }
        ...
        const command = {
            ...
            title: ( localize(1359, "Rename")),
            arguments: [source, this._renameRunnable]
        };
        const alternativeAction = {
            label: ( localize(1359, "Rename")),
    ...
    async checkRenamePrecondition(suggestItem, textModel, position, oldName, newName, lastSymbolRename) {
        ...
        try {
            const result = await this._commandService.executeCommand(
                "github.copilot.nes.prepareRename",
                textModel.uri,
                position,
                oldName,
                newName,
...
RenameSymbolProcessor = ( __decorate([( __param(0, ICommandService)), ( __param(1, ILanguageFeaturesService)), ( __param(2, ILanguageConfigurationService)), ( __param(3, IBulkEditService)), ( __param(4, IRenameSymbolTrackerService)), ( __param(5, ICodeEditorService))], RenameSymbolProcessor));
