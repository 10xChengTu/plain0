...
import { ObservableAnimatedValue, AnimatedValue, easeOutCubic } from '../../../../../base/browser/animatedValue.js';
...
let InlineCompletionsModel = class InlineCompletionsModel extends Disposable {
    ...
    constructor(
        ...
        this._fetchInlineCompletionsPromise = derivedHandleChanges({
            ...
            const userJumpedToActiveCompletion = ( this._jumpedToId.map(
                jumpedTo => !!jumpedTo && jumpedTo === this._inlineSuggestionItems.read(undefined)?.inlineEdit?.semanticId
            ));
        ...
        this._inlineSuggestionItems = derivedOpts({
            owner: this
        }, reader => {
        ...
        this._inlineCompletionItems = derivedOpts({
            owner: this,
        ...
        this.state = derivedOpts({
            ...
            if (inlineEditResult) {
                ...
                return {
                    kind: "inlineEdit",
                    inlineSuggestion: inlineEditResult,
                    edits: replacements,
                    cursorAtInlineEdit,
                    nextEditUri
                };
            ...
            if (!this._shouldShowOnSuggestConflict.read(reader) && suggestItem) {
                ...
                const validEditsAndGhostTexts = ( allPotentialEdits.map((edit, idx) => ({
                    edit,
                    ghostText: edit ? computeGhostText(edit, model, mode, positions[idx], fullEditPreviewLength) : undefined
                }))).filter((
                    {
                        edit,
                        ghostText
                    }
                ) => edit !== undefined && ghostText !== undefined);
                const edits = ( validEditsAndGhostTexts.map((
                    {
                        edit
                    }
                ) => edit));
                const ghostTexts = ( validEditsAndGhostTexts.map((
                    {
                        ghostText
                    }
                ) => ghostText));
                ...
                return {
                    ...
                    primaryGhostText,
                    ghostTexts,
                    inlineSuggestion: augmentation?.completion,
                    suggestItem
                };
            } else {
                ...
                const validEditsAndGhostTexts = ( allPotentialEdits.map((edit, idx) => ({
                    edit,
                    ghostText: edit ? computeGhostText(edit, model, mode, positions[idx], 0) : undefined
                }))).filter((
                    {
                        edit,
                        ghostText
        ...
        this._source = this._register(this._instantiationService.createInstance(
        ...
        defaultAccountService.getDefaultAccount().then(
            createDisposableCb(account => this.sku.set(skuFromAccount(account), undefined), this._store)
        );
        this._register(
            defaultAccountService.onDidChangeDefaultAccount(account => this.sku.set(skuFromAccount(account), undefined))
        );
        ...
        }));
        {
            ...
            const [diffEditor] = this._codeEditorService.listDiffEditors().filter(
                d => d.getOriginalEditor().getId() === this._editor.getId() || d.getModifiedEditor().getId() === this._editor.getId()
            );
        ...
        const inlineCompletionProviders = observableFromEvent(
        ...
        mapObservableArrayCached(this, inlineCompletionProviders, (provider, store) => {
            ...
            store.add(provider.onDidChangeInlineCompletions(changeHint => {
                ...
                transaction(tx => {
                    this._fetchSpecificProviderSignal.trigger(tx, {
                        provider,
                        changeHint: changeHint ?? undefined
                    });
                    this.trigger(tx);
                });
            }));
    ...
    async triggerExplicitly(tx, onlyFetchInlineEdits = false) {
        return this.trigger(tx, {
            onlyFetchInlineEdits,
            explicit: true
        });
    }
    ...
    async accept(editor = this._editor, alternativeAction = false) {
        ...
        try {
            ...
            if (!completion.originalTextRef.targets(this.textModel)) {
                const targetEditor = await this._codeEditorService.openCodeEditor({
                    resource: completion.originalTextRef.uri
                }, this._editor);
                if (targetEditor) {
                ...
                if (alternativeAction && action.alternativeAction) {
                    ...
                    await this._commandService.executeCommand(altCommand.id, ...(altCommand.arguments || [])).then(undefined, onUnexpectedExternalError);
                    ...
                    SnippetController2.get(editor)?.insert(action.snippetInfo.snippet, {
                    ...
                    if (completion.hint === undefined) {
                        editor.setSelections(
                            state.kind === "inlineEdit" ? selections.slice(-1) : selections,
    ...
    async _acceptNext(editor, type, getAcceptUntilIndex, kind) {
        ...
        try {
            ...
            try {
                ...
                editor.edit(
                    TextEdit.fromParallelReplacementsUnsorted(edits),
                    this._getMetadata(completion, type)
                );
            ...
            const acceptedRange = Range.fromPositions(
                completion.editRange.getStartPosition(),
...
InlineCompletionsModel = ( __decorate([( __param(7, IInstantiationService)), ( __param(8, ICommandService)), ( __param(9, ILanguageConfigurationService)), ( __param(10, IAccessibilityService)), ( __param(11, ILanguageFeaturesService)), ( __param(12, ICodeEditorService)), ( __param(13, IInlineCompletionsService)), ( __param(14, IDefaultAccountService))], InlineCompletionsModel));
...
function skuFromAccount(account) {
    if (account?.entitlementsData?.access_type_sku && account?.entitlementsData?.copilot_plan) {
        return {
            type: account.entitlementsData.access_type_sku,
            plan: account.entitlementsData.copilot_plan
        };
    }
    return undefined;
}
