...
class InlineSuggestionGutterMenuData {
    static fromInlineSuggestion(suggestion) {
        ...
        return (new InlineSuggestionGutterMenuData(
            ...
            suggestion.source.provider.displayName ?? ( localize(1366, "Inline Suggestion")),
    ...
    constructor(
        ...
        this._availableWidthForIcon = derived(this, reader => {
            ...
            for (let digits = 1; digits <= totalLinesDigits; digits++) {
                ...
                const usableWidthLeftOfLineNumber = Math.min(
                    gutterWidth,
                    Math.max(0, rightOfLineNumber - digitsWidth - layout.glyphMarginLeft)
                );
                offsetDigits.push({
        ...
        this._layout = derived(this, reader => {
            ...
            const gutterViewPortWithStickyScroll = Rect.fromLeftTopWidthHeight(
                gutterViewPortPaddingLeft,
                gutterViewPortPaddingTop,
                gutterWidthWithoutPadding,
                gutterHeightWithoutPadding
            );
            ...
            const gutterEditArea = Rect.fromRanges(OffsetRange.fromTo(
            ...
            const iconNoneDocked = customIcon ? constObservable(customIcon) : ( this._tabAction.map(
                action => action === InlineEditTabAction.Accept ? Codicon.keyboardTab : Codicon.arrowRight
            ));
            const iconDocked = customIcon ? constObservable(customIcon) : derived(this, reader => {
            ...
            if (pillIsFullyDocked) {
                ...
                if (layout.lineNumbersWidth === 0) {
                    widthUntilLineNumberEnd = Math.max(0, Math.min(
                        Math.max(layout.lineNumbersLeft - gutterViewPortWithStickyScroll.left, 0),
                        pillRect.width - idealIconAreaWidth
                    ));
                } else {
                    widthUntilLineNumberEnd = Math.max(
                        layout.lineNumbersLeft + layout.lineNumbersWidth - gutterViewPortWithStickyScroll.left,
                        0
                    );
                }
        ...
        }, ( mapOutFalsy(this._layout).map(layout => !layout ? [] : [n.div({
            style: {
                position: "absolute",
        ...
        }, this._lineNumberToRender), n.div({
            style: {
                ...
                color: ( this._data.map(
                    d => d?.customization?.icon?.color ? asCssVariable(d.customization.icon.color.id) : undefined
                ))
            }
        }, [( layout.map((l, reader) => withStyles(renderIcon(l.icon.read(reader)), {
            fontSize: toPx(Math.min(l.iconRect.width - CODICON_PADDING_PX, CODICON_SIZE_PX))
        })))])])])));
        this._originalRangeObs = mapOutFalsy(( this._data.map(d => d?.originalRange)));
        this._stickyScrollController = StickyScrollController.get(this._editorObs.editor);
        this._stickyScrollHeight = this._stickyScrollController ? observableFromEvent(
...
InlineEditsGutterIndicator = ( __decorate([( __param(6, IHoverService)), ( __param(7, IInstantiationService)), ( __param(8, IAccessibilityService)), ( __param(9, IThemeService)), ( __param(10, IUserInteractionService))], InlineEditsGutterIndicator));
