...
import * as snippetSession from './snippetSession.css';
...
import { Placeholder, Choice, Text, SnippetParser, TextmateSnippet, Variable } from './snippetParser.js';
...
let SnippetSession = SnippetSession_1 = class SnippetSession {
    ...
    static createEditsAndSnippetsFromSelections(
        ...
        const firstBeforeText = model.getValueInRange(
            SnippetSession_1.adjustSelection(model, editor.getSelection(), overwriteBefore, 0)
        );
        const firstAfterText = model.getValueInRange(
            SnippetSession_1.adjustSelection(model, editor.getSelection(), 0, overwriteAfter)
        );
        ...
        for (const {
            ...
            const snippetLineLeadingWhitespace = SnippetSession_1.adjustWhitespace(
                model,
                start,
                adjustWhitespace || (idx > 0 && firstLineFirstNonWhitespace !== model.getLineFirstNonWhitespaceColumn(selection.positionLineNumber)),
                snippet
            );
            snippet.resolveVariables(( new CompositeSnippetVariableResolver([modelBasedVariableResolver, ( new ClipboardBasedVariableResolver(
                readClipboardText,
                idx,
                indexedSelections.length,
                editor.getOption(EditorOption.multiCursorPaste) === "spread"
            )), ( new SelectionBasedVariableResolver(model, selection, idx, overtypingCapturer)), ( new CommentBasedVariableResolver(model, selection, languageConfigurationService)), ( new TimeBasedVariableResolver()), ( new WorkspaceBasedVariableResolver(workspaceService)), ( new RandomBasedVariableResolver())])));
    ...
    static createEditsAndSnippetsFromEdits(
        ...
        const workspaceBasedVariableResolver = ( new WorkspaceBasedVariableResolver(
        ...
        const indexedSnippetEdits = ( snippetEdits.map((edit, idx) => ({
            edit,
            idx
        ...
        for (let i = 0; i < indexedSnippetEdits.length; i++) {
            ...
            const preExistingVariables = ( new Set());
            snippet.walk(marker => {
            ...
            SnippetSession_1.adjustWhitespace(
                model,
            ...
            const editResolver = ( new CompositeSnippetVariableResolver([modelBasedVariableResolver, ( new ClipboardBasedVariableResolver(readClipboardText, idx, indexedSnippetEdits.length, clipboardSpread)), ( new SelectionBasedVariableResolver(model, editSelection, idx, overtypingCapturer)), ( new CommentBasedVariableResolver(model, editSelection, languageConfigurationService)), timeBasedVariableResolver, workspaceBasedVariableResolver, randomBasedVariableResolver]));
            snippet.walk(marker => {
    ...
    insert(editReason) {
        ...
        const {
            edits,
            snippets
        } = typeof this._template === "string" ? SnippetSession_1.createEditsAndSnippetsFromSelections(
            this._editor,
            this._template,
        ...
        ) : SnippetSession_1.createEditsAndSnippetsFromEdits(
            this._editor,
            this._template,
            false,
            this._options.adjustWhitespace,
