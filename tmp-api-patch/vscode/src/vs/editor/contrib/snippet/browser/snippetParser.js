...
class Scanner {
    ...
    next() {
        ...
        !isNaN(ch) && typeof Scanner._table[ch] === "undefined" && !Scanner.isDigitCharacter(ch) && !Scanner.isVariableCharacter(ch));
        this.pos += len;
        return {
            type,
            pos,
...
class FormatString extends Marker {
    ...
    _toKebabCase(value) {
        ...
        const match2 = cleaned.match(
            /\p{Lu}{2,}(?=\p{Lu}\p{Ll}+[0-9]*|[\s_-]|$)|\p{Lu}?\p{Ll}+[0-9]*|\p{Lu}(?=\p{Lu}\p{Ll})|\p{Lu}(?=[\s_-]|$)|[0-9]+/gu
        );
        if (!match2) {
...
class SnippetParser {
    ...
    _until(type) {
        ...
        while (this._token.type !== type) {
            if (this._token.type === TokenType.EOF) {
                ...
                if (nextToken.type !== TokenType.Dollar && nextToken.type !== TokenType.CurlyClose && nextToken.type !== TokenType.Backslash) {
                    return false;
                }
            }
            this._token = this._scanner.next();
    ...
    _parseEscaped(marker) {
        ...
        if (value = this._accept(TokenType.Backslash, true)) {
            value = this._accept(TokenType.Dollar, true) || this._accept(TokenType.CurlyClose, true) || this._accept(TokenType.Backslash, true) || value;
    ...
    _parseTabstopOrVariableName(parent) {
        ...
        const match = this._accept(TokenType.Dollar) && (value = this._accept(TokenType.VariableName, true) || this._accept(TokenType.Int, true));
        if (!match) {
    ...
    _parseComplexPlaceholder(parent) {
        let index;
        const token = this._token;
        const match = this._accept(TokenType.Dollar) && this._accept(TokenType.CurlyOpen) && (index = this._accept(TokenType.Int, true));
        if (!match) {
            return this._backTo(token);
        }
    ...
    _parseChoiceElement(parent) {
        ...
        while (true) {
            ...
            let value;
            if (value = this._accept(TokenType.Backslash, true)) {
                value = this._accept(TokenType.Comma, true) || this._accept(TokenType.Pipe, true) || this._accept(TokenType.Backslash, true) || value;
            } else {
    ...
    _parseComplexVariable(parent) {
        let name;
        const token = this._token;
        const match = this._accept(TokenType.Dollar) && this._accept(TokenType.CurlyOpen) && (name = this._accept(TokenType.VariableName, true));
        if (!match) {
            return this._backTo(token);
        }
