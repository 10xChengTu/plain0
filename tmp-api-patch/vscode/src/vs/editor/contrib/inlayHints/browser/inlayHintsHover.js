    ...
    computeAsync(anchor, _lineDecorations, source, token) {
        ...
        return new AsyncIterableProducer(async executor => {
            ...
            if (isNonEmptyArray(part.item.hint.textEdits)) {
                executor.emitOne(( new MarkdownHover(this, anchor.range, [( new MarkdownString()).appendText(( localize(1311, "Double-click to insert")))], false, 10001)));
            }
            let partTooltip;
            ...
            if (part.part.location || part.part.command) {
                ...
                const kb = useMetaKey ? isMacintosh ? ( localize(1312, "cmd + click")) : ( localize(1313, "ctrl + click")) : isMacintosh ? ( localize(1314, "option + click")) : ( localize(1315, "alt + click"));
                if (part.part.location && part.part.command) {
                    linkHint = ( new MarkdownString()).appendText(( localize(1316, "Go to Definition ({0}), right click for more", kb)));
                } else if (part.part.location) {
                    linkHint = ( new MarkdownString()).appendText(( localize(1317, "Go to Definition ({0})", kb)));
                } else if (part.part.command) {
                    linkHint = ( new MarkdownString(`[${( localize(1318, "Execute Command"))}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, {
                        isTrusted: true
                    }));
                }
                if (linkHint) {
...
InlayHintsHover = ( __decorate([( __param(1, IMarkdownRendererService)), ( __param(2, IKeybindingService)), ( __param(3, IHoverService)), ( __param(4, IConfigurationService)), ( __param(5, ITextModelService)), ( __param(6, ILanguageFeaturesService)), ( __param(7, ICommandService))], InlayHintsHover));
