...
import { registerEditorContribution, EditorContributionInstantiation, registerEditorAction, EditorAction } from '../../../browser/editorExtensions.js';
...
let CodeLensContribution = class CodeLensContribution {
    ...
    constructor(
        ...
        this._disposables.add(
            _languageFeaturesService.codeLensProvider.onDidChange(this._onModelChange, this)
        );
        this._onModelChange();
        this._updateLensStyle();
    }
    ...
    _onModelChange() {
        ...
        const scheduler = ( new RunOnceScheduler(() => {
            ...
            this._getCodeLensModelPromise = createCancelablePromise(
                token => getCodeLensModel(this._languageFeaturesService.codeLensProvider, model, token)
            );
...
registerEditorAction(class ShowLensesInCurrentLine extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(1024, "Show CodeLens Commands for Current Line"))
        });
    }
    async run(accessor, editor) {
        ...
        const item = await quickInputService.pick(items, {
            canPickMany: false,
            placeHolder: ( localize(1025, "Select a command"))
        });
        if (!item) {
            return;
        }
