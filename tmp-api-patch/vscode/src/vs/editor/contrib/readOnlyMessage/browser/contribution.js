...
class ReadOnlyMessageController extends Disposable {
    ...
    constructor(editor) {
        ...
        this.editor = editor;
        this._register(
            this.editor.onDidAttemptReadOnlyEdit(() => this._onDidAttemptReadOnlyEdit())
        );
    }
        ...
        if (messageController && this.editor.hasModel()) {
            ...
            if (!message) {
                if (this.editor.isSimpleWidget) {
                    message = ( new MarkdownString(( localize(1526, "Cannot edit in read-only input"))));
                } else {
                    message = ( new MarkdownString(( localize(1527, "Cannot edit in read-only editor"))));
                }
            }
