...
let RenderedContentHover = RenderedContentHover_1 = class RenderedContentHover extends Disposable {
    constructor(
        ...
        } = RenderedContentHover_1.computeHoverPositions(editor, anchor.range, parts);
...
RenderedContentHover = RenderedContentHover_1 = ( __decorate([( __param(4, IKeybindingService)), ( __param(5, IHoverService)), ( __param(6, IClipboardService))], RenderedContentHover));
...
let RenderedContentHoverParts = class RenderedContentHoverParts extends Disposable {
    ...
    constructor(
        ...
        this._renderedParts = [];
        this._perPartDisposables = ( new Map());
        this._focusedHoverPartIndex = -1;
        this._context = context;
        this._fragment = createDocumentFragment();
        this._register(
            this._renderParts(participants, hoverParts, context, keybindingService, this._hoverService)
        );
    ...
    _createListenersForPart(index, renderedPart) {
        ...
        element.tabIndex = 0;
        partDisposables.add(addDisposableListener(element, EventType.FOCUS_IN, event => {
            event.stopPropagation();
            this._focusedHoverPartIndex = index;
        }));
        partDisposables.add(addDisposableListener(element, EventType.FOCUS_OUT, event => {
            event.stopPropagation();
            this._focusedHoverPartIndex = -1;
        }));
    ...
    getAccessibleHoverContentAtIndex(index) {
        ...
        if (renderedPart.type === "statusBar") {
            const statusBarDescription = [( localize(1245, "This is a hover status bar."))];
            for (const action of renderedPart.actions) {
                ...
                if (keybinding) {
                    ...
                    statusBarDescription.push(( localize(1247, "It has an action with label {0}.", action.actionLabel)));
                }
            }
    ...
    _normalizedIndexToMarkdownHoverIndexRange(markdownHoverParticipant, index) {
        ...
        const firstIndexOfMarkdownHovers = this._renderedParts.findIndex(
            renderedPart => renderedPart.type === "hoverPart" && renderedPart.participant === markdownHoverParticipant
        );
    ...
    _findRangeOfMarkdownHoverParts(markdownHoverParticipant) {
        ...
        const inversedLastIndexOfMarkdownHovers = copiedRenderedParts.reverse().findIndex(
            renderedPart => renderedPart.type === "hoverPart" && renderedPart.participant === markdownHoverParticipant
        );
...
RenderedContentHoverParts = RenderedContentHoverParts_1 = ( __decorate([( __param(4, IKeybindingService)), ( __param(5, IHoverService)), ( __param(6, IClipboardService))], RenderedContentHoverParts));
