...
import { HIDE_LONG_LINE_WARNING_HOVER_ACTION_ID, INCREASE_HOVER_VERBOSITY_ACTION_ID, DECREASE_HOVER_VERBOSITY_ACTION_ID } from './hoverActionIds.js';
...
import { HoverPosition, ClickAction, KeyDownAction } from '../../../../base/browser/ui/hover/hoverWidget.js';
...
const $ = $$1;
const increaseHoverVerbosityIcon = registerIcon("hover-increase-verbosity", Codicon.addSmall, ( localize(1278, "Icon for increaseing hover verbosity.")));
const decreaseHoverVerbosityIcon = registerIcon("hover-decrease-verbosity", Codicon.removeSmall, ( localize(1279, "Icon for decreasing hover verbosity.")));
class MarkdownHover {
...
let MarkdownHoverParticipant = class MarkdownHoverParticipant {
    ...
    createLoadingMessage(anchor) {
        return (new MarkdownHover(
            this,
            anchor.range,
            [( new MarkdownString()).appendText(( localize(1280, "Loading...")))],
            false,
            2000
        ));
    }
    ...
    updateMarkdownHoverVerbosityLevel(action, index) {
        return Promise.resolve(
            this._renderedHoverParts?.updateMarkdownHoverPartVerbosityLevel(action, index)
        );
    }
};
MarkdownHoverParticipant = ( __decorate([( __param(1, IMarkdownRendererService)), ( __param(2, IConfigurationService)), ( __param(3, ILanguageFeaturesService)), ( __param(4, IKeybindingService)), ( __param(5, IHoverService)), ( __param(6, ICommandService))], MarkdownHoverParticipant));
...
class MarkdownRenderedHoverParts {
    ...
    _renderHoverPart(hoverPart, onFinishedRendering) {
        ...
        disposables.add(
            this._renderHoverExpansionAction(actionsContainerInner, HoverVerbosityAction.Increase, canIncreaseVerbosity)
        );
        disposables.add(
            this._renderHoverExpansionAction(actionsContainerInner, HoverVerbosityAction.Decrease, canDecreaseVerbosity)
        );
    ...
    _renderHoverExpansionAction(container, action, actionEnabled) {
        ...
        const actionElement = append(container, $(
            ThemeIcon.asCSSSelector(isActionIncrease ? increaseHoverVerbosityIcon : decreaseHoverVerbosityIcon)
        ));
        actionElement.tabIndex = 0;
        const hoverDelegate = store.add(( new WorkbenchHoverDelegate("mouse", undefined, {
            target: container,
            position: {
                hoverPosition: HoverPosition.LEFT
            }
        }, this._configurationService, this._hoverService)));
        store.add(this._hoverService.setupManagedHover(
            hoverDelegate,
            actionElement,
    ...
    async _fetchHover(hoverSource, model, action) {
        ...
        try {
            hover = await Promise.resolve(
                provider.provideHover(model, hoverSource.hoverPosition, tokenSource.token, context)
            );
        } catch (e) {
...
function labelForHoverVerbosityAction(keybindingService, action) {
    switch (action) {
    case HoverVerbosityAction.Increase:
        return keybindingService.appendKeybinding(( localize(1283, "Increase Hover Verbosity")), INCREASE_HOVER_VERBOSITY_ACTION_ID);
    case HoverVerbosityAction.Decrease:
        return keybindingService.appendKeybinding(( localize(1284, "Decrease Hover Verbosity")), DECREASE_HOVER_VERBOSITY_ACTION_ID);
    }
}
