...
import { quickFixCommandId, ApplyCodeActionReason, getCodeActions } from '../../codeAction/browser/codeAction.js';
import { CodeActionController } from '../../codeAction/browser/codeActionController.js';
import { CodeActionTriggerSource, CodeActionKind } from '../../codeAction/common/types.js';
...
let MarkerHoverParticipant = class MarkerHoverParticipant {
    ...
    getAccessibleContent(hoverPart) {
        ...
        const relatedInformation = isNonEmptyArray(marker.relatedInformation) ? ( marker.relatedInformation.map(
            related => `${basename(related.resource)}(${related.startLineNumber}, ${related.startColumn}): ${related.message}`
        )).join("\n") : undefined;
    ...
    _renderMarkerStatusbar(context, markerHover) {
        ...
        if (markerHover.marker.severity === MarkerSeverity.Error || markerHover.marker.severity === MarkerSeverity.Warning || markerHover.marker.severity === MarkerSeverity.Info) {
            ...
            if (markerController) {
                context.statusBar.addAction({
                    label: ( localize(1285, "View Problem")),
        ...
        if (!this._editor.getOption(EditorOption.readOnly)) {
                ...
                if (IMarkerData.makeKey(this.recentMarkerCodeActionsInfo.marker) === IMarkerData.makeKey(markerHover.marker)) {
                    if (!this.recentMarkerCodeActionsInfo.hasCodeActions) {
                        if (menuActions.length === 0) {
                            quickfixPlaceholderElement.textContent = ( localize(1286, "No quick fixes available"));
                        }
                    }
                } else {
                    this.recentMarkerCodeActionsInfo = undefined;
            ...
            const updatePlaceholderDisposable = this.recentMarkerCodeActionsInfo && !this.recentMarkerCodeActionsInfo.hasCodeActions ? Disposable.None : disposableTimeout(() => quickfixPlaceholderElement.textContent = ( localize(1287, "Checking for quick fixes...")), 200, disposables);
...
MarkerHoverParticipant = ( __decorate([( __param(1, IMarkerDecorationsService)), ( __param(2, IOpenerService)), ( __param(3, ILanguageFeaturesService)), ( __param(4, IMenuService)), ( __param(5, IContextKeyService))], MarkerHoverParticipant));
