...
const CutAction = supportsCut ? registerCommand(( new MultiCommand({
    ...
    (isNative ? {
        ...
        win: {
            primary: KeyMod$1.CtrlCmd | KeyCode.KeyX,
            secondary: [KeyMod$1.Shift | KeyCode.Delete]
        },
    ...
    menuOpts: [{
        ...
        group: "2_ccp",
        title: ( localize(962, "Cu&&t")),
        order: 1
    }, {
        menuId: MenuId.EditorContext,
        group: CLIPBOARD_CONTEXT_MENU_GROUP,
        title: ( localize(963, "Cut")),
...
const CopyAction = supportsCopy ? registerCommand(( new MultiCommand({
    ...
    menuOpts: [{
        menuId: MenuId.MenubarEditMenu,
        group: "2_ccp",
        title: ( localize(964, "&&Copy")),
        order: 2
    }, {
        menuId: MenuId.EditorContext,
        group: CLIPBOARD_CONTEXT_MENU_GROUP,
        title: ( localize(965, "Copy")),
        order: 2
    }, {
        menuId: MenuId.CommandPalette,
        group: "",
...
const PasteAction = supportsPaste ? registerCommand(( new MultiCommand({
    ...
    (isNative ? {
        ...
        win: {
            primary: KeyMod$1.CtrlCmd | KeyCode.KeyV,
            secondary: [KeyMod$1.Shift | KeyCode.Insert]
        },
        linux: {
            primary: KeyMod$1.CtrlCmd | KeyCode.KeyV,
            secondary: [KeyMod$1.Shift | KeyCode.Insert]
    ...
    menuOpts: [{
        menuId: MenuId.MenubarEditMenu,
        group: "2_ccp",
        title: ( localize(968, "&&Paste")),
        order: 4
    }, {
        menuId: MenuId.EditorContext,
        group: CLIPBOARD_CONTEXT_MENU_GROUP,
        title: ( localize(969, "Paste")),
        when: EditorContextKeys.writable,
        order: 4
    }, {
        menuId: MenuId.CommandPalette,
...
class ExecCommandCopyWithSyntaxHighlightingAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize2(970, "Copy with Syntax Highlighting")),
            precondition: undefined,
            kbOpts: {
...
function executeClipboardCopyWithWorkaround(editor, clipboardService) {
    ...
    if (isNative && CopyOptions.electronBugWorkaroundCopyEventHasFired === false) {
        const {
            dataToCopy
        } = generateDataToCopyAndStoreInMemory(editor._getViewModel(), undefined, isFirefox);
...
function registerExecCommandImpl(target, browserCommand) {
    ...
    target.addImplementation(10000, "code-editor", (accessor, args) => {
        ...
        const clipboardService = accessor.get(IClipboardService);
        logService.trace(
            "registerExecCommandImpl (addImplementation code-editor for : ",
            browserCommand,
            ")"
        );
    ...
    target.addImplementation(0, "generic-dom", (accessor, args) => {
        const logService = accessor.get(ILogService);
        logService.trace(
            "registerExecCommandImpl (addImplementation generic-dom for : ",
            browserCommand,
            ")"
        );
        logService.trace("registerExecCommandImpl (before execCommand " + browserCommand + ")");
            ...
            if (isWeb) {
                ...
                return (async () => {
                    ...
                    if (clipboardText !== "") {
                        ...
                        logService.trace(
                            "registerExecCommandImpl (clipboardText.length : ",
