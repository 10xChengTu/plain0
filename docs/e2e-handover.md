...
状态：**部分完成（2026-07-31）**。项 1 的 ad-hoc hardened-runtime + JIT 真实 GUI 已完成；项 2 的首次公开 Actions run 暴露前置 CI timeout 并已本地修复，但 macOS job 因前置失败而被跳过，仍待下一次 push 复验；项 3 仍为可选；项 4 继续受 Apple Developer Program 决策阻塞。`F120` S0–S7 已全部完成并转 `complete`（`features.json`），机器化契约（品牌覆盖面、bundle 段契约、entitlements 内容契约、品牌字符串扫描、声明新鲜度检查、CI 打包存在性检查）已全部落地并配真实反向测试，`pnpm check`/`pnpm test:e2e:browser`（98/98）均通过。但按本项目一贯纪律（"机器化契约通过不等于实机验证通过"），其余维度仍按下列边界登记：
...
2026-07-31 执行结果：项 1 真实通过。`APPLE_SIGNING_IDENTITY=- pnpm tauri:build:e2e` 产物经 `codesign -dv --verbose=4` 确认为 `flags=0x10002(adhoc,runtime)`，`codesign -d --entitlements -` 只含 `com.apple.security.cs.allow-jit=true`。通过真实 GUI 启动后 Workbench 正常渲染；在仓库内可写 APFS fixture 中输入 `HARDENED_RUNTIME_JIT_OK`、保存并以 shell 核对磁盘，再恢复为原始 20 字节成功。执行中同时修复了一个残余 `Cmd+N` Untitled 入口：固定 API patch 现物理移除该上游注册，并把 direct command id 纳入 Plain 稳定拒绝闭集，避免关闭 dirty Untitled 时调用被排除的 `IFileDialogService`。项 2 的真实 run `30596874766` 因 Ubuntu `check` 中重型 AST 契约超过 Vitest 默认 5 秒而失败，`build-macos` 被跳过；本地/CI 已分别改为量测后的 10 秒/20 秒有界上限，`CI=true` 全量 81 文件 / 1698 用例与本地相关契约 83/83 均通过，仍待下一个远端 run 确认。
