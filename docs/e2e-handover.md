...
状态：**部分完成（2026-07-31）**。项 1 的 ad-hoc hardened-runtime + JIT 真实 GUI 已完成；项 2 已执行六次真实 Actions run：第六次的 Ubuntu `pnpm check` 全绿，Browser E2E 97/99；余下两条均是 macOS Explorer 键位被硬编码到跨平台 Playwright 测试，Linux 实际需要 F2/Delete。现按浏览器平台选择真实键位，聚焦 2/2 与本机全量 99/99（3.3m）通过；待第七次 run 让 Linux 99/99 通过并首次启动 macOS job。项 3 仍为可选；项 4 继续受 Apple Developer Program 决策阻塞。`F120` S0–S7 已全部完成并转 `complete`（`features.json`），机器化契约（品牌覆盖面、bundle 段契约、entitlements 内容契约、品牌字符串扫描、声明新鲜度检查、CI 打包存在性检查）已全部落地并配真实反向测试。但按本项目一贯纪律（"机器化契约通过不等于实机验证通过"），其余维度仍按下列边界登记：
