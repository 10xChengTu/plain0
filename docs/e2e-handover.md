...
状态：**部分完成（2026-07-31）**。项 1 的 ad-hoc hardened-runtime + JIT 真实 GUI 已完成；项 2 已执行两次真实 Actions run：第一次暴露 Vitest timeout，第二次证明 timeout 修复生效后又暴露 Ubuntu `check` 缺少 Zig。Zig 前置及反向契约已本地修复，仍待第三次 run 让 `check` 完整通过并首次启动 macOS job；项 3 仍为可选；项 4 继续受 Apple Developer Program 决策阻塞。`F120` S0–S7 已全部完成并转 `complete`（`features.json`），机器化契约（品牌覆盖面、bundle 段契约、entitlements 内容契约、品牌字符串扫描、声明新鲜度检查、CI 打包存在性检查）已全部落地并配真实反向测试。但按本项目一贯纪律（"机器化契约通过不等于实机验证通过"），其余维度仍按下列边界登记：
...
2026-07-31 执行结果：项 1 真实通过。`APPLE_SIGNING_IDENTITY=- pnpm tauri:build:e2e` 产物经 `codesign -dv --verbose=4` 确认为 `flags=0x10002(adhoc,runtime)`，`codesign -d --entitlements -` 只含 `com.apple.security.cs.allow-jit=true`。通过真实 GUI 启动后 Workbench 正常渲染；在仓库内可写 APFS fixture 中输入 `HARDENED_RUNTIME_JIT_OK`、保存并以 shell 核对磁盘，再恢复为原始 20 字节成功。执行中同时修复了一个残余 `Cmd+N` Untitled 入口：固定 API patch 现物理移除该上游注册，并把 direct command id 纳入 Plain 稳定拒绝闭集，避免关闭 dirty Untitled 时调用被排除的 `IFileDialogService`。项 2 的 run `30596874766` 因 Ubuntu `check` 中重型 AST 契约超过 Vitest 默认 5 秒而失败；本地/CI 改为量测后的 10 秒/20 秒有界上限。第二次 run `30640499415` 已真实证明这一修复：81 文件 / 1698 用例、frontend build、architecture、bundle 与 cargo fmt 均通过；随后 `rust:clippy` 调用 `ghostty:vendor:setup` 时因 Ubuntu job 未安装 Zig，连续 5 次 `spawnSync zig ENOENT` 失败，`build-macos` 再次 skipped。workflow 现为 Ubuntu `check` 补入与 macOS job 相同的 `mlugg/setup-zig@v1`/0.15.2，并以 per-job、顺序敏感、版本敏感的契约与 missing/late/drift 三类反向测试锁定；聚焦 9/9 与完整 architecture check 通过，仍待第三次 run 确认。
