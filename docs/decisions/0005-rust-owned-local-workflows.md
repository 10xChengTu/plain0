...
- vendor built-mode 冷启动会把非默认 Sidebar active container 强制改回 Explorer，因此 Workbench 构造完成后还要对非 EMPTY workspace 二次投影已经 hydration 的同一审计快照。运行中的 topology adoption 同样不会重建 layout/view 实例；分区 seed 后必须把该 workspace 快照投影到既有 part/pane runtime。新空分区固定回到左侧 Explorer、隐藏 panel，既有分区恢复自己的位置、可见性与 active container；profile 尺寸和 pins 仍按合同共享。runtime 投影失败与 storage 分区失败同样进入 reload boundary。
