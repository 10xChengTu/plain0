...
- Source Control 在恰好一个 root 时可自动选择；出现多个 root 后必须显示仓库选择器并清除任何自动选择。SCM、Graph、Stash 与 Worktrees 共享该显式选择；File/Line History、inline blame 与 hunk stage 按活动 editor URI 的 root authority 路由，并把该 root 同步回共享选择。
- 所有 Git 虚拟资源 URI（HEAD/index quick diff、历史 commit blob 与 multi-diff source）都必须把 root id 编入 URI authority。root id 是模型缓存身份的一部分；相同 rev/sha/path 在不同仓库中不得复用内容模型或后续 hover/detail 请求。
