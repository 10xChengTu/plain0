...
2. **create 路由**：Plain `canCreateFile/createFile/createFolder` 在 provider lookup 前做 URI/options 闭集检查；新文件只接受空内容和无 overwrite，文件与目录各调用一次私有 native-create seam。两条 create command 在同一次 IPC 中返回严格 `WorkspaceEntryStat` 创建回执；Rust 只使用 create syscall 已知的 `kind + size=0`，把尚未取得的时间记为 `0`、version 记为 `null`，不允许创建成功后因第二次 pathname lookup 失败而伪装成普通未创建错误。FileService 的 Plain 分支及导出/内部 `mkdirp` tripwire 禁止 create 前 target `stat/exists`、create 后第二次 `workspace_stat/resolve`、递归建父目录、通用 write 或 overwrite fallback；provider 自身再次把 URI 同步复制为 primitive request后才调用 bridge。严格回执或响应无法认证时，provider 只发一次冻结的 root `UPDATED` 再失败；可认证的 syscall 前错误不发事件。文件首次打开时再由既有同 handle `PLR1` read receipt 取得真实 metadata 与可写 `wv1`。
