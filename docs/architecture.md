...
- Workbench storage 仍保持同步 API，但只把审计过的 layout/view-arrangement key 交给 Rust：profile 分区保存全局尺寸、固定栏位和 pinned/placeholder arrangement；workspace 分区保存可见性、位置、active container 与各 view state，并按 Rust `WorkspaceRootsIdentity` 的稳定 root-set digest 命名。启动先读取严格有界快照，再把 `PlainLayoutStorageService` 注入 `initialize()`；其余历史、认证、编辑内容和任意 storage key 只留在内存且永不跨 IPC。空 workspace 不读写 workspace entries；正常原生 close 的 `IStorageService.flush(SHUTDOWN)` 触发 Workbench 最终 state save，并在 lifecycle allow 前等待 Rust 原子发布完成。损坏 JSON 隔离为 quarantine，WebView 不接收 app-data 路径或 identity digest。
