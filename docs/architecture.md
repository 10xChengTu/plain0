...
- Workbench storage 仍保持同步 API，但只把审计过的 layout/view-arrangement key 交给 Rust：profile 分区保存全局尺寸、固定栏位和 pinned/placeholder arrangement；workspace 分区保存可见性、位置、active container 与各 view state，并按 Rust `WorkspaceRootsIdentity` 的稳定 root-set digest 命名。启动先读取严格有界快照，再把 `PlainLayoutStorageService` 注入 `initialize()`；其余历史、认证、编辑内容和任意 storage key 只留在内存且永不跨 IPC。Primary Side Bar 左右位置不再读取或写入上游全局 `workbench.sideBar.location`：固定 vendor patch 让布局命令直接更新 runtime state 与 Plain 专用 context key，冷启动只采用当前 root-set 的 workspace entry 或左侧默认值。`reinitializeWorkspace()` 只更新配置与 folder context，不会重建已存在的 layout/view runtime；因此 topology adoption 后的分区切换还必须把新 workspace 快照显式投影到现有 part visibility、position 与 pane active-container，空分区使用固定的左侧 Explorer、隐藏 panel 默认值，避免 live root-set 继承上一分区。空 workspace 不读写 workspace entries；正常原生 close 的 `IStorageService.flush(SHUTDOWN)` 触发 Workbench 最终 state save，并在 lifecycle allow 前等待 Rust 原子发布完成。损坏 JSON 隔离为 quarantine，WebView 不接收 app-data 路径或 identity digest。
