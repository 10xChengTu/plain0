...
- Command writer 不只有 `CommandsRegistry.registerCommand`：`registerCommandAlias` 会回到同一 registry，`registerAction2` 会注册 action command，`KeybindingsRegistry.registerCommandAndKeybindingRule` 也会同时注册 command。仅统计 app 源码中名为 `registerCommand` 的调用不能形成写权限闭集。来源：[commands registry](https://github.com/microsoft/vscode/blob/5264f2156cbcd7aea5fd004d29eaa10209155d66/src/vs/platform/commands/common/commands.ts)、[action registration](https://github.com/microsoft/vscode/blob/5264f2156cbcd7aea5fd004d29eaa10209155d66/src/vs/platform/actions/common/actions.ts)、[keybinding registry](https://github.com/microsoft/vscode/blob/5264f2156cbcd7aea5fd004d29eaa10209155d66/src/vs/platform/keybinding/common/keybindingsRegistry.ts)。
...
- CodinGame 根 `initialize` 会把 `configuration.commands` 交给内部 `registerCommands`，Workbench override 的 `registerCustomView({ actions })` 也会经 `registerAction2` 写 command；API 包通配导出的深层 contribution 模块还可在静态 import 时注册命令。因此命令 authority 必须锁定 app 的静态包导入形状和 `initialize` 配置，而不是只禁止几个函数名。来源：[service initialization](https://github.com/CodinGame/monaco-vscode-api/blob/d8367168c23c9d0a9ba5bc84b8034e5435e9eb93/src/services.ts)、[custom views](https://github.com/CodinGame/monaco-vscode-api/blob/d8367168c23c9d0a9ba5bc84b8034e5435e9eb93/src/service-override/tools/views.ts)。
...
只在单个文件里检查合法注册不足以阻止晚到模块覆盖 provider 或 command handler。当前 `app/` 必须禁止动态 `import()`，并由全源码 AST authority guard 按 `relativePath:moduleSpecifier` 固定所有现有 `@codingame/monaco-vscode-*` 静态导入；`monaco-editor` value import 为零允许，新增 contribution side effect 或 writer wrapper 必须显式评审后才可更新 allowlist。`registerCustomProvider`、配置 provider/factory、`CommandsRegistry` 和本地 `registerWorkspaceCommands` 还要锁定原始导出名、词法 binding、唯一直接调用和合法 dispose；根 `initialize` 只允许当前三参数调用，第三参数必须是固定四属性、无 spread/computed/`commands` 的内联配置。guarded command id 字符串只允许存在于 Plain command 闭集。未来若确需动态前端切片，必须先实现能够持续验证当前 provider/handler 身份的生命周期 guard，再单独放宽。
