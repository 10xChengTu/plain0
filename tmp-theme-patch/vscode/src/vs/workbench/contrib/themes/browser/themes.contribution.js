...
const manageExtensionIcon = registerIcon("theme-selection-manage-extension", Codicon.gear, ( localize(16201, "Icon for the 'Manage' action in the theme selection quick pick.")));
var ConfigureItem;
...
let MarketplaceThemesPicker = class MarketplaceThemesPicker {
    ...
    openQuickPick(value, currentTheme, selectTheme) {
        ...
        return ( new Promise((s, _) => {
            ...
            quickpick.placeholder = ( localize(16202, "Type to Search More. Select to Install. Up/Down Keys to Preview"));
            ...
            disposables.add(this.onDidChange(() => {
                ...
                if (this._searchOngoing) {
                    ...
                    items = [{
                        label: `$(error) ${( localize(16203, "Error while searching for themes: {0}", this._searchError))}`,
                        id: undefined,
                        alwaysShow: true
                    }];
                }
    ...
    async installExtension(galleryExtension) {
        ...
        const result = await this.dialogService.confirm({
            ...
            primaryButton: ( localize(16205, "OK"))
        });
        if (!result.confirmed) {
            return false;
        }
        try {
            await this.progressService.withProgress({
                ...
                title: ( localize(16206, "Installing Extension {0}...", galleryExtension.displayName))
            }, async () => {
    ...
    async openQuickPick(picks, currentTheme) {
        ...
        const pickInstalledThemes = activeItemId => {
            ...
            return ( new Promise((s, _) => {
                ...
                disposables.add(
                    quickpick.onDidTriggerButton(button => this.options.onButton?.(button, quickpick))
                );
                quickpick.matchOnDescription = true;
                disposables.add(quickpick.onDidAccept(async _ => {
                    isCompleted = true;
...
InstalledThemesPicker = ( __decorate([( __param(3, IQuickInputService)), ( __param(4, IExtensionGalleryService)), ( __param(5, IExtensionsWorkbenchService)), ( __param(6, IExtensionResourceLoaderService)), ( __param(7, IInstantiationService))], InstalledThemesPicker));
const SelectColorThemeCommandId = "workbench.action.selectTheme";
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: SelectColorThemeCommandId,
            title: ( localize2(16207, "Color Theme")),
    ...
    getTitle(colorScheme) {
        ...
        case ColorScheme.DARK:
            return localize(16208, "Select Color Theme for System Dark Mode");
        case ColorScheme.LIGHT:
            return localize(16209, "Select Color Theme for System Light Mode");
        case ColorScheme.HIGH_CONTRAST_DARK:
            return localize(16210, "Select Color Theme for High Contrast Dark Mode");
        case ColorScheme.HIGH_CONTRAST_LIGHT:
            return localize(16211, "Select Color Theme for High Contrast Light Mode");
        default:
            return localize(16212, "Select Color Theme (detect system color mode disabled)");
        }
    }
    async run(accessor) {
        ...
        const modeConfigureButton = {
            tooltip: preferredColorScheme ? ( localize(16213, "Detect system color mode enabled. Click to configure.")) : ( localize(16214, "Detect system color mode disabled. Click to configure.")),
            iconClass: ThemeIcon.asClassName(Codicon.colorMode),
            location: QuickInputButtonLocation.Inline
        };
        const options = {
            installMessage: ( localize(16215, "Install Additional Color Themes...")),
            browseMessage: "$(plus) " + ( localize(16216, "Browse Additional Color Themes...")),
        ...
        const lightEntries = toEntries(themes.filter(t => t.type === ColorScheme.LIGHT), ( localize(16217, "light themes")));
        const darkEntries = toEntries(themes.filter(t => t.type === ColorScheme.DARK), ( localize(16218, "dark themes")));
        const hcEntries = toEntries(themes.filter(t => isHighContrast(t.type)), ( localize(16219, "high contrast themes")));
        let picks;
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(16220, "File Icon Theme")),
            category: Categories.Preferences,
            f1: true
        });
    }
    async run(accessor) {
        const themeService = accessor.get(IWorkbenchThemeService);
        const options = {
            installMessage: ( localize(16221, "Install Additional File Icon Themes...")),
            placeholderMessage: ( localize(16222, "Select File Icon Theme (Up/Down Keys to Preview)")),
        ...
        const picks = [{
            type: "separator",
            label: ( localize(16223, "file icon themes"))
        }, {
            id: "",
            theme: FileIconThemeData.noIconTheme,
            label: ( localize(16224, "None")),
            description: ( localize(16225, "Disable File Icons"))
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(16226, "Product Icon Theme")),
            category: Categories.Preferences,
            f1: true
        });
    }
    async run(accessor) {
        const themeService = accessor.get(IWorkbenchThemeService);
        const options = {
            installMessage: ( localize(16227, "Install Additional Product Icon Themes...")),
            browseMessage: "$(plus) " + ( localize(16228, "Browse Additional Product Icon Themes...")),
            placeholderMessage: ( localize(16229, "Select Product Icon Theme (Up/Down Keys to Preview)")),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(16232, "Try New Default Themes")),
            category: Categories.Preferences,
            f1: true
        });
    }
    async run(accessor) {
        ...
        picker.items = items;
        picker.placeholder = ( localize(16233, "Pick a new default theme"));
...
function findBuiltInThemes(themes, extension) {
    ...
    ) => extensionData && extensionData.extensionIsBuiltin && equalsIgnoreCase(extensionData.extensionPublisher, extension.publisher) && equalsIgnoreCase(extensionData.extensionName, extension.name));
}
...
const defaultThemeDescriptions = {
    [ThemeSettingDefaults.COLOR_THEME_LIGHT]: ( localize(16234, "Default Light")),
    [ThemeSettingDefaults.COLOR_THEME_DARK]: ( localize(16235, "Default Dark"))
};
...
const configureButton = {
    ...
    tooltip: ( localize(16236, "Manage Extension"))
};
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: "workbench.action.generateColorTheme",
            title: ( localize2(16237, "Generate Color Theme From Current Settings")),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(16238, "Toggle between Light/Dark Themes")),
            category: Categories.Preferences,
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (configurationService.getValue(ThemeSettings.DETECT_COLOR_SCHEME)) {
            ...
            notificationService.prompt(Severity.Info, message, [{
                label: ( localize(16240, "Open Settings")),
                run: () => {
...
const PLAIN_MARKETPLACE_DISABLED = true;
if (!PLAIN_MARKETPLACE_DISABLED) {
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: browseColorThemesInMarketplaceCommandId,
            title: ( localize2(16241, "Browse Color Themes in Marketplace")),
            category: Categories.Preferences,
            f1: true
        });
    }
...
MenuRegistry.appendMenuItem(MenuId.GlobalActivity, {
    title: ( localize(16242, "Themes")),
    submenu: ThemesSubMenu,
    group: "2_configuration",
    order: 7
});
MenuRegistry.appendMenuItem(MenuId.MenubarPreferencesMenu, {
    title: ( localize(16243, "&&Themes")),
    submenu: ThemesSubMenu,
    group: "2_configuration",
    order: 7
});
MenuRegistry.appendMenuItem(ThemesSubMenu, {
    command: {
        id: SelectColorThemeCommandId,
        title: ( localize(16207, "Color Theme"))
    },
    order: 1
});
MenuRegistry.appendMenuItem(ThemesSubMenu, {
    command: {
        id: SelectFileIconThemeCommandId,
        title: ( localize(16244, "File Icon Theme"))
    },
    order: 2
});
MenuRegistry.appendMenuItem(ThemesSubMenu, {
    command: {
        id: SelectProductIconThemeCommandId,
        title: ( localize(16245, "Product Icon Theme"))
    },
    order: 3
});
