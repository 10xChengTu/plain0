...
import { IconFontDefinition, fontIdRegex, fontWeightRegex, fontStyleRegex, fontFormatRegex, getIconRegistry } from '@codingame/monaco-vscode-api/vscode/vs/platform/theme/common/iconRegistry';
...
class ProductIconThemeData {
    ...
    static get defaultTheme() {
        ...
        if (!themeData) {
            themeData = ProductIconThemeData._defaultProductIconTheme = ( new ProductIconThemeData(DEFAULT_PRODUCT_ICON_THEME_ID, ( localize(17977, "Default")), ThemeSettingDefaults.PRODUCT_ICON_THEME));
...
function _loadProductIconThemeDocument(fileService, location, warnings) {
    return fileService.readExtensionResource(location).then(content => {
        ...
        if (parseErrors.length > 0) {
            ...
            return Promise.reject(( new Error(( localize(17979, "Invalid format for product icons theme file: Object expected.")))));
        ...
        for (const font of contentValue.fonts) {
            ...
            if (isString(fontId) && fontId.match(fontIdRegex)) {
                ...
                if (isString(font.weight) && font.weight.match(fontWeightRegex)) {
                    ...
                    warnings.push(( localize(17981, "Invalid font weight in font '{0}'. Ignoring setting.", font.id)));
                }
                let fontStyle = undefined;
                if (isString(font.style) && font.style.match(fontStyleRegex)) {
                    fontStyle = font.style;
                } else {
                    warnings.push(( localize(17982, "Invalid font style in font '{0}'. Ignoring setting.", font.id)));
                }
                const sanitizedSrc = [];
        ...
        for (const iconId in contentValue.iconDefinitions) {
            ...
            if (isString(definition.fontCharacter)) {
                ...
                if (fontDefinition) {
                    ...
                    warnings.push(( localize(17986, "Skipping icon definition '{0}'. Unknown font.", iconId)));
                }
            } else {
                warnings.push(( localize(17987, "Skipping icon definition '{0}': Needs to be defined", iconId)));
            }
        }
        return {
            iconDefinitions
