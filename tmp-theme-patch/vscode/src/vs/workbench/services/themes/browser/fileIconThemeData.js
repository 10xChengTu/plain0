...
class FileIconThemeLoader {
    ...
    loadIconThemeDocument(location) {
        return this.fileService.readExtensionResource(location).then(content => {
            ...
            if (errors.length > 0) {
                ...
                return Promise.reject(( new Error(( localize(17975, "Invalid format for file icons theme file: Object expected.")))));
            }
    ...
    processIconThemeDocument(id, iconThemeDocumentLocation, iconThemeDocument) {
        ...
        function collectSelectors(associations, baseThemeClassName) {
            function addSelector(selector, defId) {
                if (defId) {
                    ...
                    if (!list) {
                        list = selectorByDefinitionId[defId] = new Builder();
                    }
                    list.push(selector);
                }
            }
            if (associations) {
                let qualifier = inline`.show-file-icons`;
                if (baseThemeClassName) {
                    qualifier = inline`${baseThemeClassName} ${qualifier}`;
                }
                const expanded = inline`.monaco-tl-twistie.collapsible:not(.collapsed) + .monaco-tl-contents`;
                if (associations.folder) {
                    addSelector(inline`${qualifier} .folder-icon::before`, associations.folder);
                    result.hasFolderIcons = true;
                }
                if (associations.folderExpanded) {
                    addSelector(
                        inline`${qualifier} ${expanded} .folder-icon::before`,
                ...
                if (rootFolder) {
                    addSelector(inline`${qualifier} .rootfolder-icon::before`, rootFolder);
                    result.hasFolderIcons = true;
                }
                if (rootFolderExpanded) {
                    addSelector(
                        inline`${qualifier} ${expanded} .rootfolder-icon::before`,
                        rootFolderExpanded
                    );
                    result.hasFolderIcons = true;
                }
                if (associations.file) {
                    addSelector(inline`${qualifier} .file-icon::before`, associations.file);
                    result.hasFileIcons = true;
                }
                const folderNames = associations.folderNames;
                if (folderNames) {
                    for (const key in folderNames) {
                        const selectors = new Builder();
                        const name = handleParentFolder(key.toLowerCase(), selectors);
                        selectors.push(inline`.${classSelectorPart(name)}-name-folder-icon`);
                        addSelector(
                            inline`${qualifier} ${selectors.join("")}.folder-icon::before`,
                            folderNames[key]
                        );
                        result.hasFolderIcons = true;
                    }
                ...
                if (folderNamesExpanded) {
                    for (const key in folderNamesExpanded) {
                        ...
                        selectors.push(inline`.${classSelectorPart(name)}-name-folder-icon`);
                        addSelector(
                            inline`${qualifier} ${expanded} ${selectors.join("")}.folder-icon::before`,
                ...
                if (rootFolderNames) {
                    for (const key in rootFolderNames) {
                        ...
                        addSelector(
                            inline`${qualifier} .${classSelectorPart(name)}-root-name-folder-icon.rootfolder-icon::before`,
                            rootFolderNames[key]
                        );
                        result.hasFolderIcons = true;
                    }
                ...
                if (rootFolderNamesExpanded) {
                    for (const key in rootFolderNamesExpanded) {
                        const name = key.toLowerCase();
                        addSelector(
                            inline`${qualifier} ${expanded} .${classSelectorPart(name)}-root-name-folder-icon.rootfolder-icon::before`,
                ...
                if (languageIds) {
                    ...
                    for (const languageId in languageIds) {
                        addSelector(
                            inline`${qualifier} .${classSelectorPart(languageId)}-lang-file-icon.file-icon::before`,
                    ...
                    for (const key in fileExtensions) {
                        ...
                        if (segments.length) {
                            for (let i = 0; i < segments.length; i++) {
                                selectors.push(
                                    inline`.${classSelectorPart(segments.slice(i).join("."))}-ext-file-icon`
                                );
                            }
                            selectors.push(inline`.ext-file-icon`);
                        }
                        addSelector(
                            inline`${qualifier} ${selectors.join("")}.file-icon::before`,
                            fileExtensions[key]
                        );
                        result.hasFileIcons = true;
                        hasSpecificFileIcons = true;
                ...
                if (fileNames) {
                    for (const key in fileNames) {
                        ...
                        selectors.push(inline`.${classSelectorPart(fileName)}-name-file-icon`);
                        selectors.push(inline`.name-file-icon`);
        ...
        collectSelectors(iconThemeDocument.light, inline`.vs`);
        collectSelectors(iconThemeDocument.highContrast, inline`.hc-black`);
        collectSelectors(iconThemeDocument.highContrast, inline`.hc-light`);
        ...
        const cssRules = new Builder();
        const fonts = iconThemeDocument.fonts;
        const fontSizes = ( new Map());
        if (Array.isArray(fonts)) {
            const defaultFontSize = this.tryNormalizeFontSize(fonts[0].size) || "150%";
            fonts.forEach(font => {
                const fontSrcs = new Builder();
                fontSrcs.push(...( font.src.map(
                    l => inline`${asCSSUrl(resolvePath(l.path))} format(${stringValue(l.format)})`
                )));
                cssRules.push(
                    inline`@font-face { src: ${fontSrcs.join(", ")}; font-family: ${stringValue(font.id)}; font-weight: ${identValue(font.weight)}; font-style: ${identValue(font.style)}; font-display: block; }`
                );
            ...
            cssRules.push(
                inline`.show-file-icons .file-icon::before, .show-file-icons .folder-icon::before, .show-file-icons .rootfolder-icon::before { font-family: ${stringValue(fonts[0].id)}; font-size: ${sizeValue(defaultFontSize)}; }`
            );
        }
        const emQuad = stringValue("\\2001");
        for (const defId in selectorByDefinitionId) {
            ...
            if (definition) {
                if (definition.iconPath) {
                    cssRules.push(
                        inline`${selectors.join(", ")} { content: ${emQuad}; background-image: ${asCSSUrl(resolvePath(definition.iconPath))}; }`
                    );
                    ...
                    if (definition.fontColor && definition.fontColor.match(fontColorRegex)) {
                        body.push(inline`color: ${hexColorValue(definition.fontColor)};`);
                    }
                    if (definition.fontCharacter) {
                        body.push(inline`content: ${stringValue(definition.fontCharacter)};`);
                    }
                    ...
                    if (fontSize && fontSize.match(fontSizeRegex)) {
                        body.push(inline`font-size: ${sizeValue(fontSize)};`);
                    }
                    if (definition.fontId) {
                        body.push(inline`font-family: ${stringValue(definition.fontId)};`);
                    }
                    if (showLanguageModeIcons) {
                        body.push(inline`background-image: unset;`);
                    }
                    cssRules.push(inline`${selectors.join(", ")} { ${body.join(" ")} }`);
                }
            }
        }
        if (showLanguageModeIcons) {
            for (const languageId of this.languageService.getRegisteredLanguageIds()) {
                if (!coveredLanguages[languageId]) {
                    ...
                    if (icon) {
                        const selector = inline`.show-file-icons .${classSelectorPart(languageId)}-lang-file-icon.file-icon::before`;
                        cssRules.push(
                            inline`${selector} { content: ${emQuad}; background-image: ${asCSSUrl(icon.dark)}; }`
                        );
                        cssRules.push(
                            inline`.vs ${selector} { content: ${emQuad}; background-image: ${asCSSUrl(icon.light)}; }`
                        );
                    }
                }
            }
...
function handleParentFolder(key, selectors) {
    ...
    if (lastIndexOfSlash >= 0) {
        ...
        selectors.push(inline`.${classSelectorPart(parentFolder)}-name-dir-icon`);
