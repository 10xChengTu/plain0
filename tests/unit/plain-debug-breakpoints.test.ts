...
describe("DebugBreakpointStore", () => {
	it("toggle adds a plain breakpoint then removes it on a second call", () => {
		...
		store.onDidChange((rootId, path) => changes.push(`${rootId}:${path}`));
		...
		store.toggle(ROOT_ID, "a.py", 5);
		expect(store.descriptorsForPath(ROOT_ID, "a.py")).toEqual([]);
		expect(store.pathsWithBreakpoints(ROOT_ID)).toEqual([]);

		expect(changes).toEqual([`${ROOT_ID}:a.py`, `${ROOT_ID}:a.py`]);
	});

	it("descriptorsForPath returns entries sorted by line regardless of insertion order", () => {
		const store = new DebugBreakpointStore();
		store.toggle(ROOT_ID, "a.py", 20);
		store.toggle(ROOT_ID, "a.py", 5);
		store.toggle(ROOT_ID, "a.py", 10);
		expect(
			store.descriptorsForPath(ROOT_ID, "a.py").map((entry) => entry.line),
		).toEqual([5, 10, 20]);
	});
	...
	it("setVerification correlates positionally with descriptorsForPath's own order", () => {
		...
		const views = store.viewsForPath(ROOT_ID, "a.py");
	...
	it("setVerification with the wrong length is ignored rather than misattributed", () => {
		...
		for (const view of store.viewsForPath(ROOT_ID, "a.py")) {
	...
	it("clearVerification only clears the named path's verification, not its breakpoints", () => {
		const store = new DebugBreakpointStore();
		store.toggle(ROOT_ID, "a.py", 5);
		store.toggle(ROOT_ID, "b.py", 7);
		store.setVerification(ROOT_ID, "a.py", [
			{ verified: true, actualLine: 5, message: null },
		]);
		store.setVerification(ROOT_ID, "b.py", [
			{ verified: true, actualLine: 7, message: null },
		]);

		store.clearVerification(ROOT_ID, "a.py");
		expect(store.viewsForPath(ROOT_ID, "a.py")[0]?.verification).toBeNull();
		expect(store.viewsForPath(ROOT_ID, "b.py")[0]?.verification).not.toBeNull();
		expect(store.descriptorsForPath(ROOT_ID, "a.py")).toHaveLength(1);
	});
	...
	it("clearAllVerification clears every path's verification and notifies once per affected path", () => {
		...
		store.onDidChange((_rootId, path) => changed.push(path));
		store.clearAllVerification();

		expect(store.viewsForPath(ROOT_ID, "a.py")[0]?.verification).toBeNull();
		expect(store.viewsForPath(ROOT_ID, "b.py")[0]?.verification).toBeNull();
	...
	it("keeps identical relative paths fully isolated between roots", () => {
		const store = new DebugBreakpointStore();
		store.toggle(ROOT_ID, "src/main.py", 5);
		store.toggle(OTHER_ROOT_ID, "src/main.py", 9);
		store.setVerification(ROOT_ID, "src/main.py", [
			{ verified: true, actualLine: 6, message: null },
		]);

		expect(store.viewsForPath(ROOT_ID, "src/main.py")).toEqual([
			{
				line: 5,
				condition: null,
				logMessage: null,
		...
		expect(store.viewsForPath(OTHER_ROOT_ID, "src/main.py")).toEqual([
			{
				line: 9,
				condition: null,
				logMessage: null,
