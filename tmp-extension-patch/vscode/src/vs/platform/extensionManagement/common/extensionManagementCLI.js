...
const notFound = id => ( localize(2026, "Extension '{0}' not found.", id));
...
let ExtensionManagementCLI = class ExtensionManagementCLI {
    ...
    async listExtensions(showVersions, category, profileLocation) {
        ...
        if (this.location) {
            this.logger.info(( localize(2028, "Extensions installed on {0}:", this.location)));
        }
    ...
    async installExtensions(extensions, builtinExtensions, installOptions, force) {
        ...
        try {
            if (extensions.length) {
                this.logger.info(this.location ? ( localize(2029, "Installing extensions on {0}...", this.location)) : ( localize(2030, "Installing extensions...")));
            }
            ...
            this.logger.error(( localize(2031, "Error while installing extensions: {0}", getErrorMessage(error))));
            throw error;
        }
        if (failed.length) {
            throw ( new Error(( localize(2032, "Failed Installing Extensions: {0}", failed.join(", ")))));
        }
    }
    async updateExtensions(profileLocation) {
        ...
        if (!extensionsToUpdate.length) {
            this.logger.info(( localize(2034, "No extension to update")));
            return;
        }
    ...
    async installGalleryExtensions(installExtensionInfos, installed, force) {
        installExtensionInfos = installExtensionInfos.filter(installExtensionInfo => {
            ...
            if (installedExtension) {
                ...
                if (version && installedExtension.manifest.version === version) {
                    this.logger.info(( localize(2039, "Extension '{0}' is already installed.", `${id}@${version}`)));
                    return false;
                }
        ...
        await Promise.all(( installExtensionInfos.map(async (
            ...
            if (installOptions.isBuiltin) {
                this.logger.info(version ? ( localize(2041, "Installing builtin extension '{0}' v{1}...", id, version)) : ( localize(2042, "Installing builtin extension '{0}'...", id)));
            } else {
                this.logger.info(version ? ( localize(2043, "Installing extension '{0}' v{1}...", id, version)) : ( localize(2044, "Installing extension '{0}'...", id)));
            }
    ...
    async installVSIX(vsix, installOptions, force, installedExtensions) {
        ...
        if (valid) {
            try {
                ...
                this.logger.info(( localize(2047, "Extension '{0}' was successfully installed.", basename(vsix))));
            } catch (error) {
                if (isCancellationError(error)) {
                    this.logger.info(( localize(2048, "Cancelled installing extension '{0}'.", basename(vsix))));
                } else {
                    throw error;
                }
            }
    ...
    async uninstallExtensions(extensions, force, profileLocation) {
        ...
        for (const extension of extensions) {
            ...
            this.logger.info(( localize(2052, "Uninstalling {0}...", id)));
            ...
            if (this.location) {
                ...
                this.logger.info(( localize(2054, "Extension '{0}' was successfully uninstalled!", id)));
            }
        }
    }
    ...
    notInstalled(id) {
        return this.location ? ( localize(2055, "Extension '{0}' is not installed on {1}.", id, this.location)) : ( localize(2056, "Extension '{0}' is not installed.", id));
    }
