...
import { PromptsType, isValidPromptType } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/chat/common/promptSyntax/promptTypes';
...
import { isUntitledChatSession, getChatSessionType } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/chat/common/model/chatUri';
...
let MainThreadChatAgents2 = class MainThreadChatAgents2 extends Disposable {
        ...
        const impl = {
            invoke: async (request, progress, history, token) => {
                ...
                this._pendingProgress.set(request.requestId, {
                    progress,
                    chatSession,
                    isSubagent: !!request.subAgentInvocationId
                });
                try {
            ...
            provideFollowups: async (request, result, history, token) => {
                ...
                return this._proxy.$provideFollowups(request, handle, result, {
                    history
                }, token);
            },
    ...
    async $handleProgressChunk(requestId, chunks) {
        ...
        if (!pendingProgress) {
            this._logService.warn(
                `MainThreadChatAgents2#$handleProgressChunk: No pending progress for requestId ${requestId}`
            );
            return;
        }
        const {
        ...
        for (const item of chunks) {
            ...
            if (progress.kind === "externalEdits") {
                if (chatSession?.editingSession && responsePartHandle !== undefined && response) {
                    const parts = progress.start ? await chatSession.editingSession.startExternalEdits(
                        response,
                        responsePartHandle,
                        revive(progress.resources),
                        progress.undoStopId
            ...
            if (progress.kind === "updateToolInvocation") {
                this._languageModelToolsService.updateToolStream(
                    progress.toolCallId,
                    progress.streamData?.partialInput,
            ...
            if (progress.kind === "usage") {
                if (isSubagent) {
                    ...
                    this._logService.warn(
                        `MainThreadChatAgents2#$handleProgressChunk: No response model for usage of non-subagent request ${requestId}; dropping usage.`
                    );
                }
                continue;
            }
            const revivedProgress = progress.kind === "notebookEdit" ? ChatNotebookEdit.fromChatEdit(progress) : revive(progress);
            ...
            if (revivedProgress.kind === "inlineReference" && revivedProgress.resolveId && response) {
                ...
                this._unresolvedAnchors.get(requestId)?.set(revivedProgress.resolveId, {
                    response
                });
            }
    ...
    $registerAgentCompletionsProvider(handle, id, triggerCharacters) {
        ...
        this._agentCompletionProviders.set(handle, this._languageFeaturesService.completionProvider.register({
            ...
            provideCompletionItems: async (model, position, _context, token) => {
                ...
                const parsedRequest = this._instantiationService.createInstance(ChatRequestParser).parseChatRequestWithReferences(
    ...
    $registerChatParticipantDetectionProvider(handle) {
        this._chatParticipantDetectionProviders.set(
            handle,
            this._chatAgentService.registerChatParticipantDetectionProvider(handle, {
                provideParticipantDetection: async (request, history, options, token) => {
                    return await this._proxy.$detectChatParticipant(handle, request, {
                        history
                    }, options, token);
                }
            })
    ...
    async $registerPromptFileProvider(handle, type, extensionId) {
        ...
        if (!extension) {
            this._logService.error(
                `[MainThreadChatAgents2] Could not find extension for prompt file provider: ${extensionId.value}`
            );
            return;
        }
    ...
    async $registerChatSessionCustomizationProvider(handle, chatSessionType, metadata, extensionId) {
        ...
        if (!extension) {
            this._logService.error(
                `[MainThreadChatAgents2] Could not find extension for customization provider: ${extensionId.value}`
            );
            return;
        }
        const emitter = ( new Emitter());
...
MainThreadChatAgents2 = __decorate([extHostNamedCustomer(MainContext.MainThreadChatAgents2), ( __param(1, IChatAgentService)), ( __param(2, IChatSessionsService)), ( __param(3, IChatService)), ( __param(4, ILanguageFeaturesService)), ( __param(5, IChatWidgetService)), ( __param(6, IInstantiationService)), ( __param(7, ILogService)), ( __param(8, IExtensionService)), ( __param(9, IUriIdentityService)), ( __param(10, IPromptsService)), ( __param(11, ILanguageModelToolsService)), ( __param(12, ICustomizationHarnessService)), ( __param(13, ITelemetryService)), ( __param(14, IAgentPluginService)), ( __param(15, IWorkbenchEnvironmentService))], MainThreadChatAgents2);
