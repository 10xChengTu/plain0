...
class ExtHostNotebookDocumentSaveParticipant {
    ...
    async participate(workingCopy, context, _progress, token) {
        ...
        const p = ( new Promise((resolve, reject) => {
            _warningTimeout = setTimeout(() => reject(( new Error(( localize(2792, "Aborted onWillSaveNotebookDocument-event after 1750ms"))))), 1750);
...
let SaveParticipant = class SaveParticipant {
    constructor(extHostContext, instantiationService, workingCopyFileService) {
        ...
        this._saveParticipantDisposable = this.workingCopyFileService.addSaveParticipant(
            instantiationService.createInstance(ExtHostNotebookDocumentSaveParticipant, extHostContext)
        );
    }
    dispose() {
...
SaveParticipant = __decorate([extHostCustomer, ( __param(1, IInstantiationService)), ( __param(2, IWorkingCopyFileService))], SaveParticipant);
