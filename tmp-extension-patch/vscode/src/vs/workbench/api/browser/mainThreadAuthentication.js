...
import { fetchAuthorizationServerMetadata } from '@codingame/monaco-vscode-api/vscode/vs/base/common/oauth';
...
import { mcpOAuthClientSecretStorageKey } from '@codingame/monaco-vscode-api/vscode/vs/workbench/contrib/mcp/common/mcpTypes';
...
let MainThreadAuthentication = class MainThreadAuthentication extends Disposable {
    constructor(
        ...
        this._register(authenticationService.registerAuthenticationProviderHostDelegate({
            ...
            createXaa: async issuer => {
                ...
                } = await fetchAuthorizationServerMetadata(( issuer.toString(true)));
                ...
                return await this._proxy.$registerXaaAuthProvider(issuer, serverMetadata, clientId, clientSecret, initialTokens);
            }
        }));
    }
    ...
    $showContinueNotification(message) {
        const yes = ( localize(2709, "Yes"));
        const no = ( localize(2710, "No"));
        const deferredPromise = ( new DeferredPromise());
        let result = false;
    ...
    async loginPrompt(provider, extensionName, recreatingSession, options) {
        ...
        if (options?.learnMore) {
            buttons.push({
                label: ( localize(2714, "Learn more")),
                run: async () => {
    ...
    async continueWithIncorrectAccountPrompt(chosenAccountLabel, requestedAccountLabel) {
        const result = await this.dialogService.prompt({
            message: ( localize(2715, "Incorrect account detected")),
            detail: ( localize(
                2716,
            ...
            cancelButton: true,
            buttons: [{
                label: ( localize(2717, "Keep {0}", chosenAccountLabel)),
                run: () => chosenAccountLabel
            }, {
                label: ( localize(2718, "Login with {0}", requestedAccountLabel)),
    ...
    async $showDeviceCodeModal(userCode, verificationUri) {
        const {
            ...
            type: Severity.Info,
            message: ( localize(2719, "Device Code Authentication")),
            detail: ( localize(
                2720,
            ...
            buttons: [{
                label: ( localize(2721, "Copy & Continue")),
                run: () => true
            }],
            cancelButton: true
        });
        if (result) {
            try {
                ...
                this.notificationService.error(( localize(2722, "Failed to open {0}", verificationUri)));
            }
        }
        return false;
    }
    async $promptForClientRegistration(authorizationServerUrl) {
        ...
        const result = await this.dialogService.prompt({
            type: Severity.Info,
            message: ( localize(2723, "Dynamic Client Registration not supported")),
            detail: ( localize(
                2724,
            ...
            buttons: [{
                label: ( localize(2725, "Copy URIs & Proceed")),
                run: async () => {
                    try {
                        await this.clipboardService.writeText(redirectUrls);
                    } catch (error) {
                        this.notificationService.error(( localize(2726, "Failed to copy redirect URIs to clipboard.")));
                    }
                    return true;
                }
            }],
            cancelButton: {
                label: ( localize(2727, "Cancel")),
                run: () => false
            }
        });
        if (!result) {
        ...
        const sharedTitle = ( localize(2728, "Add Client Registration Details"));
        const clientId = await this.quickInputService.input({
            ...
            placeHolder: ( localize(2730, "OAuth client ID (azye39d...)")),
            ignoreFocusLost: true,
            validateInput: async value => {
                if (!value || value.trim().length === 0) {
                    return localize(2731, "Client ID is required");
                }
                return undefined;
            }
        });
        ...
        const clientSecret = await this.quickInputService.input({
            ...
            placeHolder: ( localize(2733, "OAuth client secret (wer32o50f...) or leave it blank")),
            password: true,
            ignoreFocusLost: true
        });
        return {
    ...
    async $promptForResourceClientSecret(resourceClientId, resource) {
        const value = await this.quickInputService.input({
            title: ( localize(2734, "Resource Client Secret Required")),
            prompt: ( localize(
                2735,
            ...
            placeHolder: ( localize(2736, "Resource client secret")),
            password: true,
            ignoreFocusLost: true
        });
        if (value === undefined) {
        ...
        try {
            ...
            this.logService.warn(
                `[XAA] Failed to persist resource client secret for ${resource} / ${resourceClientId}: ${err.message}`
            );
        }
        return trimmed;
    }
